...
namespace Sig
{
	...
	tUnitLogic::tUnitLogic( )
		...
		, mUnitCountry( GameFlags::cCOUNTRY_DEFAULT )
	...
	void tUnitLogic::fApplyDamage( f32 damageAmount, const tDamageContext& damageContext, tDamageResult& result, b32 fromChild, b32 force )
	{
		...
		if( &damageContext != mPersistentDamageContext.fGetRawPtr( ) )
		{
			if( mUnitType == GameFlags::cUNIT_TYPE_BOSS )
				fHandleLogicEvent( Logic::tEvent( GameFlags::cEVENT_UNIT_DAMAGED, NEW Logic::tIntEventContext( damageContext.fEffectDamageType( ) ) ) );
	...
	void tUnitLogic::fReachedEnemyGoal( )
	{
		...
		tTutorialEvent reachedGoalEvent = tTutorialEvent( GameFlags::cTUTORIAL_EVENT_UNIT_REACHED_GOAL, fUnitID( ), fUnitID( ), false, fUnitType( ) );
	...
	namespace
	{
		...
		{
		public:
			tGrowableArray<tEntityPtr>& mPassengersOut;
	...
	void tUnitLogic::fScriptAudioEvent( const char *event )
	{
		if( mAudio )
		{
			mAudio->fHandleEvent( event );
		}
	}
...
namespace Sig
{
	void tUnitLogic::fExportScriptInterface( tScriptVm& vm )
	{
		...
		classDesc
			...
			.Prop( _SC( "UnitCountry" ),	&tUnitLogic::fUnitCountry, &tUnitLogic::fSetUnitCountry )
			...
			.Prop(_SC("CurrentState"),					&tUnitLogic::fCurrentState)
			...
			.Func(_SC("AudioEvent"),					&tUnitLogic::fScriptAudioEvent)
			...
			.Prop(_SC("IsDestroyed"),					&tUnitLogic::fIsDestroyed )
