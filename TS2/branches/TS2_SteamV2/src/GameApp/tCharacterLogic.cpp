...
namespace Sig
{
	...
	void tCharacterLogic::fOnSpawn( )
	{
		...
		// Make sure we test for collisions with volumes every frame.
		mContextAnimAndCollisionProximty.fSetRefreshFrequency( 0.033f, 0.f );
	...
	void tCharacterLogic::fConfigureAudio( )
	{
		...
		if( fIsCommando( ) )
		{
			u32 barrageFactionCountry = fUnitCountry( ) != GameFlags::cCOUNTRY_DEFAULT ? fUnitCountry( ) : fCountry( );
			mAudio->fSetSwitch( tGameApp::cBarrageFactionSwitchGroup, GameFlags::fCOUNTRYEnumToValueString( barrageFactionCountry ) );
		}
