...
namespace Sig
{
	...
	devvar( bool, Debug_ForceLoadDLC, true );
	...
	devvar( bool, Game_RequireOnlineUser, true );
	...
	namespace
	{
		...
		devvar( bool, Game_ResetAchievements, false );
	...
	{
#if defined( use_steam )
		struct tSteamLanguages
		{
			const char* mLanguage;
			GameFlags::tLANGUAGE mId;
			const char *mLocFolder;
		};
		tSteamLanguages sLanguages[] = {
			{ "german", GameFlags::cLANGUAGE_GERMAN, "loc/de" },
			{ "english", GameFlags::cLANGUAGE_ENGLISH, "loc/en" },
			{ "spanish", GameFlags::cLANGUAGE_SPANISH, "loc/es" },
			{ "french", GameFlags::cLANGUAGE_FRENCH, "loc/fr" },
			{ "italian", GameFlags::cLANGUAGE_ITALIAN, "loc/it" },
			{ "japanese", GameFlags::cLANGUAGE_JAPANESE, "loc/ja" },
			{ "korean", GameFlags::cLANGUAGE_KOREAN, "loc/ko" },
			{ "tchinese", GameFlags::cLANGUAGE_CHINESE, "loc/zh" },
		};
#endif
#if defined( platform_pcdx )
		// Currently only really useful for identifying the US vs Rest Of World (for ESRB display purposes)
		GameFlags::tLOCALE fGetSystemLocale( )
		{
			LANGID langID = GetUserDefaultUILanguage( );
			int primaryID = PRIMARYLANGID( langID );
			int subID = SUBLANGID( langID );
			switch( primaryID )
			{
			case LANG_ENGLISH:
				if( subID == SUBLANG_ENGLISH_US )
					return GameFlags::cLOCALE_UNITED_STATES;
				return GameFlags::cLOCALE_NO_LOCALE;

			case LANG_SPANISH:
				if( subID == SUBLANG_SPANISH_US )
					return GameFlags::cLOCALE_UNITED_STATES;
				return GameFlags::cLOCALE_NO_LOCALE;
			}
			return GameFlags::cLOCALE_NO_LOCALE;
		...
		GameFlags::tREGION fGetRegionByLocale( GameFlags::tLOCALE locale )
		{
			switch( locale )
			{
			case GameFlags::cLOCALE_AUSTRALIA:
				return GameFlags::cREGION_AUSTRALIA;

			case GameFlags::cLOCALE_GERMANY:
				return GameFlags::cREGION_GERMANY;

			case GameFlags::cLOCALE_KOREA:
				return GameFlags::cREGION_KOREA;
	...
	namespace { static const tStringPtr cTitle( "Toy Soldiers Complete" ); }
...
#if defined( platform_pcdx9 )
	tFilePathPtr tGameApp::cLocalAppData = tFilePathPtr::cNullPtr;
#endif
...
#if defined( platform_pcdx9 )
		// Get the local app data path for this title
		char buf[256];
		if (SUCCEEDED (::SHGetFolderPath (NULL, CSIDL_LOCAL_APPDATA, NULL, 0, buf))) 
		{
			cLocalAppData = tFilePathPtr::fConstructPath( tFilePathPtr( buf ), tFilePathPtr( cTitle ) );
			// Make sure the directory exists
			CreateDirectory( cLocalAppData.fCStr( ), NULL );
		}
#endif
	}
	tGameApp::~tGameApp( )
	...
	void tGameApp::fConfigureApp( tStartupOptions& opts, tPlatformStartupOptions& platformOpts )
	{
		...
		opts.mGameName = "Toy Soldiers: Complete";
...
#		undef log_flag_register_types
...
#else
		screenCreateOpts.mBackBufferHeight = mGraphicsSettings.fDisplayHeight( );
		screenCreateOpts.mBackBufferWidth = mGraphicsSettings.fDisplayWidth( );
		screenCreateOpts.mFullScreen = mGraphicsSettings.fIsFullscreen( );
		screenCreateOpts.mVsync = (Gfx::tVsync)mGraphicsSettings.fVsync( );
		
		screenCreateOpts.mShadowMapLayerCount = mGraphicsSettings.fShadowLayers( );
		screenCreateOpts.mShadowMapResolution = mGraphicsSettings.fShadowResolution( );
		// TODO: JamesP, re-enable when implemented
		//screenCreateOpts.mShadowMapLowDetailRange = mGraphicsSettings.fShadowLowDetailRange( );
		//screenCreateOpts.mShadowMapHighDetailRange = mGraphicsSettings.fShadowHighDetailRange( );
		//screenCreateOpts.mShadowsEnabled = mGraphicsSettings.fShadowsEnabled( );
		//screenCreateOpts.mLowDetailTextures = !mGraphicsSettings.fTextureDetail( );
		//screenCreateOpts.mDeferredLighting = true;
		//screenCreateOpts.mBrightness = mGraphicsSettings.fBrightness( );
#endif
	}
...
#if defined( use_steam )
		// Delegates so the engine achievement functions can use the game-provided enum<->string mappings
		Achievements::sAchievementIdToValueString.fFromFunction< GameSession::fAchievementIdToValueString >( );

		tSteamManager::fInstance( ).fStartup( );
#endif

#ifdef platform_pcdx9
		fGetProductAndVersionFromResource( );

		mGraphicsSettings.fLoad( );

		// TODO: JamesP, re-enable when implemented
		//Gfx::tLightEntity::fSetupShadowMapping( mGraphicsSettings.fShadowLowDetailRange( ), mGraphicsSettings.fShadowHighDetailRange( ) );
#endif
...
#ifdef platform_pcdx9
		mGraphicsSettings.fRegisterDeviceCallbacks( );
#endif
	...
	void tGameApp::fExportGameScriptInterfaces( tScriptVm& vm )
	{
		...
		tRailGunLogic::fExportScriptInterface( vm );
		...
		tGameController::fExportScriptInterface( vm );
		
		tAppGraphicsSettings::fExportScriptInterface( vm );
		mGraphicsSettings.fExportScriptInstance( vm, "GfxSettings" );
		mGraphicsSettings.fCaptureSettings( );
	}
	void tGameApp::fLoadPermaLoadedGameResources( )
	{
		...
		//TS1 content
		mUnitsPhysics[ GameFlags::cCOUNTRY_BRITISH ].fSet( fAddAlwaysLoadedResource( tResourceId::fMake< tDataTableFile >( "DataTables/VehiclesBRITISH.tab" ), true ) );
		mUnitsPhysics[ GameFlags::cCOUNTRY_GERMAN ].fSet( fAddAlwaysLoadedResource( tResourceId::fMake< tDataTableFile >( "DataTables/VehiclesGERMAN.tab" ), true ) );
		mUnitsPhysics[ GameFlags::cCOUNTRY_FRENCH ].fSet( fAddAlwaysLoadedResource( tResourceId::fMake< tDataTableFile >( "DataTables/VehiclesFRENCH.tab" ), true ) );
		...
		mGlobalScripts[ cGlobalScriptSniperTowerOptions ] = fAddAlwaysLoadedResource( tResourceId::fMake< tScriptFile >( "Gui/Scripts/RadialMenus/sniper_tower_options.nut" ) );
...
#define USE_GAMEREGION_FOR_REGION
		...
		auto *language = "english"; // tSteamManager::fInstance( ).fGetLanguage( );
		for( int i = 0; i < array_length( sLanguages ); ++i )
		{
			auto lang = &sLanguages[i];
			if( 0 == _stricmp( lang->mLanguage, language ) )
			{
				mLanguage = lang->mId;
				locFolder = tFilePathPtr( lang->mLocFolder );
				break;
			}
		}

		// Steam doesn't give us the locale or region, so use the system values for that
		auto locale = fGetSystemLocale( );
		mLocale = ( u32 )locale;
		mRegion = ( u32 )fGetRegionByLocale( locale );
#endif
...
#if defined(use_steam)
		// keep calling steam SDK to pump callbacks
		tSteamManager::fInstance( ).fUpdate( );
#endif

#if defined( target_game ) && defined( platform_pcdx9 )

		Gfx::tDevicePtr defaultDevice = Gfx::tDevice::fGetDefaultDevice( );
		u32 deviceWidth = ~0;
		u32 deviceHeight = ~0;
		b32 deviceIsFullScreen = false;
		b32 deviceIsVsync = false;
		
		defaultDevice->fGetDisplayMode( deviceWidth, deviceHeight, deviceIsFullScreen, deviceIsVsync );

		if( defaultDevice->fHasFocus( ) && deviceIsFullScreen )
		{
			// has focus
			RECT cursorClipRect;           // new area for ClipCursor
			GetClientRect( fGetHwnd( ), &cursorClipRect );
			MapWindowPoints( fGetHwnd( ), HWND_DESKTOP, (LPPOINT)&cursorClipRect, 2 );
...
#endif // defined( target_game ) && defined( platform_pcdx9 )
	}
...
#if defined(use_steam)
		tSteamManager::fInstance( ).fShutdown( );
#endif

		// Make sure threaded logic is not running
	...
	void tGameApp::fPollInputDevices( )
	{
		...
		Input::tMouse::fCaptureGlobalState( (Sig::Input::tMouse::tGenericWindowHandle)fGetWindowHandleGeneric( ), dt );
	...
	void tGameApp::fResetPlayerCameraAspectRatios()
	{
		for( u32 i = 0; i < mPlayers.fCount( ); ++i )
			mPlayers[ i ]->fResetCameraAspectRatios( );
	}
	...
	void tLevelLoadInfo::fFillEmptyWithDefaults( )
	{
		...
		if( mCountry2 == GameFlags::cCOUNTRY_COUNT || mCountry2 == GameFlags::cCOUNTRY_DEFAULT )
			mCountry2 = GameFlags::cCOUNTRY_USSR;
	...
	b32 tLevelLoadInfo::fIsAssetEquivalent( const tLevelLoadInfo& prevLevel ) const
	{
		if( mGameMode == prevLevel.mGameMode &&
			...
			( !mGameMode.fIsVersus( ) || ( mGameMode.fIsVersus( ) && ( mCountry == prevLevel.mCountry ) && ( mCountry2 == prevLevel.mCountry2 ) ) )
			)
			return true;
		return false;
	}
	...
	namespace 
	{
		enum tLevelResourceColumns
		{
			...
			cLevelResourcePlayerEnemyCountry,
	...
	{
		// TODO: HACK: Restrict Versus to TS2 only for now
		if( ( mapType == GameFlags::cMAP_TYPE_HEADTOHEAD ) )
		{
			for( u32 i = 0; i < table.fRowCount( ); ++i )
			{
				if( table.fIndexByRowCol< u32 >( i, cLevelResourceDlcNumber ) < GameFlags::cDLC_UNUSED )
				{
					if( levelIndex == 0 )
					{
						levelIndex = i;
		...
		GameFlags::tCOUNTRY enemyCountry	= ( GameFlags::tCOUNTRY )GameFlags::fCOUNTRYValueStringToEnum( table.fIndexByRowCol<tStringPtr>( levelIndex, cLevelResourcePlayerEnemyCountry ) );
		levelLoadInfo.mCountry2				= ( ( enemyCountry == GameFlags::cCOUNTRY_DEFAULT ) || ( enemyCountry == GameFlags::cCOUNTRY_COUNT ) ) ? tPlayer::fEnemyCountry( levelLoadInfo.mCountry ) : enemyCountry;
	...
	void tGameApp::fConfigureAppForGameMode( const tLevelLoadInfo& levelLoadInfo )
	{
		...
		else
		{
			...
			mPlayers[ 0 ]->fSetCountry( (GameFlags::tCOUNTRY)levelLoadInfo.mCountry );
			mPlayers[ 1 ]->fSetCountry( gameMode.fIsVersus( ) ? (GameFlags::tCOUNTRY)levelLoadInfo.mCountry2 : levelLoadInfo.mCountry );
		}
	...
	void tGameApp::fConfigureViewportsForGameMode( const tGameMode& gameMode )
	{
		...
		if( mSoundSystem )
		{
			...
			for( u32 i = 0; i < mPlayers.fCount( ); ++i ) 
			{
				u32 listenerIndex = mPlayers[ i ]->fListenerIndex( );
				if( listenerIndex != ~0 )
				{
					masterMask = fSetBits( mask, ( 1 << listenerIndex ) ); 
					if( mPlayers[ i ]->fUser( )->fIsLocal( ) ) 
						mask = fSetBits( mask, ( 1 << listenerIndex ) ); 
				}
			}
	...
	void tGameApp::fOnDeviceReset( )
	{
		fSetViewportProportions( fGameMode( ) );
	}
	...
	{
		// Adjust for non 16_9 resolutions
		float currentAspectRatio = (float)fScreen()->fCreateOpts().mBackBufferWidth / (float)fScreen()->fCreateOpts().mBackBufferHeight;
		...
		if( splitScreen )
		{
			...
			fFrontEndPlayer( )->fUser( )->fViewport( )->fSet16_9ClipBox( Math::tRect( 0.0f+vClip, 0.0f+hClip, 1.0f-vClip, 1.0f-hClip ) );
			fSecondaryPlayer( )->fUser( )->fViewport( )->fSet16_9ClipBox( Math::tRect( 0.0f+vClip, 0.0f+hClip, 1.0f-vClip, 1.0f-hClip ) );

			// Create black bar to separate the viewports
			Gfx::tDevicePtr device = Gfx::tDevice::fGetDefaultDevice( );
			int w = device->fCreationPresentParams( ).BackBufferWidth;
			int h = device->fCreationPresentParams( ).BackBufferHeight;
		...
		{
			Gfx::tViewport *vp = fFrontEndPlayer( )->fUser( )->fViewport( ).fGetRawPtr( );
			vp->fSetClipBox( Math::tRect( 0.0f, 0.0f, 1.0f, 1.0f ) );
			vp->fSet16_9ClipBox( Math::tRect( 0.0f+vClip, 0.0f+hClip, 1.0f-vClip, 1.0f-hClip ) );

			if( tPlayer * otherPlayer = fSecondaryPlayer( ) )
			{
				if( otherPlayer->fUser( )->fViewport( ) != vp &&
	...
	void tGameApp::fOnLevelSpawnBegin( ) 
	{
		...
		mGameAppSession.fOnLevelSpawnBegin( );
	}
	void tGameApp::fOnLevelSpawnEnd( )
	{
		mSpawningCurrentLevel = false;

		mGameAppSession.fOnLevelSpawnEnd( );
		if( mCurrentSaveGame )
		{
	...
	u32 tGameApp::fNumLevelsInTable( u32 mapType ) const
	{
		...
		const tDataTableFile& levelResources = fDataTable( cDataTableLevelResources );
		const tDataTable& table = levelResources.fIndexTable( mapType );
		auto levelCount = table.fRowCount( );
		// TODO: HACK: Restrict Versus to TS2 only for now
		if( ( mapType == GameFlags::cMAP_TYPE_HEADTOHEAD ) )
		{
			levelCount = 0;
	...
	void tGameApp::fStartupSfx( )
	{
		...
		const tFilePathPtr root = fResourceDepot( )->fRootPath( );
		mStartupParams.mAudioOptions.mBaseResourcePath = root;

		// All platforms have these core banks
		mStartupParams.mAudioOptions.mSoundBanks.fPushBack( tFilePathPtr( "\\Audio\\Init.bnk" ) ); // WWise required initialization bank
		mStartupParams.mAudioOptions.mSoundBanks.fPushBack( tFilePathPtr( "\\Audio\\MainBank.bnk" ) ); // Core game audio assets

		// If we have any dlc packages, load the dlc sound content.
		if( mDLCEnumerator.fCountByType( tDLCPackage::cDLCPackage ) || mDLCEnumerator.fCountByType( tDLCPackage::cDLCCompatibilityPackage ) || Debug_ForceLoadDLC )
		{
#if defined( platform_xbox360 )
			// Only xbox has seperate DLC packs.
			if( Debug_ForceLoadDLC )
			{
				mStartupParams.mAudioOptions.mFilePackages.fPushBack( tFilePathPtr( "\\Audio\\DLC01.pck" ) );
				mStartupParams.mAudioOptions.mFilePackages.fPushBack( tFilePathPtr( "\\Audio\\DLC02.pck" ) );
			}
			else
			{
				...
				for( u32 p = 0; p < mDLCEnumerator.fPackageCount( ); ++p )
				{
					...
					switch( pck->mType )
					{
					case tDLCPackage::cDLCPackage:
					case tDLCPackage::cDLCCompatibilityPackage:
						p = 9001; // all full packages contain the same data
						// intentional fallthrough
					case tDLCPackage::cSingleDLCPackage:
					case tDLCPackage::cSingleDLCCompatibilityPackage:
						{
							//// log all dlc files
						...
						break;
					case tDLCPackage::cTitleUpdatePackage:
						// ignore
						break;
					}
				}
...
#else
			// All platforms have seperate DLC banks.
			mStartupParams.mAudioOptions.mSoundBanks.fPushBack( tFilePathPtr( "\\Audio\\DLC01.bnk" ) );
			mStartupParams.mAudioOptions.mSoundBanks.fPushBack( tFilePathPtr( "\\Audio\\DLC02.bnk" ) );
#endif
		}

#if defined( platform_pcdx ) && defined( target_game )
		// tell the audio system the handle of our main window.
		mStartupParams.mAudioOptions.hWnd = mMainWindow.fGetHwnd( );
#endif
	...
	u32 tGameApp::fFindLocalUserWithGameControlPressed( u32 gameControlId )
	{
		u32 gamepadButton = 0;
		u32 keyboardKey = 0;
		u32 mouseButton = Input::tMouse::cButtonCount;

		switch( gameControlId )
		{
		case GameFlags::cGAME_CONTROLS_MENU_ACCEPT:
			{
				gamepadButton = Input::tGamepad::cButtonA;
				keyboardKey = Input::tKeyboard::cButtonEnter;
			}
			break;

		case GameFlags::cGAME_CONTROLS_MENU_PAUSE:
			{
				gamepadButton = Input::tGamepad::cButtonStart;
				keyboardKey = Input::tKeyboard::cButtonEscape;
			}
			break;

		default:
			{
				log_assert( false, "tGameApp::fFindLocalUserWithGameControlPressed - unsupported gameControlId: " << GameFlags::fGAME_CONTROLSEnumToString( gameControlId ) );
				return ~0;
			}
			break;
		}
		...
		for( u32 u = 0; u < userCount; ++u )
		{
			...
			if( mLocalUsers[ u ]->fRawGamepad( ).fButtonDown( gamepadButton ) || mLocalUsers[ u ]->fRawKeyboard( ).fButtonDown( keyboardKey ) || mLocalUsers[ u ]->fRawMouse( ).fButtonDown( mouseButton ) )
				return u;
		}
	...
	u32 tGameApp::fFindLocalUserWithAnyPressStartInputPressed( ) const
	{
		const u32 userCount = mLocalUsers.fCount( );
		for( u32 u = 0; u < userCount; ++u )
		{
			...
			if( mLocalUsers[ u ]->fRawGamepad( ).fButtonDown( Input::tGamepad::cButtonStart ) || mLocalUsers[ u ]->fRawGamepad( ).fButtonDown( Input::tGamepad::cButtonSelect ) ||
				mLocalUsers[ u ]->fRawGamepad( ).fButtonDown( Input::tGamepad::cButtonA ) || mLocalUsers[ u ]->fRawGamepad( ).fButtonDown( Input::tGamepad::cButtonB ) ||
				mLocalUsers[ u ]->fRawMouse( ).fButtonUp( Input::tMouse::cButtonLeft ) )
			{
				return u;
			}

			const Input::tKeyboard::tStateData& stateData = mLocalUsers[ u ]->fRawKeyboard( ).fGetStateData( );
			u32 keyCount = fMin( stateData.mKeysOn.fCount( ), stateData.mPrevKeysOn.fCount( ) );
			// Start at cButtonBackspace, skipping all the mouse VK codes.
			for( u32 key = Input::tKeyboard::cButtonBackspace; key < keyCount; ++key )
			{
				if( stateData.mKeysOn.fGetBit( key ) && !stateData.mPrevKeysOn.fGetBit( key ) )
				{
					return u;
				}
			}
	...
	tPlayer * tGameApp::fGetPlayerByUser( tUser* user )
	{
		...
		//sigassert( playerIndex != ~0 && playerIndex < mPlayers.fCount( ) && "Invalid player index from fWhichUser" ); // it's valid for there to be a user that isn't assigned to a player. fGetPlayer returns null in this case.
	...
	tPlayer* tGameApp::fAnyPlayerWithButtonsDown( const u32* pressedButtons, u32 pressedButtonsCount, const u32* notPressedButtons, u32 notPressedButtonsCount, b32 localOnly )
	{
		sigassert( pressedButtons );
		if( !pressedButtons )
		{
			return 0;
		}
		...
		for( u32 i = 0; i < mPlayers.fCount( ); ++i )
		{
			if( mPlayers[ i ]->fIsActive( ) && ( localOnly ? mPlayers[ i ]->fUser( )->fIsLocal( ) : true ) )
			{
				b32 pressed = false;
				for( u32 button = 0; button < pressedButtonsCount; ++button )
				{
					if( mPlayers[ i ]->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, pressedButtons[ button ] ) )
					{
						pressed = true;
						break;
					}
				...
				{
					b32 notPressed = true;
					if( notPressedButtons )
					{
						for( u32 button = 0; button < notPressedButtonsCount; ++button )
						{
							if( mPlayers[ i ]->fGameController( )->fButtonHeld( tUserProfile::cProfileCamera, notPressedButtons[ button ] ) )
							{
								notPressed = false;
								break;
							}
	...
	tPlayer* tGameApp::fAnyLocalPlayerWithButtonsDown( const u32* pressedButtons, u32 pressedButtonsCount, const u32* notPressedButtons, u32 notPressedButtonsCount )
	{
		return fAnyPlayerWithButtonsDown( pressedButtons, pressedButtonsCount, notPressedButtons, notPressedButtonsCount, true);
	}

	tPlayer* tGameApp::fAnyPlayerOpeningPauseMenu( )
	{
		u32 pressed = GameFlags::cGAME_CONTROLS_MENU_PAUSE;
		u32 notPressed = GameFlags::cGAME_CONTROLS_MENU_REWIND;
		return fAnyPlayerWithButtonsDown( &pressed, 1, &notPressed, 1, false );
	}
	...
	static const u32 cGameControlsNotPressedWhenPressingRewindButton[] =
	{
		GameFlags::cGAME_CONTROLS_SELECT,
		GameFlags::cGAME_CONTROLS_CANCEL,
		GameFlags::cGAME_CONTROLS_LAUNCH_WAVE,
		GameFlags::cGAME_CONTROLS_ENTER_EXIT_UNIT,
		GameFlags::cGAME_CONTROLS_MENU_PAUSE,
		GameFlags::cGAME_CONTROLS_UNIT_QUICKSWITCH_BEHIND
	};
	...
	tPlayer* tGameApp::fAnyPlayerOpeningRewindMenu( )
	{
		...
		{
			u32 pressed = GameFlags::cGAME_CONTROLS_MENU_REWIND;
			return fAnyPlayerWithButtonsDown( &pressed, 1, cGameControlsNotPressedWhenPressingRewindButton, array_length( cGameControlsNotPressedWhenPressingRewindButton ), false );
		}
		else
			return NULL;
	}
	...
	tPlayer* tGameApp::fHostPlayerOpeningRewindMenu( ) const
	{
		...
		for( u32 i = 0; i < mPlayers.fCount( ); ++i )
		{
			...
			if( mPlayers[ i ]->fIsActive( ) )
			{
				b32 pressed = mPlayers[ i ]->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_MENU_REWIND );

				if( pressed )
				{
					b32 notPressed = true;
					for( int button = 0; button < array_length( cGameControlsNotPressedWhenPressingRewindButton ); ++button )
					{
						if( mPlayers[ i ]->fGameController( )->fButtonHeld( tUserProfile::cProfileCamera, cGameControlsNotPressedWhenPressingRewindButton[ button ] ) )
						{
							notPressed = false;
							break;
						}
	...
	void tGameApp::fApplyRichPresence( )
	{
		...
		// TODO: HACK: Remove TS1 levels for Versus and Survival for now
		...
		else if( info.mMapType == GameFlags::cMAP_TYPE_HEADTOHEAD )
		{
			for( u32 i = 0; i < mLocalUsers.fCount( ); ++i )
			{
				...
				if( player != ~0 )
				{
					// Not set up for PC version and having more teams, because we don't use rich presence
	...
	u32 tGameApp::fDefaultTeamFromCountry( u32 country ) const
	{
		switch( country )
		{
		// setting up for the TS1 content - trying to keep just the two teams (blue and red)
		...
		case GameFlags::cCOUNTRY_BRITISH:
			return GameFlags::cTEAM_BLUE;
		case GameFlags::cCOUNTRY_GERMAN:
			return GameFlags::cTEAM_RED;
		case GameFlags::cCOUNTRY_FRENCH:
			return GameFlags::cTEAM_BLUE;
		}
...
//		}
...
#if defined( use_steam )
	bool tGameApp::fReportBug( )
	{
		SteamFriends( )->ActivateGameOverlayToWebPage( "http://steamcommunity.com/app/262120/discussions/0/540734792351421574/" );
		return true;
	}
#else
	bool tGameApp::fReportBug( )
	...
	void tLevelLoadInfo::fExportScriptInterface( tScriptVm& vm )
	{
		...
		classDesc
			...
			.Var( _SC( "Country2" ),			&tLevelLoadInfo::mCountry2 )
	...
	void tGameApp::fExportScriptInterface( tScriptVm& vm )
	{
		...
		classDesc
			...
			.Func( _SC( "FindLocalUserWithGameControlPressed" ),	&tGameApp::fFindLocalUserWithGameControlPressed )
			.Func( _SC( "FindLocalUserWithAnyPressStartInputPressed" ),	&tGameApp::fFindLocalUserWithAnyPressStartInputPressed )
			...
			.Prop( _SC( "ProductName" ),							&tGameApp::fGetProductName )
			.Prop( _SC( "ProductVersion" ),							&tGameApp::fGetProductVersion )
			.Func( _SC( "ReportBug" ),								&tGameApp::fReportBug )
			.Func( _SC( "UnitResourcePath" ),						&tGameApp::fUnitResourcePath )
			;
...
namespace Sig
{
	bool tGameApp::fGetProductAndVersionFromResource( )
	{
		...
		tDynamicArray<BYTE> data( dwSize );

		// load the version info
		if ( !GetFileVersionInfo( szFilename, NULL, dwSize, data.fBegin( ) ) )
		{
		...
		u32 iProductNameLen = 0;
		LPVOID pvProductVersion = NULL;
		u32 iProductVersionLen = 0;

		// replace "040904b0" with the language ID of your resources
		if ( !VerQueryValueW( data.fBegin( ), L"\\StringFileInfo\\040904b0\\ProductName", &pvProductName, &iProductNameLen ) ||
			!VerQueryValueW( data.fBegin( ), L"\\StringFileInfo\\040904b0\\ProductVersion", &pvProductVersion, &iProductVersionLen ) )
		{
			//TRACE( "Can't obtain ProductName and ProductVersion from resources\n" );
			return false;
		}

		mProductName = tLocalizedString( ( LPCWSTR )pvProductName );

		std::wstring temp = std::wstring( ( LPCWSTR )pvProductVersion );
		mProductVersion = tLocalizedString( temp );

		return true;
	}

	const tLocalizedString &tGameApp::fGetProductName( ) const
	{
		return mProductName;
	}

	const tLocalizedString &tGameApp::fGetProductVersion( ) const
	{
	...
	void tGameApp::fConfigureAppPlatform( tStartupOptions& opts, tPlatformStartupOptions& platformOptsOut )
	{
		...
		platformOptsOut.mDefaultCursor = MAKEINTRESOURCE( IDC_DEFAULTCURSOR );
		opts.mFixedTimeStep = 1.f / 30.f;
		opts.mUpdateDelay = opts.mFixedTimeStep;
		opts.mFrameTimeDeltaClamp = 4.0f * opts.mFixedTimeStep;
	}
}
