...
namespace Sig
{
	...
	public:
		...
		static const u32 cVersion = 15; // Please remove the fSaveLoad( mouseSensitivity ) call if you are updating this number ( and this comment ).
		...
		enum tControlProfile
		{
			...
			cProfileEnd,
			cProfileBegin = cProfileCharacters,
			cProfileCount = cProfileEnd - cProfileBegin
		};
		...
		b32 fSouthPaw( u32 profile )
		{
			...
			case cProfileUI: return false; break;
		...
		static const tStringPtr& fControlProfileEnumToString( u32 enumValue );
		static u32 fControlProfileStringToEnum( const tStringPtr& enumName );
	...
	public:
		...
		void fSaveLoad( tArchive& archive )
		{
			...
			u8 mouseSensitivity = 0;
			archive.fSaveLoad( mouseSensitivity ); // TODO: Remove on next version increase. Left in so that existing saves don't break.

			// Note: Adding or changing any members of this archive requires incrementing cVersion at the top of this class
		}
	};
