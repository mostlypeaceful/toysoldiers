...
namespace Sig
{
	...
	private:
		...
		f32 mReverseTimer;
		f32 mRestTimer;
		f32 mRobotAttackTimer;
		b32 mForceFireState;
		u32 mForceFireTier;
		f32 mForceFireTimer;
		f32 mForceFireTimerInterval;
		tEntityPtr mTrailer;
		b8 mIsTrailer;
		b8 mBurnout;
		b8 mParachuting;
		...
		void fDeployTurrets( );
		void fEnableForceFire( b32 enable ) { mForceFireState = enable; }
		void fSetForceFireTier( const u32 forceFireTier ) {  mForceFireTier = forceFireTier; }
		void fSetForceFireTimerInterval( const f32 forceFireTimerInterval ) {  mForceFireTimer = mForceFireTimerInterval = forceFireTimerInterval; }
		void fForceFireTurrets( );
		void fForceFireSecondaryTurrets( );
		void fForceFireTertiaryTurrets( );
		void fSetReverseTimer( const f32 reverseTimer ) {mReverseTimer = reverseTimer; }
		f32 fRestTimer( ) const { return mRestTimer; }
		void fSetRestTimer( const f32 restTimer );
		f32 fRobotAttackTimer( ) const { return mRobotAttackTimer; }
		void fSetRobotAttackTimer( const f32 robotAttackTimer );
