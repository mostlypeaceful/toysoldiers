...
namespace Sig
{
	...
	{
		// JPODESTA - Don't reset the viewport data here because this is called on Device Reset and this data needs to be the same after reset
		//		for( u32 i = 0; i < mPerViewportData.fCount( ); ++i )
	...
	void tGamePostEffectManager::fSetupFilmGrain( Gfx::tScreen& screen )
	{
		...
		for( u32 i = 0; i < mPerViewportData.fCount( ); ++i )
		{
			...
			Gfx::tPostEffectsMaterial::tParameters filmGrainParams;
			if( perVpData.mFilmGrainOnOverride )
				filmGrainParams = perVpData.mFilmGrainMaterial->mParameters;
			...
			// If there is a film grain override active, and the device is reset, restore the custom effect parameters.
			if( perVpData.mFilmGrainOnOverride )
				perVpData.mFilmGrainMaterial->mParameters = filmGrainParams;
