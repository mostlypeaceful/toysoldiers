...
namespace Sig
{
	...
	Gui::tRadialMenuPtr tVehicleLogic::fCreateSelectionRadialMenu( tPlayer& player )
	{
		...
		Gui::tRadialMenuPtr radialMenu = Gui::tRadialMenuPtr( NEW Gui::tRadialMenu( tGameApp::fInstance( ).fGlobalScriptResource( tGameApp::cGlobalScriptVehicleOptions ), player.fUser( ), player.fGameController( ) ) );
	...
	void tVehicleLogic::fAddRumbleEvent( const Input::tRumbleEvent& event )
	{
		for( u32 i = 0; i < mSeats.fCount( ); ++i )
		{
			...
			if( player ) player->fUser( )->fRawGamepad( ).fRumble( ).fAddEvent( event );
		}
	}
	...
	void tVehicleLogic::fSetExplicitRumble( f32 rumble )
	{
		...
		{
			tPlayer* player = fGetPlayer( i );
			if( player ) player->fUser( )->fRawGamepad( ).fRumble( ).fSetExplicitRumble( rumble );
		}
	}
	...
	{
		if( fSeatOccupied( cVehicleDriverSeat ) && fGetPlayer( cVehicleDriverSeat )->fGameController( )->fButtonDown( tUserProfile::cProfileVehicles, GameFlags::cGAME_CONTROLS_BOOST ) && mBoostEffect.fExists( ) )
		{
		...
		case GameFlags::cEVENT_UNIT_DESTROYED:
			{
				...
				// TS2: COLD WAR ACHIEVEMENTS
				if( mUnitID == GameFlags::cUNIT_ID_BOSS_FLYINGTANK && ( mCountry == GameFlags::cCOUNTRY_USA || mCountry == GameFlags::cCOUNTRY_USSR ) )
					tGameApp::fInstance( ).fAwardAchievementToAllPlayers( GameFlags::cACHIEVEMENTS_SHOCKING_RESULTS );
				else if( mUnitID == GameFlags::cUNIT_ID_BOSS_SUB && ( mCountry == GameFlags::cCOUNTRY_USA || mCountry == GameFlags::cCOUNTRY_USSR ) )
					tGameApp::fInstance( ).fAwardAchievementToAllPlayers( GameFlags::cACHIEVEMENTS_SUNK );
				else if( mUnitID == GameFlags::cUNIT_ID_BOSS_TANK && ( mCountry == GameFlags::cCOUNTRY_USA || mCountry == GameFlags::cCOUNTRY_USSR ) )
					tGameApp::fInstance( ).fAwardAchievementToAllPlayers( GameFlags::cACHIEVEMENTS_A_FEW_LOOSE_SCREWS );
				else if( mUnitID == GameFlags::cUNIT_ID_BOSS_SUPERTANK && ( mCountry == GameFlags::cCOUNTRY_USA || mCountry == GameFlags::cCOUNTRY_USSR ) )
				{
				...
				else if( mUnitID == GameFlags::cUNIT_ID_BOSS_MI12 && ( mCountry == GameFlags::cCOUNTRY_USA || mCountry == GameFlags::cCOUNTRY_USSR ) )
				{
					for( u32 i = 0; i < tGameApp::fInstance( ).fPlayerCount( ); ++i )
					{
						tPlayer* player = tGameApp::fInstance( ).fGetPlayer( i );
				...
				// TS1 ACHIEVEMENTS
				// K-WAGEN AND ROBOBOB ACHIEVEMENTS - can't set up properly ATM as the unitIDs aren't set up in data etc yet - probably best to actually set this up in the boss logic NUT file (like has been done for FLYING_PIG)
				/*
				else if( mUnitID == GameFlags::cUNIT_ID_BOSS_UBER && (mUnitCountry == GameFlags::cCOUNTRY_BRITISH || mUnitCountry == GameFlags::cCOUNTRY_GERMAN || mUnitCountry == GameFlags::cCOUNTRY_FRENCH))
				  tGameApp::fInstance( ).fAwardAchievementToAllPlayers( GameFlags::cACHIEVEMENTS_TS1_LAND_SHIP );
				else if( mUnitID == GameFlags::cUNIT_ID_BOSS_UBER && (mUnitCountry == GameFlags::cCOUNTRY_BRITISH || mUnitCountry == GameFlags::cCOUNTRY_GERMAN || mUnitCountry == GameFlags::cCOUNTRY_FRENCH))
				  tGameApp::fInstance( ).fAwardAchievementToAllPlayers( GameFlags::cACHIEVEMENTS_TS1_MR_ROBOTO );*/
	...
	void tVehicleLogic::fStopToDropCargo( )
	{
		if( mDroppingCargo.mStopWhileSpawning )
		{
			...
			// Make sure we have more waypoints.
			if( path->fCurrentPathSequence( ) != ~0u && path->fHasWaypoints( ) )
			{
	...
	void tVehicleLogic::fExportScriptInterface( tScriptVm& vm )
	{
		...
		}
		{
			...
			classDesc
				...
				.Prop(_SC("ThrottleScaleTarget"),			&tVehicleLogic::fThrottleScaleTarget, &tVehicleLogic::fSetThrottleScaleTarget)
