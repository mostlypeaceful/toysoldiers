...
namespace Sig
{
	...
	f32 tArtilleryBarrage::fProcessST( tPlayer* player, f32 dt )
	{
		...
		if( mLastProjLogic )
		{
			...
			{
				if( player->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_LAUNCH_BARRAGE ) )
	...
	f32 tRamboBarrage::fProcessST( tPlayer* player, f32 dt )
	{
		...
		if( mCharacter && player->fCurrentUnit( ) != mCharacter && ((canUse && mForceInto) || (!mForceInto && player->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_LAUNCH_BARRAGE ))) )
		{
	...
	f32 tUsableWaveLaunchBarrage::fProcessST( tPlayer* player, f32 dt )
	{
		...
		if( mUsable && (mForceInto || mPlayer->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_LAUNCH_BARRAGE )) )
		{
			mUsable = false;
			mForceInto = false;
	...
	public:
		...
		virtual f32 fProcessST( tPlayer* player, f32 dt )
		{
			...
			else
			{
				if( !mTargetSet )
				{
					if( player->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_CANCEL ) )
						fPushPopCamera( false );
					else if( player->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_LAUNCH_BARRAGE ) )
						fPushPopCamera( true );
				}

				if( mCameraPushed && mCamera->fHasBlendedIn( ) && player->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_SELECT ) )
				{
					sigassert( mCamera );
	...
	public:
		...
		virtual f32 fProcessST( tPlayer* player, f32 dt )
		{
			...
			else
			{
				...
				{
					if( player->fGameController( )->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_SELECT ) )
						fFire( true );
					else if( player->fGameController( )->fButtonUp( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_SELECT ) )
						fFire( false );
				}
			}
