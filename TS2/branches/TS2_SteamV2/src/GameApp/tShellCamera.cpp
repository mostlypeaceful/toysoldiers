...
namespace Sig
{
		...
		, mInputFilter( ~0 )
	{
		...
		mInputFilter = mPlayer.fUser( )->fIncInputFilterLevel( );
		mPlayer.fGameController( )->fPushMouseCursorRestrictFilterLevel( mInputFilter );
	}
	...
	tShellCamera::~tShellCamera( )
	{
		...
		if( mInputFilter != ~0 )
		{
			mPlayer.fGameController( )->fPopMouseCursorRestrictFilterLevel( mInputFilter );
			mPlayer.fUser( )->fDecInputFilterLevel( mInputFilter );
			mInputFilter = ~0;
		}
	}
	...
	void tShellCamera::fUserTick( f32 dt, Gfx::tTripod& tripod )
	{
		...
		if( mProjLogics.fCount( ) )
		{
			...
			for( u32 i = 0; i < mProjLogics.fCount( ); ++i )
			{
				lean = mProjLogics[ i ]->fUpdateShellCam( mPlayer, dt, mInputFilter );
