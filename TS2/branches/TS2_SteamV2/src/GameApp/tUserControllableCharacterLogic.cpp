...
namespace Sig
{
		...
		if( fHasSelectionFlag( ) )
		{
			if( fUnderUserControl( ) )
			{
				if( fIsCommando( ) )
				{
					...
					else if( fCountry( ) == GameFlags::cCOUNTRY_USSR )
						fTeamPlayer( )->fStats( ).fIncQuick( GameFlags::cSESSION_STATS_TIME_USING_IVAN, dt );
					//HACK for TS1 content - need to know correct session stats to use for TS1 content
					else if( fCountry( ) == GameFlags::cCOUNTRY_BRITISH )
						fTeamPlayer( )->fStats( ).fIncQuick( GameFlags::cSESSION_STATS_TIME_USING_COMMANDO, dt );
					else if( fCountry( ) == GameFlags::cCOUNTRY_GERMAN )
						fTeamPlayer( )->fStats( ).fIncQuick( GameFlags::cSESSION_STATS_TIME_USING_COMMANDO, dt );
					else if( fCountry( ) == GameFlags::cCOUNTRY_FRENCH )
						fTeamPlayer( )->fStats( ).fIncQuick( GameFlags::cSESSION_STATS_TIME_USING_COMMANDO, dt );
				}
			}
			else
	...
	{
		Gui::tRadialMenuPtr radialMenu = Gui::tRadialMenuPtr( NEW Gui::tRadialMenu( tGameApp::fInstance( ).fGlobalScriptResource( tGameApp::cGlobalScriptInfantryOptions ), player.fUser( ), player.fGameController( ) ) );
	...
	void tUserControllableCharacterLogic::fComputeUserInput( f32 dt )
	{
		const tGameControllerPtr gc = fTeamPlayer( )->fGameController( );
		tVec2f moveStick = gc->fMoveStick( tUserProfile::cProfileCharacters );
		tVec2f aimStick = gc->fAimStick( tUserProfile::cProfileCharacters );
		moveStick.x = -moveStick.x;

		if( mExtraMode )
		{
			fComputeUserInputExtra( moveStick, aimStick, dt );
		}
		else
		{
	/*		if( mVehicleBaseStarted )
		...
		if( !mExtraMode || !fInVehicle( ) )
		{
			...
			// jumping
			if( !mFirstUserInput && gc->fButtonDown( tUserProfile::cProfileCharacters, GameFlags::cGAME_CONTROLS_JUMP ) )
			{
			...
			if( !mSprinting )
			{
				if( gc->fButtonDown( tUserProfile::cProfileCharacters, GameFlags::cGAME_CONTROLS_BOOST )
		...
		if( fHasWeaponStation( 0 ) )
		{
			...
			if( gc->fButtonDown( tUserProfile::cProfileCharacters, rocketBank->fTriggerButton( ) ) && rocketBank->fShouldFire( ) ) 
				rocketBank->fFire( );
			else if( gc->fButtonUp( tUserProfile::cProfileCharacters, rocketBank->fTriggerButton( ) ) ) 
				rocketBank->fEndFire( );
	...
	void tUserControllableCharacterLogic::fComputeUserInputExtra( const Math::tVec2f& moveStick, const Math::tVec2f& aimStick, f32 dt )
	{
		if( !fInVehicle( ) )
		{
			if( mLookToggle ) 
			...
			if( fTeamPlayer( )->fGameController( )->fButtonDown( tUserProfile::cProfileCharacters, GameFlags::cGAME_CONTROLS_CAMERA_CYCLE ) )
			{
				if( mLookToggle )
				{
					mAimBlendDownUp = 0.f;
	...
	void tUserControllableCharacterLogic::fHandleEnterExitVehicle( )
	{
		if( !fInVehicle( ) )
		{
			if( mNearestVehicle && fTeamPlayer( )->fGameController( )->fButtonDown( tUserProfile::cProfileCharacters, GameFlags::cGAME_CONTROLS_ENTER_EXIT_UNIT ) )
			{
		...
		{
			sigassert( mRidingVehicle );
			if( fTeamPlayer( )->fGameController( )->fButtonDown( tUserProfile::cProfileCharacters, GameFlags::cGAME_CONTROLS_ENTER_EXIT_UNIT ) )
			{
	...
	class tInfantryContinuousFireUserGoal : public tInfantryContinuousFireGoal
	{
		...
		virtual void fOnProcess( AI::tGoalPtr& goalPtr, tLogic* logic, f32 dt )
		{
			...
			const tGameControllerPtr gc = fLogic( )->fTeamPlayer( )->fGameController( );
			const tWeaponBankPtr& gunBank = fLogic( )->fWeaponStation( 0 )->fBank( 0 );

			if( !gc->fButtonHeld( tUserProfile::cProfileCharacters, gunBank->fTriggerButton( ) ) )
			{
				fMarkAsComplete( );
			}
		...
		virtual void fOnProcess( AI::tGoalPtr& goalPtr, tLogic* logic, f32 dt )
		{
			if( fLogic( )->fTeamPlayer( ) )
			{
				...
				if( tp && fLogic( )->fHasWeaponStation( 0 ) )
				{
					...
					const tGameControllerPtr gc = tp->fGameController( );
					if( fLogic( )->fExtraMode( ) ) 
						fLogic( )->fHandleEnterExitVehicle( );
					...
					if( gc->fButtonDown( tUserProfile::cProfileCharacters, GameFlags::cGAME_CONTROLS_MELEE ) )
						logic->fHandleLogicEvent( Logic::tEvent( GameFlags::cEVENT_USER_MELEE ) );
				}
			}
