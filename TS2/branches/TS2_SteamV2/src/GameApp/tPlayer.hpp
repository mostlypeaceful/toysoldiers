...
namespace Sig
{
	...
	define_smart_ptr( base_export, tRefCounterPtr, tPlayer );
	...
	public:
		...
		const tUserProfilePtr& fProfile( ) const { return mProfile; }
		tUserProfile* fProfileFromScript( ) const { return mProfile.fGetRawPtr( ); }
	...
	public:
		...
		f32 fTimeSinceUnitExit( ) { return mTimeSinceUnitExit; }
	...
	private:
		...
		// Followpathcam is done, but we're waiting for input to switch cameras.
		b32 mWaitingToLeaveFollowPathCam;
		b32 mStepCameras;

		// For display case:
		Sqrat::Function mSelectedUnitChangedCallback;
		Sqrat::Function mCurrentUnitChangedCallback;
		...
		tAchievementsReaderPtr mAchievementsReader;

		f32 mTimeSinceUnitExit;
	};
}
