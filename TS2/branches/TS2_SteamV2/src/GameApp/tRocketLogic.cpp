...
namespace Sig
{
	...
	f32 tRocketLogic::fUpdateShellCam( tPlayer& player, f32 dt, u32 inputFilter )
	{
		mGuidanceMode = cStraightFire;
		mInitialDistanceToTarget = 0.f;

		const tVec2f aimStick = Input::tGamepad::fMapStickCircleToRectangle( player.fGameController( )->fAimStick( tUserProfile::cProfileShellCam, inputFilter ) );
		const tVec2f speedStick = Input::tGamepad::fMapStickCircleToRectangle( player.fGameController( )->fMoveStick( tUserProfile::cProfileShellCam, inputFilter ) );
			...
			virtual void fMoveST( f32 dt )
			{
				...
				if( mWaitingToPass < mTargets.fCount( ) && !mEntityHitWithLogic )
				{
					...
					else if( mShellCamera && dist < Gameplay_Weapon_ShellCam_RocketHall_VibeDist )
					{
						...
						mShellCamera->fPlayer( ).fUser( )->fRawGamepad( ).fRumble( ).fSetExplicitRumble( Gameplay_Weapon_ShellCam_RocketHall_VibeStrength * scale );
					}
				}
			}
