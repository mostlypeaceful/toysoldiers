...
namespace Sig
{
	...
	namespace
	{
		void fUserStatsText( std::ostream& o, u32 i, tUser* user )
		{
			...
			o << ( ( user->fAddOnsLicensed( ) & GameFlags::cDLC_BRITISH )? "b" : "-" );
			o << ( ( user->fAddOnsLicensed( ) & GameFlags::cDLC_GERMAN )? "h" : "-" );
			o << ( ( user->fAddOnsLicensed( ) & GameFlags::cDLC_INVASION )? "i" : "-" );
			o << ( ( user->fAddOnsLicensed( ) & GameFlags::cDLC_KAISER )? "k" : "-" );
			o << ")" << std::endl;
		}
	}
	...
	void tGameAppSession::fPollInputDevices( tUserArray & localUsers )
	{
			...
			( mGameSessionNetwork.fIsInactive( ) || !mGameSessionNetwork.fConnectionCount( ) ) )
		{
			...
			else if( !mLoadingLevel )
			{
				...
				if( !mReplayer->fHasInputAvailableGamepad( ) && !mReplayer->fHasInputAvailableKeyboard( ) && !mReplayer->fHasInputAvailableMouse( ) )
				{
					gameApp.fQuitAsync( );
				}
				else
				{
					...
					sigassert( playerCount == mReplayer->fGetInputCountGamepad( ) );	
					sigassert( playerCount == mReplayer->fGetInputCountKeyboard( ) );	
					sigassert( playerCount == mReplayer->fGetInputCountMouse( ) );	
					for( u32 p = 0; p < playerCount; ++p )
					{
						players[ p ]->fUser( )->fAddGamepadState( mReplayer->fGetInputGamepad( p ) );
						players[ p ]->fUser( )->fAddKeyboardState( mReplayer->fGetInputKeyboard( p ) );
						players[ p ]->fUser( )->fAddMouseState( mReplayer->fGetInputMouse( p ) );
					}
				}
			}
		}
		...
		else if( mGameSessionNetwork.fIsRunning( ) && fAllPeersReadyForLevelStart( ) )
		{
			const f32 frameDelta = gameApp.fGetFrameDeltaTime( );
	...
	void tGameAppSession::fAddToStatText( std::stringstream & statsText )
	{
		...
		statsText << "   SteamID = " << tPlatformUserId( SteamUser( )->GetSteamID( ) ) << std::endl;
#endif
		...
		statsText << "simulate: " << ( tGameApp::fInstance( ).fSimulate( ) ? "true" : "false" ) << std::endl;
	}
	...
	//------------------------------------------------------------------------------
	void tGameAppSession::fOnLevelSpawnBegin( )
	{
		...
		if( gameApp.fCurrentLevelLoadInfo( ).mGameMode.fIsNet( ) )
		{
			...
			if( mGameSessionNetwork.fIsRunningWithConnections( ) )
			{
				...
				// Disable simulation until all peers are ready
				gameApp.fSetSimulate( false );
#endif
				gameApp.fEnableFixedTimeStep( true );
	...
	//------------------------------------------------------------------------------
	void tGameAppSession::fOnLevelSpawnEnd( )
	{
		log_line( Log::cFlagSession, __FUNCTION__ );
		mLoadingLevel = false;
		mLevelUnloaded = false;
		...
		else
		{
			...
			gameApp.fSetSimulate( true );
#if !defined( build_debug ) // debug builds force fixed time step.
			gameApp.fEnableFixedTimeStep( false );
#endif
		}
	...
	void tGameAppSession::fBeginGameSearch( tUser& user, u32 gameType, u32 gameMode )
	{
		...
		// Only 
		tUserData gameVersion;
		gameVersion.fSet( ( VERSION_MAJOR << 24 ) | ( VERSION_MINOR << 16 ) | ( VERSION_REVISION << 8 ) );
		...
		user.fSetContext( tUser::cUserContextGameType, gameType );
		user.fSetContext( tUser::cUserContextGameMode, gameMode );
		user.fSetProperty( tUser::cUserPropertyGameVersion, gameVersion );

		mGameSessionSearch->fSetProperty( tUser::cUserPropertyGameVersion, gameVersion );
	...
	void tGameAppSession::fAddGameModeFlags( tLevelLoadInfo& info )
	{
		...
		else if( ( sessionGameMode == GameSession::cContextGameModeSurvival ) && ( ( info.mDlcNumber == GameFlags::cDLC_COLD_WAR ) || ( info.mDlcNumber == GameFlags::cDLC_EVIL_EMPIRE ) || ( info.mDlcNumber == GameFlags::cDLC_NAPALM ) ) ) // Only TS2 Survival maps support co-op
			info.mGameMode.fAddCoOpFlag( );
	...
	{
		if( !Game_Record || !mGameSessionNetwork.fHasSession( ) || mReplayer || ( mGameSessionNetwork.fFilledSlots( ) == 0 ) )
			return;
		...
		if( mGameSessionNetwork.fRemoteUserCount( ) )
		{
			...
			for( u32 p = 0; p < playerCount; ++p )
			{
				...
				if( player->fUser( )->fIsLocal( ) )
				{
					...
					tGameArchiveSave gameControllerArchive;
					player->fGameController( )->fSaveLoad( gameControllerArchive );
					info.mUserId = player->fUser( )->fPlatformId( );
					info.mProfile.fInitialize( archive.fBuffer( ).fBegin( ), archive.fBuffer( ).fTotalSizeOf( ) );
					info.mGameControllerConfig.fInitialize( gameControllerArchive.fBuffer( ).fBegin( ), gameControllerArchive.fBuffer( ).fTotalSizeOf( ) );

					log_line( Log::cFlagSession, "Sending PlayerInfo" );
		...
		for( u32 p = 0; p < playerCount; ++p )
		{
			if( !players[ p ]->fUser( )->fIsLocal( ) )
			{
				...
				if( data && data->mProfile != players[ p ]->fProfile( ) )
				{
					...
					tGameArchiveLoad gameControllerConfig( data->mGameControllerConfigData.fBegin( ), data->mGameControllerConfigData.fTotalSizeOf( ) );
					players[ p ]->fGameController( )->fSaveLoad( gameControllerConfig );
				}
			}
		}
	}
	...
	//------------------------------------------------------------------------------
	bool tGameAppSession::fAllPeersReadyForLevelStart( )
	{
		return !!fReadyForInput( );
	}
	...
	void tGameAppSession::fTryCaptureInputFrame( tUserArray & localUsers )
	{
		...
		if( !mReplayer )
		{
			...
			for( u32 u = 0; u < userCount; ++u )
			{
				...
				gamepad.fCaptureStateUnbuffered( frame.mStateDataGamepad, user->fLocalHwIndex( ), frameDelta );

				Input::tKeyboard keyboard;
				keyboard.fCaptureStateUnbuffered( frame.mStateDataKeyboard, frameDelta );

				Input::tMouse mouse;
				mouse.fStartup( const_cast<Input::tMouse&>( user->fRawMouse( ) ).fWindowHandle( ) );
				mouse.fCaptureStateUnbuffered( frame.mStateDataMouse, frameDelta );

				fPushInputFrame( frame );
				fSendMsgToPeers( mGameSessionNetwork, frame, cGameChannelDefault, true );

				//log_line( 0, "User " << user->fLocalHwIndex( ) << " fTryCaptureInputFrame  " << frame.mStateDataMouse.mCursorPosX << " " << frame.mStateDataMouse.mCursorPosY );
			}
		...
		else if( !mLoadingLevel )
		{
			...
			else
			{
				...
				for( u32 p = 0; p < playerCount; ++p )
				{
					...
					frame.mStateDataGamepad = mReplayer->fGetInputGamepad( p );
					frame.mStateDataKeyboard = mReplayer->fGetInputKeyboard( p );
					frame.mStateDataMouse = mReplayer->fGetInputMouse( p );
					fPushInputFrame( frame );
					fSendMsgToPeers( mGameSessionNetwork, frame, cGameChannelDefault, true );
				}
			}
	...
	void tGameAppSession::fTryApplyInputFrame( tUserArray & localUsers )
	{
		...
		for( u32 u = 0; u < localUserCount; ++u )
		{
			...
			user->fAddGamepadState( frame.mStateDataGamepad );
			user->fAddKeyboardState( frame.mStateDataKeyboard );
			user->fAddMouseState( frame.mStateDataMouse );
	...
	{
		log_line( Log::cFlagSession, __FUNCTION__ " userId " << userId );
		...
		for( u32 i = 0; i < inputCount; ++i )
		{
			...
			if( test.mUserId == tUser::cInvalidUserId )
			{
				test.mUserId = userId;
#if defined( platform_pcdx )
				if( fAllPeersReadyForLevelStart( ) )
				{
					// If all peers are ready, start the simulation
					tGameApp::fInstance( ).fSetSimulate( true );
				}
#endif
				return;
			}
	...
	void tGameAppSession::fOnError( u32 failedState )
	{
		...
		if( !mStartingReplaySession )
		{
			...
			{
#if defined( use_steam )
			case tGameSessionNetwork::cStateRequestingLobbyData:
				...
				fSendSessionEvent( ApplicationEvent::cSessionInviteNeedSameVersion, true );
	...
	b32  tGameAppSession::fIsCompatible( )
	{
		if( mGameSessionNetwork.fSessionDetails( ).mGameVersion != ( ( VERSION_MAJOR << 24 ) | ( VERSION_MINOR << 16 ) | ( VERSION_REVISION << 8 ) ) )
		{
			return false;
		}
	...
	namespace{
		tStringPtr fEventIdToString( u32 id )
		{
			...
			sEventNames[ ApplicationEvent::cSessionInviteNeedSameVersion ] = tStringPtr( "SESSION_INVITE_NEED_SAME_VERSION" );
		...
		const Net::tPeer & peer, u32 channelId, const byte * data, u32 dataLength )
	{
		...
		log_line( Log::cFlagSession, __FUNCTION__ " " << ready.mUserId );
		fMarkReadyForInput( ready.mUserId );
	}
		...
		const Net::tPeer & peer, u32 channelId, const byte * data, u32 dataLength )
	{
		...
		clientData.mGameControllerConfigData.fInitialize( info.mGameControllerConfig.fBegin( ), info.mGameControllerConfig.fTotalSizeOf( ) );
...
namespace Sig
{
	...
	void tGameAppSession::fExportScriptInterface( tScriptVm& vm )
	{
		...
		classDesc
			...
			.Prop( _SC( "AllPeersReadyForLevelStart" ),		&tGameAppSession::fAllPeersReadyForLevelStart )
