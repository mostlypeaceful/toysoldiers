...
namespace Sig
{
	...
	void tVehicleLagCamera::fUserTick( f32 dt, Gfx::tTripod& tripod )
	{
		...
		else
		{
			...
			if( gc->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_CAMERA_CENTER ) )
				mCenterLook = true;

			if( gc->fButtonDown( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_AIM_MINMAG ) )
			{
				mCenterLook = false;
			...
			if( gc->fButtonHeld( tUserProfile::cProfileCamera, GameFlags::cGAME_CONTROLS_AIM_MINMAG ) )
			{
				mCenterLook = false;
