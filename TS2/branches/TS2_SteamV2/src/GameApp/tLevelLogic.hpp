...
namespace Sig
{
	...
	public:
		...
		u32 mControlProfile; // used for input events to determine which control profile to check
		u32 mDamageType;

		tTutorialEvent( u32 id = ~0, u32 value = ~0, u32 currentUnitID = ~0, b32 shellCaming = false, u32 eventUnitType = ~0, tEntity* ent = NULL, tPlayer* player = NULL, GameFlags::tDAMAGE_TYPE dt = GameFlags::cDAMAGE_TYPE_COUNT )
			: mEventID( id )
			, mEventValue( value )
		...
		{ 
			return other 
				...
				&& ( other->mControlProfile == ~0 || mControlProfile == other->mControlProfile ); 
		}
	...
	public:
		...
		b32	fLightUpOnlyTutorialPlatforms( ) { return mLightUpOnlyTutorialPlatforms; }
	...
	private:
		...
		u32 mRewindCount;
		f32 mStopWavesTimer;
		f32 mLastMoveStickMag;

		// Showcase stuff
		tEntityPtr mBackDrop;
		tEntityPtr mUSAFlag;
		tEntityPtr mUSSRFlag;
		//tEntityPtr mBRITISHFlag;
		//tEntityPtr mGERMANFlag;
		//tEntityPtr mFRENCHFlag;
