...
namespace Sig
{
	...
	void tLevelLogic::fThinkST( f32 dt )
	{
		...
		if( mBackDropCamera )
		{
			...
			if( mBackDropCamera->fIsActive( ) )
			{
				...
				fIncCameraAngle( -gc->fAimStick( tUserProfile::cProfileCamera ).x * cPiOver4 * dt );

				f32 mag = gc->fMoveStickMagnitude( tUserProfile::cProfileCamera );
				if( mLastMoveStickMag <= 0.1f && mag > 0.0f )
				{
					f32 moveStick = -gc->fMoveStick( tUserProfile::cProfileCamera ).x;
	...
	{
		if( mWaveManager )
			mWaveManager->fShowWaveListUI( show );
	}
	...
	namespace
	{
		...
		static const tStringPtr cBRITISHFlag( "britishFLAG" );
		static const tStringPtr cGERMANFlag( "germanFLAG" );
		static const tStringPtr cFRENCHFlag( "frenchFLAG" );
	...
	void tLevelLogic::fSpawnShowcaseTurrets( )
	{
		...
		//mBRITISHFlag.fReset( fNamedObject( cBRITISHFlag ) );
		//sigassert( mBRITISHFlag && "Need backdrop named: britishFLAG" );
		//mGERMANFlag.fReset( fNamedObject( cGERMANFlag ) );
		//sigassert( mGERMANFlag && "Need backdrop named: germanFLAG" );
		//mFRENCHFlag.fReset( fNamedObject( cFRENCHFlag ) );
		//sigassert( mFRENCHFlag && "Need backdrop named: frenchFLAG" );
	...
	void tLevelLogic::fToggleDisplayCaseCountry( )
	{
		...
		else if( mDisplayCaseCountry == GameFlags::cCOUNTRY_USSR )
			mDisplayCaseCountry = GameFlags::cCOUNTRY_BRITISH;
		else if( mDisplayCaseCountry == GameFlags::cCOUNTRY_BRITISH )
			mDisplayCaseCountry = GameFlags::cCOUNTRY_GERMAN;
		else if( mDisplayCaseCountry == GameFlags::cCOUNTRY_GERMAN )
			mDisplayCaseCountry = GameFlags::cCOUNTRY_FRENCH;
		else if( mDisplayCaseCountry == GameFlags::cCOUNTRY_FRENCH )
			mDisplayCaseCountry = GameFlags::cCOUNTRY_USA;
	...
	void tLevelLogic::fApplyAngles( )
	{
		...
		if( mWaitOneFrameToSwitchCountry != ~0 && --mWaitOneFrameToSwitchCountry == 0 )
		{
			...
			//Gfx::tRenderableEntity::fSetDisabled( *mBRITISHFlag, mDisplayCaseCountry == GameFlags::cCOUNTRY_USSR );
			//Gfx::tRenderableEntity::fSetDisabled( *mGERMANFlag, mDisplayCaseCountry == GameFlags::cCOUNTRY_BRITISH );
			//Gfx::tRenderableEntity::fSetDisabled( *mFRENCHFlag, mDisplayCaseCountry == GameFlags::cCOUNTRY_GERMAN );
		}
	}
	...
	void tLevelLogic::fIncRankProgress( tPlayer* player, s32 val )
	{
		...
		tUserProfilePtr profile = player->fProfile( );
		sigassert( profile );
		...
		for( u32 p = 0; p < playerCount; ++p ) // for now
		{
			...
			if( !scores->fIsGoalComplete( index ) )
			{
				...
				for( u32 i = 0; i < numLevels; ++i )
				{
					...
					//if( info.mDlcNumber == fDlcNumber( ) ) // Earn 11/22 decorations from any levels, not just from the same campaign/dlc.
					{
	...
	{
		{
			...
			classDesc
				...
				.Var( _SC( "ControlProfile" ),	&tTutorialEvent::mControlProfile )
				.Var( _SC( "DamageType" ),		&tTutorialEvent::mDamageType )
				;
		...
		}
		{
			...
			classDesc
				...
				.Func(_SC("GoalBox"),					&tLevelLogic::fGoalBox)
