...
namespace Sig
{
	...
	{ 
		// Fire if we "can", and we already are or we're pointing at the target. being under user control, or ai with a target
		return fCanFire( ) && 
	...
	b32 tWeapon::fShouldShellCam( ) const
	{
		if( mPlayer )
		{
			...
			if( mShellCamPushed )
			{
				u32 shellCamIndex = mPlayer->fCameraStack( ).fIndexOfType<tShellCamera>( );
				if( ( shellCamIndex != ~0 ) && ( shellCamIndex > 0 ) )
				{
					tShellCamera* shellCam = mPlayer->fCameraStack( )[ shellCamIndex ].fDynamicCast<tShellCamera>( );
					if( shellCam )
					{
						filterLevel = shellCam->fInputFilterLevel( );
					}
				}
			}

			return ( mDesc.mFireMode == cWeaponFireModeOneShotMinigameMode ) || mPlayer->fGameController( )->fButtonHeld( tUserProfile::cProfileShellCam, mInst.mTriggerButton, filterLevel );
		}
	...
	void tWeapon::fUpdateTargetRelatedData( f32 dt )
	{
		mWithinReach = false;
		mReticalOverTarget = false;
		...
		{
			if( !fUnderUserControl( ) )
				mPredictedTargetPosition = fPredictTargetPosition( *target, dt );
		...
		if( fUnderUserControl( ) && mShellCamEligible && mLastProjectiles.fCount( ) && (!mBehaveContinuously || fReloading( ) || fNeedsReload( )) )
		{
			...
			if( mShellCaming || fShouldShellCam( ) )
			{
				...
				if( !mShellCamPushed && mShellCamInitiateTimer >= mDesc.mShellCamInitiateTimer )
				{
					...
					if( level )
					{
						tTutorialEvent tutorialEvent( GameFlags::cTUTORIAL_EVENT_INPUT, GameFlags::cGAME_CONTROLS_TRIGGER_PRIMARY, ~0, 1 ); 
						tutorialEvent.mControlProfile = tUserProfile::cProfileShellCam;
						level->fHandleTutorialEvent( tutorialEvent );
					}
				}
			}
			else
	...
	tVec3f tWeapon::fPredictTargetPosition( const tEntityPtr& target, f32 dt )
	{
		...
		if( mDesc.mLeadTargets && (unit = target->fLogicDerived< tUnitLogic >( )) )
		{
			...
			f32 time = fEstimateTimeToImpact( currentPos ) + dt * 2.f;
			...
			//f32 time = fEstimateTimeToImpact( currentPos ) + dt;
	...
	void tWeapon::fProcessShellBursting( f32 dt )
	{
		if( mPlayer )
		{
			...
			if( mPlayer->fGameController( )->fButtonHeld( tUserProfile::cProfileShellCam, GameFlags::cGAME_CONTROLS_TRIGGER_SECONDARY, filterLevel ) )
			{
