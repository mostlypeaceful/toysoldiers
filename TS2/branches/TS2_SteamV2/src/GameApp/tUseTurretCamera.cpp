...
namespace Sig
{
	...
	void tUseTurretCamera::fUserTick( f32 dt, Gfx::tTripod& tripod )
	{
		...
		{
			f32 targetScope = 0.f;
			if( fPlayer( ).fGameController( )->fButtonHeld( tUserProfile::cProfileTurrets, GameFlags::cGAME_CONTROLS_TRIGGER_SECONDARY ) )
				targetScope = 1.f;
	...
	void tUseTurretCamera::fUpdateZoom( f32 dt )
	{
		...
		{
			tGameControllerPtr gc = fPlayer( ).fGameController( );
			f32 yVal = 0.0f;
			if( gc->fMode( ) == tGameController::KeyboardMouse )
			{
				if( gc->fButtonDown( tUserProfile::cProfileTurrets, GameFlags::cGAME_CONTROLS_CAMERA_MOVE_IN ) || gc->fButtonHeld( tUserProfile::cProfileTurrets, GameFlags::cGAME_CONTROLS_CAMERA_MOVE_IN ) )
				{
					yVal = 1.0f;
				}
				else if( gc->fButtonDown( tUserProfile::cProfileTurrets, GameFlags::cGAME_CONTROLS_CAMERA_MOVE_OUT ) || gc->fButtonHeld( tUserProfile::cProfileTurrets, GameFlags::cGAME_CONTROLS_CAMERA_MOVE_OUT ) )
				{
					yVal = -1.0f;
				}
			}
			else
			{
				...
				tVec2f stick = gc->fMoveStick( tUserProfile::cProfileTurrets );
				yVal = stick.y;
				if( inversion )
					yVal *= -1.f;
			}
