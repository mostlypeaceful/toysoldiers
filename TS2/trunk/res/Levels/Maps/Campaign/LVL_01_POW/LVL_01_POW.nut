...
{
	RegisterLevelLogic( entity, LevelLogic_Campaign_lvl_pow( ) ) // RENAME to current level (e.g. LevelLogic_Campaign_lvl01)
}

class LevelLogic_Campaign_lvl_pow extends GameStandardLevelLogic // RENAME to current level (e.g. LevelLogic_Campaign_lvl01)
{
	wave01 = null
	wave02 = null
	wave03 = null
	...
	function OnSpawn( )
	{
		...
		wave02 = AddWaveList( "AllyWaveList01" )
		wave03 = AddWaveList( "EnemyWaveList01" )

		LevelIntro2P( "nis_intro_campath", "nis_intro_campath_02", null, true, false ) // not skipable due to ramp
		onLevelIntroComplete = EndOfCamera
		
		//shoot down 3 helicopters using commando.
		//earn a barrage with a machine gun turret.
		//rank: machine gun ii kills
		InitDecoration( 0, 3, function( event ) { return PlayerKilledTheseUnitsWithOneOfTheseUnits( event, [ UNIT_TYPE_AIR ], [ UNIT_ID_INFANTRY_OFFICER_01 ] ) }.bindenv( this ) )
		InitDecoration( 1, null, function( event ) { return event.EventID == TUTORIAL_EVENT_BARRAGE_RECEIVED
			&& ( event.CurrentUnitID == UNIT_ID_TURRET_MG_01 || event.CurrentUnitID == UNIT_ID_TURRET_MG_02 || event.CurrentUnitID == UNIT_ID_TURRET_MG_03 ) }.bindenv( this ) )
		InitChallenge( function( event ) { return PlayerKilledTheseUnitsWithOneOfTheseUnits( event, [ UNIT_TYPE_INFANTRY, UNIT_TYPE_VEHICLE, UNIT_TYPE_AIR ], [ UNIT_ID_TURRET_MG_02 ] ) }.bindenv( this ) )
	}
	...
	function SpawnPreIntroWaves( )
	{
		...
		if( ::GameApp.GetDifficulty( ) != DIFFICULTY_GENERAL )
		{
			...
			player.GiveInstantBarrage( "BARRAGE_RAMBO", NamedObject( "ramboDropFirst" ) )
	...
	{
		//specialLevelObject.Logic.RegisterForLevelEvent( LEVEL_EVENT_UNIT_DESTROYED, OnTurretDestroyed.bindenv( this ) )
