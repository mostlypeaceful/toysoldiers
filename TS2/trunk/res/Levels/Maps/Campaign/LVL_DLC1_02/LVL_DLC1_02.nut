...
{
	RegisterLevelLogic( entity, LevelLogic_Campaign_lvl04( ) ) // RENAME to current level (e.g. LevelLogic_Campaign_lvl01)
}

class LevelLogic_Campaign_lvl04 extends GameStandardLevelLogic // RENAME to current level (e.g. LevelLogic_Campaign_lvl01)
{
	wave01 = null
	...
	function OnSpawn( )
	{
		...
		//-Biggs Arclighter: Call in a Bombing Run.
		//-Master of Unloading: Destroy a vehicle with a Machine Gun turret.
		// Challenge: Mortar III kills
		InitDecoration( 0, null, function( event ) { return event.EventID == TUTORIAL_EVENT_BARRAGE_ACTIVATED && event.WeaponID == "BARRAGE_B52" }.bindenv( this ) )
		InitDecoration( 1, 1, function( event ) { return PlayerKilledTheseUnitsWithOneOfTheseUnits( event, [ UNIT_TYPE_VEHICLE ], [ UNIT_ID_TURRET_MG_01, UNIT_ID_TURRET_MG_02, UNIT_ID_TURRET_MG_03 ] ) }.bindenv( this ) )
		InitChallenge( function( event ) { return PlayerKilledTheseUnitsWithOneOfTheseUnits( event, [ UNIT_TYPE_INFANTRY, UNIT_TYPE_VEHICLE, UNIT_TYPE_AIR ], [ UNIT_ID_TURRET_MORTAR_03 ] ) }.bindenv( this ) )
	}
	...
	function EndOfCamera( )
	{
		...
		if( ::GameApp.GetDifficulty( ) != DIFFICULTY_GENERAL )
		{
			...
			player.LockInUnit( "start_helicopter" )
