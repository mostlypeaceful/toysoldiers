...
namespace Sig
{
	...
	namespace
	{
		...
		static tFixedArray<tVec4f, GameFlags::cTEAM_COUNT> cCaptureTints;
	...
	void tBuildSiteLogic::fOnSpawn( )
	{
		...
		if( mCaptureShape )
		{
			...
			cCaptureTints[ GameFlags::cTEAM_RED ] = tVec4f( 1, 0, 0, 0 );
			cCaptureTints[ GameFlags::cTEAM_BLUE ] = tVec4f( 0, 0, 1, 0 );
	...
	{
		Gui::tRadialMenuPtr radialMenu = Gui::tRadialMenuPtr( NEW Gui::tRadialMenu( tGameApp::fInstance( ).fGlobalScriptResource( tGameApp::cGlobalScriptBuildSiteOptions ), player.fUser( ) ) );
