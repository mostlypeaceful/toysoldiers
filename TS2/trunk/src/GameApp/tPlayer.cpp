...
namespace Sig
{
	...
	void AAACheatsUnlockAllLevels( tDevCallback::tArgs& args )
	{
		...
		for( u32 i = 0; i < players.fCount( ); ++i )
		{
			if( players[ i ]->fUser( ).fGetRawPtr( ) == args.mUser )
			{
				tUserProfile* profile = players[ i ]->fProfile( );
				if( profile )
				{
...
namespace Sig
{
	...
	void tPlayer::fResetProfile( )
	{
		...
		mProfile->fSetRealDefaults( );
		mProfile->fSetInversionDefaults( mUser->fYAxisInvertedDefault( ) );
		mProfile->fSetSouthpawDefaults( mUser->fSouthpawDefault( ) );
		mCachedProfileInfo = false;
	...
	void tPlayer::fOnTickPlaying( f32 dt )
	{
		...
		if( !app.fSceneGraph( )->fIsPaused( ) )
		{
			...
			if( Debug_RandyMode && mScoreUI )
			{
				...
				const u32 buttons[ buttonCount ] = { Input::tGamepad::cButtonA, Input::tGamepad::cButtonB, Input::tGamepad::cButtonX, Input::tGamepad::cButtonY, 
					Input::tGamepad::cButtonRShoulder, Input::tGamepad::cButtonLShoulder };

				for( u32 i = 0; i < buttonCount; ++i )
				{
					if( fGamepad( ).fButtonHeld( Input::tGamepad::cButtonSelect ) && fGamepad( ).fButtonDown( buttons[ i ] ) )
						mScoreUI->Test( i, buttons[ i ], this, false );
					if( fGamepad( ).fButtonHeld( Input::tGamepad::cButtonSelect ) && fGamepad( ).fButtonHeld( buttons[ i ] ) )
	...
	void tPlayer::fProcessQuickSwitch( )
	{
		...
		if( fGamepad( ).fButtonDown( Input::tGamepad::cButtonDPadUp | Input::tGamepad::cButtonDPadLeft | Input::tGamepad::cButtonDPadDown | Input::tGamepad::cButtonDPadRight ) )
		{
			...
			if( fGamepad( ).fButtonDown( Input::tGamepad::cButtonDPadUp ) )		dpadInput.z += 1.f;
			if( fGamepad( ).fButtonDown( Input::tGamepad::cButtonDPadDown ) )	dpadInput.z -= 1.f;
			if( fGamepad( ).fButtonDown( Input::tGamepad::cButtonDPadLeft ) )	dpadInput.x += 1.f;
			if( fGamepad( ).fButtonDown( Input::tGamepad::cButtonDPadRight ) )	dpadInput.x -= 1.f;
	...
	b32 tPlayer::fActuallyReadProfile( )
	{
		...
		tAchievementsReader achievements;
		achievements.fSelectUser( mUser->fLocalHwIndex( ) );
		achievements.fRead( 0, GameFlags::cACHIEVEMENTS_COUNT, tAchievementsReader::cDetailsLabel | tAchievementsReader::cDetailsDescription );
	...
	{
		mCountry = country;
		mEnemyCountry = fEnemyCountry( country );
		if( mSoundSource )
	...
	void tPlayer::fEnemyKilled( const tDamageContext& dc, f32 damageAmount, tUnitLogic& killedLogic, u32 shareIndex, u32 shareCount )
	{
		...
		if( barrageKill || comboIncKill || !level->fDisablePropMoney( ) )
		{
			...
			tTutorialEvent killEvent( GameFlags::cTUTORIAL_EVENT_UNIT_DESTROYED, killedLogic.fUnitID( ), killedLogic.fUnitID( ), false, killedLogic.fUnitType( ), killedLogic.fOwnerEntity( ), killedLogic.fDestroyedByPlayer( ) );
...
#pragma optimize( "", off )
	...
	void tPlayer::fOnPathCameraFinished( Gfx::tFollowPathCamera& camera )
	{
		...
		Gfx::tCameraControllerStack* stack = camera.fStack( );
		if( stack ) 
		{
			mFollowPathCamActive = false;
			if( stack->fPopCamerasOfType<tFollowPathCam>( ) )
	...
	{
#ifdef platform_xbox360 // TEMPORARILY DISABLED FOR PC BUILD (for PAX East Demo, capturing footage)
...
#pragma optimize( "", off )
	...
	void tPlayer::fStepFocus( f32 dt )
	{
		...
		if( !mFocusCameraPushed )
		{
			if( mFocusShown && fGamepad( ).fButtonHeld( Input::tGamepad::cButtonY ) )
			{
				fPushFocusCamera( true );
			}
		}
		else
		{
			...
			if( !fGamepad( ).fButtonHeld( Input::tGamepad::cButtonY ) )
			{
	...
	void tPlayer::fUpdateWaveLaunchMenu( )
	{
		if( tGameApp::fInstance( ).fGameMode( ).fIsVersus( ) )
		{
			if( mWaveLaunchMenu )
			{
				const Input::tGamepad& gamepad = mWaveLaunchMenu->fGamepad( );
				b32 closeMenu = false;

				// Close the menu if the user presses B (for back) or Y (what they used to open it)
				if( gamepad.fButtonDown( Input::tGamepad::cButtonB ) || gamepad.fButtonDown( Input::tGamepad::cButtonX ) )
					closeMenu = true;
				else
				{
					mWaveLaunchMenu->fHighlightByAngle( gamepad.fLeftStickAngle( ), gamepad.fLeftStickMagnitude( ) );
					if( gamepad.fButtonDown( Input::tGamepad::cButtonA ) && mWaveLaunchMenu->fSelectActiveIcon( ) )
						closeMenu = true;
				}
			...
			{
				if( fFilteredGamepad( 0 ).fButtonDown( Input::tGamepad::cButtonX ) )
	...
	void tPlayer::fHideShowOffensiveWaveMenu( b32 show )
	{
		if( show )
		{
			if( !mWaveLaunchMenu )
			{
				mWaveLaunchMenu = Gui::tRadialMenuPtr( NEW Gui::tRadialMenu( tGameApp::fInstance( ).fGlobalScriptResource( tGameApp::cGlobalScriptWaveLaunch ), fUser( ) ) );
...
namespace Sig
{
	...
	void tPlayer::fExportScriptInterface( tScriptVm& vm )
	{
		...
		classDesc
			...
			.Prop(_SC("Gamepad"),				&tPlayer::fGamepad)
			...
			.Func(_SC("GetUserProfile"),		&tPlayer::fProfile)
