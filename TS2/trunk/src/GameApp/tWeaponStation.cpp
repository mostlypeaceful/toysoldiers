...
namespace Sig
{
	...
	tWeaponBank::tWeaponBank( tUnitLogic* unitLogic, tWeaponStation* station, u32 id )
		...
		, mNextFireIndex( 0 )
		, mTriggerButton( Input::tGamepad::cButtonRTrigger )
		, mFireRate( -1.f )
		, mFireRateTimer( -1.f )
	...
	void tWeaponBank::fProcessUserInput( )
	{
		...
		const Input::tGamepad &gamepad = mStation->fPlayer( )->fGamepad( );

		b32 firing = fFiring( );
		if( !firing && gamepad.fButtonHeld( mTriggerButton ) )		
			fFire( );
		else if( firing && !gamepad.fButtonHeld( mTriggerButton ) )	
			fEndFire( );
...
found:
	...
	void tWeaponStation::fProcessUserInput( )
	{
		...
		if( mWeaponUI && mToggleCameraButton != ~0 )
		{
			...
			if( mPlayer->fProfile( )->fSouthPaw( tUserProfile::cProfileVehicles ) )
				cameraButton = ( cameraButton == Input::tGamepad::cButtonRThumb )? Input::tGamepad::cButtonLThumb : Input::tGamepad::cButtonRThumb;
			
			if( mPlayer->fGamepad( ).fButtonDown( cameraButton ) )
			{
	...
	void tWeaponStation::fApplyRumbleEvent( const tFireEvent& fireEvent, const tPlayer& player )
	{
		...
		if( Gameplay_Weapon_EnableRumble ) 
			player.fGamepad( ).fRumble( ).fAddEvent( event );
