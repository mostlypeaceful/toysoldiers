...
namespace Sig
{
	...
	f32 tShellLogic::fUpdateShellCam( tPlayer& player, f32 dt )
	{
		const tMat3f& transform = fOwnerEntity( )->fObjectToWorld( );
		const tVec2f aimStick = player.fAimStick( tUserProfile::cProfileShellCam );
		const tVec2f moveStick = player.fMoveStick( tUserProfile::cProfileShellCam );
