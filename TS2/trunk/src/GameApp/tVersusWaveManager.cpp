...
namespace Sig
{
	...
	tVersusWaveManager::tVersusWaveManager( )
	{
		...
		const u32 di = game.fFrontEndPlayer( )->fCountry( ) - 1;
		const u32 si = game.fSecondaryPlayer( )->fCountry( ) - 1;
		...
		if( game.fGameMode( ).fIsNet( ) )
		{
			...
			mVersusUi[ di ]->fCanvas( ).fCanvas( )->fSetInvisible( player1->fUser( )->fIsViewportVirtual( ) );
			mVersusUi[ si ]->fCanvas( ).fCanvas( )->fSetInvisible( player1->fUser( )->fIsViewportVirtual( ) );
			mNetVersusUi[ di ]->fCanvas( ).fCanvas( )->fSetInvisible( player2->fUser( )->fIsViewportVirtual( ) );
			mNetVersusUi[ si ]->fCanvas( ).fCanvas( )->fSetInvisible( player2->fUser( )->fIsViewportVirtual( ) );
	...
	b32 tVersusWaveManager::fLaunchOffensiveWave( tOffensiveWaveDesc& waveDesc, tPlayer* player )
	{
		...
		const u32 index = player->fCountry( ) - 1;
