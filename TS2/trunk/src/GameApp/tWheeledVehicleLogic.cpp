...
namespace Sig
{
	...
	void tWheeledVehicleLogic::fComputeUserInput( f32 dt )
	{
		...
		if( fSeatOccupied( cVehicleDriverSeat ) )
		{
			...
			{
				// Rotate camera
				tVec2f stick = player->fAimStick( tUserProfile::cProfileVehicles );	
			...
			tVec2f moveStick = player->fMoveStick( tUserProfile::cProfileVehicles );
			...
			if( mFull3DPhysics )
			{
				const Input::tGamepad &gamepad = player->fGamepad( );
				...
				if( inputLocked )
				{
					...
					input.mAccelerate = gamepad.fRightTriggerPressure( );
				}
				else
				{
					input.mAccelerate = gamepad.fRightTriggerPressure( );
					input.mDeccelerate = gamepad.fLeftTriggerPressure( );
					input.mHandBrake = gamepad.fButtonHeld( Input::tGamepad::cButtonX );
					...
					if( velMag < cMinSpeed )
					{
						if( gamepad.fRightTriggerPressure( ) > cMaxStickActivation )
							mReverseTimer = fMax( mReverseTimer - dt, 0.f );
						else if( gamepad.fLeftTriggerPressure( ) > cMaxStickActivation )
					...
					{
						//reversing
						input.mAccelerate = -gamepad.fLeftTriggerPressure( );
						input.mDeccelerate = gamepad.fRightTriggerPressure( );
					}
				}
				...
				{
					if( velMag < cMinSpeed && gamepad.fButtonDown( Input::tGamepad::cButtonRTrigger ) )
						mBurnout = true;
		...
		if( fSeatOccupied( cVehicleGunnerSeat ) )
		{
			...
			{
				// Rotate camera
				tVec2f stick = fGetPlayer( cVehicleGunnerSeat )->fAimStick( tUserProfile::cProfileVehicles );
