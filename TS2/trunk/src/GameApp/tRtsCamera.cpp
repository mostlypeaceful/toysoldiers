...
namespace Sig
{
	...
	void tRtsCamera::fEvaluateStickValuesFromGamepad( f32& motionMag, f32& rotateMag, Math::tVec2f& motionStick, Math::tVec2f& rotateStick )
	{
		...
		const Input::tGamepad& gamepad = (mStepWhenPaused || (level && level->fLockPlaceMenuUpUntilBuild( )))
											? mPlayer.fRawGamepad( ) : mPlayer.fGamepad( );

		const b32 southPawMode = mPlayer.fProfile( )->fSouthPaw( tUserProfile::cProfileCamera );
		if( southPawMode )
		{
			rotateMag	= gamepad.fLeftStickMagnitude( );
			motionMag	= gamepad.fRightStickMagnitude( );
			rotateStick	= gamepad.fLeftStick( );
			motionStick	= gamepad.fRightStick( );
		}
		else // standard mode
		{
			motionMag	= gamepad.fLeftStickMagnitude( );
			rotateMag	= gamepad.fRightStickMagnitude( );
			motionStick	= gamepad.fLeftStick( );
			rotateStick	= gamepad.fRightStick( );
		}

		const b32 inverted = mPlayer.fProfile( )->fInversion( tUserProfile::cProfileCamera );
		if( inverted )
			rotateStick.y *= -1.0f;
		...
		if( gamepad.fLeftTriggerHeld( ) )
			motionMag *= Cameras_RTSCam_SuperSpeedScale;

		const b32 blendingInNoMove = !fHasBlendedIn( );
		const b32 motionDisabled = blendingInNoMove || mLockPositionButAllowZoom;
		const b32 rotateDisabled = blendingInNoMove || mLockPositionButAllowZoom || mBombDropper;
		...
		const b32 rthumbDown = !mBombDropper && gamepad.fButtonDown( mPlayer.fAimThumbButton( tUserProfile::cProfileCamera ) );
