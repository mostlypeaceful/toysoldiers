...
namespace Sig
{
	...
	Gui::tRadialMenuPtr tBatteryCharger::fCreateSelectionRadialMenu( tPlayer& player )
	{
		if( !mHasPurchased )
		{
			Gui::tRadialMenuPtr radialMenu = Gui::tRadialMenuPtr( NEW Gui::tRadialMenu( tGameApp::fInstance( ).fGlobalScriptResource( tGameApp::cGlobalScriptPurchaseVehicleOptions ), player.fUser( ) ) );
