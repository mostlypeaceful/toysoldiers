...
namespace Sig
{
	...
	Gui::tRadialMenuPtr tTurretLogic::fCreateSelectionRadialMenu( tPlayer& player )
	{
		...
		Gui::tRadialMenuPtr radialMenu = Gui::tRadialMenuPtr( NEW Gui::tRadialMenu( tGameApp::fInstance( ).fGlobalScriptResource( tGameApp::cGlobalScriptTurretOptions ), player.fUser( ) ) );
	...
	b32 tTurretLogic::fTryToSell( tPlayer* player )
	{
		...
		if( level ) level->fHandleTutorialEvent( tTutorialEvent( GameFlags::cTUTORIAL_EVENT_TURRET_SELL, fUnitID( ), fUnitID( ) ) );
	...
	b32 tTurretLogic::fTryToRepair( tPlayer* player )
	{
		...
		tLevelLogic* level = tGameApp::fInstance( ).fCurrentLevel( );
		if( level ) level->fHandleTutorialEvent( tTutorialEvent( GameFlags::cTUTORIAL_EVENT_TURRET_REPAIR, fUnitID( ), fUnitID( ) ) );
	...
	void tTurretLogic::fApplyUserControl( f32 dt )
	{
		...
		const Input::tGamepad& gamepad = controllingPlayer->fGamepad( );
		...
		if( !mainWeap->fShellCaming( ) && !mDisableControl )
		{
			...
			if( blendedIn )
			{
				...
				tVec2f stick = controllingPlayer->fAimStick( tUserProfile::cProfileTurrets );
...
namespace Sig
{
	...
	namespace
	{
		void fCheckUserFire( tTurretLogic* logic )
		{
			if( !logic->fFlipping( ) )
			{
				...
				const Input::tGamepad& gp = logic->fControllingPlayer( )->fGamepad( );
				tWeaponBankPtr& bank = logic->fWeaponStation( 0 )->fBank( 0 );

				b32 continuousFire = bank->fIsContinuousFire( );
				b32 fire = continuousFire ?  gp.fRightTriggerHeld( ) : 
					(bank->fWantsLocks( ) ? (bank->fAcquiringLocks( ) && gp.fRightTriggerUp( )) : gp.fRightTriggerDown( ));
	...
	public:
		virtual void fOnActivate( tLogic* logic )
		{
			...
			{
				const Input::tGamepad& gp = fLogic( )->fControllingPlayer( )->fGamepad( );
				if( !fLogic( )->fDisableControl( ) && bank->fWantsLocks( ) && gp.fRightTriggerHeld( ) )
		...
		virtual void fOnProcess( AI::tGoalPtr& goalPtr, tLogic* logic, f32 dt )
		{
			...
			{
				const Input::tGamepad& gp = fLogic( )->fControllingPlayer( )->fGamepad( );
				if( !fLogic( )->fDisableControl( ) && bank->fWantsLocks( ) && gp.fRightTriggerDown( ) )
				{
		...
		virtual void fOnProcess( AI::tGoalPtr& goalPtr, tLogic* logic, f32 dt )
		{
			...
			if( fLogic( )->fUnderUserControl( ) )
			{
				...
				if( !level->fDisableVehicleInput( ) )
				{
					...
					if( !weap->fIsContinuousFire( ) && !gp.fRightTriggerHeld( ) && !station->fBank( 0 )->fWantsLocks( ) )
