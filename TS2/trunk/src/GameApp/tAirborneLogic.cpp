...
{	
	...
	void tAirborneLogic::fComputeUserInput( f32 dt )
	{
		...
		if( fSeatOccupied( cVehicleDriverSeat ) )
		{
			...
			if( input.mActive )
			{
				...
				{
					const tVec2f speedStick = player->fMoveStick( tUserProfile::cProfilePlanes );
					const tVec2f aimStick = player->fAimStick( tUserProfile::cProfilePlanes );
					...
					if( !mLockedInBombCam && gamepad.fButtonDown( player->fAimThumbButton( tUserProfile::cProfilePlanes ) ) )
				...
				if( !mSpecialMove )
				{
					//special anim
					u32 id = ~0;
					b32 lShoulder = gamepad.fButtonHeld( Input::tGamepad::cButtonLShoulder );
					b32 rShoulder = gamepad.fButtonHeld( Input::tGamepad::cButtonRShoulder );
	...
	void tAirborneLogic::fReactToDamage( const tDamageContext& dc, const tDamageResult& dr )
	{
		if( dc.fAttackingTeam( ) != fTeam( ) 
			&& !mDisableEvasion 
			&& !fUnderUserControl( ) 
			&& fUnitPath( )->fPathMode( ) == tUnitPath::cPathModeFollow )
			fAvert( );
