...
{	
	...
	void tHoverLogic::fComputeUserInput( f32 dt )
	{
		...
		if( fSeatOccupied( cVehicleDriverSeat ) )
		{
			...
			{
				const tVec2f aimStick = player->fAimStick( tUserProfile::cProfileVehicles );
				const tVec2f moveStick = player->fMoveStick( tUserProfile::cProfileVehicles );
				...
				const Input::tGamepad &gamepad = fGetPlayer( cVehicleDriverSeat )->fGamepad( );
				if( gamepad.fButtonHeld( Input::tGamepad::cButtonLShoulder ) ) input.mHeightAdjustment -= 1.f;
				if( gamepad.fButtonHeld( Input::tGamepad::cButtonRShoulder ) ) input.mHeightAdjustment += 1.f;
