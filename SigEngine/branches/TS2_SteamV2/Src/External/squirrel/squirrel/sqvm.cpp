	...
	//
	{
		for(;;)
		{
			...
			case _OP_LINE:
				if(type(_debughook) != OT_NULL && _rawval(_debughook) != _rawval(ci->_closure)){
					bool suspend = false;
					CallDebugHook(_SC('l'),arg1,&suspend);
					if(suspend){
						_suspended = SQTrue;
...
void SQVM::CallDebugHook(SQInteger type,SQInteger forcedline,bool *suspend)
{
	SQObjectPtr temp_reg;
	SQInteger nparams=5;
	SQFunctionProto *func=_funcproto(_closure(ci->_closure)->_function);
	Push(_roottable); Push(type); Push(func->_sourcename); Push(forcedline?forcedline:func->GetLine(ci->_ip)); Push(func->_name);
	Call(_debughook,nparams,_top-nparams,temp_reg,SQFalse,suspend);
	Pop(nparams);
}
...
bool SQVM::Call(SQObjectPtr &closure,SQInteger nparams,SQInteger stackbase,SQObjectPtr &outres,SQBool raiseerror, bool *suspend)
{
#ifdef _DEBUG
