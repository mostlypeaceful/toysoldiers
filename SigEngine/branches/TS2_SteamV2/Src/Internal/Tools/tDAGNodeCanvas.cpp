...
namespace Sig
{
	...
	void tDAGNodeCanvas::fFrame( b32 selectedOnly )
	{
		...
		else
		{
			...
			const wxPoint windowCenter = wxPoint( GetSize( ).x / 2, GetSize( ).y / 2 );
	...
	void tDAGNodeCanvas::fOnMouseMove( wxMouseEvent& event )
	{
		...
		const wxPoint delta = event.GetPosition( ) - mLastMouseMove;
	...
	void tDAGNodeCanvas::fRenderNodeWindows( wxDC& dc )
	{
		...
		for( u32 i = mDAGNodes.fCount( ); i > 0; --i )
			mDAGNodes[ i - 1 ]->fOnPaint( mTopLeft, size, dc, mOutputsBeforeInputs );
	}
