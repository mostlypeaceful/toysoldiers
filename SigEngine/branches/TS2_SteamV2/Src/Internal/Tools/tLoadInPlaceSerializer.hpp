...
namespace Sig
{
	...
	private:
		...
		public:
			...
			tHashTable<void*,b32> mEndianSwappedPtrs;

			tPointee( const void* addr, u32 dataSize, const Rtti::tReflector& reflector ) 
				: mAddress( addr ), mFullDataSize( dataSize ), mReflector( reflector ), mCid( Rtti::cInvalidClassId ), mOwned( false ), mFilePos( 0 ), mEndianSwappedPtrs( 2048 ) { }
