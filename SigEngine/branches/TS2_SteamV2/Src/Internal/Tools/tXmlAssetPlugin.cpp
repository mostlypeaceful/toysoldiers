...
namespace Sig
{
	...
	b32 tXmlFileConverter::fWriteFile( const tFilePathPtr& outPath, const tPlatformId& pid ) const
	{
		...
		if( !ofile.fIsOpen( ) )
		{
			log_warning( Log::cFlagResource, "Failed to open file " << outPath );
			return 0;
		}
		...
		const tFilePathPtrList& immediateInputFiles )
	{
		for( u32 i = 0; i < immediateInputFiles.fCount( ); ++i )
		{
			if( StringUtil::fCheckExtension( immediateInputFiles[ i ].fCStr( ), ".xml" ) )
			{
				...
				inputOutputsOut.fBack( ).mOutput = ToolsPaths::fMakeResRelative( tResourceConvertPath<tXmlFile>::fConvertToBinary( immediateInputFiles[ i ] ) );
			}
		}
	}
			...
			tFilePathPtrList& indirectGenFilesAddTo )
	{
		...
		for( u32 i = 0; i < inOut.mInputs.fCount( ); ++i )
		{
			...
			for( u32 j = 0; j < inOut.mPlatformsToConvert.fCount( ); ++j )
			{
				...
				c.fSetSignature( pid, Rtti::fGetClassId<tXmlFile>( ), tXmlFile::cVersion );


				//read in file
				if( !c.fReadFile( inOut.mInputs[ i ] ) )
				{
					log_warning( Log::cFlagResource, "Failed reading file " << inOut.mInputs[ i ] );
					continue;
				}
				...
				{
					log_warning( Log::cFlagResource, "Failed writing file " << outputPath );
					continue;
				}
			}
		}
