...
namespace Sig
{
	...
	template< class tSerializer >
	void fSerializeXmlObject( tSerializer& s, Attractors::tData& o )
	{
		o.fSerializeXmlObject< tSerializer >( s );
	}
...
namespace Sig { namespace Fxml
{
	...
	tEntityDef* tFxParticleSystemObject::fCreateEntityDef( tFxmlConverter& fxmlConverter )
	{
		...
		if( !mat )
		{
			log_warning( 0, "You need to assign a custom material to your particle system!" );
			return 0;
		}
		...
		for( u32 i = 0; i < mStates.fCount( ); ++i )
			entityDef->fAddFromToolState( mStates[ i ]->fToolState( ) );
	...
	tEntityDef* tFxAttractorObject::fCreateEntityDef( tFxmlConverter& fxmlConverter )
	{
		...
		entityDef->fFromToolData( mAttractorData->fToolData( ) );		
		return entityDef;
	}
	...
	void fSerializeXmlObject( tSerializer& s, tFile& o )
	{
		...
		{
			log_warning( 0, "Fxml file format is out of date -> Please re-export." );
			return;
		}
	...
	b32 tFile::fSaveXml( const tFilePathPtr& path, b32 promptToCheckout )
	{
		...
		{
			log_warning( 0, "Couldn't save Fxml file [" << path << "]" );
			return false;
		}
	...
	b32 tFile::fLoadXml( const tFilePathPtr& path )
	{
		...
		{
			log_warning( 0, "Couldn't load Fxml file [" << path << "]" );
			return false;
		}
