...
namespace Sig { namespace Sklml
{
	...
	void fSerializeXmlObject( tSerializer& s, tFile& o )
	{
		...
		{
			log_warning( 0, "Sklml file format is out of date (exported from " << o.mSrcFile << ") -> Please re-export." );
			return;
		}
	...
	b32 tFile::fSaveXml( const tFilePathPtr& path, b32 promptToCheckout )
	{
		...
		{
			log_warning( 0, "Couldn't save Sklml file [" << path << "]" );
			return false;
		}
	...
	b32 tFile::fLoadXml( const tFilePathPtr& path )
	{
		...
		{
			log_warning( 0, "Couldn't load Sklml file [" << path << "]" );
			return false;
		}
