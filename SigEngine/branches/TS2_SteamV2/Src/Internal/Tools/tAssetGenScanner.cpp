...
namespace Sig
{
	...
	class tInputFileListHandler
	{
		...
		tFilesProcessed&				mFilesAlreadyProcessed;
		tFilePathPtrList&				mIndirectGenFiles;
		b32								mReconOnly;
	...
	void tInputFileListHandler::fDeleteOutputsWithMissingDependencies( )
	{
		...
		for( u32 i = 0; i < mIndirectGenFiles.fCount( ); ++i )
		{
				...
				ToolsPaths::fIsUnderSourceFolder( mIndirectGenFiles[ i ] ) )
			{
				...
				log_warning( 0, "Dependency file [" << mIndirectGenFiles[ i ] << "] does not exist." );
	...
	b32 tInputFileListHandler::fVerifyInputs( iAssetGenPlugin::tInputOutput& inOut ) const
	{
		...
		for( u32 i = 0; i < inOut.mInputs.fCount( ); ++i )
		{
			...
			{
				log_warning( 0, "Input file [" << ToolsPaths::fMakeResRelative( inOut.mInputs[ i ] ) << "] required by output file [" << inOut.mOutput << "] does not exist." );
				success = false;
			}
		}
	...
	b32 tInputFileListHandler::fVerifyOutput( iAssetGenPlugin::tInputOutput& inOut ) const
	{
		...
		{
			log_warning( 0, "The file name [" << inOut.mOutput << "] is too long - maximum of " << cMaxCharsSimple << " is allowed." );
			return false;
		}
	...
	{
		// if the user specified an extension filter, then cull files lacking that extension
		if( opts.mExt.length( ) > 0 )
		{
			for( u32 i = 0; i < files.fCount( ); ++i )
			{
				if( !StringUtil::fCheckExtension( files[ i ].fCStr( ), opts.mExt.c_str( ) ) )
				{
					files.fEraseOrdered( i );
					--i;
				}
			}
		}
		...
		if( FileSystem::fReadFileToBuffer( depends, dependsPath, "" ) )
		{
			...
			for( u32 i = 0; i < dependsFiles.fCount( ); ++i )
			{
				...
				else
				{
					...
					log_warning( 0, "Dependency file [" << dependsFiles[ i ] << "] does not exist, or is under a ~src folder." );
	...
	void tAssetGenScanner::fWriteProcessDependencies( const std::string& file, const tFilePathPtrList& dependFiles, const tGrowableArray<std::string>& bufferedLogs )
	{
		...
		tFileWriter depends( dependsPath, false );
	...
	void tAssetGenScanner::fGeneratePackFiles( )
	{
		...
		{
			log_warning( 0, "Multiple input .sacml files found, but a single output path was specified: " << mOpts.mSacmlOutput );
		}
