...
namespace Sig
{
	// SINGLE VECTOR ENTITY, JUST FOR VISUALIZATION
		...
		: tEditableObject( container )
	{
		...
		u32 id = mGuid;
		if( fxa.mOpened )
			id = fxa.mAttractorData->fId( );
		mAttractor->fSetData( FX::Attractors::tAttractorDataPtr( new FX::Attractors::tData( fxa.mAttractorData.fGetRawPtr( ), id ) ) );
	...
	void tSigFxAttractor::fClone( const tSigFxAttractor& attractor )
	{
		...
		mAttractor->fSetData( FX::Attractors::tAttractorDataPtr( new FX::Attractors::tData( data->fGetAttractorData( ).fGetRawPtr( ), mGuid ) ) );
	...
	Sigml::tObjectPtr tSigFxAttractor::fSerialize( b32 clone ) const
	{
		...
		fxa->mAttractorData.fReset( mAttractor->fGetAttractorData( ).fGetRawPtr( ) );
