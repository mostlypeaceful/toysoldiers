...
namespace Sig
{
		...
		: tEditableObject( container )
	{
		...
		for( u32 i = 0; i < fxo.mStates.fCount( ); ++i )
		{
			const FX::tStatePtr state = fxo.mStates[ i ];
			mParticleSystem->fAddState( FX::tStatePtr( new FX::tState( new FX::tToolParticleSystemState( state->fToolState( ) ) ) ) );
		}
	...
	void tSigFxParticleSystem::fClone( const tSigFxParticleSystem& system )
	{
		...
		for( u32 i = 0; i < ps->fStateCount( ); ++i )
		{
			FX::tStatePtr state( &ps->fState( i ) );
			mParticleSystem->fAddState( FX::tStatePtr( new FX::tState( new FX::tToolParticleSystemState( state->fToolState( ) ) ) ) );
		}
	...
	void tSigFxParticleSystem::fCommonCtor( tResourceDepot& resDep )
	{
		...
		mParticleSystem.fReset( new FX::tParticleSystem( mMaterial ) );
	...
	void tSigFxParticleSystem::fThinkST( f32 dt )
	{
		...
		for( u32 i = 0; i < mBubbles.fCount( ); ++i )
		{
			if( mParticleSystem->fParticleCount( ) < 1 )
			{
				...
				const Math::tVec4f color = mParticleSystem->fState( ).fSamplePerParticleGraph< Math::tVec4f >( mParticleSystem->fSystemRandomNumberGenerator( ), FX::cColorGraph, 0.f );
	...
	Sigml::tObjectPtr tSigFxParticleSystem::fSerialize( b32 clone ) const
	{
		...
		for( u32 i = 0; i < mParticleSystem->fStateCount( ); ++i )
		{
			fxo->mStates.fPushBack( FX::tStatePtr( &mParticleSystem->fState( i ) ) );
		}
	...
	void tSigFxParticleSystem::fGenerateShaders( const Derml::tFile& dermlFile )
	{
		...
		{
			log_warning( 0, "Invalid geometry style on shader [" << mDermlMtlFile.mShaderPath << "]; expected GeometryType: 'FacingParticleQuads'" );
		}
