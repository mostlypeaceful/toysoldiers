...
namespace Sig
{
	...
	protected:
		...
		f32 mLabelXOffset;
	public:
		tDAGNodeOutput( const wxString& name, tDAGNode& owner, const wxColour& color, u32 maxConnections = ~0, tEdge edge = cRight, f32 radius = 6.5f );

		const wxString& fName( ) const { return mName; }
		void fSetName( const wxString& name ) { mName = name; }
	...
	{ 
	public:
		tDAGNodeInput( const wxString& name, tDAGNode& owner, const wxColour& color, u32 maxConnections = ~0, tEdge edge = cLeft, f32 radius = 6.5f );
		virtual u32 fIndex( ) const;
	};
	...
	protected:
		...
		b32 mHideIfNullText;
	public:
		tDAGNodeText( const wxString& name, const wxString& value, tDAGNode& owner, b32 hideIfNullText );
		void fOnPaint( wxPoint parentTopLeft, f32 offsets[tDAGNodeOutput::cEdgeCount], wxDC& dc );
		const tDAGNode& fOwner( ) const { return mOwner; }
		u32 fHeight( ) const;

		void fSetLabel( const wxString& label ) { mName = label; }
		void fSetValue( const wxString& value ) { mValue = value; }
		wxString fTotalString( ) const { return mName + " - " + mValue; }
	};
	...
	public:
		tDAGNode( const wxString& name, const wxColour& titleBarColor, const wxColour& titleBarTextColor, const wxPoint& topLeft = wxPoint( 0, 0 ) );
		virtual ~tDAGNode( );
		
		void fSetOwner( tDAGNodeCanvas* owner ) { mOwner = owner; }
		tDAGNodeCanvas* fOwner( ) const { return mOwner; }
		void fOnPaint( const wxPoint& topLeftOrigin, const wxSize& clipDims, wxDC& dc, b32 outputsBeforeInputs );

		const wxString& fName( ) const { return mName; }
		void fSetName( const wxString& name );
		virtual wxString fDisplayName( ) const { return mName; }
	...
	public:
		...
		void fSetText( const wxString& text ) { mText = text; }
		const wxString& fText( ) const { return mText; }
