...
namespace Sig
{
	...
	namespace
	{
		D3DFORMAT fConvertFormat( Gfx::tTextureFile::tFormat fmt, Gfx::tTextureFile::tSemantic semantic, tTextureSysRam::tAlphaLevel alphaLevel )
		{
			...
			switch( fmt )
			{
			case Gfx::tTextureFile::cFormatA8R8G8B8:	o = D3DFMT_A8R8G8B8;
				break;
			case Gfx::tTextureFile::cFormatDXT1:		o = D3DFMT_DXT1;
				break;
			case Gfx::tTextureFile::cFormatDXT3:		o = D3DFMT_DXT3;
				break;
			case Gfx::tTextureFile::cFormatDXT5:		o = D3DFMT_DXT5;
				break;
			case Gfx::tTextureFile::cFormatR5G6B5:		o = D3DFMT_R5G6B5;
				break;
			case Gfx::tTextureFile::cFormatA8:			o = D3DFMT_A8;
				break;
			default:									sigassert( !"un-recognized texture format!" );
				break;
			}
	...
	void tTextureSysRam::tSurface::fUnfilteredCopyR5G6B5( const tSurface& src )
	{
		...
		for( u32 xDst = 0; xDst < dst.fWidth( ); ++xDst )
		{
			...
			Math::tVec3u tlSrcRgb;

			Gfx::tTextureVRam::fUnpackColorR5G6B5( *srcMip.fGetTexel<u16>( l, t ), tlSrcRgb );

			*dstMip.fGetTexel<u16>( xDst, yDst ) = Gfx::tTextureVRam::fPackColorR5G6B5( tlSrcRgb.x, tlSrcRgb.y, tlSrcRgb.z );
		}
	...
	b32 tTextureSysRam::fLoad( const tFilePathPtr& fileName, b32 autoHandleMipsAndNormalMapFormat, u32 ithImage )
	{
		...
		if( !fDetailsValid( ) )
		{
			log_warning( 0, "No formatting was specified for this texture [" << fileName << "]; be sure to specify _d, _i, _n, etc. at the end of your texture name." );
			return false;
		}
	...
	b32 tTextureSysRam::fImagesValid( ) const
	{
		...
		for( u32 iface = 0; iface < mImages.fCount( ); ++iface )
		{
			...
			if( fSemanticRequiresMipMaps( ) )
			{
				...
				{
					log_warning( 0, "Image dimensions are not multiples of 4." );
					return false;
				}
			}
		}
	...
	void tTextureSysRam::fSaveGameBinary( const tFilePathPtr& outputFileName, tPlatformId pid ) const
	{
		...
		if( !ofile.fIsOpen( ) )
		{
			log_warning( 0, "Couldn't open file [" << outputFileName << "] for writing." );
			return;
		}
	...
	void tTextureSysRam::fSaveGameBinary( tRawFaceMipMapSet& rawFaceMips, tFileWriter& ofile, tPlatformId pid ) const
	{
		...
		texFile.mDiscardSurfaceBuffers = true; // TODO figure out per-platform, some may need it to persist
		texFile.mType = mType;
		...
		texFile.fSetSignature( pid, Rtti::fGetClassId<Gfx::tTextureFile>( ), Gfx::tTextureFile::cVersion );
	...
	void tTextureSysRam::fOutputMipChain( Gfx::tTextureFile& texFile, tRawFaceMipMapSet& rawFaceMips, tFileWriter& ofile, tPlatformId pid ) const
	{
		...
		{
			log_warning( 0, "Unsupported platform (" << fPlatformIdString( pid ) << ") for texture generation." );
		}
	}
