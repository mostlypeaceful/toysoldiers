...
namespace Sig { namespace Animl
{
	...
	void tBone::fDecomposeKeys( tPRSKeyFrameList& decomposed, const tKeyFrameList& keys )
	{
		...
		{
			if( decomposed[ i - 1 ].mR.fAntiPodal( decomposed[ i ].mR ) )
				decomposed[ i ].mR = -decomposed[ i ].mR;
		}
	}
	...
	void tBone::fDeltaCompress( tDeltaCompressionResult& result, const tKeyFrameList& keys, f32 pMaxErrorInMeters, f32 rMaxErrorInPctg, f32 sMaxErrorInPctg )
	{
		...
		for( u32 i = 0; i < result.mCompressionKeys.fCount( ); ++i )
		{
			...
			{
				++result.mPKeyCount;
				const f32 pMin = result.mPRSKeys[ i ].mP.fMin( );
				const f32 pMax = result.mPRSKeys[ i ].mP.fMax( );
			...
			{
				++result.mSKeyCount;
				const f32 sMin = result.mPRSKeys[ i ].mS.fMin( );
				const f32 sMax = result.mPRSKeys[ i ].mS.fMax( );
		...
		if( result.mPKeyCount == 2 && result.mPRSKeys.fFront( ).mP == result.mPRSKeys.fBack( ).mP )
		{
			result.mPKeyCount -= 1;
			result.mCompressionKeys.fBack( ).mP = false;
		}
		if( result.mRKeyCount == 2 && result.mPRSKeys.fFront( ).mR == result.mPRSKeys.fBack( ).mR )
		{
			result.mRKeyCount -= 1;
			result.mCompressionKeys.fBack( ).mR = false;
		}
		if( result.mSKeyCount == 2 && result.mPRSKeys.fFront( ).mS == result.mPRSKeys.fBack( ).mS )
		{
			result.mSKeyCount -= 1;
	...
	struct tDeltaCompressPosition
	{
		...
		{
			return ( actual.mP - Math::fLerp( l.mP, r.mP, t ) ).fLengthSquared( );
		}
	...
	struct tDeltaCompressRotation
	{
		f32 fSqError( const Math::tPRSXformf& l, const Math::tPRSXformf& r, const Math::tPRSXformf& actual, f32 t )
		{
			return ( actual.mR - Math::fNLerp( l.mR, r.mR, t ) ).fLengthSquared( );
		}
	...
	struct tDeltaCompressScale
	{
		f32 fSqError( const Math::tPRSXformf& l, const Math::tPRSXformf& r, const Math::tPRSXformf& actual, f32 t )
		{
			return ( actual.mS - Math::fLerp( l.mS, r.mS, t ) ).fLengthSquared( );
		}
	...
	void fSerializeXmlObject( tSerializer& s, tFile& o )
	{
		...
		{
			log_warning( 0, "Animl file format is out of date (exported from " << o.mSrcFile << ") -> Please re-export." );
			return;
		}
	...
	b32 tFile::fSaveXml( const tFilePathPtr& path, b32 promptToCheckout )
	{
		...
		{
			log_warning( 0, "Couldn't save Animl file [" << path << "]" );
			return false;
		}
	...
	b32 tFile::fLoadXml( const tFilePathPtr& path )
	{
		...
		{
			log_warning( 0, "Couldn't load Animl file [" << path << "]" );
			return false;
		}
