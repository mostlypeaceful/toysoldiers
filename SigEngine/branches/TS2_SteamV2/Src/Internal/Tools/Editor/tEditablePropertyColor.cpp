...
namespace Sig
{
	...
	void tEditablePropertyColor::fCreateGui( wxWindow* parent, const std::string& label )
	{
		mColorPicker = new tColorPicker( this, parent, label.c_str( ) );

		fRefreshGui( );

		if( mAllowDelete )
			fAddRemoveButton( parent, *mColorPicker );
	}
