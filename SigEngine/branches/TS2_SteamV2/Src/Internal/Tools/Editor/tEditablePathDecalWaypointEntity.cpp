...
namespace Sig { namespace Sigml
{
	...
	tEntityDef* tPathDecalObject::fCreateEntityDef( tSigmlConverter& sigmlConverter )
	{
		...
		if( mDiffuseTextureFilePath.fLength( ) > 0 && FileSystem::fFileExists( ToolsPaths::fMakeResAbsolute( mDiffuseTextureFilePath ) ) )
			...
			log_warning( 0, "No diffuse texture specified or texture doesn't exist: " << mDiffuseTextureFilePath );
		...
		if( mNormalMapFilePath.fLength( ) > 0 )
		{
			if( FileSystem::fFileExists( ToolsPaths::fMakeResAbsolute( mNormalMapFilePath ) ) && mAcceptsLights )
				...
				log_warning( 0, "Normal map specified but texture doesn't exist: " << mNormalMapFilePath );
		}
...
namespace Sig
{
	...
	{
		if( !mDiffuseTextureRes.fNull( ) )
			mDiffuseTextureRes->fUnload( this );

		if( !mNormalMapRes.fNull( ) )
			mNormalMapRes->fUnload( this );
	}
	...
	namespace
	{
		...
		struct tCollectTrisIntersectCallback : public tEntityBVH::tIntersectVolumeCallback<tVolume>
		{
			...
			{
				tSpatialEntity* collectFrom = static_cast< tSpatialEntity* >( octreeObject );

				if( !mApplyToAllMeshes )
				{
					tEditableTerrainEntity* terrainEnt = collectFrom->fFirstAncestorOfType< tEditableTerrainEntity >( );
	...
	void tEditablePathDecalEntity::fRefreshGeometry( )
	{
		...
		for( u32 i = 0;  i < mFullWithSubs.fCount( )-1; ++i )
		{
			...
			Math::tVec3f forwardAxis	= toNext * 0.5 + toNextUnit * currentScale.z * 0.5f + toNextUnit * nextScale.z * 0.5f;
