...
namespace Sig { namespace Sigml
{
	...
	tEntityDef* tNavGraphRootObject::fCreateEntityDef( tSigmlConverter& sigmlConverter )
	{
		...
		{
			log_warning( 0, "Detected nav graph root disconnected from a graph." );
			return NULL;
		}
...
namespace Sig
{
	...
	tEditableNavGraphNodeEntity::tEditableNavGraphNodeEntity( tEditableObjectContainer& container )
		...
		, mConnectorOverride( 
			Gfx::tRenderState::cColorBuffer | Gfx::tRenderState::cDepthBuffer | Gfx::tRenderState::cAlphaBlend | Gfx::tRenderState::cCutOut | Gfx::tRenderState::cPolyTwoSided, 
		...
		for( u32 i = 0; i < mConnections.fCount( ); ++i )
		{
			...
			if( !connHid && !nodeHid )
			{
				...
				newQuad.fReset( new tDummyObjectEntity( 
					mGeometry.fBack( )->fGetModifiedRenderBatch( &mShellRenderState ), 
