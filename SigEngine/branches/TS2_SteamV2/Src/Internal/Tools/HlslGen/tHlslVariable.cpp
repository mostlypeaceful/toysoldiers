...
namespace Sig { namespace HlslGen
{
	...
	void tShaderRequirements::fComputeVertexFormat( Gfx::tVertexFormat& vtxFormat ) const
	{
		tGrowableArray<Gfx::tVertexElement> vertexElems;
		for( u32 i = 0; i < mInputs.fCount( ); ++i )
		{
			Gfx::tVertexElement ve = mInputs[ i ]->fVertexElement( );
			if( ve.mSemantic != Gfx::tVertexElement::cSemanticInvalid )
