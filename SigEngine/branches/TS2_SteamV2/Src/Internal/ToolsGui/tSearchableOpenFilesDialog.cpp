...
namespace Sig
{
	...
	void tSearchableOpenFilesDialog::fPrepData( )
	{
		...
		if( mAdditionalExtensionFilters.fCount( ) == 0 )
			files = mSourceBrowser->fGetRelevantPaths( );
		else
			files = mSourceBrowser->fGetRelevantPathsOfType( mAdditionalExtensionFilters );
	...
	void tSearchableOpenFilesDialog::fUpdateResults( wxCommandEvent& event )
	{
		...
		StringUtil::fSplit( includes, mFilterText->GetLineText( 0 ).c_str( ), " " );

		for( u32 i = 0; i < includes.fCount( ); ++i )
		{
			std::string thisString = includes[i];
			...
			if( *thisString.begin( ) == '-' )
			{
				...
				if( thisString.length( ) >= 2 )
					excludes.fPushBack( StringUtil::fFirstCharAfter( thisString.c_str( ), "-" ) );
			}
		}
		...
		for( u32 i = 0; i < mRows.fCount( ); ++i )
		{
			...
			if( !fAndSearch( mRows[i].fFileName( ).c_str( ), includes ) )
				continue;

			// Filter for all exclusions.
			if( !fNotSearch( mRows[i].fFileName( ).c_str( ), excludes ) )
				continue;
