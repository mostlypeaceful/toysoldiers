...
namespace Sig
{
	...
	tObjectScaleGizmo::tObjectScaleGizmo( const tGizmoGeometryPtr& geom )
	...
	void tObjectScaleGizmo::fSpecifyWorldCoords( 
		...
		const Math::tVec3f& worldCoords, b32 doX, b32 doY, b32 doZ )
	{
		...
		for( u32 i = 0; i < selectedObjects.fCount( ); ++i )
		{
			...
			editorXform.fScaleGlobal( scale );
	...
	void tObjectScaleGizmo::fExtractWorldCoords( 
		const tEditorSelectionList& selectedObjects,
	...
	void tObjectScaleGizmo::fComputeLocalToWorld( Math::tMat3f& xform, const tEditorSelectionList& selectedObjects, const Gfx::tCamera& camera, f32 scale, b32 orientToWorld ) const
	{
	...
	void tObjectScaleGizmo::fOnDrag( 
		tToolsGuiMainWindow& mainWindow,
		const tEditorSelectionList& selectedObjects, 
