	...
	*/
	...
	public:
		...
		void fOnControlUpdated( wxCommandEvent& event )
		{
			for( u32 i = 0; i < mCheckListBox->GetCount( ); ++i )
			{
				...
				{
					const b32 hasCurFlag = ( *mList )[ j ]->fGetParticleSystem( )->fHasFlag( flag );
					( *mList )[ j ]->fGetParticleSystem( )->fRemoveFlag( flag );
					...
					if( mCheckListBox->IsChecked( i ) )
					{
						...
						( *mList )[ j ]->fGetParticleSystem( )->fAddFlag( flag );
					}
				}					
			}
			mSigFxMainWindow->fGuiApp( ).fActionStack( ).fForceSetDirty( true );
	...
	void tIgnoreAttractorChecklist::fOnControlUpdated( wxCommandEvent& event )
	{
		...
		for( u32 i = 0; i < systems.fCount( ); ++i )
		{
			...
			{
				systems[ i ]->fGetParticleSystem( )->fState( ).fClearAttractorIgnoreIds( );
				...
				if( mCheckListBox->IsChecked( i ) )
				{
					...
					if( !strcmp( s1, s2 ) )
					{
						for( u32 s = 0; s < systems.fCount( ); ++s )
						{
							...
							{
								systems[ s ]->fGetParticleSystem( )->fState( ).fAddAttractorIgnoreId( attractors[ j ]->fGetAttractor( )->fId( ) );
