...
namespace Sig
{
	...
	void tSigFxKeyline::fSetPaused( const b32 pause )
	{
		...
		mPaused = pause;
		fMainWindow( )->fGuiApp( ).fSceneGraph( )->fPause( mPaused  );
	...
	void tSigFxKeyline::fBuildKeyline( )
	{
		...
		if( fxps )
		{
			...
			for( u32 i = 0; i < cEmissionGraphCount; ++i )
			{
				...
				tGraphPtr graph = ps->fState( ).fToolState( ).mEmissionGraphs[ i ];
			...
			for( u32 i = 0; i < cParticleGraphCount; ++i )
			{
				...
				const wxColour hil( (u32)h.x, (u32)h.y, (u32)h.z );
				const wxColour sel( (u32)s.x, (u32)s.y, (u32)s.z );
				tGraphPtr graph = ps->fState( ).fToolState( ).mPerParticleGraphs[ i ];
			...
			for( u32 i = 0; i < cMeshGraphCount; ++i )
			{
				...
				const wxColour hil( (u32)h.x, (u32)h.y, (u32)h.z );
				const wxColour sel( (u32)s.x, (u32)s.y, (u32)s.z );
				tGraphPtr graph = ps->fState( ).fToolState( ).mMeshGraphs[ i ];
		...
		{
			tParticleAttractorPtr pa = fxa->fGetAttractor( );
			...
			for( u32 i = 0; i < cAttractorGraphCount; ++i )
			{
				...
				tGraphPtr graph = pa->fGetAttractorData( )->fGraph( i );
