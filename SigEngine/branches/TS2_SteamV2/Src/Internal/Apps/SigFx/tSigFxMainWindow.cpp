...
namespace Sig
{
	...
	BEGIN_EVENT_TABLE( tSigFxMainWindow, wxFrame )
		...
		EVT_MENU(				cActionPreviewProfile,  tSigFxMainWindow::fOnAction )
	...
	b32 tSigFxMainWindow::fImportFxmlFile( const tFilePathPtr& filePath )
	{
		...
		for( u32 j = previousAttractors.fCount( ); j < allAttractors.fCount( ); ++j )
		{
			...
			{
				previousParticleSystems[ p ]->fGetParticleSystem( )->fState( ).fAddAttractorIgnoreId( allAttractors[ j ]->fGetAttractor( )->fId( ) );
			}
		...
		for( u32 j = 0; j < previousAttractors.fCount( ); ++j )
		{
			...
			{
				allParticleSystems[ p ]->fGetParticleSystem( )->fState( ).fAddAttractorIgnoreId( previousAttractors[ j ]->fGetAttractor( )->fId( ) );
			}
	...
	b32 tSigFxMainWindow::fSaveDoc( b32 saveAs )
	{
		...
		if( saveAs || fGuiApp().fDocName( ) == tToolsGuiApp::cNewDocTitle )
		{
			...
			fGuiApp( ).fSetDocName( openFileDialog->GetPath( ).c_str( ) );
	...
	void tSigFxMainWindow::fPreview( u32 id )
	{
		...
		if( FileSystem::fFileExists( relSigbPath ) )
		{
			...
			case cActionPreviewProfile : cmdLine += "-profile ";	break;
	...
	void tSigFxMainWindow::fOnAction( wxCommandEvent& event )
	{
		...
		else
		{
			...
			case cActionPreviewProfile:		fPreview( id );	break;
	...
	void tSigFxMainWindow::fAddMenus( )
	{
		...
		// add file menu
		{
			...
			subMenu->Append( cActionPreviewProfile, "Preview Profile XBOX" );
	...
	void tSigFxMainWindow::fAddToolbar( )
	{
		...
		const Math::tVec4f rgbaClearColor = mRenderPanelContainer->fGetFocusRenderPanel( )->fGetScreen( )->fRgbaClearColor( );
	...
	void tSigFxMainWindow::fOnShowLevelInBackgroundClicked( wxCommandEvent& event )
	{
		if( event.IsChecked( ) )
		{
			tResourcePtr resource = fGuiApp( ).fEditableObjects( ).fGetResourceDepot( )->fQuery( tResourceId::fMake< tSceneGraphFile >( tFilePathPtr( "Effects\\editor_preview\\fx_preview_level.sigb" ) ) );
			mBackgroundLevelEntity.fReset( new tEditableSgFileRefEntity( fGuiApp( ).fEditableObjects( ), resource ) );
		}
		else
		{
	...
	void tSigFxMainWindow::fOnBackgroundColourChanged( wxColourPickerEvent& event )
	{
		...
		mRenderPanelContainer->fGetFocusRenderPanel( )->fGetScreen( )->fSetRgbaClearColor( r, g, b );
	}
