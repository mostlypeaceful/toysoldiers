...
wxFont tGraphButton::mFontSel( 10, wxFONTFAMILY_MODERN, wxFONTSTYLE_NORMAL | wxFONTFLAG_ANTIALIASED, wxFONTWEIGHT_BOLD, true, wxT( "Georgia" ) );
	...
	void tSigFxGraphline::fBuildPageFromEntities( tEditorSelectionList& selected )
	{
		...
		if( fxps )	// Fill out Particle Systems
		{
			...
			FX::tParticleSystemPtr ps = fxps->fGetParticleSystem( );

			for( u32 i = 0; i < cEmissionGraphCount; ++i )
			{
				tSigFxGraph* graph = new tSigFxGraph( &ps->fState( ).fToolState( ).mEmissionGraphs[ i ], mSelectedEntity, wxString( tToolParticleSystemState::mEmissionGraphNames[ i ].fCStr( ) ), mTheKeyline->fMainWindow( ) );
			...
			for( u32 i = 0; i < cParticleGraphCount; ++i )
			{
				...
				tSigFxGraph* graph = new tSigFxGraph( &ps->fState( ).fToolState( ).mPerParticleGraphs[ i ], mSelectedEntity, wxString( tToolParticleSystemState::mPerParticleGraphNames[ i ].fCStr( ) ) + "*", mTheKeyline->fMainWindow( ) );
				mGraphs.fPushBack( graph );
				mGraphButtons[ gbIdx++ ]->fSetGraph( graph );
			...
			{
				// since these graphs x-axis are determined per-particle we add an asterick to the end of the name to denote that
				tSigFxGraph* graph = new tSigFxGraph( &ps->fState( ).fToolState( ).mMeshGraphs[ i ], mSelectedEntity, wxString( tToolParticleSystemState::mMeshGraphNames[ i ].fCStr( ) ) + "*", mTheKeyline->fMainWindow( ) );
				mGraphs.fPushBack( graph );
				mGraphButtons[ gbIdx++ ]->fSetGraph( graph );
		...
		{
			FX::tParticleAttractorPtr pa = fxa->fGetAttractor( );
			...
			for( u32 i = 0; i < cAttractorGraphCount; ++i )
			{
				...
				tSigFxGraph* graph = new tSigFxGraph( &pa->fGetAttractorData( )->fGraph( i ), mSelectedEntity, wxString( tToolAttractorData::mGraphNames[ i ].fCStr( ) ), mTheKeyline->fMainWindow( ) );
				mGraphs.fPushBack( graph );
				mGraphButtons[ gbIdx++ ]->fSetGraph( graph );
		...
		else if( fxms )		// Okay now fill out the Mesh Systems
		{
