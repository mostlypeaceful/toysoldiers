...
/**
 * @brief This file contains the implementation of functions related to asset generation.
 *
 * The functions in this file handle parsing command line options, logging options, registering plugins,
 * generating assets, and scanning for duplicate class names in .nut files. The code is organized into
 * several private functions, each responsible for a specific task. The main entry point is the `fParseOptions`
 * function, which takes in command line arguments and populates the `tAssetGenOptions` structure. The `fLogOptions`
 * function logs the parsed options. The `fRegisterPlugins` function loads and registers asset plugins based on
 * the current project file. The `fGenerateAssets` function scans directories for files to convert. The `GetClassNamesFromFile`
 * function extracts class names from a given file. Finally, the `fDumpDuplicateSquirrelClasses` function scans .nut files
 * for duplicate class names and logs any warnings. These functions work together to facilitate the asset generation process.
 */
namespace
{
	...
	void fParseOptions(tAssetGenOptions &opts, const std::string &cmdLineBuffer)
	{
		...
		if (file.fFound())
			opts.mFile = file.fGetTypedOption<std::string>();

		const tCmdLineOption ext("ext", cmdLineBuffer);
		if (ext.fFound())
			opts.mExt = ext.fGetTypedOption<std::string>();
		...
		if (sacmlOutput.fFound())
			opts.mSacmlOutput = sacmlOutput.fGetTypedOption<std::string>();
	...
	void fLogOptions(const tAssetGenOptions &opts)
	{
		...
		if (opts.mExt.length() > 0)
			log_line(0, "Found option: ext=[" << opts.mExt << "]");
		if (opts.mRecursive)
	...
	void fDumpDuplicateSquirrelClasses(tConsoleApp &app, const tAssetGenOptions &opts)
	{
		log_newline();
		...
		{
			log_warning(0, "The current directory [" << curDir << "] is not a subfolder of the current project's Res folder; aborting.");
			return;
		}
		...
		for (u32 i = 0; i < classNames.fCount(); ++i)
		{
			...
			if (fileCount > 1)
			{
				...
				output << "!WARNING! >> Duplicate class: " << classNames[i];
				for (u32 j = 0; j < fileCount; ++j)
					output << "\n\t" << filePaths[i][j];

				output << "\n";
				log_output(0, output.str());
			}
		}
	...
	{
		if (opts.mSquirrelCheck)
			fDumpDuplicateSquirrelClasses(app, opts);
