...
namespace Sig
{
		   ...
		   , mChangesTimer( 0 )
	{
		...
		mMainSizer->Add( vert, 1, wxEXPAND );
		vert->Add( mToolbar, 0, wxEXPAND );
		...
		vert->Add( mScriptNotebook, 1, wxEXPAND );

		wxScrolledWindow* rightPanel = new wxScrolledWindow( this, wxID_ANY, wxDefaultPosition );
		mMainSizer->Add( rightPanel, 0, wxEXPAND | wxALL, 0 );
		rightPanel->SetMinSize( wxSize( 320, -1 ) );
		wxBoxSizer* rightPanelSizer = new wxBoxSizer( wxVERTICAL );
		rightPanel->SetSizer( rightPanelSizer );

		{
			wxPanel* group = new wxPanel( rightPanel, wxID_ANY, wxDefaultPosition, wxSize(-1, -1), wxBORDER_SIMPLE );
			group->SetForegroundColour( cTextColor );
			rightPanel->GetSizer( )->Add( group, 1, wxEXPAND | wxALL, 5 );
			group->SetSizer( new wxBoxSizer( wxVERTICAL ) );

			// Add text title.
			wxStaticText* text = new wxStaticText( group, wxID_ANY, "Script Browser" );
			text->SetForegroundColour( cGroupTitleColour );
			group->GetSizer( )->Add( text, 0, wxALIGN_LEFT | wxLEFT | wxTOP, 5 );

			mBrowser = new tScriptBrowser( group, this, wxDefaultSize.y );
			...
			group->GetSizer( )->Add( mBrowser, 1, wxEXPAND | wxALL, 5 );
			mBrowser->fRefresh( );
		}
