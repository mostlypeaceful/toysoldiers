...
namespace Sig
{
		...
		, mRecentFiles( ToolsPaths::fGetSignalRegistryKeyName( ) + "\\Shade\\MainWindow\\" + ToolsPaths::fGetCurrentProjectName( ) + "\\RecentFiles" )
	{
		...
		mMatEd = new tMatEdMainWindow( this, mCanvas->fEditorActions( ), ToolsPaths::fGetSignalRegistryKeyName( ) + "\\Shade\\MatEd", false );
	...
	void tShadeMainWindow::fOnAction( wxCommandEvent& event )
	{
		...
		else
		{
			...
			case cActionAbout:
				wxMessageBox( "SigShade is a visual, node-based shader creation tool.", "About SigShade", wxOK | wxICON_INFORMATION );
				break;
