...
namespace Sig
{
	...
	void tShadeNodeCanvas::fFromDermlFile( const Derml::tFile& file, b32 addToScene )
	{
	...
	b32 tShadeNodeCanvas::fPrepareClipboardForNextPaste( )
	{
		const u32 nextUid = tShadeNode::fNextUniqueNodeId( );
		mClipboard.fSaveXml( cScratchFilePath, false );
		mClipboard = Derml::tFile( );
		if( !mClipboard.fLoadXml( cScratchFilePath ) )
			return false;
		tShadeNode::fSetNextUniqueNodeId( nextUid );
		for( u32 i = 0; i < mClipboard.mNodes.fCount( ); ++i )
		{
			mClipboard.mNodes[ i ]->fGrabNextUniqueNodeId( );
			mClipboard.mNodes[ i ]->fMove( wxPoint( 8, 8 ) );
		}
		return true;
	}
