...
namespace Sig
{
	...
	MStatus tMayaSigmlExporter::fImport( const MString& path )
	{
		...
		{
			log_warning( 0, "Error trying to import [" << path.asChar( ) << "] - the file either doesn't exist or is corrupt." );
	...
	MStatus tMayaSigmlExporter::fImportSigml( const Sigml::tFile& sigmlFile, const Math::tMat3f& curMatrix )
	{
		for( u32 i = 0; i < sigmlFile.mObjects.fCount( ); ++i )
		{
			...
			if( status.error( ) )
				log_warning( 0, "Error importing object: " << status.errorString( ).asChar( ) );
		}
	...
	MStatus tMayaSigmlExporter::fImportSceneRef( const Sigml::tFile& parentFile, const Sigml::tSigmlReferenceObject& sigmlObject, const Math::tMat3f& localToWorld )
	{
		...
		{
			log_warning( 0, "Error importing sub-scene [" << sigmlObject.mReferencePath << "] - the file either doesn't exist or is corrupt." );
			return MStatus::kFailure;
		}
	...
	MStatus tMayaSigmlExporter::fImportGeomObject( const Sigml::tFile& parentFile, const Sigml::tGeomObject& sigmlObject, const Math::tMat3f& localToWorld )
	{
		...
		float mm[4][4]={0};
		fMemCpy( mm[0], &curMatrix.mRow0, sizeof( mm[0] ) );
		fMemCpy( mm[1], &curMatrix.mRow1, sizeof( mm[1] ) );
		fMemCpy( mm[2], &curMatrix.mRow2, sizeof( mm[2] ) );
		fMemCpy( mm[3], &row3, sizeof( mm[3] ) );

		MFnTransform transformFn( meshObj );
		transformFn.set( MTransformationMatrix( MMatrix( mm ).transpose( ) ) );

		if( status.error( ) )
			log_warning( 0, "Error creating mesh node" );
	...
	void tMayaSigmlExporter::fExportCommonBegin( const MString& path )
	{
		...
		mExportFile->mModellingPackage = tStringPtr( "Maya.2010" );
	...
	void tMayaSigmlExporter::fVisitGeometry( MDagPath& path, MFnDagNode& nodeFn, MObject& mobject )
	{
		...
		f32 stateIndex = 0;
		if( MayaUtil::fGetFloatAttribute( mobject, nodeFn, stateIndex, tExporterToolbox::fNameStateIndex( ) ) )
			object->mStateIndex = ( s16 )stateIndex;
		...
		fAcquireObjectMesh( object.fGetRawPtr( ), path, nodeFn, mobject );
	...
	void tMayaSigmlExporter::fAcquireObjectMesh( Sigml::tGeomObject* sigmlObject, MDagPath& path, MFnDagNode& nodeFn, MObject& mobject )
	{
	...
	Sigml::tSkinPtr tMayaSigmlExporter::fBuildSkin( MDagPath& path, MFnMesh& meshFn )
	{
		...
		for( u32 i = 0; i < numVerts; ++i, iVertex.next( ) )
		{
			...
			if( wgtsLen != joints.length( ) )
				log_warning( 0, "wgtsLen = " << wgts.length( ) << ", joints.length( ) = " << joints.length( ) );
	...
	MObject tMayaSigmlExporter::fFindSkinCluster( MFnMesh& fnMesh )
	{
		for( u32 i = 0; i < mSkinClusters.fCount( ); ++i )
		{
			...
			{
				log_warning( 0, "found a skin cluster with no outputs [" << skin.name( ).asChar( ) << "]" );
