...
namespace AutoTest
{
    public delegate void OnOutputDebugStringHandler( string text );

    public class DebugMonitor_PCDX11 : DebugMonitor
    {
        /// <summary>
        /// Private constructor so no one can create a instance
        /// of this static class
        /// </summary>
        public DebugMonitor_PCDX11( )
        {
            OnOutputDebugString += new OnOutputDebugStringHandler( pcdx9_OnStdNotify );

            InitializeManagers( "127.0.0.1" );
        }
        ...
        /// </summary>
        private void pcdx9_OnStdNotify( string text )
        {
            // Log the message.
            string message = text.TrimEnd( " \t\n".ToCharArray( ) );
