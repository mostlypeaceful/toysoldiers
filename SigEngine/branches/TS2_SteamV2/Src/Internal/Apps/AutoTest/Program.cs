...
namespace AutoTest
{
    class Program
    {
        ...
        static void RunMaps( MapList mapList )
        {
            ...
            DebugMonitor_PCDX11 monitor_pcdx11 = null;

            for( int i = 0; i < mapList.MapCount; ++i )
            {
                Map map = mapList[ i ];

                string launchArgs = "-min -nowatson -noprogress -nopause";
                ...
                switch( map.Platform )
                {
                    ...
                    case Platforms.pcdx9:
                        if (monitor_pcdx9 == null)
                        {
                            ...
                            if (!Debugger.IsAttached || DialogResult.Yes == MessageBox.Show
                                ("Pausing execution of AutoTest via the debugger may cause massive lockups and slowdowns as global debug output locks get held.\n"
                                + "Do you still want AutoTest to attach to the debug output?  Do note that AutoTest will hardly function without it."
                                , "Warning!"
                        ...
                        break;
                    case Platforms.pcdx11:
                        if (monitor_pcdx11 == null)
                        {
                            monitor_pcdx11 = new DebugMonitor_PCDX11();
                            if( !Debugger.IsAttached || DialogResult.Yes == MessageBox.Show
                                ( "Pausing execution of AutoTest via the debugger may cause massive lockups and slowdowns as global debug output locks get held.\n"
                                + "Do you still want AutoTest to attach to the debug output?  Do note that AutoTest will hardly function without it."
                                , "Warning!"
                            ...
                            {
                                DebugMonitor_PCDX11.Start( );
                            }
                        }

                        monitor = monitor_pcdx11;

                        launchArgs += " -platform pcdx11";
                        break;
                }
