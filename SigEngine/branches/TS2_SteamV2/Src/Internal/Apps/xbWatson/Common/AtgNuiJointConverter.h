...
namespace ATG
{
    ...
    public:
        NuiJointConverter();
        VOID  ApplyJointSmoothingAndConstraintsToSkeleton( const NUI_SKELETON_DATA* pNuiSkeleton, BOOL bFilterJointOrientations );
        VOID  ConvertCameraToAvatarSkeleton( const NUI_SKELETON_DATA* pNuiSkeleton, XAVATAR_SKELETON_POSE_JOINT* pAvatarJoints );
        VOID  DrawSkeleton( ::D3DDevice* pd3dDevice, XMMATRIX matWVP ) const;
        VOID  AddJointConstraint( INTERMEDIATE_JOINT_INDEX joint, XMVECTOR dir, FLOAT angle ); 
