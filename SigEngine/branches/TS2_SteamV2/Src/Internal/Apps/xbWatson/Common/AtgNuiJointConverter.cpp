...
const IntermediateJointDesc g_IntJointDesc[ INTERMEDIATE_JOINT_COUNT ] =
{
...
//--------------------------------------------------------------------------------------
VOID NuiJointConverter::AddJointConstraint(INTERMEDIATE_JOINT_INDEX joint, XMVECTOR dir, FLOAT angle)
{
    m_jointConstraints[m_dwNumJointConstraints].m_intJoint = joint;
    m_jointConstraints[m_dwNumJointConstraints].m_vDir = dir;
    m_jointConstraints[m_dwNumJointConstraints].m_fAngle = angle;
...
//--------------------------------------------------------------------------------------
VOID NuiJointConverter::ConvertCameraToAvatarSkeleton( const NUI_SKELETON_DATA* pNuiSkeleton, XAVATAR_SKELETON_POSE_JOINT* pAvatarJoints )
{
    ApplyJointSmoothingAndConstraintsToSkeleton( pNuiSkeleton, FALSE );
...
//--------------------------------------------------------------------------------------
// Name: ApplyJointSmoothingAndConstraintsToSkeleton()
...
//--------------------------------------------------------------------------------------
VOID NuiJointConverter::ApplyJointSmoothingAndConstraintsToSkeleton( const NUI_SKELETON_DATA* pNuiSkeleton, BOOL bFilterJointOrientations )
{
...
//--------------------------------------------------------------------------------------
// Name: ConvertCoordinateSystem()
// Desc: Flip the Z direction to match our coordinate space.
//--------------------------------------------------------------------------------------
VOID NuiJointConverter::ConvertCoordinateSystem( )
{
    // Create a scale vector.
    static const XMVECTOR vConvertToOurSpace = XMVectorSet
    ( 
        1.0f,
        1.0f,  
        -1.0f, // Flip in Z
    ...
    {
        // Scale all values to get into our space.
        m_NuiSkeleton.SkeletonPositions[nJoint] = XMVectorMultiply( m_NuiSkeleton.SkeletonPositions[ nJoint ], vConvertToOurSpace );
    }
}
...
VOID NuiJointConverter::CalculateJointRotations()
{
    ...
    for( int i = 0; i < ARRAYSIZE( m_IntJoints ); ++i )
    {
        ...
        else
        {
            ...
            // Transform the bone direction into the parent's world space
            XMVECTOR vBoneDir = XMVector3Transform( g_IntJointDesc[i].vBoneDir, matParentRot );

            // Find the rotation between the skeleton's direction and the desired bone direction
            XMVECTOR qRot = GetShortestRotationBetweenVecs( vDir, vBoneDir );
...
VOID NuiJointConverter::ApplyJointConstraints()
{
    ...
    for(int i = 0; i < ARRAYSIZE(g_IntJointDesc); i++)
    {
        ...
        {
            m_IntJoints[ i ].quatWorld = XMQuaternionMultiply( m_IntJoints[ g_IntJointDesc[i].parentIdx].quatWorld, m_IntJoints[i].quatLocal);
