...
namespace Sig
{
	...
	public:
			...
			, mVSizer( 0 )
		{
			...
			{
				mColumnSizer->Insert( layerIndex - 1, mRowSizer, 0, wxEXPAND | wxTOP, 4 );
			}
			fApplyBGColor( false );
			...
			// object list
			mItemList = new wxListBox( this, wxID_ANY );
		...
		void fRefreshList( )
		{
			...
			if( mItemList->IsShown( ) )
			{
				...
				mOwner->fEditableObjects( ).fGetLayer( objectsInLayer, fEffectiveName( ) );
		...
		{
			std::string name = obj->fGetName( );

			if( name.length( ) )
				mItemList->Append( name, obj );
			else
			{
				tEditableSgFileRefEntity* sceneRef = obj->fDynamicCast<tEditableSgFileRefEntity>( );
				if( sceneRef )
				{
					std::string name = StringUtil::fNameFromPath( sceneRef->fResourcePath( ).fCStr( ) );
					if( !name.length( ) )
						name = "Sigml or Mshml";
					mItemList->Append( name, obj );
				}
		...
		void fOnAction( wxCommandEvent& event )
		{
			...
			case cActionSelectObjectsInLayer:
				{
					tGrowableArray<tEditableObject*> objectsInLayer;
					mOwner->fEditableObjects( ).fGetLayer( objectsInLayer, layerName );
			...
			case cActionRemoveSelectedFromLayer:
				{
					if( !mDefaultLayer )
					{
						...
						mOwner->fEditableObjects( ).fGetLayer( objects, fEffectiveName( ) );
		...
		void fOnColorButtonPressed( wxCommandEvent& )
		{
			...
			if( tWxSlapOnColorPicker::fPopUpDialog( mButtonColor, cpd ) )
			{
				...
				tGrowableArray<tEditableObject*> objects;
				mOwner->fEditableObjects( ).fGetLayer( objects, layerName );
				for( u32 i = 0; i < objects.fCount( ); ++i )
	...
	tLayerPanel::tLayerPanel( tWxToolsPanel* parent )
		: tWxToolsPanelTool( parent, "Layers", "Layers", "Layer" )
		, mLayerGroup( 0 )
	...
	void tLayerPanel::fReset( const Sigml::tFile& file )
	{
		...
		if( file.mLayers.fCount( ) > 0 )
		{
			// update default layer
			mLayers[ 0 ]->fUpdateLayer( file.mLayers[ 0 ] );
		}

		wxSizer* vSizer = mLayerGroup->fGetMainPanel( )->GetSizer( );
		for( u32 layerIdx = 1, i = 1; i < file.mLayers.fCount( ); ++layerIdx, ++i )
			mLayers.fPushBack( new tObjectLayerGui( this, mLayerGroup->fGetMainPanel( ), vSizer, file.mLayers[ i ], false, mBaseLayerSizerIndex + layerIdx ) );
	...
	void tLayerPanel::fMergeLayers( const Sigml::tFile& file )
	{
		...
		wxSizer* vSizer = mLayerGroup->fGetMainPanel( )->GetSizer( );
		for( u32 fileIdx = 0; fileIdx < file.mLayers.fCount( ); ++fileIdx )
		{
			const tObjectLayer& thisFileLayer = file.mLayers[ fileIdx ];
			b32 found = false;
			...
			// Add unique layers.
			if( !found )
				mLayers.fPushBack( new tObjectLayerGui( this, mLayerGroup->fGetMainPanel( ), vSizer, file.mLayers[ fileIdx ], false, mLayers.fCount( ) ) );
		}
	...
	void tLayerPanel::fSaveLayers( Sigml::tFile& file, const tEditorSelectionList* selected ) const
	{
		if( selected )
		{
			for( u32 l = 0; l < mLayers.fCount( ); ++l )
			{
				...
				tGrowableArray<tEditableObject*> objectsInLayer;
				fEditableObjects( ).fGetLayer( objectsInLayer, layer.fName( ) );

				if( objectsInLayer.fCount( ) )
					file.mLayers.fPushBack( layer );
			}
		}
		else
		{
			file.mLayers.fSetCount( mLayers.fCount( ) );
			for( u32 i = 0; i < mLayers.fCount( ); ++i )
				file.mLayers[ i ] = static_cast<tObjectLayer&>( *mLayers[ i ] );
		}
	}
	...
	b32 tLayerPanel::fRenameLayer( tObjectLayerGui* layer, const std::string& name )
	{
		...
		tGrowableArray<tEditableObject*> objects;
		fEditableObjects( ).fGetLayer( objects, prevName );
		for( u32 i = 0; i < objects.fCount( ); ++i )
			objects[ i ]->fSetLayer( name );
		return true;
	}
