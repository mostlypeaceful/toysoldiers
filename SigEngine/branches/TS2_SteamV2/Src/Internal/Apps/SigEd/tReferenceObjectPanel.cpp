...
namespace Sig
{
	...
	{
	public:
		tSceneRefObjectCursor( tToolsGuiMainWindow& mainWindow, const tResourcePtr& sigbResource )
			: tEditorCursorController( mainWindow.fGuiApp( ) )
			, tPlaceObjectCursorBase( mainWindow, tEntityPtr( new tEditableSgFileRefEntity( mainWindow.fGuiApp( ).fEditableObjects( ), sigbResource ) ) )
		{
	...
	public:
		...
		{			
			...
			parent->GetSizer( )->Add( filtersPanel, 0, wxALL | wxEXPAND , 5 );
			{
				...
				filtersSizer->Add( searchPanel, 1, wxALL | wxEXPAND, 5 );
				{
					...
					mGetTiemls = new wxCheckBox( searchPanel, wxID_ANY, wxT("Tieml") );
					mGetTiemls->SetValue( true );
					mGetTiemls->Connect( wxEVT_COMMAND_CHECKBOX_CLICKED, wxCommandEventHandler(tResourceBrowserTree::fOnNeedRefresh), NULL, this );
					searchSizer->Add( mGetTiemls, 0, wxALL, 0 );
				}
				...
				mFilterText->Connect( wxEVT_COMMAND_TEXT_UPDATED, wxCommandEventHandler(tResourceBrowserTree::fOnNeedRefresh), NULL, this );
		...
		virtual void fOnSelChanged( wxTreeEvent& event, const tFileEntryData& fileEntryData )
		{
			...
			else
			{
				...
				mEditorWindow->fGuiApp( ).fSetCurrentCursor( tEditorCursorControllerPtr( new tSceneRefObjectCursor( *mEditorWindow, sigbResource ) ) );
			}
		...
		virtual wxColour fProvideCustomEntryColor( const tFileEntryData& fileEntryData )
		{
			...
			if( StringUtil::fStrStrI( fileEntryData.fXmlPath( ).fCStr( ), ".tieml" ) )
				return wxColour( 0x00, 0x55, 0x77 );
			return wxColour( 0, 0, 0 );
		}
		virtual b32 fFilterPath( const tFilePathPtr& path )
		{
			// Disregard everything that isn't a reference file.
			if( !Sigml::fIsSigmlFile( path ) && !Fxml::fIsFxmlFile( path ) && !Tieml::fIsTiemlFile( path ) )
				return true;
			...
			if( !( (mGetSigmls->GetValue( ) && tSceneGraphFile::fIsSigmlFileExclusive( path ))
				...
				|| (mGetTiemls->GetValue( ) && Tieml::fIsTiemlFile( path )) ) )
				return true;
		...
		virtual std::string fConvertDisplayName( const std::string& simplePath, u32& sortValue )
		{
			...
			else if( StringUtil::fStrStrI( simplePath.c_str( ), ".tieml" ) )
			{
				displayName << "[tile] ";
				sortValue = 2;
			}
			...
			case cActionRunAssetGen:
				{
					...
					if( fileEntryData )
					{
						...
						const tResourceId rid = tResourceId::fMake( isFx ? Rtti::fGetClassId<FX::tFxFile>( ) : Rtti::fGetClassId<tSceneGraphFile>( ), pathToReload, false );
