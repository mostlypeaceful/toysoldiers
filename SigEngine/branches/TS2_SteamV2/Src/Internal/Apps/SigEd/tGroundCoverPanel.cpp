...
namespace Sig
{
	...
	public:
		...
		tGroundCoverLayerVisibilityAction(u32 id, tGroundCoverPanel * panel );
	...
	public:
		...
		void fSetVisibility( u32 vis, b32 isEdit )
		{
			if( isEdit )
				fStartVisibilityEdit( );

			tGroundCoverLayer::fSetVisibility( vis );
	...
	private:
		...
		void fStartVisibilityEdit( )
		{
			sigassert( mCurrentAction.fNull( ) );
			mCurrentAction.fReset( new tGroundCoverLayerVisibilityAction( fUniqueId( ), mOwner ) );
			mOwner->fGuiApp( ).fActionStack( ).fAddAction( mCurrentAction );
		}
	...
	//------------------------------------------------------------------------------
	tGroundCoverLayerVisibilityAction::tGroundCoverLayerVisibilityAction(u32 id, tGroundCoverPanel * panel )
		: mId( id )
		, mPanel( panel )
	{
		mStart = panel->fFindLayer( id )->fVisibility( );
	}

	//------------------------------------------------------------------------------
	void tGroundCoverLayerVisibilityAction::fUndo( )
	{
		mPanel->fFindLayer( mId )->fSetVisibility( mStart, false );
	}

	//------------------------------------------------------------------------------
	void tGroundCoverLayerVisibilityAction::fRedo( )
	{
		mPanel->fFindLayer( mId )->fSetVisibility( mEnd, false );
	}

	//------------------------------------------------------------------------------
	void tGroundCoverLayerVisibilityAction::fEnd( )
	{
		mEnd = mPanel->fFindLayer( mId )->fVisibility( );
	}
	...
	public:
		...
		tWxLayerVisibilityComboBox( wxWindow * parent )
			: tWxSlapOnComboBox( parent, "Range", 0, wxEXPAND )
			, mLayer( 0 )
		{
			fAddString( "Near" );
			fAddString( "Medium" );
		...
		void fSetLayer( tGroundCoverLayerGui * layer )
		{
			...
			mLayer = layer;
			if( layer )
				fSetSelection( layer->fVisibility( ) );
		}
	...
	private:
		...
		{
			if( mLayer )
				mLayer->fSetVisibility( fGetSelection( ), true );
		}
	...
	public:
			...
			, mOwner( owner )
		{
			...
			// Visibility
			{
				...
				mVisBox = new tWxLayerVisibilityComboBox( group->fGetMainPanel( ) );
			}
	...
	private:
		...
		tWxLayerVisibilityComboBox * mVisBox;
