...
namespace Sig
{
	...
	BEGIN_EVENT_TABLE(tSigTileMainWindow, wxFrame)
		...
		EVT_MENU(				cActionTileSetPalettesDialog, tSigTileMainWindow::fOnAction)
		...
		, mViewMode( cTiles )
	{
		...
		mCanvas.fReset( new tTileCanvas( this, 500, 500 ) );
		...
		mTileSetBrushesDialog = new tTileSetPigmentsDialog( this );
		mEditorDialogs.fPushBack( mTileSetBrushesDialog );
	...
	void tSigTileMainWindow::fSetupRendering( )
	{
		...
		mDatabase.fLoadRandomTileAssets( resourceDepot );
		...
		{
			mDatabase.fAddTileSet( resourceDepot, ToolsPaths::fMakeResAbsolute( tFilePathPtr( "Art/Dungeons/tileset_default" ) ), NULL );
		}
		...
		//const tCmdLineBufferPtr& cmdLineBuffer = fGuiApp( ).fGetCmdLine( );
	...
	void tSigTileMainWindow::fOnTick( )
	{
		...
		if( onTop )
		{
			...
			// step cursor
			if( mViewMode == cTiles && !fGuiApp( ).fCurrentCursor( ).fNull( ) )
	...
	void tSigTileMainWindow::fClearScene( b32 closing )
	{
		...
		mDatabase.fClearTileSetPigments( );
		mTileSetBrushesDialog->fClear( );
	...
	void tSigTileMainWindow::fOpenPalettesDialog( )
	{
		mTileSetBrushesDialog->Show( true );
	}
	...
	void tSigTileMainWindow::fAddToolbar( )
	{
		...
		mainToolBar->AddSeparator( );
		mainToolBar->AddTool( cActionTileSetPalettesDialog, "Toggle Tile Set Palette Dialog", wxBitmap( "tilesetbrushdialog" ), wxNullBitmap, wxITEM_NORMAL, "Open the tile set palettes dialog" );
	...
	void tSigTileMainWindow::fOnAction(wxCommandEvent& event)
	{
		...
		{	
			...
			case cActionCycleRandom:	mCanvas->fBakeRandomized( ); break;

			case cActionTileSetPalettesDialog: mTileSetBrushesDialog->fToggle( ); break;
	...
	{
		return std::string( Tieml::fGetFileExtension( ) );
	}
	...
	b32 tSigTileMainWindow::fSerializeDoc( const tFilePathPtr& path )
	{
		...
		mCanvas->fSerialize( tiemlFile );
		mDatabase.fSerializeTileSetPigments( tiemlFile );
