...
namespace Sig
{
	namespace
	{
		...
		std::string fKeyframeEventStringFromKey( u32 key )
		{
			...
			const tProjectFile::tEvent* event = tProjectFile::fGetCurrentProjectFileCached( ).fFindKeyframeEventByKey( key );
			if( event )
				eventType = event->mName;
			else
				log_warning( 0, "Could not find key frame event. key: " << key );
			return eventType;
		}
	}
	...
	namespace
	{
		...
		public:
				...
				, mEventType( 0 )
			{
				...
				u32 typeIndex = 0;
				const tProjectFile& pf = tProjectFile::fGetCurrentProjectFileCached( );
			...
			std::string fGetText( ) const { return mText->fIsEmpty( ) ? "NO_TAG" : mText->fGetValue( ); }
	...
	void tSigAnimEdDialog::fAddKeyframeEventToListBox( f32 time, const std::string& type, const std::string& tag )
	{
		...
		entryText << std::setprecision( 4 ) << std::fixed << time << ": " << type << ": " << tag;
	...
	b32 tSigAnimEdDialog::fExtractKeyFrameTag( s32 i, Anipk::tKeyFrameTag& kft ) const
	{
		...
		if( frags.fCount( ) != 3 )
		{
			log_warning( 0, "Wrong number of frags in fExtractKeyFrameTag" );
			return false;
		}


		const tProjectFile::tEvent* event = tProjectFile::fGetCurrentProjectFileCached( ).fFindKeyframeEventByName( frags[ 1 ] );
		if( !event )
		{
			log_warning( 0, "could not find event in fExtractKeyFrameTag, name: " << frags[ 1 ] );
			return false;
		}
