...
namespace Sig
{
	...
	public:
		...
		virtual void fSaveAllAnimPacks( )
		{
			...
			for( u32 p = 0; p < packCount; ++p )
			{
				...
				data.mAnipkFile.fSaveXml( absolutePath, true );
				data.mDirty = false;
			}
		...
		virtual f32 fGetAnimTime( const std::string& animName ) const
		{
			...
			{
				const tAnimatedSkeletonPtr & skeleton = ents[e]->fSkeleton( );
				if( !skeleton )
					continue;

				tAnimTrackPtr animTrack = skeleton->fFirstTrackWithTag( tStringPtr( animName ) );
				if( !animTrack )
					continue;
	...
	private:
		...
		void fOnResourceLoaded( tResource & theResource, b32 success )
		{
			...
			while( nextItem.IsOk( ) )
			{
				...
				if( data->mLoading && data->mAnimPackResource == resPtr )
				{
					...
					DeleteChildren( item );
					fAddAnimations( item, resPtr, animFilter );	
					data->mLoading = false;
				}
			}
		...
		void fOnAnimActivated( wxTreeEvent & e )
		{
			...
			tKeyFrameAnimDesc animTrackDesc( anim, 0.2f, oneShot ? 0.2f : 0.f );
			...
			for( u32 e = 0; e < selectedCount; ++e )
			{
				...
				const tAnimatedSkeletonPtr & skeleton = entity->fSkeleton( );
				if( skeleton.fNull( ) )
					continue;
				...
				tAnimTrackPtr track( new tKeyFrameAnimTrack( animTrackDesc ) );
				track->fSetTag( anim->mName->fGetStringPtr( ) );
