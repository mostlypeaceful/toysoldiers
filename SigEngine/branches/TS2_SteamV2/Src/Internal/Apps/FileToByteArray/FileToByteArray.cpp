...
namespace
{
	...
	s32 fConvertFile( const char* inputFile, const char* outputFile, b32 appendNull )
	{
		...
		{
			log_warning( 0, "Couldn't open input file [" << inputFile << "] for reading, aborting." );
			return -1;
		}

		tFileWriter dstFile( tFilePathPtr(outputFile), false );
		if( !dstFile.fIsOpen( ) )
		{
			log_warning( 0, "Couldn't open output file [" << inputFile << "] for writing, aborting." );
			return -1;
		}
...
int WINAPI WinMain( HINSTANCE h1, HINSTANCE h2, LPSTR cmdLine, int showCmd )
{
	...
	{
		log_warning( 0, "Input (-i) and Output (-o) filename options are required to run FileToByteArray; Append Null (-n) is optional." );
		return -1;
	}
