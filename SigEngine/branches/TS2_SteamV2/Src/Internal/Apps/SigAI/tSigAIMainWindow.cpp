...
namespace Sig
{
	...
	void tSigAIMainWindow::fCreateMainMenu( )
	{
...

		{
			...
			subMenu->Append( cActionEditMoMap, "&Edit MoMap" );
		...
		}
		{
			...
			subMenu->AppendSeparator( );
			subMenu->Append( cActionFind, "&Find" );
			subMenu->Append( cActionReplace, "R&eplace" );
		}
	...
	void tSigAIMainWindow::fOnAction( wxCommandEvent& event )
	{
		...
		else
		{
			...
			case cActionEditMoMap:  fEditMoMap( ); break;
	...
	{
		return "SigAI ~ " + mDocName + ( mCanvas->fEditorActions( ).fIsDirty( ) ? "*" : "" );
	}
	...
	{
		wxString file = wxFileSelector( "Choose MotionMap file", NULL, NULL, ".nut", "Momap Files (*.nut)|*.nut" );
		if( !file.empty( ) )
		{
	...
	void tSigAIMainWindow::fEditMoMap( )
	{
		if( _stricmp( mControlPanel->fMoMapPath( ).fCStr( ), tSigAIControlPanel::cNoMoMapText.c_str( ) ) == 0 )
			wxMessageBox( "No MoMap chosen yet.", "Oops" );
		else
		{
			std::string command( "sigscript.exe -c \"" + std::string( ToolsPaths::fMakeResAbsolute( mControlPanel->fMoMapPath( ) ).fCStr( ) ) + "\"" );
