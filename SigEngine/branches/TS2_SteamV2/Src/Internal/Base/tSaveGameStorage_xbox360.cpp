...
namespace Sig
{
	...
	Threads::tThreadReturn thread_call tSaveGameStorageEnumerator::fThreadMain( void* threadParam )
	{
		...
		{
			CloseHandle( handle );
			log_warning( 0, "tSaveGameStorageEnumerator::fThreadMain: XContentCreateEnumerator failed with error code: " << ret );
		...
		{
			CloseHandle( handle );
			log_warning( 0, "tSaveGameStorageEnumerator::fThreadMain: XEnumerate failed with error code: " << ret );
			enumerator->fSetFinished( );
			return ret;
		}
...
namespace Sig
{
	Threads::tThreadReturn thread_call tSaveGameStorage::fThreadMain( void* threadParam )
	{
		...
		if( saveGame->fSaveGameDesc( ).mDisplayName.length( ) )
			wcscpy_s( contentData.szDisplayName, saveGame->fSaveGameDesc( ).mDisplayName.c_str( ) );
		...
		if( ret != ERROR_SUCCESS )
		{
			log_warning( 0, "tSaveGameStorage::fThreadMain: XContentCreate failed with error code " << ret );
