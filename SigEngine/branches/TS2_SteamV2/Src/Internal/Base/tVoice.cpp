...
namespace Sig
{
	...
	sig_static_assert( tVoice::cProcessingModeCount <= 32 );
	...
	{
		tPlatformUserId * remoteTalkers = ( tPlatformUserId * )alloca( 
			mSystem.mMaxRemoteTalkers * sizeof( tPlatformUserId ) );
		const u32 numRemoteTalkers = fGetRemoteTalkers( remoteTalkers );
	...
	{
		tPlatformUserId * remoteTalkers = (tPlatformUserId *)alloca( 
			sizeof( tPlatformUserId ) * mSystem.mMaxRemoteTalkers );
		const u32 remoteTalkerCount = fGetRemoteTalkers( remoteTalkers );
	...
	b32 tVoice::fStartProcessingMode( u32 localHwIndex, u32 mode )
	{
		...
		if( !voice.mIsRegistered )
		{
			log_warning( 0, __FUNCTION__ " error: User must be registered" );
			return false;
		}
	...
	void tVoice::fSubmitVoiceData( const byte * buffer, u32 bufferSize )
	{
		while( bufferSize >= sizeof( tVoiceHeader ) )
		{
			...
			if( submitted != header->mByteCount )
			{
				...
				log_warning( 0, "Incoming voice buffer is full - lost " << header->mByteCount - submitted << " bytes of voice data" );
			}
	...
	void tVoice::fHandleMuteMsg( const byte * msgBytes, u32 msgLength )
	{
		...
		tMuteMsg msg; archive.fSaveLoad( msg );
