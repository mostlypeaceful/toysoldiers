...
namespace Sig 
{
	...
	void tLeaderboard::fReadByRank( u32 rankStart, u32 toRead )
	{
		...
		{
			log_warning( 0, "Error " << result << " creating stats enumerator" );
		...
		{
			log_warning( 0, "Error " << std::hex << result << " enumerating stats" );
			mReadData.fCloseEnumerator( );
			return;
		}
	...
	void tLeaderboard::fReadByPlatformId( const tPlatformUserId userIds[], u32 userIdCount )
	{
		...
		if( result != ERROR_IO_PENDING && result != ERROR_SUCCESS )
		{
			log_warning( 0, "Error " << std::hex << result << " reading user stats" );
			return;
		}
	...
	void tLeaderboard::fOnFriendsRead( )
	{
		...
		const DWORD friendsFound = ( mReadData.mFriendBuffer.fCount( ) / sizeof( XONLINE_FRIEND ) );
		...
		DWORD result = XUserReadStats( 
			0,								// Title ID
			friendsFound + 1,				// Num XUIDS
			(XUID *)1,						// XUIDs
			mBoardsToReadCount,				// Boards
			mReadData.mSpecData.fBegin( ),	// Specs
			&bufferSize,					// Result buffer size
			NULL, NULL );
		log_line( Log::cFlagSession, "XUserReadStats, Xuids: " << friendsFound + 1 << " Boards: " << mBoardsToReadCount << " Func: " __FUNCTION__ );
		sigassert( result == ERROR_INSUFFICIENT_BUFFER && "Unknown error getting XUserReadStats buffer size" );

		// Allocate a big enough buffer
		const u32 totalBufferSize = bufferSize + ( friendsFound + 1 ) * sizeof( tPlatformUserId );
		mReadData.mBuffer.fNewArray( totalBufferSize );
		...
		log_line( Log::cFlagSession, "XUserReadStats 2, Xuids: " << friendsFound + 1 << " Boards: " << mBoardsToReadCount << " Func: " __FUNCTION__ );
