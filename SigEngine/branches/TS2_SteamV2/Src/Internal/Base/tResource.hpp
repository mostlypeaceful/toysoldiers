...
namespace Sig
{
	...
	public:
		...
		struct base_export tTypeSpecificProperties
		{
			...
			inline tTypeSpecificProperties( )
				: mPostLoad( 0 ), mPreDestroy( 0 ), mIsSubResource( 0 ), mGatherSubResources( 0 ) { }
		};
		...
		///
		/// \brief Provides a backwards link to a file package and the header describing this resource.
		struct base_export tFilePackageLink
		{ 
			tFilePackage* mPackage; u32 mIthHeader;
			explicit inline tFilePackageLink( tFilePackage* package=0, u32 ithHeader=0 ) 
				: mPackage( package ), mIthHeader( ithHeader ) { }
			inline b32 fNull( ) const { return mPackage==0; }
			inline b32 operator==( const tFilePackage* package ) const { return mPackage == package; }
		};
	...
	private:
		...
		tLoadCallerIdArray					mLoadCallerIds;
		tGrowableArray<tFilePackageLink>	mFilePackageLinks;
		u64									mFileTimeStamp;
	...
	public:
		...
		tResource( const tResourceId& rid, tResourceDepot* owner );
		~tResource( );
	...
	public:
		...
		///
		/// \brief Access the array of load caller ids.
		inline const tLoadCallerIdArray& fGetLoadCallerIds( ) const { return mLoadCallerIds; }
		...
		///
		/// \brief Loads a file from the most suitable package file.
		/// \return false if a suitable package cannot be found (i.e., if one hasn't been registered).
		/// It will also return false if the file is already loading or loaded.
		/// \note If the file is already loaded or loading (whether from a file package or not), this
		/// method has no effect. If the file is not loaded, then a load will be initiated, and the file 
		/// package will be considered the load owner for the resource, and the resource will get unloaded 
		/// when the file package does.
		b32							fLoadFromPackage( const tLoadCallerId& lcid );

		///
		/// \brief Loads a stand-alone file (i.e., a file not in a package file). Unlike the fLoadFromPackage
		/// variant, the caller must supply a load caller id, as the caller is considered the load owner.
		void						fLoadStandard( const tLoadCallerId& lcid );
		...
		/// Additionally, all loading works based on ownership through load caller ids. This allows a form of
		/// reference-counted loading. If the supplied caller id has already been registered, this method will
		/// have no effect (as the file should already be loading/loaded).
		void						fLoad( const tLoadCallerId& lcid, const tResourceLoaderPtr& loader );

		///
		/// \brief Unload the specified resource. This generally just means deallocating the resource
		/// buffer, but will also invoke any pre-destroy callbacks that need to fire.
		/// \note If the supplied caller id has not previously been registered via a call to fLoad, then
		/// this method will have no effect. Additionally, if there are currently other caller ids still
		/// registered, then this method will remove the supplied caller id from the registered list,
		/// without actually unloading the file.
		void						fUnload( const tLoadCallerId& lcid );
		...
		///
		/// \brief Query for whether the supplied caller id is a currently registered loader.
		/// E.g., if the caller has called fLoad on this resource without calling fUnload, this
		/// method should return true.
		b32							fIsLoadCaller( const tLoadCallerId& lcid ) const;
		...
		///
		/// \brief Add a file package link.
		void						fAddFilePackageLink( tFilePackage* package, u32 ithHeader );

		///
		/// \brief Remove a file package link.
		void						fRemoveFilePackageLink( const tFilePackage* package );

		///
		/// \brief Retrieve a file package link.
		tFilePackageLink			fGetFilePackageLink( const tFilePackage* package );
	...
	private:
		...
		///
		/// \brief Adds a load-caller-id.
		/// \return true If the lcid was added (bcz it didn't already exist); false if the lcid
		/// already existed in the list of load caller ids.
		b32							fAddLoadCallerId( const tLoadCallerId& lcid );
		...
		///
		/// \brief Internal method for initiating an unload.
		void						fInitiateUnload( );

		///
		/// \brief Called internally from the destructor, not for external use.
		void						fRemoveFromDepot( );
		...
		///
		/// \brief In case there are multiple packages registered for this resource, this method
		/// will try to determine the best one to load from (if there's any difference).
		tFilePackageLink*			fFindBestPackageToLoadFrom( );

		///
		/// \brief Called internally to perform post-load cleanup/notifications.
		void						fOnLoadComplete( tResourceLoader::tLoadResult loadResult );
	};
