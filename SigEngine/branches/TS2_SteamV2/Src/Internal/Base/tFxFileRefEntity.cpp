...
namespace Sig
{
	...
	void tEffectRefEntityDef::fCollectEntities( tEntity& parent, const tEntityCreationFlags& creationFlags ) const
	{
		// TODO REFACTOR: Consider doing this more like the tSceneRefEntityDef fCollectEntities. Consider case of being attached to a bone and wanting to move freely within the bone ref-frame.
		const tResourcePtr& resource = mReferenceFile->fGetResourcePtr( );
		FX::tFxFileRefEntity* entity = NEW FX::tFxFileRefEntity( resource, mLoopCount, mStartupFlags & cStartupFlagLocalSpace, mPreLoadTime, this );
		fApplyPropsAndSpawnWithScript( *entity, parent, creationFlags );

		if( mLODSettings )
		{
			sigassert( mLODSettings->mFadeOverride || mLODSettings->mFadeSetting );
			Gfx::tRenderableEntity::fSetFadeSetting( *entity, ( Gfx::tRenderableEntity::tFadeSetting )mLODSettings->mFadeSetting, mLODSettings->mFadeOverride );
		}
	}
}
...
namespace Sig { namespace FX
{
	...
	tFxFileRefEntity::tFxFileRefEntity( const tResourcePtr& fxResource, s32 playCount, b32 local, f32 preloadTime, const tEffectRefEntityDef* entityDef )
	...
	void tFxFileRefEntity::fOnFxFileResourceLoaded( tResource& theResource, b32 success )
	{
		...
		fxFile->fCollectEntities( *this, tEntityCreationFlags( ) );
	...
	void tFxFileRefEntity::fPropagateSkeleton( tAnimatedSkeleton& skeleton )
	{
