...
namespace Sig { namespace FX
{
	...
	{
		const Math::tMat3f parentObjToWorld = mParent ? mParent->fObjectToWorld( ) : Math::tMat3f::cIdentity;
	...
	void tTracerTrailEntity::fHeavyLiftingMTSafeInternal( f32 dt, f32 parentLerp, const Math::tMat3f& parentObjToWorld )
	{
		...
		if( mParent )
		{
			...
			const Math::tMat3f trans = Math::fLerp( mPrevXform, parentObjToWorld, parentLerp );
			...
			if( mLastSpawn >= mDef.mSpawnRate )
			{
				...
				if( mDef.mSmooth )
				{
					...
					{
						tElement dummy, head;
						mElements.fPopLIFO( head );
						mElements.fPopLIFO( dummy );
						mElements.fPut( head );
					}
					...
					if( mElements.fNumItems( ) >= 3 )
					{
						...
						tElement oldHead;
						mElements.fPopLIFO( oldHead );
						mElements.fPut( fixup0 );
						mElements.fPut( fixup1 );
						mElements.fPut( oldHead );
						...
						const tElement tween1 = fCatmullRom( 
							...
							mHead,
							0.666f );
						mElements.fPut( tween0 );
						mElements.fPut( tween1 );
					}
				}

				mElements.fPut( mHead );
			}
		}
		else
		{
