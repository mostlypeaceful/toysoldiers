...
namespace Sig { namespace FX
{
	...
	void tMeshParticleList::fFastUpdate( f32 dt )
	{
		for( u32 i = 0; i < mParticles.fCount( ); )
		{
			...
			if( !p || ( p->mCurrentLife + dt ) > p->mLifetime )
			{
				mParticles.fErase( i );
				if( p )
	...
	void tMeshParticleList::fEmitParticles( tParticleListUpdateParams& updateParams, u32 emitCount )
	{
		...
		for( u32 iemit = 0; iemit < emitCount; ++iemit )
		{
			...
			const Math::tVec3f scale0 = state.fSampleEmissionGraph< Math::tVec3f >( updateParams.mSystemRand, cSpawnSizeGraph, systemDelta ) * xformScale;			
			...
			p->mYAxis = state.fSampleMeshGraph< Math::tVec3f >( updateParams.mSystemRand, cMeshInitialAxis, systemDelta ); 
			p->mYAxis.fNormalizeSafe( Math::tVec3f::cYAxis );

			p->mYAxisDelta = state.fSampleMeshGraph< Math::tVec3f >( updateParams.mSystemRand, cMeshAxisDelta, systemDelta ); 
	...
	void tMeshParticleList::fUpdateParticleList( tGrowableArray< tMeshParticle* >& particleList, tGrowableArray< u32 >& removalList, tParticleListUpdateParams& updateParams )
	{
		...
		for( u32 i = particleUpdateCount; i < particleList.fCount( ); ++i )
		{
			...
			p1->mCurrentColor = state.fSamplePerParticleGraph< Math::tVec4f >( updateParams.mSystemRand, cColorGraph, normalizedParticleLife );

			if( cUpdatePos )
			{
				p1->mPosition += p1->mDirection * state.fSamplePerParticleGraph< f32 >( updateParams.mSystemRand, cEnergyGraph, normalizedParticleLife ) * dt;
	...
	void tMeshParticleList::fWriteToVB( tParticleListUpdateParams& updateParams, tMeshParticle* p1, tMeshParticle* p2, tMeshParticle* p3, tMeshParticle* p4, f32 d1, f32 d2, f32 d3, f32 d4 )
	{
		...
		p1->mSpin = p1->mYaw0 * state.fSamplePerParticleGraph< f32 >( updateParams.mSystemRand, cSpinGraph, d1 );
		p2->mSpin = p2->mYaw0 * state.fSamplePerParticleGraph< f32 >( updateParams.mSystemRand, cSpinGraph, d2 );
		p3->mSpin = p3->mYaw0 * state.fSamplePerParticleGraph< f32 >( updateParams.mSystemRand, cSpinGraph, d3 );
		p4->mSpin = p4->mYaw0 * state.fSamplePerParticleGraph< f32 >( updateParams.mSystemRand, cSpinGraph, d4 );

		p1->mScale = Math::tVec3f(p1->mScale0, p1->mScale0z) * state.fSamplePerParticleGraph< Math::tVec3f >( updateParams.mSystemRand, cScaleGraph, d1 );
		p2->mScale = Math::tVec3f(p2->mScale0, p2->mScale0z) * state.fSamplePerParticleGraph< Math::tVec3f >( updateParams.mSystemRand, cScaleGraph, d2 );
		p3->mScale = Math::tVec3f(p3->mScale0, p3->mScale0z) * state.fSamplePerParticleGraph< Math::tVec3f >( updateParams.mSystemRand, cScaleGraph, d3 );
		p4->mScale = Math::tVec3f(p4->mScale0, p4->mScale0z) * state.fSamplePerParticleGraph< Math::tVec3f >( updateParams.mSystemRand, cScaleGraph, d4 );
	...
	void tMeshParticleList::fWriteToVB( tParticleListUpdateParams& updateParams, tMeshParticle* particle, f32 delta )
	{
		...
		particle->mSpin		= particle->mYaw0 * state.fSamplePerParticleGraph< f32 >( updateParams.mSystemRand, cSpinGraph, delta );
		particle->mScale	= Math::tVec3f( particle->mScale0, particle->mScale0z ) * state.fSamplePerParticleGraph< Math::tVec3f >( updateParams.mSystemRand, cScaleGraph, delta );
