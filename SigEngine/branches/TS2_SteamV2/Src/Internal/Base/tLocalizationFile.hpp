...
namespace Sig
{
	...
	{
		declare_reflector( );
		sig_make_loggable( tLocalizedString, StringUtil::fWStringToString( c_str( ) ) );
	public:
	...
	public:
		...
		const wchar_t* c_str( ) const { return ( const wchar_t* )fBegin( ); }
		u32 length( ) const { return fCount( ) ? fCount( ) - 1 : 0; }
		tLocalizedString& fJoinWithLocString( const tLocalizedString& locStr );
		tLocalizedString& fJoinWithCString( const char* str );
		tLocalizedString fClone( ) const;
		std::string fToCString( ) const { return StringUtil::fWStringToString( c_str( ) ); }
...
{
	template<>
	class tResourceConvertPath<tLocalizationFile>
	{
	public:
