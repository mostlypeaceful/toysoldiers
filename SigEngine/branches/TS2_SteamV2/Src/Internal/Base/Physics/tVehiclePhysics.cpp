...
namespace Sig { namespace Physics
{
	...
	void tVehiclePhysics::fComputeAndApplyWheelforcesMT( tLogic* logic, f32 dt )
	{
		...
		if( mTransform.fIsNan( ) )
		{
			...
			log_warning( Log::cFlagPhysics, "Killed vehicle because of NAN" );
		}
			...
			if( wheelProps.mFollowIndex == ~0 )
			{
				...
				if( mDoGroundCheck )
				{
					...
					if( rayCastCallback.mHit.fHit( ) )
					{
						...
						if( m3DPhysicsMode )
						{
							const tVec3f contactPt = wheelState.mRay.fEvaluate( rayCastCallback.mHit.mT );
	...
	void tVehiclePhysics::fKinematicVehicleMT( tLogic* logic, f32 dt )
	{
		...
		if( mDoGroundCheck && Physics_Vehicle_Enables_DoHeightLock )
		{
			...
			for( u32 i = 0; i < 4; ++i )
			{
				...
				mCornerRays[ i ] = ray;
				tGroundRayCastCallback rayCastCallback( *logic->fOwnerEntity( ), mProperties.mGroundMask );
				...
				if( rayCastCallback.mHit.fHit( ) )
				{
					tVec3f point = ray.fEvaluate( rayCastCallback.mHit.mT );
	...
	void tVehiclePhysics::fDrawDebugInfo( tLogic* logicP )
	{
		if( Physics_Vehicle__DrawDebug )
		{
			...
			for (u32 i = 0; i < 4; ++i)
			{
				...
				logicP->fSceneGraph( )->fDebugGeometry( ).fRenderOnce( mCornerRays[i].fEvaluate( 0.5f ), mCornerRays[i].fEvaluate( 0.5f ) + tVec3f(2,0,0), tVec4f( 0,1,0,1 ) );
			}
