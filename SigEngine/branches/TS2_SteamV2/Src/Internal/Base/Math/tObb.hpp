...
namespace Sig { namespace Math
{
	...
	public:
		...
		inline tObb( const tVector3<t>& center, const tVector3<t>& x, const tVector3<t>& y, const tVector3<t>& z )
			: mCenter( center )
		{
			mAxes[ 0 ] = x; mAxes[ 0 ].fNormalizeSafe( mExtents.x );
			mAxes[ 1 ] = y; mAxes[ 1 ].fNormalizeSafe( mExtents.y );
			mAxes[ 2 ] = z; mAxes[ 2 ].fNormalizeSafe( mExtents.z );
		}
		...
		{
			tVector3<t> result = fCornerWorldVector( 0 );
			t maxD = result.fDot( worldD );
			for( u32 i = 1; i < 8; ++i )
			{
				tVector3<t> temp = fCornerWorldVector( i );
				t d = temp.fDot( worldD );
				if( d > maxD )
				{
					maxD = d;
					result = temp;
		...
		tObb fInterpolate( const tObb& other, t amountOfOther ) const
		{
			...
			for( u32 i = 0; i < 3; ++i )
				o.mAxes[ i ] = fNLerp( mAxes[ i ], other.mAxes[ i ], amountOfOther );
