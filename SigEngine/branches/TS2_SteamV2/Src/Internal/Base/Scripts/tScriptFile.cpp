...
namespace Sig
{
	...
	const u32		tScriptFile::cVersion = 1;
	...
	void tScriptFile::fOnSubResourcesLoaded( const tResource& ownerResource )
	{
		for( u32 i = 0; i < mExportedFunctions.fCount( ); ++i )
			mExportedFunctions[ i ].mScriptObject.fConstruct( );
		for( u32 i = 0; i < mStandardExportedFunctions.fCount( ); ++i )
			mStandardExportedFunctions[ i ].mScriptObject.fConstruct( );
...
// TODO REFACTOR need to resolve the fact that scripts are dependent on game-specific code registration - 
	...
	void tScriptFile::fOnFileUnloading( )
	{
		for( u32 i = 0; i < mExportedFunctions.fCount( ); ++i )
