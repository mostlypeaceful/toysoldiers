...
{
	sig_static_assert( sizeof( tChunkyHeapAllocHeader ) == 20 + sizeof( tAllocStamp ) );
	...
	tChunkyHeapAllocHeader* tChunkySubHeap::fNewHeader( byte* startOfNewMemory, u32 numBytes, tChunkyHeapAllocHeader::tState state, const tAllocStamp& stamp )
	{
		...
		*header = tChunkyHeapAllocHeader( startOfNewMemory + mOccupyingHeaderSize, numBytes - mOccupyingHeaderSize, state );
		header->mStamp = stamp;
		return header;
	}
	...
	void tChunkySubHeap::fFreeInternal( tChunkyHeapAllocHeader* header )
	{
		...
		//header->mStamp = cHeaderStampFree;
	...
	void tChunkySubHeap::fDumpNamedAllocations( tMemoryDumpOptions& memoryDumpOptions, tPageDump* page )
	{
#ifdef sig_memory_dump
		if( page )
		{
			...
			for( tChunkyHeapAllocHeader* i = mInUseHead; i; i = i->mNext )
			{
				page->mAllocations.fPushBack( tAllocDump( tStampDump( i->mStamp, (u32)i->mClientAddr ) ) );
			}
			page->mFreed.fSetCapacity( 300 );
			for( tChunkyHeapAllocHeader* i = mFreeChunkHead; i; i = i->mNext )
			{
				page->mFreed.fPushBack( tAllocDump( tStampDump( i->mStamp, (u32)i->mClientAddr ) ) );
			}
		}
	...
	void tChunkySubHeap::fDumpSubset( const char* nameToLookFor )
	{
#ifdef sig_memory_dump
		// TODO, UNMAINTAINED
	...
	tChunkyHeapAllocHeader* tChunkySubHeap::fFindBestFit( u32 bytes )
	{
	...
	void tChunkySubHeap::fConsolidate()
	{
		if( !mFreeChunkHead )
		{
			//log_warning( Log::cFlagMemory, "tChunkySubHeap::Consolidate() being called with no free chunks!" );
			return;
		}
	...
	b32 tChunkySubHeap::fValidateChunk( tChunkyHeapAllocHeader* header ) const
	{
		...
		else
		{
			...
			{
				if( valid )
					Log::fPrintf( Log::cFlagMemory, onlyReportThisTextOnce, header, header->mStamp.mFile );
			...
			if( mOccupyingHeaderSize > 0 && header->mState != tChunkyHeapAllocHeader::cStateAllocd )
			{
				...
				for( u32 i=0; i < header->mClientByteCount; i+=4 )
				{
					...
					{
						if( valid )
							Log::fPrintf( Log::cFlagMemory, onlyReportThisTextOnce, header, header->mName);
