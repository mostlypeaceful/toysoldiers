...
namespace Sig { namespace AI
{
	...
	inline Sqrat::Object fGetLogicForScript( tLogic* logic ) 
	{ 
		sigassert( logic );
		sigassert( logic->fOwnerEntity( ) );
		sigassert( !logic->fOwnerEntity( )->fScriptLogicObject( ).IsNull( ) );
		return logic->fOwnerEntity( )->fScriptLogicObject( ); 
	}
	
	class tGoal : public tUncopyable, public tRefCounter
	{
	...
	public:
		...
		void			fProcess( tGoalPtr& goalPtr, tLogic* logic, f32 dt );
	...
	protected:
		...
		virtual b32		fHandleLogicEvent( tLogic* logic, const Logic::tEvent& e ) { return false; }
	...
	class tGoalPtr : public tScriptOrCodeObjectPtr<tGoal>
	{
		f32 mRelativePriority;
	public:
		tGoalPtr( ) { }
		tGoalPtr( const Sqrat::Object& o, f32 relativePriority ) 
			: tScriptOrCodeObjectPtr<tGoal>( o ), mRelativePriority( relativePriority ) { }
		...
		f32				fComputePriority( tLogic* logic );

		///
		/// \brief Notify the goal that it's being inserted into the goal stack.
		void			fOnInsertion( tLogic* logic ) const;
		...
		b32				fActivateIfNotActive( tLogic* logic );
		...
		b32				fSuspendIfActive( tLogic* logic );
	...
	class tCompositeGoal : public tGoal
	{
		friend class tGoalPtr;
	...
	public:
		...
		void			fPushGoalList( const tGoalPtrList& goals, tLogic* logic );

		///
		/// \brief Removes any existing goals.
		void			fRemoveSubGoals( tLogic* logic );
		...
		void			fSuspendAfterSubGoals( tLogic* logic ) { mSuspendAfterSubGoals = true; }
	...
	protected: // interface for derived types to manipulate
		...
		void			fProcessTriggeredGoals( tLogic* logic, f32 dt, b32 forceCheck );
		void			fProcessPotentialGoals( tLogic* logic, f32 dt );
