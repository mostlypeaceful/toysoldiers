...
namespace Sig { namespace Gfx
{
	...
	{
		const tGeometryBufferVRamSlice *geom = &fGeometry( );
		u32 startVert = geom->mStartVertex;
		u32 nVerts = geom->mNumVerts;
		Sig::byte* gpuVerts = geom->mBuffer->fQuickLock(startVert,nVerts);
		const tVertexFormatVRam *vertRam = &geom->mBuffer->fVertexFormat();
		u32 copySize = numVerts * vertRam->fVertexSize( );
		fMemCpyToGpu( gpuVerts, vertBuffer, copySize );
