...
namespace Sig { namespace Gfx
{
	...
	void tDebugFreeCamera::fGlobalAppTick( )
	{
		...
		if( gDebugEntity )
		{
			...
			if( gDebugEntity->fLogicPtr( ) )
			{
				std::string statsText;
				gDebugEntity->fLogicPtr( )->fComputeDebugText( statsText );
				gStatsText->fBakeBox( 400, statsText.c_str( ), 0, Gui::tText::cAlignLeft );
				gStatsText->fSetPosition( Math::tVec3f( 1240.f - 400.f, 40.f, 0.0f ) );
	...
	void tDebugFreeCamera::fOnTick( f32 dt )
	{
		...
		const Input::tGamepad& gamepad = mUsers.fFront( )->fFilteredGamepad( mGamepadFilterIndex );
