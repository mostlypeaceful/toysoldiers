...
namespace Sig { namespace Gfx
{
	...
	tDebugGeometryContainer::~tDebugGeometryContainer( )
	{
		if( mImpl->mSolidColorMaterialFile )
			mImpl->mSolidColorMaterialFile->fUnload( this );
	...
	{
		const Math::tVec3f normal = plane.fGetNormal( );
		const Math::tVec3f left = plane.fInverse( ).fGetNormalUnit( ) * scale;
