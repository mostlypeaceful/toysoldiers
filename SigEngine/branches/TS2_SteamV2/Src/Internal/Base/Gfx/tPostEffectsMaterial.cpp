...
namespace Sig { namespace Gfx
{
	namespace
	{
		...
		{
			tVertexElement( tVertexElement::cSemanticPosition, tVertexElement::cFormat_f32_2 ),
		};
	...
	void tPostEffectsMaterial::fApplyShared( const tDevicePtr& device, const tRenderContext& context, const tRenderBatchData& renderBatch ) const
	{
		...
		case cPShaderGaussBlurV:
			{
				sigassert( mInputCount == 1 );
				const f32 uScale = ( mPS == cPShaderGaussBlurH ) ? ( 1.f/mInputs[0].mWidth  ) : 0.f;
				const f32 vScale = ( mPS == cPShaderGaussBlurV ) ? ( 1.f/mInputs[0].mHeight ) : 0.f;
	...
	void tPostEffectsMaterial::fAddInput( const tRenderToTexturePtr& rtt )
	{
		...
		mInputs[ mInputCount++ ] = tInput( rtt.fGetRawPtr( ), rtt->fRenderTarget( )->fWidth( ), rtt->fRenderTarget( )->fHeight( ) );
	}
	...
	devrgb_clamp( Renderer_PostEffects_FilmGrain_Saturation, Math::tVec3f( 0.0f, 0.0f, 0.0f ), 0.f, 3.f, 2 );
	devrgb_clamp( Renderer_PostEffects_FilmGrain_Exposure, Math::tVec3f( 1.70f, 1.50f, 1.35f ), -10.f, +10.f, 2 );
	...
	void tPostEffectsMaterial::fStepFilmGrain( f32 dt, tRandom& rand, f32 scale )
	{
		// smudges
		{
			...
			const Math::tVec2f uvDelta = ( dt * rand.fFloatZeroToOne( ) * rand.fFloatMinusOneToOne( ) ) * Renderer_PostEffects_FilmGrain_Smudges_Speed;
		...
		// hairs/scratches
		{
			...
			const Math::tVec2f uvDelta = ( dt * rand.fFloatZeroToOne( ) * rand.fFloatMinusOneToOne( ) ) * Renderer_PostEffects_FilmGrain_Hairs_Speed;
		...
		// vertical lines
		{
			...
			const Math::tVec2f uvDelta = ( dt * rand.fFloatZeroToOne( ) * rand.fFloatMinusOneToOne( ) ) * Renderer_PostEffects_FilmGrain_Lines_Speed;
		...
		// grain
		{
			...
			const Math::tVec2f uvDelta = ( dt * rand.fFloatZeroToOne( ) * rand.fFloatMinusOneToOne( ) ) * Renderer_PostEffects_FilmGrain_Grain_Speed;
