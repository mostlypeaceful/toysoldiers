...
namespace Sig
{
	...
	void tFilePackageResourceLoader::fOnOpenSuccess( )
	{
		...
		const b32 willResize = fGetResource( )->fWillResizeAfterLoad( );

		if( resBuffer->fCanAlloc( mFileSizeUncompressed, willResize ) )
		{
			resBuffer->fAlloc( mFileSizeUncompressed, willResize, fMakeStamp( ) );
