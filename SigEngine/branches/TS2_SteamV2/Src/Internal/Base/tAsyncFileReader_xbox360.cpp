	...
	b32 tAsyncFileReader::fReadFileInThreadForPlatform( )
	{
		...
		while( absolutePos < lastBytePlus1 )
		{
			...
			if( mReadParams.mDecompressAfterRead )
			{
				...
				//if ~0 then some error occurred during decompression. GTFO
		...
		if( overwriteAmount < 0 )
		{
			...
			log_warning( 0, "File: " << mDebugContext << " could not read whole buffer." );
			return false;
		}
		...
		if( overwriteAmount > 0 )
		{
			...
			log_warning( 0, "File: " << mDebugContext << " is trampling memory by " << overwriteAmount << "; mReadParams.mDecompressAfterRead = " << mReadParams.mDecompressAfterRead );
