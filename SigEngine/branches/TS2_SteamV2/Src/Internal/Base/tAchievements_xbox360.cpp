...
namespace Sig
{
	...
	void tAchievementsWriter::fFinish( b32 wait )
	{
		...
		if( wait )
		{
			...
			{
				mState = cStateFail;
				log_warning( 0, __FUNCTION__ << " failed with GetOverlappedResult: " << error );
				return;
			}
	...
	void tAchievementsReader::fRead( u32 startIdx, u32 toRead, u32 details, tPlatformUserId target )
	{
		...
		{
			log_warning( 0, "Error " << std::hex << result << " creating achievements enumerator" );
		...
		{
			log_warning( 0, "Error " << std::hex << result << " enumerating achievements" );
			mReadData.fCloseEnumerator( );
			return;
		}
	...
	b32	tAchievementsReader::fIsAwarded( u32 id )
	{
		...
		{
			log_warning( 0, __FUNCTION__ << " called while not in Success state" );
			return false;
		}
		...
		log_warning( 0, __FUNCTION__ << ": Achievement (" << id << ") could not be found" );
		return false;
	}
