...
namespace Sig
{
	...
	b32 tContent::tContentOp::fCreate( 
		...
		const XCONTENT_CROSS_TITLE_DATA & contentData,
		u32 contentFlags )
	{
		if( !fPreExecute( ) )
			return false;
		...
		DWORD result = XContentCrossTitleCreate( 
			userIndex, 
			rootName.fCStr( ), 
			&mContentData, 
			contentFlags, 
			...
			1024,					// io cache (random guess between 20 and 900 KB )
			size,					// set to zero will not limit size of content
			fOverlapped( ) );

		if( result != ERROR_IO_PENDING )
		{
			log_warning( 0, "XContentCreate failed with error: " << result );
			return false;
		}
	...
	b32 tContent::tContentOp::fFlush( const tStringPtr & rootName )
	{
		...
		if( result != ERROR_IO_PENDING )
		{
			log_warning( 0, "XContentFlush failed with error: " << result );
			return false;
		}
	...
	b32 tContent::tContentOp::fClose( const tStringPtr & rootName )
	{
		...
		if( result != ERROR_IO_PENDING )
		{
			log_warning( 0, "XContentClose failed with error: " << result );
			return false;
		}
		...
		u32 contentFlags )
	{
		...
		log_warning( 0, "Creating content for user: " << mUserIndex << " XUserGetSigninState: " << XUserGetSigninState( mUserIndex ) );
	...
	b32 tContent::fAdvance( )
	{
		...
		DWORD error = mContentOp.fGetResult( result, false );
		...
		if( mState == cStateCreating )
		{
			...
			{
				log_warning( 0, "Content creation failed with error: ERROR_FILE_CORRUPT" );
				mState = cStateCorrupt;
			}
			else if( error == ERROR_ACCESS_DENIED )
			{
				log_warning( 0, "Content creation failed with error: ERROR_ACCESS_DENIED" );
				mState = cStateAccessDenied;
			}
			else
			{
				log_warning( 0, "Content creation failed with error: " << error );
				mState = cStateFailed;
			}
		}
		...
		else if( mState == cStateFlushing )
		{
			if( result != ERROR_SUCCESS )
				log_warning( 0, "Content flush failed with error: " << result );
		...
		{
			if( result != ERROR_SUCCESS )
				log_warning( 0, "Content close failed with error: " << result );
