...
namespace Sig
{
	//------------------------------------------------------------------------------
	template< class t >
	static void fWrite( tGrowableArray< byte > & buffer, const t * objects, u32 count )
	{
	...
	//------------------------------------------------------------------------------
	template< class t >
	static void fWrite( tGrowableArray< byte > & buffer, const t & obj )
	{
		fWrite( buffer, &obj, 1 );
	}
	...
	//------------------------------------------------------------------------------
	template< >
	static void fWrite(  tGrowableArray< byte > & buffer, const std::string & str )
	{
		u32 len = str.length( );
		fWrite( buffer, len );
	...
	//------------------------------------------------------------------------------
	template< >
	static void fWrite( tGrowableArray< byte > & buffer, const tFilePathPtr & path )
	{
	...
	//------------------------------------------------------------------------------
	template< class t >
	static const byte * fRead( tPlatformId source, const byte * data, u32 dataLength, t * objects, u32 count )
	{
	...
	//------------------------------------------------------------------------------
	template< class t >
	static const byte * fRead( tPlatformId source, const byte * data, u32 dataLength, t & obj )
	{
		sigassert( tIsBuiltInType< t >::cIs );
	...
	//------------------------------------------------------------------------------
	template< >
	static const byte * fRead( tPlatformId source, const byte * data, u32 dataLength, std::string & str )
	{
		...
		char * mem = ( char * )_alloca( sizeof( char ) * len );
		data = fRead( source, data, term - data, mem, len );

		str = std::string( mem, mem + len );
		return data;
	}

	//------------------------------------------------------------------------------
	template< >
	static const byte * fRead( tPlatformId source, const byte * data, u32 dataLength, tFilePathPtr & path )
	{
		const byte * term = data + dataLength;
		...
		path = tFilePathPtr( std::string( mem, mem + len ) );
		return data;
	}
