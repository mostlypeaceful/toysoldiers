...
namespace Sig
{
		...
		if( !mDocPtr->SaveFile( path.fCStr( ) ) )
		{
			...
			if( promptToCheckout && ToolsPaths::fPromptToCheckout( path ) )
			{
				if( displayWarnings )
				{
					...
					if ( ToolsPaths::fIsCheckedOut( path ) )
	...
	b32 tXmlFileData::fLoad( const tFilePathPtr& path, b32 dummy )
	{
        ...
        {
			log_warning( Log::cFlagNone, "Error Loading Xml File. Row: " << mDocPtr->ErrorRow( ) << " Column: " << mDocPtr->ErrorCol( ) << " Error: " << mDocPtr->ErrorDesc( ) );
			return false;
        }
	...
	b32 tXmlFileData::fParse( const char * text )
	{
		...
		if( mDocPtr->Error( ) )
		{
			log_warning( Log::cFlagNone, "Error parsing Xml text. Row: " << mDocPtr->ErrorRow( ) << " Column: " << mDocPtr->ErrorCol( ) << " Error: " << mDocPtr->ErrorDesc( ) );
			return false;
		}
