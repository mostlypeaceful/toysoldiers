...
namespace Sig
{
	...
	{
		sig_make_loggable(tFilePathPtr, ( fCStr( ) ? fCStr( ) : "(null)" ) );
	private:
...
#undef overload_comparison_operator
	...
	template<>
	class base_export tHash<tFilePathPtr> : public tHash<tHashTablePtrInt>
	{
	public:
		inline u32 operator()( const tFilePathPtr& key, const u32 maxSize ) const
		{
			return tHash<tHashTablePtrInt>::operator()( key.fGetHashValue( ), maxSize );
		}
	};
}
