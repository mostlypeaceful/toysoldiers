...
namespace Sig { namespace Input
{
	...
	public:
		...
		struct tStateData
		{
			...
			{
				// All fields must be sent over the wire because angle and strength are calculated from stick values
				// before normalization, and the stick values we have here are after normalization.
	...
	private:
		...
		static f32 fComputeStickMagnitude( const Math::tVec2f& stick, f32 rawMag, f32 angle );
