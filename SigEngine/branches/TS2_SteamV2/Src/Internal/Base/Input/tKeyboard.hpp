...
namespace Sig { namespace Input
{
	...
	public:
		...
		struct tStateData
		{
			typedef tFixedBitArray< 256 > tKeyBitBuffer;
			typedef tFixedArray< u8, 256 > tKeyBuffer;

			tKeyBitBuffer mKeysOn;
			tKeyBitBuffer mPrevKeysOn;
			...
			{
				archive.fSaveLoad( mKeysOn );
				archive.fSaveLoad( mPrevKeysOn );
				// The number of frames held is not used by TS:C code at all. Don't send it.
				//archive.fSaveLoad( mFramesHeld );
			}
		};
	...
	public:
		...
		static void fCaptureGlobalState( f32 dt = 1.f );

		///
		/// \brief Update the state of this instance (this will overwrite the
		...
		static void fCaptureGlobalStateUnbuffered( tStateData& stateData, f32 dt = 1.f );

		///
		/// \brief Capture this instances current state and put it in the output variable,
		/// without putting it storing it in this keyboard object's state data variable.
		/// Prefer using the tKeyboard class directly to query the current state.
		...
		///
		/// \brief set state into the state buffer.
	...
	public:
		...
		static const tButton cButtonComma;
		static const tButton cButtonPeriod;
		static const tButton cButtonSemiColon;
		static const tButton cButtonQuote;
		static const tButton cButtonBackslash;
		static const tButton cButtonTilde;
		static const tButton cButtonQuestionMark;
