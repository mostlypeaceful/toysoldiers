...
namespace Sig { namespace Input
{
	...
	/*static*/ Math::tVec2f tGamepad::fMapStickCircleToRectangle( const Math::tVec2f &stick, u32 innerRoundness )
	{
		...
		Math::tVec2f stickNormalized = stick;
		if( stickLen > 1.0f )
		{
			stickNormalized = stickNormalized.fNormalizeSafe( );
		}

		// Determine the theta angle
		f32 angle = Math::fAtan2( stickNormalized.y, stickNormalized.x ) + Math::cPi;
	 
		Math::tVec2f squared;
		// Scale according to which wall we're clamping to
		// X+ wall
		if( angle <= Math::cPiOver4 || angle > 7 * Math::cPiOver4 )
			squared = stickNormalized / Math::fCos(angle);
		// Y+ wall
		else if( angle > Math::cPiOver4 && angle <= 3 * Math::cPiOver4 )
			squared = stickNormalized / Math::fSin(angle);
		// X- wall
		...
		if (innerRoundness == 0)
			return squared * stickLen;
	 
		// Find the inner-roundness scaling factor and LERP
		f32 length = stickNormalized.fLength();
		f32 factor = pow( length, (f32)innerRoundness );
		return fLerp( stickNormalized, squared, factor ) * stickLen;
	}
	...
	void tGamepad::fExportScriptInterface( tScriptVm& vm )
	{
		...
		classDesc
			...
			.Func(_SC("Connected"), &tGamepad::fConnected)
