...
namespace CheckIn
{
    public partial class CheckInForm : Form
    {
        ...
        private void InitializeLists(object sender, DoWorkEventArgs e)
        {
            ...
            string resDir = Environment.GetEnvironmentVariable("SigCurrentProject") + "\\res";

            string workspaceName = GetWorkspaceName();
            string workspaceDir = GetWorkspaceDirectory();

            // Get a list of all files in the res directory
            List<string> fileList = new List<string>();
            List<string> ignoredList = ignoredForm.GetIgnoredList();
            ProcessDirectory(resDir, ref fileList, ref ignoredList);
            ...
            process = new P4Process("have \"" + resDir + "\\...\"");
            process.Start();
            int fileCount = 0;
            file = "";
            while (!process.StandardOutput.EndOfStream || !process.HasExited)
