	...
	public:
			...
			, mDirtyingAction( true )
		{
			...
			for( u32 idx = 0; idx < mSystems.fCount( ); ++idx )
			{
				...
				if( fxps )
				{
					...
					{
						tGraphPtr old;
						tSigFxGraph::fSetOldGraph( fxps->fGetToolState( )->mEmissionGraphs[ i ], &old );
						mOldGraphs.fPushBack( old );
					}
					for( u32 i = 0; i < cParticleGraphCount; ++i )
					{
						tGraphPtr old;
						tSigFxGraph::fSetOldGraph( fxps->fGetToolState( )->mPerParticleGraphs[ i ], &old );
						mOldGraphs.fPushBack( old );
					}
					for( u32 i = 0; i < cMeshGraphCount; ++i )
					{
						tGraphPtr old;
						tSigFxGraph::fSetOldGraph( fxps->fGetToolState( )->mMeshGraphs[ i ], &old );
						mOldGraphs.fPushBack( old );
					}
				}
				else if( fxa )
				{
					tToolAttractorDataPtr data = fxa->fGetToolData( );
		...
		virtual void fEnd( )
		{
			...
			for( u32 idx = 0; idx < mSystems.fCount( ); ++idx )
			{
				...
				{
					for( u32 i = 0; i < cEmissionGraphCount; ++i )
						mActions.fPushBack( new tGraphlineAction( &fxps->fGetToolState( )->mEmissionGraphs[ i ], mOldGraphs[ oldGraphIdx++ ] ) );
					for( u32 i = 0; i < cParticleGraphCount; ++i )
						mActions.fPushBack( new tGraphlineAction( &fxps->fGetToolState( )->mPerParticleGraphs[ i ], mOldGraphs[ oldGraphIdx++ ] ) );
					for( u32 i = 0; i < cMeshGraphCount; ++i )
						mActions.fPushBack( new tGraphlineAction( &fxps->fGetToolState( )->mMeshGraphs[ i ], mOldGraphs[ oldGraphIdx++ ] ) );
				}
				else if( fxa )
				{
					for( u32 i = 0; i < cAttractorGraphCount; ++i )
					{
						mActions.fPushBack( new tGraphlineAction( &fxa->fGetToolData( )->fGraph( i ), mOldGraphs[ oldGraphIdx++ ] ) );
					}
				}
				else if( fxms )
				{
