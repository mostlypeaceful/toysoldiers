...
namespace Sig
{
	...
	BEGIN_EVENT_TABLE(tEditableObjectProperties, wxFrame)
		...
		EVT_SET_FOCUS(	tEditableObjectProperties::fOnFocus )
	END_EVENT_TABLE()
		...
		, mDoGlobalProps( globalProps )
	{
		...
		mCaptureCameraDistance.fFromMethod< tEditableObjectProperties, &tEditableObjectProperties::fCaptureCameraDistance >( this );
		...
		tEditablePropertyCDCFloat::tCallbackData captureDistance;
		captureDistance.mFunc = mCaptureCameraDistance;
		tEditablePropertyCDCFloat::fAddCallback( Sigml::tObjectProperties::fEditablePropLODMediumDistanceName(), captureDistance );
		tEditablePropertyCDCFloat::fAddCallback( Sigml::tObjectProperties::fEditablePropLODFarDistanceName(), captureDistance );
		...
		mMainPanel->Connect(wxEVT_SET_FOCUS, wxFocusEventHandler( tEditableObjectProperties::fOnFocus ), NULL, this );
	}

	void tEditableObjectProperties::fSetFocusHooks( )
	{
		wxWindowList panelChildren = mMainPanel->GetChildren();
		fSetFocusHooksFromList( panelChildren );
	}

	// Recursively hook up SET_FOCUS hooks to all children
	void tEditableObjectProperties::fSetFocusHooksFromList( wxWindowList& list )
	{
		for( u32 i = 0; i < list.GetCount( ); ++i )
		{
			list[ i ]->Connect(wxEVT_SET_FOCUS, wxFocusEventHandler( tEditableObjectProperties::fOnFocus ), NULL, this );

			wxWindowList children = list[ i ]->GetChildren( );
			if( children.GetCount( ) > 0 )
				fSetFocusHooksFromList( children );
		}
	}
	...
	void tEditableObjectProperties::fOnSelectionChanged( )
	{
		...
		else
		{
			...
			std::string path = obj->fGetAssetPath();

			if( !path.empty() )
			{
				// Add a slash to the beginning of the path to make it easier to paste into a bare \res\ folder if you want.
				// The real cool thing to do would be intercept Ctrl-C and make it convert to an absolute path.
				tWxSlapOnTextBox* filePath = new tWxSlapOnTextBox( mMainPanel, "Asset Path:", -1, false );
				path = "\\" + std::string( tResource::fConvertPathB2ML( tFilePathPtr(path) ).fCStr() );
				filePath->fSetValue( path );
				wxButton* exploreButton = new wxButton( mMainPanel, wxID_ANY, "O", wxDefaultPosition, wxSize( 20, 20 ) );
				exploreButton->SetToolTip( "Open File In Explorer" );
				exploreButton->Connect( wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler( tEditableObjectProperties::fOnOpenExplorer ), NULL, this );
				filePath->fAddWindowToSizer( exploreButton, true );
			}
		}

		tEditablePropertyTable::fAddPropsToWindow( mMainPanel, mCommonProps, true, &selection );
	...
	void tEditableObjectProperties::fOnFocus( wxFocusEvent & event )
	{
		if( event.GetEventType() == wxEVT_SET_FOCUS )
		{
			tEditorAppWindow * app = fEditorWindow( );
			if ( app != NULL )
				if ( mOnFocusCallback != NULL )
					(app->*mOnFocusCallback)( );
		}
		event.Skip();
	}

	void tEditableObjectProperties::fOnOpenExplorer( wxCommandEvent& )
	{
		const tEditorSelectionList& selection = fEditorWindow( )->fGuiApp( ).fSelectionList( );
		if( selection.fCount() == 0 )
		{
			log_warning( "Explore button pressed with no objects selected. That's kind of weird." );
			return;
		}

		tFilePathPtr filePath = tFilePathPtr( selection[0]->fDynamicCast< tEditableObject >()->fGetAssetPath() );
		if( !filePath.fExists() )
		{
			log_warning( "Explore button pressed with object that has no asset path. That's kind of weird." );
			return;
		}

		// Turn a partial binary path into a full intermediate path.
		filePath = tResource::fConvertPathB2ML( filePath );
		filePath = ToolsPaths::fMakeResAbsolute( filePath );

		Win32Util::fExploreToDirectoryAndSelectFile( filePath.fCStr() );
	}
	...
	void tEditableObjectProperties::fOnAction(wxCommandEvent& event)
	{
		...
		else if( event.GetId( ) == cEOPAddProperty )
		{
			...
			{
				const tProjectFile::tGameTag* tag = tProjectFile::fInstance( ).fFindTagByName( std::string( propertyName ) );
				if( !tag )
				{
			...
			else if( type == cPropertyTypeEnum )
			{
				...
				if( !tProjectFile::fInstance( ).fFindEnumeratedTypeByName( std::string( propertyName ), &key ) )
				{
	...
	b32 tEditableObjectProperties::fCaptureCameraDistance( tEditablePropertyCDCFloat& property, f32& cameraDistanceOut )
	{
		tEditorSelectionList& selList = fEditorWindow()->fGuiApp().fSelectionList();

		if( selList.fCount() == 0 )
		{
			wxMessageBox( "There is no object selected.", "Select An Object" );
			return false;
		}
		else if( selList.fCount() > 1 )
		{
			wxMessageBox( "There is more than one object selected. Camera distance can only be determined from a single object.", "Select An Object" );
			return false;
		}

		tEditableObject* eo = selList[0]->fDynamicCast< tEditableObject >( );
		const Math::tVec3f pos = eo->fObjectToWorld().fGetTranslation();

		// hooooooooooooooly....
		const Math::tVec3f eye = fEditorWindow()->fRenderPanelContainer()->fGetFocusRenderPanel()->fGetScreen()->fViewport(0)->fRenderCamera().fGetTripod().mEye;

		cameraDistanceOut = (pos - eye).fLength();
	...
	void tEditableObjectProperties::fRefreshUserPropertyNames( )
	{
		...
		else if( type == cPropertyTypeEnum )
		{
			...
			log_warning( "Invalid property type." );
	...
	void tEditableObjectProperties::fSetOnFocusCallback( tOnFocusCallback focusCB )
	{
		mOnFocusCallback = focusCB;
	}
}
