...
namespace Sig
{
	enum tAction
	{
		...
		cActionRedo,
		cActionDuplicate,
		cActionGroupSelected,
		cActionBreakSelectedGroup,
		cActionSelectAll,
	...
	BEGIN_EVENT_TABLE(tSigEdMainWindow, wxFrame)
		...
		EVT_MENU(				cActionOpenBrowse,		tSigEdMainWindow::fOnAction)
		...
		EVT_MENU(				cActionPreviewPlaytest,	tSigEdMainWindow::fOnAction)
		...
		EVT_MENU(				cActionDuplicate,		tSigEdMainWindow::fOnAction)
		EVT_MENU(				cActionGroupSelected,	tSigEdMainWindow::fOnAction)
		EVT_MENU(				cActionBreakSelectedGroup,	tSigEdMainWindow::fOnAction)
		EVT_MENU(				cActionSelectAll,		tSigEdMainWindow::fOnAction)
		...
		EVT_MENU(				cActionMemoryViewer,	tSigEdMainWindow::fOnAction)
		...
		, mPlatformComboBox( 0 )
	{
		...
		// Load and apply project file settings
		{
			const tProjectFile & file = tProjectFile::fInstance( );
			for( u32 i = 0; i < file.mEngineConfig.mRendererSettings.mRenderableFarFadeSettings.cDimension; ++i )
				Gfx::tRenderableEntity::fSetGlobalFarFadeSetting( i, file.mEngineConfig.mRendererSettings.mRenderableFarFadeSettings[ i ] );
			for( u32 i = 0; i < file.mEngineConfig.mRendererSettings.mRenderableNearFadeSettings.cDimension; ++i )
				Gfx::tRenderableEntity::fSetGlobalNearFadeSetting( i, file.mEngineConfig.mRendererSettings.mRenderableNearFadeSettings[ i ] );
		}
	...
	void tSigEdMainWindow::fSetupRendering( )
	{
		...
		mLightEdDialog = new tSigEdLightEdDialog( this, fGuiApp( ).fGfxDevice( ), fGuiApp( ).fActionStack( ), fGuiApp( ).fRegKeyName( ) + "\\LightEd" );
		mLightEdDialog->SetTitle( "Light Ed" );
		//mLightEdDialog->fToggle( );

		mLenseFlareEdDialog = new tSigEdLenseFlareEdDialog( this, fGuiApp( ).fGfxDevice( ), fGuiApp( ).fActionStack( ), fGuiApp( ).fRegKeyName( ) + "\\LenseFlareEd" );
		mLenseFlareEdDialog->SetTitle( "Lense Flare Ed" );
		//mLenseFlareEdDialog->Show( true );
	}

	void tSigEdMainWindow::fOnTick( )
	{
		// needs to happen before the render panels are ticked
		if( mLightEdDialog )
			mLightEdDialog->fOnTick( );
		if( mLenseFlareEdDialog ) 
			mLenseFlareEdDialog->fOnTick( );
	...
	void tSigEdMainWindow::fAddMenus( )
	{
		...
		// add file menu
		{
			...
			subMenu->Append( cActionOpenBrowse, "&Browse...\tCtrl+Shift+O" );
			...
			subMenu->Append( cActionPreviewPlaytest, "Preview Playtest\tCtrl+Shift+P" );
			subMenu->Append( cActionPreviewRelease, "Preview Release" );
		...
		// add edit menu
		{
			...
			subMenu->AppendSeparator( );
			subMenu->Append( cActionDuplicate, "&Duplicate\tCtrl+D" );
			subMenu->Append( cActionGroupSelected, "&Group Selected\tCtrl+G" );
			subMenu->Append( cActionBreakSelectedGroup, "&Break Selected Group\tCtrl+Alt+G" );
			subMenu->AppendSeparator( );
			subMenu->Append( cActionSelectAll, "Select &All\tCtrl+A" );
		}
		...
		// add "window" menu; might change this later
		{
			...
			subMenu->Append( cActionMemoryViewer, "&Memory Viewer" );
			subMenu->AppendSeparator();
	...
	void tSigEdMainWindow::fAddToolbar( )
	{
		...
		mainToolBar->AddTool( cActionOpenBrowse, "Browse to Scene", wxBitmap( "opendoc" ), wxNullBitmap, wxITEM_NORMAL, "Browse for an existing scene" );
		...
		choices[ cPlatformPcDx11 - cPlatformFirst ] = wxString( "PcDx11" );
		choices[ cPlatformXbox360 - cPlatformFirst ] = wxString( "Xbox360" );
		choices[ cPlatformMetro - cPlatformFirst ] = wxString( "Metro" );
		choices[ cPlatformPs3Ppu - cPlatformFirst ] = wxString( "Ps3" );
		choices[ cPlatformPs3Spu - cPlatformFirst ] = wxString( "Ignore(Ps3Spu)" );
		choices[ cPlatformiOS - cPlatformFirst ] = wxString( "iOS" );
		choices[ cPlatformAndroid - cPlatformFirst ] = wxString( "Android" );
	...
	void tSigEdMainWindow::fAddTools( )
	{
		// create misc panel and tools
		{
			...
			mLayerPanel =	new tLayerPanel( panel, false );
			mVisibilityPanel =	new tLayerPanel( panel, true );
			...
			mTilePaintPanel = new tTilePaintPanel( this, panel );
		}

		// Has to be created after the object browser is ready.
		mOpenDialog = new tSearchableOpenFilesDialog( this, ToolsPaths::fGetCurrentProjectResFolder(), wxLC_SINGLE_SEL );
	...
	void tSigEdMainWindow::fOnClose(wxCloseEvent& event)
	{
		...
		b32 objectPropHidden = mObjectProperties->Hide( );
		b32 globalPropHidden = mGlobalProperties->Hide( );
		...
		{
			if( objectPropHidden )
				mObjectProperties->Show( );
			if( globalPropHidden )
				mGlobalProperties->Show( );
			event.Veto( );
		}
	}
	...
	void tSigEdMainWindow::fOnAction(wxCommandEvent& event)
	{
		...
		else
		{
			...
			case cActionOpenBrowse: fBrowseDoc( ); break;
			...
			case cActionSaveAs:		
				{
					...
					mOpenDocName = fGuiApp( ).fDocName( );
					mOpenDocName.Remove( 0, mOpenDocName.find_last_of( "\\" ) + 1 );

					// Remove everything after the last '\' 
					mOpenDocDirectory = StringUtil::fStripExtension( fGuiApp( ).fDocName( ).c_str( ) ) ;
					mOpenDocDirectory.Remove( mOpenDocDirectory.find_last_of( "\\" ) );

					wxString directory = mOpenDocDirectory;
					wxString fileName = StringUtil::fStripExtension( mOpenDocName ) + "_logic.nut";

					// Set filename and directory
					tEditablePropertyFileNameString::fSetDefaultBrowseDirectory(Sigml::tObjectProperties::fEditablePropScriptName( ), directory.c_str());
					tEditablePropertyFileNameString::fSetDefaultFileName(Sigml::tObjectProperties::fEditablePropScriptName( ), fileName.c_str());

					break;
				}
			case cActionImport:		fImportDoc( ); break;
			case cActionPreview:	fPreview( cPreviewBuildConfigInternal ); break;
			case cActionPreviewPlaytest: fPreview( cPreviewBuildConfigPlaytest ); break;
			case cActionPreviewRelease:	fPreview( cPreviewBuildConfigRelease ); break;
			...
			case cActionDuplicate:	fDuplicateSelected( ); break;
			case cActionGroupSelected: fGroupSelected( ); break;
			case cActionBreakSelectedGroup: fBreakSelected( ); break;
			case cActionSelectAll:	fSelectAll( ); break;
			...
			case cActionObjectBrowser:
				...
				break;
			//case cActionTextEditor goes here
			case cActionMemoryViewer:
				mMemoryViewerDialog->fToggle( );
				break;
