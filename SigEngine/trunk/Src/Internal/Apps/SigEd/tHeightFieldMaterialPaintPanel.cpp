...
namespace Sig
{
	...
	class tHeightFieldMaterialCheckBox : public tWxSlapOnCheckBox
	{
		tHeightFieldMaterialPaintPanel* mOwner;
	public:
		tHeightFieldMaterialCheckBox( tHeightFieldMaterialPaintPanel* owner, wxWindow* parent, const char* label )
		...
		{
			mOwner->fOnHeightFieldMaterialChange( );
		}
	};
		...
		, mLuminositySlider( 0 )
	{
		...
		mHeightFieldMaterial = new tHeightFieldMaterialCheckBox( this, browseGroup->fGetMainPanel( ), "Transition Material" );
	...
	void tHeightFieldMaterialPaintPanel::fFromSigmlFile( const Sigml::tFile& sigml )
	{
		...
		if( Sigml::tHeightFieldMeshObject* hfmo = sigml.fFindObjectByType<Sigml::tHeightFieldMeshObject>( ) )
        {
			mHeightFieldMaterial->fSetValue( hfmo->mHFMatType == tTerrainGeometry::cMaterialTypeTransition );
        }	
		...
		if( mHeightFieldMaterial->fGetValueBool( ) )
		{
			for( u32 i = 0; i < mMaterialButtons.fCount( ); ++i )
			{
				const f32 tf = sigml.mHeightFieldMaterialTileFactors[ i * tTerrainGeometry::cTransitionMaterialSpan ];
				mMaterialButtons[ i ]->fSetTileFactor( tf );
			}
		}
		else
		{
	...
	{		
		if( mHeightFieldMaterial->fGetValueBool( ) )
		{
			tilingFactors.fNewArray( mMaterialButtons.fCount( ) * tTerrainGeometry::cTransitionMaterialSpan );
			for( u32 i = 0; i < tilingFactors.fCount( ); ++i )
			{
				tilingFactors[ i ] = mMaterialButtons[ i / tTerrainGeometry::cTransitionMaterialSpan ]->fTileFactor( );
			}
		}
		else
		{
	...
	void tHeightFieldMaterialPaintPanel::fOnHeightFieldMaterialChange( )
	{
		fGuiApp( ).fActionStack( ).fForceSetDirty( );
		tEditableObjectContainer& editableObjects = fGuiApp( ).fEditableObjects( );
		editableObjects.fChangeHeightFieldMaterial( mHeightFieldMaterial->fGetValueBool( ) ? tTerrainGeometry::cMaterialTypeTransition : tTerrainGeometry::cMaterialTypeDefault );

		//refresh editor stuff
		if( dynamic_cast<tHeightFieldMaterialPaintCursor*>( fGuiApp( ).fCurrentCursor( ).fGetRawPtr( ) ) )
			mAppWindow->fSetSelectionCursor( ); //if we are trying to paint the heightfield then choose a different cursor so we don't crash trying to access freed memory.
		mMaterialGroup->fDeleteButtons( ); //clear out all material painting buttons
		mMaterialGroup->fClearSelection( );
		fRefreshAtlases( true, true ); //refresh all material painting buttons
	}
		...
		if( diffuse )
		{
			...
			if( mDiffuseMapAtlas.fLength( ) > 0 )
			{
				...
				if( tatml.fLoadXml( ToolsPaths::fMakeResAbsolute( mDiffuseMapAtlas ) ) )
				{
					...
					u32 numMtls = tatml.mTexturePaths.fCount( );
					u32 mtlSpan = 1;

					const b32 isTransitionMtl = mHeightFieldMaterial->fGetValueBool( );

					if( isTransitionMtl )
					{
						numMtls /= tTerrainGeometry::cTransitionMaterialSpan;
						mtlSpan = tTerrainGeometry::cTransitionMaterialSpan;
					}

					mMaterialButtons.fNewArray( numMtls );
					for( u32 imtl = 0; imtl < numMtls; ++imtl )
					{
						const u32 mtlOffset = imtl * mtlSpan;
						tHeightFieldMaterialButton* mtlButton = new tHeightFieldMaterialButton( mAppWindow, this, mMaterialGroup, mtlOffset );
						mMaterialButtons[ imtl ] = mtlButton;
						mtlButton->fSetDiffuseMapPath( tatml.mTexturePaths[ mtlOffset ] );

						std::string texName = StringUtil::fNameFromPath( tatml.mTexturePaths[ mtlOffset ].fCStr( ) );
