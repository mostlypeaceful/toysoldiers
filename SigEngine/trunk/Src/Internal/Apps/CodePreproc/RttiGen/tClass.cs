...
namespace CodePreproc.RttiGen
{
    class tClass : tParser
    {
        ...
        {
            Group defaultExpressionGroup = match.Groups["defaultexpr"];
            Group typenameGroup = match.Groups["typename"];
            // Remove default expression first, to avoid changing the typenameGroup index:
            text = text.Remove(defaultExpressionGroup.Index, defaultExpressionGroup.Length);
            text = text.Remove(typenameGroup.Index, typenameGroup.Length);
            text = text.Trim();
            match = re.Match(text);
            return true;
        }
        ...
        private string fRemoveDefaultExpression(Match m)
        {
            Group defaultexpr = m.Groups["defaultexpr"];
            if (defaultexpr.Success)
                return m.Value.Remove(defaultexpr.Index - m.Index, defaultexpr.Length);
            else
                return m.Value;
        }
        ...
        private void fParseTagAndName(string text, string parentClassName)
        {
            ...
            if (match.Success)
            {
                ...
                mTemplateParams = mRegExTemplateParameter.Replace(mTemplateParams, fRemoveDefaultExpression);
            }
        ...
        private bool fParseMemberVariable(Regex re, ref Match match, ref string text)
        {
            ...
            // clean any 'mutable' specifiers, also dont care about those
            decl = mRegExMutable.Replace(decl, "").Trim();
