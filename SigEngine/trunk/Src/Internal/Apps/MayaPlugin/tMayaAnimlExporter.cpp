...
namespace Sig
{
	...
	void tMayaAnimlExporter::fExportCommonBegin( const MString& path )
	{
		...
		if( minUnit != maxUnit )
			log_warning( "Different units for start and end of animation timeline! Not sure how this can happen." );
		mTimeUnit = minUnit;
	...
	b32 tMayaAnimlExporter::fVisitNode( MDagPath& path, MObject& component )
	{
		...
		if( found && refFrame )
		{
			if( mNumRefFrames == 0 )
			{
				...
				log_warning( "Skipping reference frame [" << nodeFn.name( ).asChar( ) << "]; only 1 reference frame can be exported at a time in a .sklml file." );
			++mNumRefFrames;
		}
	...
	void tMayaAnimlExporter::fVisitReferenceFrame( MDagPath& path )
	{
		...
		{
			log_warning( "Invalid root node name specified on reference frame." );
			return;
		}
