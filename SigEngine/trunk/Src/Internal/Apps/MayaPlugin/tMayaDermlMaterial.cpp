...
namespace Sig
{
	...
	{
		renderContext.mSceneCamera = &camera;
	...
	void tMayaDermlMaterial::fGenerateShaders( const Derml::tFile& dermlFile )
	{
		...
		{
			log_warning( "Invalid geometry style on shader [" << mMaterialFile.mShaderPath << "] (Material Name: " << name( ).asChar( ) << "); expected GeometryType: 'MeshModel'" );
		}
