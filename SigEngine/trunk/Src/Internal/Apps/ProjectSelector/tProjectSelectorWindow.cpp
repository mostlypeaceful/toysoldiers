...
namespace Sig
{
	...
	BEGIN_EVENT_TABLE(tProjectSelectorWindow, wxFrame)
		...
		EVT_MENU(				ID_RollBackBuild,	tProjectSelectorWindow::fRollBackBuild)
	...
	void tProjectSelectorWindow::fRollBackBuild( wxCommandEvent& event )
	{
		const s32 index = mListBox->GetSelection( );
		ToolsPaths::tProjectProfile* profile = fGetProfileAt( index );
		if( !profile )
		...
		// Normally, tFilePathPtr does not allow leading slashes, but this is
		// a network path so we have to tell it we don't need sanitizing
		tFilePathPtr buildsDirectory( tFilePathPtr::tGuaranteeTag( ),
										std::string( "\\\\build-" + profile->mProfileName + "\\" + profile->mProfileName + "\\Project\\Builds" ).c_str( ) );

		tRollBackBuildDialog* dialog = new tRollBackBuildDialog( this, buildsDirectory );
	...
	void tProjectSelectorWindow::fOnListBoxRUp(wxMouseEvent& event)
	{
		...
		menu.Append( ID_RollBackBuild, _T("&Roll Back Build") );
