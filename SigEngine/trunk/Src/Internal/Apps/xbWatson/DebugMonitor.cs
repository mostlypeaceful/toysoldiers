...
namespace Atg.Samples.xbWatson
{
    ...
    public class DebugMonitor
    {
        ...
        /// </summary>
        private void ShowNotificationDialog(IXboxEventInfo eventInfo, string title, string overview, string details, bool setReturnValue)
        {
            // Update UI.
            try
            {
            ...
            if (setting.Action == AutomationAction.Prompt)
            {
                ...
                try
                {
                    ...
                    {
                        Forms.DebugNotificationDialog dialog = new Forms.DebugNotificationDialog(owner, eventInfo);
            ...
            else
            {
                ...
                // Directly cause the appropriate action to occur here
                HandleNotificationDialogAction(setting.Action, eventInfo);
            }
        }
        ...
        private void OnAssertionFailed(IXboxEventInfo eventInfo)
        {
            ...
            // Display dialog.
            ShowNotificationDialog(eventInfo, title, overview.ToString(), details.ToString(), true);
        }

        public void HandleNotificationDialogAction(AutomationAction result, IXboxEventInfo eventInfo)
        {

            // Handle user action.
            switch (result)
            {
                case AutomationAction.Continue:
                    {
                        if (eventInfo.Info.Event == XboxDebugEventType.AssertionFailed)
        ...
        private void OnException(IXboxEventInfo eventInfo)
        {
            ...
            ShowNotificationDialog(eventInfo, title, overview.ToString(), details.ToString(), false);
        }
