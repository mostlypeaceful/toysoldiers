...
namespace Sig
{
	...
	{
		const tFilePathPtrList& paths = mBrowser->fGetFilteredPaths( );

		wxProgressDialog progress( "Finding/Replacing...", "123", paths.fCount(), NULL, wxDEFAULT_DIALOG_STYLE | wxSTAY_ON_TOP | wxPD_CAN_ABORT );
		progress.SetSize( 600, progress.GetSize().GetY() );
		for( u32 i = 0; i < paths.fCount( ) && progress.Update(i,paths[i].fCStr()); ++i )
		{
