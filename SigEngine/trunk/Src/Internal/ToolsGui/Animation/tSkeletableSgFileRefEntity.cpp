...
namespace Sig
{
	...
	tSkeletableSgFileRefEntity::tSkeletableSgFileRefEntity( 
		...
		: tEditableSgFileRefEntity( container, rid, sigbResource )
		, mIsCursor( false )
		, mIsPaused( false )
		, mForceStep( 0 )
		, mTimeScale( 1 )
	...
	tEntityPtr tSkeletableSgFileRefEntity::fClone( )
	{
		...
		for( ;; )
		{
			...
			if( !mSkelResource )
			{
				ptr = new tSkeletableSgFileRefEntity( mContainer, mSgId, mSgResource );
				break;
			}
			...
			{
				ptr = new tSkeletableSgFileRefEntity( mContainer, mSgId, mSgResource, mSkelResource );
				break;
			}
	...
	void tSkeletableSgFileRefEntity::fOnSkelLoaded( tResource& theResource, b32 success )
	{
		...
		mSkeleton.fReset( new Anim::tAnimatedSkeleton( mSkelResource, *this ) );
	...
	//------------------------------------------------------------------------------
	void tSkeletableSgFileRefEntity::fSetMotionMap( Anim::tSigAnimMoMap* moMap )
	{
		mMoMap.fReset( moMap );
	}
}
