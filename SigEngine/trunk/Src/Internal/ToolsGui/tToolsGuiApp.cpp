...
namespace Sig
{
	namespace
	{
		...
		{
			Dx9Util::tTextureCache* cache = static_cast<tToolsGuiApp&>( tToolsGuiApp::fInstance( ) ).fEditableObjects( ).fTextureCache( ).fGetRawPtr( );
			sigassert( cache );

			Dx9Util::tBaseTexturePtr ptr = cache->fFindLoad2D( path );
			sizeOut = cache->fTextureDims2D( *ptr );

			Gfx::tTextureReference ref;
			ref.fSetRaw( (Gfx::tTextureReference::tPlatformHandle)ptr.fGetRawPtr( ) );
	...
	{
		mResourceDepot->fSetResourceLoadingTimeoutMS( 10.0f );
	...
	void tToolsGuiApp::fStartupApp( )
	{
		...
		mEditableObjects->fSetTextureCache( Dx9Util::tTextureCachePtr( new Dx9Util::tTextureCache( mGfxDevice ) ) );
		mResourceDepot->sToolsTextureDelegate = make_delegate_cfn( tResourceDepot::tToolsTextureDelegate, fToolsTextureQuery );
	...
	{
		Gfx::tDefaultRenderContext& d = Gfx::tDefaultRenderContext::fInstance( );
		mSavedFogValues = d.mFogValues;
		mSavedFogColor = d.mFogColor;
		d.mFogValues = Math::tVec4f::cZeroVector;
		d.mFogColor = Math::tVec3f::cZeroVector;
	}

	void tToolsGuiApp::fEnableFog( )
	{
		Gfx::tDefaultRenderContext& d = Gfx::tDefaultRenderContext::fInstance( );
		d.mFogValues = mSavedFogValues;
		d.mFogColor = mSavedFogColor;
	}
	...
	void tToolsGuiApp::fSyncDefaultLight( tEditablePropertyTable& properties )
	{
		...
		mDefaultLight->fSetColors( lightDesc.mFrontColor, lightDesc.mRimColor, lightDesc.mBackColor, lightDesc.mAmbientColor );
	...
	tEditorSelectionList&				tToolsGuiApp::fPrevSelectionList( ) { return fEditableObjects( ).fGetPrevSelectionList( ); }
