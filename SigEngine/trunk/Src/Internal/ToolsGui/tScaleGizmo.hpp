...
namespace Sig
{
	...
	private:
		Math::tVec3f fHandleLockedAxes( const Math::tVec3f& vec, const f32 maximum ) const;
		f32 fDetermineLargestScaleFactor( const tEditorSelectionList& selectedObjects, Math::tAabbf& outObjBox, Math::tVec3f& outScaleFactor ) const;
	};
