...
namespace Sig
{
	...
	class toolsgui_export tGroupSelectedObjectsAction : public tEditorButtonManagedCursorAction
	{
		tEntityPtr mMaster;
		tEditorSelectionList mEntities;
	public:
		tGroupSelectedObjectsAction( tToolsGuiMainWindow& mainWindow );
		virtual void fUndo( );
		virtual void fRedo( );
	};

	class toolsgui_export tBreakSelectedGroupsAction : public tEditorButtonManagedCursorAction
	{
		struct tGroupRecord
		{
			tEntityPtr mMaster;
			...
			tGroupRecord& operator=( const tGroupRecord& rhs )
			{
				mMaster = rhs.mMaster;
				mEntities.fReset( rhs.mEntities, false );
				return *this;
			}
		};

		tGrowableArray< tGroupRecord > mRecords;
	public:
		tBreakSelectedGroupsAction( tToolsGuiMainWindow& mainWindow );
		virtual void fUndo( );
		virtual void fRedo( );
	};
	...
	public:
		...
		inline void fSetIsGizmoWorldSpace( b32 is ) { mIsGizmoInWorldSpace = is; mGizmo->fSetOrientToWorld( mIsGizmoInWorldSpace ); }
