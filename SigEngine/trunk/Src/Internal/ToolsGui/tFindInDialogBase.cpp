...
namespace Sig
{
	...
	static const u32 cDialogFlags = wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER | wxTAB_TRAVERSAL | wxFRAME_FLOAT_ON_PARENT;
	...
	void tFindInDialogBase::fOpenFind( )
	{
		...
		mHorizontalSizer->Hide( mMatchMultiplePerLine );
	...
	void tFindInDialogBase::fOpenFindInFiles( )
	{
		...
		// Turn some things on.
		mHorizontalSizer->Show( mMatchMultiplePerLine );
	...
	void tFindInDialogBase::fBuildGui( )
	{
		...
		// Multiple per line.
		mMatchMultiplePerLine = new wxCheckBox( this, wxID_ANY, "Multiple per line" );
		mMatchMultiplePerLine->SetValue( true );
		mHorizontalSizer->Add( mMatchMultiplePerLine, 0, wxALL | wxALIGN_CENTER_VERTICAL, cItemSpacer );
		mHorizontalSizer->AddSpacer( cItemSpacer );
	...
	{
		const b32 multipleResultsPerLine = mMatchMultiplePerLine->GetValue( );

		const char* const start = (const char*)text.fBegin();
		const char* lineStart = start;
		const char* lineNext = start;
		u32 lineNumber = 0; // N.B. 1 based

		for( const char* result = fFindStr( start, findStr.c_str( ) ); result; result = fFindStr( multipleResultsPerLine ? result+findStr.size( ) : lineNext, findStr.c_str( ) ) )
		{
			while( lineNext && lineNext <= result )
			{
				lineStart = lineNext;
				lineNext = StringUtil::fReadLine(lineStart);

				++lineNumber;
			}

			const char* lineEnd = lineNext ? lineNext - 1 : lineStart + StringUtil::fStrLen( lineStart ); // don't count EOL

			if( lineStart <= result && result < lineEnd ) // preserve old behavior of only counting the line if result was entirely in this line
			{
				tOccurence newOccurence;
				newOccurence.mFile = file;
				newOccurence.mLineNum = lineNumber - 1; // N.B. 0 based
				newOccurence.mColNum = result - lineStart;
				...
				thing << lineNumber;
				thing << "):            ";
				thing.write( lineStart, lineEnd-lineStart );
	...
	void tFindInDialogBase::fProcessEnter( wxCommandEvent& event )
	{
