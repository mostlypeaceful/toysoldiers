...
namespace Sig
{
	...
	public:
		tPlanePreviewGeometryConverter( b32 facingParticleQuad, const Math::tMat3f& formVertices );
		virtual void operator( )( u32 ivtx, Sig::byte* vertexElem, const Gfx::tVertexElement& vertexElemDesc );
	};

	tPlanePreviewGeometryConverter::tPlanePreviewGeometryConverter( b32 facingParticleQuad, const Math::tMat3f& xformVertices )
		: mFacingParticleQuad( facingParticleQuad )
	{
		...
		{
			mP[ 0 ] = xformVertices.fXformPoint( Math::tVec3f( -1.f, +1.f, 0.f ) );
			mP[ 1 ] = xformVertices.fXformPoint( Math::tVec3f( -1.f, -1.f, 0.f ) );
			mP[ 2 ] = xformVertices.fXformPoint( Math::tVec3f( +1.f, +1.f, 0.f ) );
			mP[ 3 ] = xformVertices.fXformPoint( Math::tVec3f( +1.f, -1.f, 0.f ) );
		}

		Math::tVec3f n = Math::tVec3f::cZAxis;
		Math::tVec3f t = Math::tVec3f::cYAxis.fCross( n ).fNormalizeSafe( );
		Math::tVec3f b = n.fCross( t ).fNormalizeSafe( );

		if( !facingParticleQuad )
		{
			n = xformVertices.fXformVector( n );
			t = xformVertices.fXformVector( t );
			b = xformVertices.fXformVector( b );
		}
	...
	tPlanePreviewGeometry::tPlanePreviewGeometry( b32 facingParticleQuad, const Math::tMat3f& xformVertices, b32 xparentBatch )
		: mFacingParticleQuad( facingParticleQuad )
		, mXformVertices( xformVertices )
	{
		mRenderState = xparentBatch ? Gfx::tRenderState::cDefaultColorTransparent : Gfx::tRenderState::cDefaultColorOpaque; //todo figure out where to put this, possibly on the bundle.
	...
	void tPlanePreviewGeometry::fRegenerateGeometry( const Gfx::tDevicePtr& device, const Gfx::tVertexFormatVRam* vtxFormat, const Gfx::tMaterial* mtl )
	{
		...
		tPlanePreviewGeometryConverter converter( mFacingParticleQuad, mXformVertices );
