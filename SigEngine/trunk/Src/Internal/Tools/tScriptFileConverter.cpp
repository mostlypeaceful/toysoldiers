...
namespace Sig
{
	namespace
	{
		...
		const char* fFindSpecialTag( const char* src, const char* exportTag, u32 exportTagLen, b32& commentedOut )
		{
		...
		b32 fReadScriptClass( tScriptFile& scriptFile, tDynamicBuffer& scriptString )
		{
			...
			const char* className = fFindSpecialTag( src, classTag, classTagLen, commentedOut );
			if( className )
			{
		...
		b32 fParseAndReplaceImports( tScriptFile& scriptFile, tDynamicBuffer& scriptString, const std::string& fileBasedUniqueTag, tFilePathPtrList& indirectGenFilesAddTo )
		{
			...
			b32 commentedOut = false;
			const char* next = fFindSpecialTag( src, importTag, importTagLen, commentedOut );
			while( next )
			{
				...
				src		= fileNameEnd;
				next	= fFindSpecialTag( src, importTag, importTagLen, commentedOut );
			}
		...
		b32 fParseAndReplaceExports( tScriptFile& scriptFile, tDynamicBuffer& scriptString, const std::string& fileBasedUniqueTag )
		{
			...
			b32 commentedOut = false;
			const char* next = fFindSpecialTag( src, exportTag, exportTagLen, commentedOut );
			while( next )
			{
				...
				src		= next;
				next	= fFindSpecialTag( src, exportTag, exportTagLen, commentedOut );
			}
		...
		b32 fConvertToScriptFile( tScriptFile& scriptFile, const tFilePathPtr& inputFileName, tDynamicBuffer& scriptString, tScriptVm& svm, tFilePathPtrList& indirectGenFilesAddTo, tPlatformId pid )
		{
			...
			{
				log_warning( "Error parsing script file [" << inputFileName << "] for special tags, conversion failed." );
				return false;
			}
			...
			if( compileToByteCode )
			{
				...
				{
					log_warning( "Error compiling script file [" << inputFileName << "], conversion failed." );
					return false;
				}
				...
				{
					log_warning( "Error generating byte code for script file [" << inputFileName << "], conversion failed." );
					return false;
				}
	...
	b32 tScriptFileConverter::fParseAndReplaceSigvars( tScriptFile* scriptFile, tDynamicBuffer& scriptString, const std::string& fileBasedUniqueTag, tGrowableArray< tExportedVariable >& variablesOut, tGrowableArray< std::string >& groupsOut, b32 skipReplacement )
	{
		...
		while( line )
		{
			...
			else
			{
				...
				// sigimport can't handle variables
				const b32 dumbLine = StringUtil::fStartsWith(lineStr.c_str(),"sigimport ") || StringUtil::fStartsWith(lineStr.c_str(),"sigimport\t");
				...
				for( u32 i = 0; i < variablesOut.fCount( ); ++i )
				{
					...
					if( releaseMode || dumbLine ) replaceWith = variablesOut[ i ].fValue( );
	...
	std::string tScriptFileConverter::tExportedVariable::fValue( ) const
	{
		if( mType == cVariableTypeVector )
		{
			...
			if( count < 2 )
			{
				log_warning( "Too few values in sigvar vector type." );
				count = 2;
			}
			else if( count > 4u )
			{
				log_warning( "Too many values in sigvar vector type." );
				count = 4;
			}
	...
	tLoadInPlaceResourcePtr* tScriptFileConverter::fAddDependencyInternal( 
	...
	void tScriptFileConverter::fConvertScript( tDynamicBuffer scriptString, const tFilePathPtr& inputFileName, const tFilePathPtr& output, tPlatformId pid, tFilePathPtrList& indirectGenFilesAddTo, const tFilePathPtrList& additionalDependencies )
	{
		...
		if( !ofile.fIsOpen( ) )
		{
			log_warning( "Couldn't open output script file [" << output << "], conversion failed for platform [" << fPlatformIdString( pid ) << "]" );
			return;
		}

		// set signature for this platform
		scriptFile.fSetSignature<tScriptFile>( pid );
		...
		tFilePathPtrList& indirectGenFilesAddTo )
	{
		...
		{
			log_warning( "Empty script file [" << inputFileName << "] - was this intended?" );
			...
			do 
			{
				...
				if( *start != 0 )
				{
					...
					else
					{
						...
						log_warning( "End of file found inside of string! Path replacement aborted" );
						return 0;
					}
				}
