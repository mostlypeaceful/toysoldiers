...
namespace Sig { namespace Anipk
{
	...
	void tAnimationMetaData::fSetTagTime( u32 idx, f32 newTime )
	{
		sigassert( idx <= mKeyFrameTags.fCount() );
		mKeyFrameTags[ idx ].mTime = newTime;
		fSetDirty();
	}
	...
	void fSerializeXmlObject( tSerializer& s, tFile& o )
	{
		...
		{
			log_warning( "Anipk file format is out of date -> Please re-export." );
			return;
		}
	...
	b32 tFile::fSaveXml( const tFilePathPtr& path, b32 promptToCheckout, b32 displayWarnings )
	{
		tXmlSerializer ser;
		if( !ser.fSave( path, "Anipk", *this, promptToCheckout, displayWarnings ) )
		{
			log_warning( "Couldn't save Anipk file [" << path << "]" );
			return false;
		}
	...
	b32 tFile::fLoadXml( const tFilePathPtr& path )
	{
		...
		{
			log_warning( "Couldn't load Anipk file [" << path << "]" );
			return false;
		}
