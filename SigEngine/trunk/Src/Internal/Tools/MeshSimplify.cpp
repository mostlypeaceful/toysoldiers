...
namespace Sig { namespace MeshSimplify
{
	...
	void fSimplify( 
		...
		const f32 optimizeTarget, 
		const b32 disregardEdges, 
		const b32 reIndex )
	{
		...
		log_line( 0, 
			"Mesh simplification results: tris=" << (100.f*numOutputTris)/numInputTris 
			<< "%, verts=" << (100.f*numOutputVerts)/numInputVerts 
			<< "%, time=" << Time::fGetElapsedS( timeBegin, timeEnd ) << "s" );
	}
}}
