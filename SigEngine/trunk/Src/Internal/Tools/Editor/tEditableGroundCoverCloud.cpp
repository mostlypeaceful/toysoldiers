...
namespace Sig
{
	...
	void tEditableGroundCoverCloud::fUpdateDef( 
		...
		b32 updateCover )
	{
		b32 newVisibility = mVisible != layer.fVisible( );
		...
		if( mDefInstance.mFarVisibility != layer.fFarVisibility( ) )
		{
			mDefInstance.mFarVisibility = (Gfx::tGroundCoverCloudDef::tVisibility)layer.fFarVisibility( );
			sigassert( mDefInstance.mFarVisibility < Gfx::tGroundCoverCloudDef::cVisibilityCount );
			newVisibility = true;
		}

		if( mDefInstance.mNearVisibility != layer.fNearVisibility( ) )
		{
			mDefInstance.mNearVisibility = (Gfx::tGroundCoverCloudDef::tVisibility)layer.fNearVisibility( );
			sigassert( mDefInstance.mNearVisibility < Gfx::tGroundCoverCloudDef::cVisibilityCount );
			newVisibility = true;
		}
		...
		else
		{
			for( u32 e = 0; e < elementCount; ++e )
			{
				...
				tFilePathPtr binPath = tSceneGraphFile::fConvertToBinary( to.mSgPath );
		...
		if( newElements )
		{
			...
			for( u32 e = 0; e < elementCount; ++e )
			{
				...
				if( !ptr )
				{
					...
					ptr->mFilePathPtr.fTreatAsObject( ) = tFilePathPtr( ptr->mRawPath.fBegin( ) );
		...
		if( newUnitSize || newElements || newVisibility )
		...
		const tTerrainGeometry::tGroundCover & gc, b32 updateCover  )
	{
		...
		mDefInstance.mUnitSize = gc.mUnitSize;
	...
	void tEditableGroundCoverCloud::fUpdateShadows( const Sigml::tGroundCoverLayer & layer )
	{
		...
		for( u32 e = 0; e < elementCount; ++e )
		{
			...
			const Sigml::tGroundCoverLayer::tElement * from = layer.fFindElement(
				tSceneGraphFile::fConvertToSource( to.mSgFile->fGetFilePathPtr( ) ) );
			sigassert( from );
			...
			{
				if( mCells[ c ].fIsDeadOrEmpty( ) )
					continue;
				
				mCells[ c ].fRenderList( ).fApplyProperties( &fDef( ), mCuller, to );
			}
		}
	}
	...
	{
		if( mOnLoadComplete.fOwnerEventCount( ) )
			return;
	...
	void tEditableGroundCoverCloud::fRefreshXforms( u32 minX, u32 minZ, u32 maxX, u32 maxZ )
	{
		...
		for( u32 z = minZ; z < maxZ; ++z, zP += unitSize  )
		{
			...
			for( u32 x = minX; x < maxX; ++x, xP += unitSize )
			{
				...
				cell.fRenderList( ).fMoveTo( &mDefInstance, mCuller, objectToWorld, x, z, xP, zP );
			}
		}
	}
}
