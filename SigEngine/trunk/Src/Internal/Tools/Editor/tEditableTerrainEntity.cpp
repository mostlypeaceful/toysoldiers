...
namespace Sig
{
	namespace
	{
		...
		static inline const char* fStrHeightFieldPreserveSpace( ) { return "HeightField.PreserveSpace"; }
		static inline const char* fStrPreserveSpace( ) { return "PreserveSpace.Enabled"; }
		static inline const char* fStrPreserveTop( ) { return "PreserveSpace.AlignTop"; }
		static inline const char* fStrPreserveLeft( ) { return "PreserveSpace.AlignLeft"; }
	...
	public:
		tChangeTerrainResolutionAction( 
			...
			const Math::tVec3f & entityMoved = Math::tVec3f::cZeroVector )
			: mEntity( entity )
			, mOldGeometry( oldGeometry )
			, mNewGeometry( newGeometry )
			, mEntityMoved( entityMoved )
		{
		...
		virtual void fUndo( )
		{
			...
			Math::tMat3f world = mEntity->fObjectToWorld( );
			world.fTranslateGlobal( -mEntityMoved );
			mEntity->fMoveTo( world );
		}
		virtual void fRedo( )
		{
	...
	void tEditableTerrainEntity::fChangeHeightFieldMaterial( tTerrainGeometry::tMaterialType hfType )
	{
		mTerrainGeometry->fAcquireMaterial(
			mContainer.fGetDevice( ),
			mContainer.fGetResourceDepot( ),
			hfType,
			mEditableProperties.fGetValue( fStrMaterialRes( ), cDefaultMaterialRes ),
	...
	u32 tEditableTerrainEntity::fGetHeightFieldMaterialType( ) const
	{
		return mTerrainGeometry->fHeightFieldMaterialType( );
	}
		...
		: tEditableTerrainGeometry( container )
	{
		...
		sigassert( sigmlObject.mHFMatType < tTerrainGeometry::cMaterialTypeCount );
		fChangeHeightFieldMaterial( (tTerrainGeometry::tMaterialType)sigmlObject.mHFMatType );
	...
	void tEditableTerrainEntity::fCommonCtor( )
	{
		...
		// Space preservation when editing the terrain
		mEditableProperties.fInsert(
			tEditablePropertyPtr( new tEditablePropertyBool( fStrPreserveSpace( ), false ) ) );
		mEditableProperties.fInsert(
			tEditablePropertyPtr( new tEditablePropertyBool( fStrPreserveTop( ), true ) ) );
		mEditableProperties.fInsert(
			tEditablePropertyPtr( new tEditablePropertyBool( fStrPreserveLeft( ), true ) ) );
		mEditableProperties.fRemove( fStrHeightFieldPreserveSpace( ) );
	...
	{
		mHeightFieldRoot->fClearChildren( );
		mTerrainGeometry->fCollectEntities( tCollectEntitiesParams( *mHeightFieldRoot, tEntityCreationFlags( ) ) );
	...
	void tEditableTerrainEntity::fUpdateGroundCoverHeights( tEditableGroundCoverHeights & editor )
	{
		...
		for( u32 z = 0; z < dimz; ++z, zP += unitSize )
		{
			...
			{
				spawns.fSetCount( 0 );
				Gfx::tGroundCoverCloud::fGenSpawnInfo( 
					&entity->fDef( ),
					x + editor.fStartX( ), 
					z + editor.fStartZ( ), 
					xP, zP, 
				...
				for( u32 s = 0; s < spawnCount; ++s )
				{
					...
					ray.mOrigin = world.fXformPoint( 
						Math::tVec3f( 
							info.mLocalXform(0,3), 
							1000.f, 
							info.mLocalXform(2,3) ) 
					);
					...
					if( hit.fHit( ) )
						editor.fIndex( x, z, s ) = worldInv.fXformPoint( ray.fPointAtTime( hit.mT ) ).y;
					else
	...
	Sigml::tObjectPtr tEditableTerrainEntity::fSerialize( b32 clone ) const
	{
		...
		o->mHFMatType = mTerrainGeometry->fHeightFieldMaterialType( );
	...
	void tEditableTerrainEntity::fNotifyPropertyChanged( tEditableProperty& property )
	{
		...
		Math::tVec2f dims = mEditableProperties.fGetValue( fStrDimensions( ), cDefaultDimensions );
		const Math::tVec2i mtlRes = mEditableProperties.fGetValue( fStrMaterialRes( ), cDefaultMaterialRes );
		
		// We only preserve space if the box is checked and dimensions changed
		const b32 preserveSpace = mEditableProperties.fGetValue( fStrPreserveSpace( ), false ) && 
			...
			property.fGetName( ) == fStrDimensions( ) )
		{
			...
			// In what directions do we preserve space
			const b32 preserveLeft = mEditableProperties.fGetValue( fStrPreserveLeft( ), true );
			const b32 preserveTop = mEditableProperties.fGetValue( fStrPreserveTop( ), true );

			Math::tVec2f preserveOffset( 0.f );
			if( preserveSpace )
			{
				if( !preserveLeft )
					preserveOffset.x = mTerrainGeometry->fRez( ).fWorldLengthX( ) - oldGeometry->fRez( ).fWorldLengthX( );

				if( !preserveTop )
					preserveOffset.y = mTerrainGeometry->fRez( ).fWorldLengthZ( ) - oldGeometry->fRez( ).fWorldLengthZ( );
			}
			...
			for( u32 gc = 0; gc < gcCount; ++gc )
			{
				...
				b32 diff = mTerrainGeometry->fUpdateGroundCoverDims(
					cover.mCoverId, cover.mUnitSize, cover.mMaxUnitSpawns, preserveSpace, preserveLeft, preserveTop );
			...
			// If we're not preserving vert spacing then we want to normalize our terrain
			// over the new space
			if( !preserveSpace )
			{
				// Normalized sample
			...
			// Otherwise we want to fill the space with the old space
			else
			{
				// Positional sample
				for( u32 z = 0; z < editableVerts.fDimZ( ); ++z )
				{
					for( u32 x = 0; x < editableVerts.fDimX( ); ++x )	
					{	
						const f32 h = oldGeometry->fSampleHeight( 
							oldMinX - preserveOffset.x + x * mTerrainGeometry->fRez( ).fVertexDeltaX( ),
							oldMinZ - preserveOffset.y + z * mTerrainGeometry->fRez( ).fVertexDeltaZ( ) );
			...
			// Tracks the motion of the entity, if any
			Math::tVec3f entityMoved = Math::tVec3f::cZeroVector; 

			// Copy the textures straight across
			...
			mTerrainGeometry->fCopyMaterialTilingFactors( *oldGeometry );

			if( preserveSpace )
			{
				// We need to move the entity so it stays under the objects already placed
				entityMoved = Math::tVec3f(
					oldMinX - preserveOffset.x - mTerrainGeometry->fRez( ).fMinX( ), 0.f,
					oldMinZ - preserveOffset.y - mTerrainGeometry->fRez( ).fMinZ( ) );

				Math::tMat3f world = fObjectToWorld( );
				world.fTranslateGlobal( entityMoved );
				fMoveTo( world );

				// Now we need to handle the mask/mtl id textures
				const Math::tVec2f oldDims( 
					oldGeometry->fRez( ).fWorldLengthX( ), 
					oldGeometry->fRez( ).fWorldLengthZ( ) );

				// Does our old space use less texture?
				if( oldDims.x < dims.x || oldDims.y < dims.y )
				{
					sigassert( oldDims.x <= dims.x && oldDims.y <= dims.y );

					const Math::tVec2u miniTexSize(
						fRoundUp<u32>( mtlRes.x * ( oldDims.x / dims.x ) ),
						fRoundUp<u32>( mtlRes.y * ( oldDims.y / dims.y ) ) );

					// Mask
					{
						tTextureSysRam::tSurface& maskTex = mTerrainGeometry->fGetMaskTexture( );
						tTextureSysRam::tSurface miniTex; miniTex.fAllocate( 
							maskTex.fGetReferenceDevice( ), 
							miniTexSize.x, miniTexSize.y, 
							maskTex.fFormat( ) );

						// Filter down to the space after the change
						miniTex.fFilteredCopyA8R8G8B8( maskTex );

						// Copy into the new texture
						maskTex.fUnfilteredCopyA8R8G8B8ToSubRect( 
							miniTex, 
							preserveLeft ? 0 : maskTex.fWidth( ) - miniTex.fWidth( ),
							preserveTop ? 0 : maskTex.fHeight( ) - miniTex.fHeight( ) );
					...
					// Mtl ids
					{
						tTextureSysRam::tSurface& mtlTex = mTerrainGeometry->fGetMtlIdsTexture( );
						tTextureSysRam::tSurface miniTex; miniTex.fAllocate( 
							mtlTex.fGetReferenceDevice( ),
							miniTexSize.x, miniTexSize.y, 
							mtlTex.fFormat( ) );

						// Copy down to the space after the change
						miniTex.fUnfilteredCopyR5G6B5( mtlTex );

						// Copy into the new texture
						mtlTex.fUnfilteredCopyR5G6B5ToSubRect( 
							miniTex,
							preserveLeft ? 0 : mtlTex.fWidth( ) - miniTex.fWidth( ),
							preserveTop ? 0 : mtlTex.fHeight( ) - miniTex.fHeight( ) );
				...
				// Or does it use more?
				else if( oldDims.x > dims.x || oldDims.y > dims.y )
				{
					sigassert( oldDims.x >= dims.x && oldDims.y >= dims.y );

					const Math::tVec2u miniTexSize(
						fRoundUp<u32>( mtlRes.x * ( dims.x / oldDims.x ) ),
						fRoundUp<u32>( mtlRes.y * ( dims.y / oldDims.y ) ) );
					...
					// Mask
					{
						...
						// Copy the needed section
						miniTex.fUnfilteredCopyA8R8G8B8FromSubRect(
							maskTex,
							preserveLeft ? 0 : maskTex.fWidth( ) - miniTex.fWidth( ),
							preserveTop ? 0 : maskTex.fHeight( ) - miniTex.fHeight( ) );

						// Filter up to the space after the change
						maskTex.fFilteredCopyA8R8G8B8( miniTex );
					...
					// Mtl ids
					{
						...
						// Copy the needed section
						miniTex.fUnfilteredCopyR5G6B5FromSubRect(
							mtlTex,
							preserveLeft ? 0 : mtlTex.fWidth( ) - miniTex.fWidth( ),
							preserveTop ? 0 : mtlTex.fHeight( ) - miniTex.fHeight( ) );

						// Copy up to the space after the change
						mtlTex.fUnfilteredCopyR5G6B5( miniTex );
			...
			mContainer.fGetActionStack( ).fAddAction( 
					tEditorActionPtr( new tChangeTerrainResolutionAction( fToSmartPtr( ), oldGeometry, mTerrainGeometry, entityMoved ) ) );
	...
	void tEditableTerrainEntity::fUpdateGroundCover( const Sigml::tGroundCoverLayer & layer )
	{
		b32 gcChanged = mTerrainGeometry->fUpdateGroundCoverDims( 
			layer.fUniqueId( ), layer.fUnitSize( ), layer.fComputeMaxUnitSpawns( ), false, false, false );
