...
namespace Sig
{
	...
	u32 tEditablePropertyTable::fAddPropsToWindow( wxWindow* panel, tEditablePropertyTable& commonProps, b32 collapsibleGroups, const tEditorSelectionList* selection )
	{
		...
		for( u32 i = 0; i < commonPropsSorted.fCount( ); ++i )
		{
			...
			tEditableProperty::tCreateGuiData data;
			data.mParent = ( *group )->fGetMainPanel( );
			data.mLabel = realName;
			data.mSelection = selection;
			commonPropsSorted[ i ]->fCreateGui( data );
		}
	...
	namespace
	{
		b32 fWantsProperty( const tGrowableArray< tEditablePropertyPtr >& props, const std::string& name )
		{
			for( u32 i = 0; i < props.fCount( ); ++i )
				if( props[ i ]->fGetName( ) == name )
					return true;
	...
	void tEditablePropertyTable::fAssignPreferExisting( const tGrowableArray< tEditablePropertyPtr >& properties )
	{
		tEditablePropertyTable& table = *this;

		// Remove old properties that we dont want.
		// Have to use a while loop because mEditableProperties will be reallocating if we remove something.
		bool done = false;
		while( !done )
		{
			done = true;
			...
			for( ; it.fNotDone( ); ++it )
			{
				if( !fWantsProperty( properties, it->mValue->fGetName( ) ) )
				{
		...
		// Insert all new properties if they dont exist
		for( u32 i = 0; i < properties.fCount( ); ++i )
			table.fInsert( properties[ i ] );
	}

	b32 tEditablePropertyTable::fContainsProperty( const tEditableProperty& ofProperty ) const
	{
		tEditablePropertyTable::tConstIteratorNoNullOrRemoved it( fBegin( ), fEnd( ) );
		for( ; it.fNotDone( ); ++it )
		{
			if( ofProperty.fEqual( *it->mValue ) )
				return true;
		}
	...
	b32 tEditablePropertyTable::fEqual( const tEditablePropertyTable& other ) const
	{
		if( fGetItemCount( ) != other.fGetItemCount( ) )
			return false;

		tConstIteratorNoNullOrRemoved it( fBegin( ), fEnd( ) );
		for( ; it.fNotDone( ); ++it )
		{
			tEditablePropertyPtr* v = other.fFind( it->mKey );
			if( !v || !(*v)->fEqual( *it->mValue) )
				return false;
		}
