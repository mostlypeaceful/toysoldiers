...
namespace Sig
{
	...
	void tEditablePropertyBool::fCreateGui( tCreateGuiData& data )
	{
		mCheckBox = new tCheckBox( this, data.mParent, data.mLabel.c_str( ) );
	...
	void tEditablePropertyMask::fCreateGui( tCreateGuiData& data )
	{
		mUI = new tMask( this, data.mParent, data.mLabel.c_str( ) );
	...
	void tEditablePropertyEnum::fCreateGui( tCreateGuiData& data )
	{
		tDynamicArray<wxString> choices( mEnumNames.fCount( ) );
		for( u32 i = 0; i < choices.fCount( ); ++i )
			choices[ i ] = mEnumNames[ i ];
		mComboBox = new tComboBox( this, data.mParent, data.mLabel.c_str( ), choices, mRawData );
		mComboBox->fSetToolTip( fDisplayOptions( ).mToolTip );
		fRefreshGui( );
		if( mAllowDelete )
			fAddRemoveButton( data.mParent, *mComboBox );
	}
	...
	void tEditablePropertyEnum::fUpdate( tEditableProperty& other )
	{
		tEditablePropertyEnum* enumType = dynamic_cast<tEditablePropertyEnum*>( &other );
		sigassert( enumType );

		mEnumNames = enumType->mEnumNames;

		// Make sure our old selected index is not past the new bounds
		u32 oldIndex;
		fGetData( oldIndex );

		if( oldIndex >= mEnumNames.fCount( ) )
		{
			log_warning( "Enum property was adjusted, had to clamp selected index: " << fGetName( ) );
			fSetData( mEnumNames.fCount( ) - 1, false );
		}
	}

	///
	/// \section tEditablePropertyLenseFlare
	///

	namespace
	{
		const u32 cNoneEntryOffset = 1;
	}

	tEditablePropertyLenseFlare::tComboBox::tComboBox( tEditablePropertyLenseFlare* ownerProp, wxWindow* parent, const char* label, const tDynamicArray<wxString>& enumNames, u32 defChoice )
		: tWxSlapOnChoice( parent, label, enumNames.fBegin( ), enumNames.fCount( ), defChoice )
		, mOwnerProp( ownerProp )
	{
	}

	register_rtti_factory( tEditablePropertyLenseFlare, false )

	tEditablePropertyLenseFlare::tEditablePropertyLenseFlare( const std::string& name, u32 initState, b32 allowDelete )
		: tRawDataEditableProperty<u32>( name, initState )
		, mComboBox( NULL )
		, mEditButton( NULL )
		, mAllowDelete( allowDelete )
	...
	void tEditablePropertyLenseFlare::fCreateGui( tCreateGuiData& data )
	{
		const tProjectFile& projectFile = tProjectFile::fInstance( );

		tDynamicArray<wxString> choices;

		const tGrowableArray< tProjectFile::tLenseFlare >& flares = projectFile.mEngineConfig.mRendererSettings.mLenseFlares;
		choices.fNewArray( flares.fCount( ) + cNoneEntryOffset );
		choices[ 0 ] = "None";
		for( u32 i = 0; i < flares.fCount( ); ++i )
			choices[ i + cNoneEntryOffset ] = flares[ i ].mName;

		mComboBox = new tComboBox( this, data.mParent, "Lense Flare", choices, mRawData );
		mComboBox->fSetToolTip( fDisplayOptions( ).mToolTip );
		
		mEditButton = new wxButton( data.mParent, wxID_ANY, "Edit", wxDefaultPosition, wxSize( 22, 20 ) );
		mEditButton->SetForegroundColour( wxColour( 0x22, 0x22, 0xff ) );
		mComboBox->fAddWindowToSizer( mEditButton, true );
		mEditButton->Connect( wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler( tEditablePropertyLenseFlare::fOnEdit ), NULL, this );

		fRefreshGui( );
		if( mAllowDelete )
			fAddRemoveButton( data.mParent, *mComboBox );
	}
	void tEditablePropertyLenseFlare::fRefreshGui( )
	{
		if( fHasConflict( ) )
			mComboBox->fSetValue( ~0 );
		else
		{
			const tProjectFile& projectFile = tProjectFile::fInstance( );			
			u32 index = tProjectFile::fFindItemIndexByKey( mRawData, projectFile.mEngineConfig.mRendererSettings.mLenseFlares );

			if( index == ~0 )
				index = 0;
			else
				index += cNoneEntryOffset;

			mComboBox->fSetValue( index );
		}
	}
	void tEditablePropertyLenseFlare::fClearGui( )
	{
		mComboBox = NULL;
		mEditButton = NULL;
	}
	tEditableProperty* tEditablePropertyLenseFlare::fClone( ) const
	{
		tEditablePropertyLenseFlare* o = new tEditablePropertyLenseFlare( fGetName( ), mRawData );
		o->mAllowDelete = mAllowDelete;
		return o;
	}
	void tEditablePropertyLenseFlare::fOnEdit( wxCommandEvent& )
	{
		tSigEdLenseFlareEdDialog::fShow( true );
	}
	void tEditablePropertyLenseFlare::fSetByIndex( u32 index )
	{
		u32 key = ~0;

		if( index >= cNoneEntryOffset )
		{
			const tProjectFile& projectFile = tProjectFile::fInstance( );
			key = projectFile.mEngineConfig.mRendererSettings.mLenseFlares[ index - cNoneEntryOffset ].mKey;
		}
	...
	{
		return tProjectFile::fInstance( ).fFindEnumeratedTypeByKey( fEnumTypeKey( ) );
	}
	void tEditablePropertyProjectFileEnum::fCreateGui( tCreateGuiData& data )
	{
		const tProjectFile& projectFile = tProjectFile::fInstance( );
		...
		mComboBox = new tComboBox( this, data.mParent, name.c_str( ), choices, mRawData );
		mComboBox->fSetToolTip( fDisplayOptions( ).mToolTip );
		fRefreshGui( );
		if( mAllowDelete )
			fAddRemoveButton( data.mParent, *mComboBox );
	...
	void tEditablePropertyProjectFileTag::fCreateGui( tCreateGuiData& data )
	{
		const tProjectFile::tGameTag* tag = tProjectFile::fInstance( ).fFindTagByKey( mRawData );

		std::string name;
		if( !tag ) name == "Error!";
		else name = tag->mName;

		mTextBox = new tTextBox( this, data.mParent, name.c_str( ) );
		mTextBox->fSetToolTip( fDisplayOptions( ).mToolTip );
		fRefreshGui( );
		if( mAllowDelete )
			fAddRemoveButton( data.mParent, *mTextBox );
	}
	...
	void tEditablePropertyString::fCreateGui( tCreateGuiData& data )
	{
		mTextBox = new tTextBox( this, data.mParent, data.mLabel.c_str( ) );
		mTextBox->fSetToolTip( fDisplayOptions( ).mToolTip );
	...
	void tEditablePropertyScriptString::fCreateGui( tCreateGuiData& data )
	{
		mTextBox = new tTextBox( this, data.mParent, data.mLabel.c_str( ) );
		mTextBox->fSetToolTip( fDisplayOptions( ).mToolTip );
		...
		wxButton* button = new wxButton( data.mParent, wxID_ANY, "Edit", wxDefaultPosition, wxSize( 20, 20 ) );
	...
	void tEditablePropertyFileNameString::tTextBox::fOnLabelButtonClicked( wxCommandEvent& event )
	{
		...
		if( text.empty( ) )
		{
			...
			{
				const std::string& defaultDirectory = tFileBrowserFilterMap::fInstance( )[ Sigml::tObjectProperties::fEditablePropScriptName( ) ].mDefaultBrowseDirectory;
				const std::string& defaultFilename = tFileBrowserFilterMap::fInstance( )[ Sigml::tObjectProperties::fEditablePropScriptName( ) ].mDefaultFileName;
	...
	void tEditablePropertyFileNameString::fSetDefaultFileName( const std::string& propName, const std::string& defaultFileName )
	{
		tFileBrowserFilterMap::fInstance( )[ propName ].mDefaultFileName = defaultFileName;
	}	
	...
	void tEditablePropertyFileNameString::fCreateGui( tCreateGuiData& data )
	{
		mTextBox = new tTextBox( this, data.mParent, data.mLabel.c_str( ), fButtonSupportedFile(data.mLabel) );
		mTextBox->fSetToolTip( fDisplayOptions( ).mToolTip );
		mBrowseButton = new wxButton( data.mParent, wxID_ANY, "...", wxDefaultPosition, wxSize( 22, 20 ) );
	...
	void tEditablePropertyFileNameString::fOnBrowse( wxCommandEvent& )
	{
		...
		// dont set default directory on the browse button, only on the create button.
		if( WxUtil::fBrowseForFile( path, mBrowseButton->GetParent( ), "Select File", "", 0, filter.length( ) > 0 ? filter.c_str( ) : 0, wxFD_OPEN ) )
		{
	...
	void tEditablePropertyCustomString::fCreateGui( tCreateGuiData& data )
	{
		mTextBox = new tTextBox( this, data.mParent, data.mLabel.c_str( ) );
		mTextBox->fSetToolTip( fDisplayOptions( ).mToolTip );
		mBrowseButton = new wxButton( data.mParent, wxID_ANY, "...", wxDefaultPosition, wxSize( 22, 20 ) );
	...
	register_rtti_factory( tEditablePropertyButtons, false )

	tEditablePropertyButtons::tEditablePropertyButtons( )
	{ }

	tEditablePropertyButtons::tEditablePropertyButtons( const std::string& name, const std::string& button, const std::string& rawData )
		: tEditablePropertyString( name, rawData )
	{
		mButtonNames.fPushBack( button );
	}

	tEditablePropertyButtons::tEditablePropertyButtons( const std::string& name, const tDynamicArray<std::string>& buttons, const std::string& rawData )
		: tEditablePropertyString( name, rawData )
		, mButtonNames( buttons )
	{
	}

	void tEditablePropertyButtons::fCreateGui( tCreateGuiData& data ) 
	{ 
		for( u32 i = 0; i < mButtonNames.fCount( ); ++i )
		{
			tWxSlapOnButton* genButton = new tWxSlapOnButton( data.mParent, mButtonNames[ i ].c_str( ), " " );
			genButton->fButton( )->Connect( wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler( tEditablePropertyButtons::fButtonClicked ), (wxObject*)genButton, this );
		}
	}

	void tEditablePropertyButtons::fButtonClicked( wxCommandEvent& x )
	{
		tWxSlapOnButton* genButton = (tWxSlapOnButton*)x.m_callbackUserData;
		fSetData( std::string( genButton->fButton( )->GetLabelText( ) ) );
	}
	...
	{
		sigassert( min > -Math::cInfinity && "Testing has shown that using Infinity as extents will not behave correctly. Continue if you dare." );
		sigassert( max < Math::cInfinity && "Testing has shown that using Infinity as extents will not behave correctly. Continue if you dare." );
	}
	...
	///
	/// \section tEditablePropertyCameraDistanceCaptureFloat
	///

	register_rtti_factory( tEditablePropertyCDCFloat, false )

	tEditablePropertyCDCFloat::tEditablePropertyCDCFloat( 
		const std::string& name, 
		f32 initState,
		f32 min, f32 max, f32 increment, u32 precision, b32 allowDelete )
		: tEditablePropertyVectorBase<Math::tVec1f>( name, initState, min, max, increment, precision, allowDelete )
	...
	tEditableProperty* tEditablePropertyCDCFloat::fClone( ) const
	{
		return new tEditablePropertyCDCFloat( mName, mRawData.x, mMin, mMax, mIncrement, mPrecision, mAllowDelete );
	}

	void tEditablePropertyCDCFloat::fCreateGui( tCreateGuiData& data )
	{
		// Hold off on creating the delete button, we need to do some work.
		const b32 cachedAllowDelete = mAllowDelete;
		mAllowDelete = false;
		tEditablePropertyVectorBase<Math::tVec1f>::fCreateGui( data );

		// Add the capture button.
		wxButton* button = new wxButton( data.mParent, wxID_ANY, "C", wxDefaultPosition, wxSize( 20, 20 ) );
		button->SetForegroundColour( wxColour( 0x00, 0x99, 0x00 ) );
		button->SetToolTip( "Copy the current camera distance" );
		mSpinners.fBack( )->fAddWindowToSizer( button, true );
		button->Connect( wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler( tEditablePropertyCDCFloat::fOnCapturePressed ), NULL, this );

		// Ok now the delete button can get added.
		mAllowDelete = cachedAllowDelete;
		if( mAllowDelete )
			fAddRemoveButton( data.mParent, *mSpinners.fBack( ) );
	}

	typedef tHashTable< std::string, tEditablePropertyCDCFloat::tCallbackData > tCDCCallbackMapBase;
	class tCDCCallbackMap : public tCDCCallbackMapBase
	{
		declare_singleton_define_own_ctor_dtor( tCDCCallbackMap );
		tCDCCallbackMap( ) : tCDCCallbackMapBase( 32 ) { }
		~tCDCCallbackMap( ) { }
	};

	void tEditablePropertyCDCFloat::fAddCallback( const std::string& propName, const tCallbackData& callback )
	{
		tCDCCallbackMap::fInstance( )[ propName ] = callback;
	}

	void tEditablePropertyCDCFloat::fOnCapturePressed( wxCommandEvent& )
	{
		tCallbackData cb = tCDCCallbackMap::fInstance( )[ fGetName( ) ];
		...
		if( !cb.mFunc.fNull( ) )
		{
			...
			if( cb.mFunc( *this, newValue ) )
			{
				fSetData<Math::tVec1f>( newValue );
				if( fHasConflict( ) )
					mSpinners.fBack()->fSetValue( 0 );
				else
					mSpinners.fBack()->fSetValue( mRawData.x );
			}
		}
	}
