...
namespace Sig
{
	namespace
	{
		...
		{
			mutable tEditableObjectContainer::tEntityMasterList& mIntersection;
			...
			inline tEditableObject* fCull( const Math::tFrustumf& v, const tEntityBVH::tObjectPtr& i, b32 aabbWhollyContained ) const
			{
				...
				if( !o )											return 0;
				o = o->fGetGroupParent( );
				if( o->fIsSelectionBoxEntity( *spatial ) )			return 0;
	...
	Math::tAabbf tEditableObjectContainer::fComputeBounding( ) const
	{
		...
		for( u32 i = 0; i < shown.fCount( ); ++i )
		{
			...
			if( !worldBox.fIsValid( ) )
				log_warning( "Invalid fWorldSpaceBox found!" );
			else if( worldBox.mMin.fIsNan( ) || worldBox.mMax.fIsNan( ) )
				log_warning( "NaN fWorldSpaceBox found!" );
			else
				o |= eo->fWorldSpaceBox( );
		}
		...
		const Gfx::tIndexBufferVRamAllocatorPtr& solidColorIndices )
	{
		...
		mDummyCylinderTemplate.fResetDeviceObjects( device, solidColorMaterial, solidColorGeometry, solidColorIndices );
	...
	void tEditableObjectContainer::fChangeHeightFieldMaterial( u32 hfMatType )
	{
		for( u32 i = 0; i < mObjectSet.fCount( ); ++i )
		{
			tEntityMasterList& list = mObjectSet[ i ];
			for( u32 j = 0; j < list.fCount( ); ++j )
			{
				if( tEditableTerrainEntity* etg = list[ j ]->fDynamicCast< tEditableTerrainEntity >( ) )
					etg->fChangeHeightFieldMaterial( (tTerrainGeometry::tMaterialType)hfMatType );
			}
		}
	}

	u32 tEditableObjectContainer::fGetHeightFieldMaterialType( ) const
	{
		for( u32 i = 0; i < mObjectSet.fCount( ); ++i )
		{
			const tEntityMasterList& list = mObjectSet[ i ];
			...
			{
				if( tEditableTerrainEntity* etg = list[ j ]->fDynamicCast< tEditableTerrainEntity >( ) )
					return etg->fGetHeightFieldMaterialType( );
			}
		}
		return ~0;
	}
	...
	void tEditableObjectContainer::fOnSharedHeightFieldMapsModified( )
	{
		for( u32 i = 0; i < mObjectSet.fCount( ); ++i )
		{
			...
			for( u32 j = 0; j < list.fCount( ); ++j )
			{
				if( tEditableTerrainGeometry* etg = list[ j ]->fDynamicCast< tEditableTerrainGeometry >( ) )
					etg->fUpdateDynamicTextureReferences( );
			}
		}
	}
	...
	void tEditableObjectContainer::fGetVisibilitySet( tGrowableArray< tEditableObject* >& output, const std::string& set ) const
	{
		for( u32 istate = 0; istate < tEditableObject::cStateCount; ++istate )
		{
			const tEntityMasterList& objs = mObjectSet[ istate ];
			...
			{
				tEditableObject* o = objs[ i ]->fDynamicCast< tEditableObject >( );
				if( o && o->fVisibilitySets( ).mSet.fFind( set ) )
					output.fPushBack( o );
			}
		}
	}
	...
	void tEditableObjectContainer::fSerialize( Sigml::tObjectPtrArray& objects, tOnObjectSerialized* perObjectCb, b32 onlySelected )
	{
		...
		{
			log_warning( "Duplicate and/or invalid GUIDs were detected. Correcting, but could introduce problems for existing paths." );
	...
	void tEditableObjectContainer::fGenerateDummyTemplates( )
	{
		...
		mDummyCylinderTemplate.fSetRenderStateOverride( &mDummyRenderStateOverride );
		mDummyBoxTemplate.fGenerate( 1.f );
		mDummySphereTemplate.fGenerate( 1.f );
		mDummyCylinderTemplate.fGenerate( 1.f );
