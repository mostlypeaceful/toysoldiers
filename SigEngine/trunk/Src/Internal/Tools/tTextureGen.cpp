...
namespace Sig
{
    ...
    tLoadInPlaceResourcePtr* tTextureGen::fAddDependencyInternal( 
        tLoadInPlaceFileBase& outputFileObject, 
        const tFilePathPtr& dependencyPath )
    {
        tLoadInPlaceResourcePtr* o = 0;

        if( tTextureSysRam::fRecognizedExtension( dependencyPath.fCStr( ) ) )
        {
		...
		tFilePathPtrList& indirectGenFilesAddTo )
	{
		...
		if( Tatml::fIsTatmlFile( inputFileName ) )
		{
			...
			{
				log_warning( "Invalid source Tatml file [" << inputFileName << "], conversion failed." );
				return;
			}
		...
		else
		{
			...
			{
				log_warning( "Invalid source texture file [" << inputFileName << "], conversion failed." );
				return;
			}
		}
