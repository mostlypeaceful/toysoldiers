...
namespace Sig
{
	...
	public:
		explicit tShadeNode( const std::string& titleText, const wxColour& titleBarColor, const wxColour& titleTextColor, b32 editable, const wxPoint& topLeft = wxPoint( 0, 0 ) );
		...
		void fAddInput( const std::string& inputName, const wxColor& inputColor = wxColour( 0x11, 0xee, 0x11 ) );
		void fAddOutput( const std::string& outputName = "", const wxColor& outputColor = wxColour( 0xff, 0xff, 0xff ) );
	};
