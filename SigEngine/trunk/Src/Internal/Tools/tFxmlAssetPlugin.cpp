...
namespace Sig
{
    ...
    tLoadInPlaceResourcePtr* tFxmlAssetGenPlugin::fAddDependencyInternal( 
        ...
        )
    {
        ...
        if( Fxml::fIsFxmlFile( dependencyPath ) )
        {
	...
	void tFxmlAssetGenPlugin::fProcessInputOutputs( const tInputOutput& inOut, tFilePathPtrList& indirectGenFilesAddTo )
	{
		...
		for( u32 i = 0; i < inOut.mPlatformsToConvert.fCount( ); ++i )
		{
			...
			if( !ofile.fIsOpen( ) )
			{
				log_warning( "Couldn't open output Fxb file [" << outputPath << "] for writing." );
				continue;
			}
