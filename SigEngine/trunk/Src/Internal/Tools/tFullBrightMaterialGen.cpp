...
namespace Sig
{
	...
	namespace
	{
		void fVs30( std::string& o )
		{
			...
			ss << "   o.vOutUv.xy = vInUv.xy;" << std::endl;
			ss << "   return o;" << std::endl;
			ss << "}" << std::endl;
			o = ss.str( );
		}

		void fPs30( std::string& o, b32 gbuffer )
		{
			std::stringstream ss;
			...
			if( gbuffer )
			{
				ss << "void main( in float4 vInColor : COLOR0, in float2 vInUv : TEXCOORD0, out float4 oColor : COLOR0, out float4 oColor1 : COLOR1, out float4 oColor2 : COLOR2 ) {" << std::endl;
				ss << "   oColor = float4(0,0,0,0);" << std::endl;
				ss << "   oColor1 = float4(0,0,0,0);" << std::endl;
				ss << "   oColor2 = gTint * vInColor * tex2D( gColorMap, vInUv );" << std::endl;
			}
			else
			{
	...
	void tFullBrightMaterialGen::fGenerateMaterialFilePcDx9( tPlatformId pid )
	{
		...
		fAddPixelShader( pid, 0, Gfx::tFullBrightMaterial::cShaderSlotPS_Gbuffer, mtlFile, shaderBuffers, gBuff_ps );
