...
namespace Sig
{
	...
	void tWxDirectoryBrowser::fSelectItem( const tFilePathPtr& xmlFilePath )
	{
		wxTreeItemId* item = mXmlToIdMap.fFind( xmlFilePath.fGetHashValue() );
		if( !item )
			return;
	...
	void tWxDirectoryBrowser::fRebuildResDir( )
	{
		...
		mFilteredPaths.fDeleteArray();
		for( u32 i = 0; i < mAllPaths.fCount( ); ++i )
		{
			tFilePathPtr thisPath = mAllPaths[i];
			if( ToolsPaths::fIsUnderSourceFolder( thisPath ) )
				continue;
			if( fFilterPath( thisPath ) )
				continue;

			mFilteredPaths.fPushBack( thisPath );

			// record the paths that match this filter.
			const tFilePathPtr file = ToolsPaths::fMakeResRelative( thisPath );
	...
	void tWxDirectoryBrowser::fRefreshPartTwo( )
	{
		...
		mXmlToIdMap.fClear( mXmlToIdMap.fGetCapacity() );
		DeleteAllItems( );
		fRebuildResDir( );
	...
	wxTreeItemId tWxDirectoryBrowser::fAddDirectory( wxTreeItemId curRoot, const tTreeViewDirectoryData& dir )
	{
		...
		for( u32 ifile = 0; ifile < dir.mFiles.fCount( ); ++ifile )
		{
			...
			mXmlToIdMap.fInsert( dir.mFiles[ ifile ].mXmlPath.fGetHashValue(), fileId );
	...
	tFilePathPtrList tWxDirectoryBrowser::fGetAllPathsOfType( const tGrowableArray<const char*>& extList ) const 
	{ 
		return fGetPathsOfType( mAllPaths, extList );
	}

	tFilePathPtrList tWxDirectoryBrowser::fGetFilteredPathsOfType( const tGrowableArray<const char*>& extList ) const 
	{ 
		return fGetPathsOfType( mFilteredPaths, extList );
	}

	tFilePathPtrList tWxDirectoryBrowser::fGetPathsOfType( const tFilePathPtrList& files, const tGrowableArray<const char*>& extList )
	{
		tFilePathPtrList filteredPaths;
		for( u32 i = 0; i < files.fCount( ); ++i )
		{
			const tFilePathPtr& thisPath = files[ i ];
