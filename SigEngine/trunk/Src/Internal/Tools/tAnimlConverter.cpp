...
namespace Sig
{
	...
	b32 tAnimlConverter::fLoadAnimlFiles( const tFilePathPtrList& animlFilePaths, const tFilePathPtr& outputResourcePath )
	{
		if( animlFilePaths.fCount( ) < 2 )
		{
			...
			log_warning( "Not enough input files to convert an .anipk file; requires one .anipk and one .sklml file." );
			return false;
		}
	...
	b32 tAnimlConverter::fOutput( tFileWriter& ofile, tPlatformId pid )
	{
		...
		this->fSetSignature<tAnimPackFile>( pid );
		...
		const tExportAnim& ianim )
	{
		...
		for( u32 i = 0; i < animMetaData.mKeyFrameTags.fCount( ); ++i )
		{
			...
			oanim.mEvents[ i ].mEventTypeCppValue = tProjectFile::fInstance( ).fFindKeyframeEventIndexByKey( animMetaData.mKeyFrameTags[ i ].mEventTypeKey );
		}
		...
		b32 refFrame )
	{
		...
		{
			log_warning( "Bone [" << ibone.mName << "] not found in master skeleton file; aborting conversion of current animation." );
			return false;
		}
		...
		for( u32 i = 0, ikey = 0; i < compressionResult.mCompressionKeys.fCount( ); ++i )
		{
			if( compressionResult.mCompressionKeys[ i ].mP )
			{
				...
				obone.mPositionKeys[ ikey ] = tKeyFrameAnimation::fToPositionKey( compressionResult.mPRSKeys[ i ].mPosition, compressionResult.mPMin, compressionResult.mPMax );
				++ikey;
			}
		}
		...
		for( u32 i = 0, ikey = 0; i < compressionResult.mCompressionKeys.fCount( ); ++i )
		{
			if( compressionResult.mCompressionKeys[ i ].mR )
			{
				...
				obone.mRotationKeys[ ikey ] = tKeyFrameAnimation::fToRotationKey( compressionResult.mPRSKeys[ i ].mRotation );
				++ikey;
			}
		}
		...
		for( u32 i = 0, ikey = 0; i < compressionResult.mCompressionKeys.fCount( ); ++i )
		{
			if( compressionResult.mCompressionKeys[ i ].mS )
			{
				...
				obone.mScaleKeys[ ikey ] = tKeyFrameAnimation::fToScaleKey( compressionResult.mPRSKeys[ i ].mScale, compressionResult.mSMin, compressionResult.mSMax );
				++ikey;
			}
		}
	...
	b32 tAnimlConverter::fConvertBoneToParentSpace( const Animl::tFile& animl, const Animl::tBone& animlBone, Animl::tKeyFrameList& newKeysOut )
	{
		...
		if( !animlParent )
		{
			log_warning( "Couldn't find bone [" << sklmlParent->mName << "] in Animl file; parenting to reference frame." );
