...
namespace Sig
{
	...
	tWxSlapOnDialog::tWxSlapOnDialog( const char* title, wxWindow* parent, const std::string regKeyName )
		: wxFrame( parent, wxID_ANY, title, wxDefaultPosition, wxDefaultSize, parent ? (cDefaultStyles | wxFRAME_FLOAT_ON_PARENT) : cDefaultStyles )
		, mRawParent( 0 )
		, mWasTopMost( false )
	...
	{
		// TODO: In 2.9, maybe HasFocus() will handle this more elegantly?
		wxWindow* focusWin = wxWindow::FindFocus();
		if( !IsShown() || !focusWin )
			return false;

		return FindWindow( focusWin->GetId() ) != NULL;
	}
