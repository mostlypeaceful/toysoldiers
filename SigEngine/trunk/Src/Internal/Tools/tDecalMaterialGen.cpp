...
namespace Sig
{
	...
	struct tDecalMaterialGenStruct : public Gfx::tDecalMaterial
	{
		...
		void fVSOutputPsInputStruct( std::stringstream& ss, b32 psIn, b32 tangent, b32 shadows = false, u32 lightCount = 0 )
		{
			if( psIn )
		...
		std::string& fPS( std::string& o, b32 tangent, b32 shadows, u32 numUvs, u32 diffuseUvSet, u32 normalUvSet, u32 lightCount, tPlatformId pid )
		{
			...
			fVSOutputPsInputStruct( ss, true, tangent, shadows, lightCount );
			...
			if( lightCount >= 2 )
			{
				ss << "for( int iLight = 1; iLight < " << lightCount << "; ++iLight )" << std::endl;
				ss << "{" << std::endl;
				tMaterialGenBase::fCallAccumulateLight(
					ss,
					false,
		...
		void fGenerate( Gfx::tMaterialFile& mtlFile, tMaterialGenBase::tShaderBufferSet& shaderBuffers, tPlatformId pid )
		{
			...
			fWriteCompiledShader( tFilePathPtr::fConstructPath( ToolsPaths::fGetEngineRootFolder(), tFilePathPtr( "tmp\\matgen\\Fullbright.vsh" ) ), vs );
			fWriteCompiledShader( tFilePathPtr::fConstructPath( ToolsPaths::fGetEngineRootFolder(), tFilePathPtr( "tmp\\matgen\\Fullbright.psh" ) ), ps );
			...
			fWriteCompiledShader( tFilePathPtr::fConstructPath( ToolsPaths::fGetEngineRootFolder(), tFilePathPtr( "tmp\\matgen\\DiffuseOnly.vsh" ) ), s );
			fWriteCompiledShader( tFilePathPtr::fConstructPath( ToolsPaths::fGetEngineRootFolder(), tFilePathPtr( "tmp\\matgen\\DiffuseNormal.vsh" ) ), s );

			for( u32 iLightCount = 0; iLightCount < Gfx::tMaterial::cLightSlotCount; ++iLightCount )
			{
				// add non normal-mapped non-shadowed pixel shaders
				tMaterialGenBase::fAddPixelShader( pid, Gfx::tDecalMaterial::cShaderCategoryPS, 
					...
					fPS( s, false, false,  1,  0, ~0, iLightCount, pid ) );


				// add non normal-mapped shadowed pixel shaders
				tMaterialGenBase::fAddPixelShader( pid, Gfx::tDecalMaterial::cShaderCategoryPS, 
					fPixelShaderSlot( cPSDiffuseShadow, iLightCount ), mtlFile, shaderBuffers,
					fPS( s, false, true,  1,  0, ~0, iLightCount, pid ) );

				// add normal-mapped non-shadowed shaders
				tMaterialGenBase::fAddPixelShader( pid, Gfx::tDecalMaterial::cShaderCategoryPS, 
					fPixelShaderSlot( cPSDiffuseNormal, iLightCount ), mtlFile, shaderBuffers,
					fPS( s, true, false, 1,  0,  0, iLightCount, pid ) );

				// add normal-mapped shadowed shaders
				tMaterialGenBase::fAddPixelShader( pid, Gfx::tDecalMaterial::cShaderCategoryPS, 
					fPixelShaderSlot( cPSDiffuseNormalShadow, iLightCount ), mtlFile, shaderBuffers,
					fPS( s, true, true, 1,  0,  0, iLightCount, pid ) );
			}
		}
		...
		void fAssignVertexAndPixelShaderSlots( Gfx::tDecalMaterial* mtl, b32 hasTangent )
		{
			...
			if( hasTangent )
			{
				...
				mtl->mPS = cPSDiffuseNormalShadow;
			}
			else
			{
				// non normal mapped versions
				mtl->mVS = cVSDiffuse;
				mtl->mPS = cPSDiffuseShadow;
			}
			// TODO: Fullbright
		}
	};
