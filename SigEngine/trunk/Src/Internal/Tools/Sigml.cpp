...
namespace Sig { namespace Sigml
{
	...
	void tMaterialRenderOptions::fToRenderState( Gfx::tRenderState& rs )
	{
		...
		if( mAdditive )
		{
			...
			if( mTransparency )
				rs.fSetSrcBlendMode( Gfx::tRenderState::cBlendSrcAlpha );
			else 
	...
	void fSerializeXmlObject( tSerializer& s, tMesh& o )
	{
		...
		s( "LodRatioHigh", o.mHighLodRatio );
		s( "LodRatioMedium", o.mMediumLodRatio );
		s( "LodRatioLow", o.mLowLodRatio );
			...
			for( u32 i = 0; i < gameProperties.fCount( ); ++i )
			{
				...
				if( prop->fClassId( ) == Rtti::fGetClassId< tEditablePropertyProjectFileTag >( ) )
				{
					...
					if( tagBitIndex != ~0 )
						...
						log_warning( "Could not find game tag by key: " << value );
				}
				else
					log_warning( "Unrecognized game tag type [" << std::hex << prop->fClassId( ) << "], name = [" << prop->fGetName( ) << "]" );
			}
		}
			...
			const char* groupName )
		{
			...
			for( u32 i = 0; i < gameProperties.fCount( ); ++i )
			{
				...
				if( prop->fClassId( ) == Rtti::fGetClassId< tEditablePropertyProjectFileEnum >( ) )
				{
					...
					if( !enumT )
					{
						log_warning( "Could not find enum type by key: " << enumProp->fEnumTypeKey( ) );
					}
					else
					{
						u32 key = enumT->mKey;
						...
						if( key != ~0 && valueKey != ~0 )
						{
							...
							if( index == ~0 )
							{
								log_warning( "Could not find enum (key: " << key << ") value by key: " << valueKey );
							}
							else
								entityDefProps.fPushBack( tEntityDefProperties::tEnumProperty( key, index ) );
						}
						else
							log_warning( "Programmer Error key: " << key << " valueKey: " << valueKey );
					}
				}
				else
					log_warning( "Unrecognized game property type [" << std::hex << prop->fClassId( ) << "], name = [" << prop->fGetName( ) << "]" );
			}
		}
	}
	...
	b32	tObjectProperties::fGetBoneRelativeAttachmentProperty( ) const
	{
		return mEditableProperties.fGetValue< b32 >( fEditablePropBoneRelativeAttachment( ), false );
	}
	...
	b32 tObjectProperties::fGetPhysicsDisableChildCollision( ) const
	{
		return mEditableProperties.fGetValue< b32 >( fEditablePropPhysicsIgnoreChildCollision( ), false );
	}

	b32 tObjectProperties::fGetPhysicsCreateCollision( ) const
	{
		return mEditableProperties.fGetValue< b32 >( fEditablePropPhysicsCreateStatic( ), false );
	}

	b32 tObjectProperties::fGetPhysicsCreateStatic( ) const
	{
		return mEditableProperties.fGetValue< b32 >( fEditablePropPhysicsCreateStatic( ), false );
	}
	...
	void tObjectProperties::fAddEntityDefProperties( tEntityDefProperties* entityDef, tLoadInPlaceFileBase& converter ) const
	{
		...
		if( boneAttachment.fLength( ) > 0 )
		{
			...
			if( fGetBoneRelativeAttachmentProperty( ) )
				entityDef->mCreationFlags.mCreateFlags |= tEntityCreationFlags::cFlagBoneRelativeAttachment;
		}
	...
	void tObject::fConvertEntityDefBase( tEntityDef* entityDef, tLoadInPlaceFileBase& converter )
	{
		...
		creationFlags.mCreateFlags |= ( fGetPhysicsDisableChildCollision( ) ? tEntityCreationFlags::cFlagPhysicsDisableChildCollision : 0 );
		creationFlags.mCreateFlags |= ( fGetPhysicsCreateCollision( ) ? tEntityCreationFlags::cFlagPhysicsCreateStatic : 0 );

		creationFlags.mVisibilitySet.mSerializedVisibilitySets.fNewArray( mVisibilitySets.mSet.fCount( ) );
		for( u32 i = 0; i < mVisibilitySets.mSet.fCount( ); ++i )
			creationFlags.mVisibilitySet.mSerializedVisibilitySets[ i ] = converter.fAddLoadInPlaceStringPtr( mVisibilitySets.mSet[ i ].c_str( ) );
	...
	void fSerializeXmlObject( tSerializer& s, tGeomObject& o )
	{
		...
		if( s.fIn( ) )
		{
			// convert existing indexes to masks.
			s32 stateIndex = -2;
			...
			if( stateIndex != -2 )
			{
				...
				if( stateIndex == -1 )
					...
					o.mStateMask = (1 << stateIndex);
			}
		}

		s( "StateMask", o.mStateMask );
	}
	...
	tEntityDef* tGeomObject::fCreateEntityDef( tSigmlConverter& sigmlConverter )
	{
		...
		if( mStateMask == ~0 )
			entityDef->mStateMask = 0xFFFF;
		else
		{
			if( (mStateMask & 0xFFFF0000) != 0 )
				log_warning( "State mask use out of bounds! Mask: " << mStateMask );
			entityDef->mStateMask = mStateMask;
		}
	...
	{
		log_warning( "Geometry Objects (meshes) not supported as editable objects. Skipping object." );
		return 0;
	}
	...
	tEntityDef* tSigmlReferenceObject::fCreateEntityDef( tSigmlConverter& sigmlConverter )
	{
		...
		const u32 mediumOverride = mEditableProperties.fGetValue( fEditablePropLODMediumDistanceName( ), 0.f );
		const f32 farOverride = mEditableProperties.fGetValue( fEditablePropLODFarDistanceName( ), 0.f );
		if( fadeSetting || fadeOverride )
		{
			...
			entityDef->mLODSettings->mLODMediumDistanceOverride = mediumOverride;
			entityDef->mLODSettings->mLODFarDistanceOverride = farOverride;
		}
	...
	// tGroundCoverLayer
	//------------------------------------------------------------------------------
	tGroundCoverLayer::tGroundCoverLayer( )
		: mUniqueId( ~0 )
		, mUnitSize( 10.f )
		, mPaintUnits( 2 )
		, mVisible( true )
		...
		, mFarVisibility( Gfx::tGroundCoverCloudDef::cVisibilityNear )
		, mNearVisibility( Gfx::tGroundCoverCloudDef::cVisibilityNoFade ){ }

	//------------------------------------------------------------------------------
	tGroundCoverLayer::tGroundCoverLayer( u32 uniqueId, const char * name )
		: mUniqueId( uniqueId )
		, mName( name )
		, mUnitSize( 10.f )
		, mPaintUnits( 2 ) 
	...
	//------------------------------------------------------------------------------
	tGroundCoverLayer::tGroundCoverLayer( const tGroundCoverLayer & other )
		...
		, mFarVisibility( other.mFarVisibility ) 
		, mNearVisibility( other.mNearVisibility ) { }
	...
	void fSerializeXmlObject( tSerializer& s, tFile& o )
	{
		...
		{
			log_warning( "Sigml file format is out of date (exported from " << o.mSrcFile << ") -> Please re-export." );
			return;
		}
		...
		s( "Layers", o.mLayers );
		s( "PotentialVisibility", o.mPotentialVisibility );
	...
	b32 tFile::fSaveXml( const tFilePathPtr& path, b32 promptToCheckout )
	{
		...
		{
			log_warning( "Couldn't save Sigml file [" << path << "]" );
			return false;
		}
	...
	b32 tFile::fLoadXml( const tFilePathPtr& path )
	{
		...
		{
			log_warning( "Couldn't load Sigml file [" << path << "]" );
			return false;
		}
	...
	b32 tFile::fApplySkinFile( const tFile& skinFile )
	{
		...
		{
			log_warning( "Error applying skin file to mesh file: mis-match in mesh count." );
			return false;
		}
		...
		for( u32 i = 0; i < mMeshes.fCount( ); ++i )
		{
			...
			{
				log_warning( "Error applying skin file to mesh file: mis-match in the number of vertices within mesh." );
				return false;
			}
		}
	...
	void tFile::fAddEntityDefProperties( tSceneGraphFile* sgFile, tSigmlConverter& sigmlConverter ) const
	{
		...
		if( fadeSetting || fadeOverride )
		{
			...
			sgFile->mLODSettings->mLODMediumDistanceOverride = mediumOverride;
			sgFile->mLODSettings->mLODFarDistanceOverride = farOverride;
		}
	}
	...
	void tFile::fAddGlobalProperties( )
	{
		...
		mEditableProperties.fInsert( tEditablePropertyPtr( new tEditablePropertyCDCFloat( tObjectProperties::fEditablePropLODMediumDistanceName( ), 0.f, 0.f, 10000.f, 1.f, 0 ) ) );
		mEditableProperties.fInsert( tEditablePropertyPtr( new tEditablePropertyCDCFloat( tObjectProperties::fEditablePropLODFarDistanceName( ), 0.f, 0.f, 10000.f, 1.f, 0 ) ) );
