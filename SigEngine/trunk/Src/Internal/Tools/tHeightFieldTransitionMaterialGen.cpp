...
namespace Sig
{
	...
	register_rtti_factory( tHeightFieldTransitionMaterialGen, false );

	struct tHeightFieldTransitionLightingPassGen : public Gfx::tHeightFieldTransitionMaterial::tLightingPass
	{
		...
		static void fAtlasSampler( std::stringstream& ss, b32 normalMap )
		{
			...
			if( normalMap )
			{
				ss << "   float2 DtoN_ratio = float2( gTexAtlasDiffuseCount_NormalCount.x / gTexAtlasDiffuseCount_NormalCount.z, gTexAtlasDiffuseCount_NormalCount.y / gTexAtlasDiffuseCount_NormalCount.w );" << std::endl;
				ss << "   uv = uvOffset * invTexAtlasDims + gutter + tiledWorldUv * ( invTexAtlasDims * DtoN_ratio - 2.f * gutter );" << std::endl;
		...
		static void fPsGlobals( std::stringstream& ss, b32 normalMap, b32 shadows, tPlatformId pid, b32 multiObject )
		{
			ss << "#pragma pack_matrix(row_major)" << std::endl;
			...
			// time
			const char cGlobalTimeName[] = "gTime";
			ss << "float4 " << cGlobalTimeName << " : register( " << fAppend("c", cPSTime) << " );" << std::endl;

			// Transition info
			Gfx::tTransitionObjectMaterialHelper::fAddDefaultIdDeclaration( ss, cPSDefaultAlignment );
			Gfx::tTransitionObjectMaterialHelper::fAddEdgeColorDeclaration( ss, cPSAlignmentColors );
			Gfx::tTransitionObjectMaterialHelper::fAddObjectsDeclaration( ss, cPSTransitionObjects );
			...
			ss << std::endl;
			if( multiObject )
				Gfx::tTransitionObjectMaterialHelper::fAddFunctionDefinition( ss, cGlobalTimeName );
		}
		...
		static void fVSOutputPsInputStruct( std::stringstream& ss, b32 psIn, b32 normalMapped, b32 shadows, b32 deferred, u32 lightCount )
		{
			...
			ss << "   float4 vWorldPos : TEXCOORD2;" << std::endl;
		...
		static std::string& fVS( std::string& o, b32 normalMap, b32 useCompressedInputFormat, b32 deferred, tPlatformId pid )
		{
			...
			if( tHeightFieldMaterialGenBase::fShadowMapLayerCount( pid ) == 1 )
				ss << "   o.vWorldPos = mul( gWorldToLight, float4( pworld, 1.f ) );" << std::endl;
			else
				ss << "   o.vWorldPos = float4( pworld, 1.f );" << std::endl;
		...
		static std::string& fPS( std::string& o, b32 normalMap, b32 shadows, b32 deferred, u32 lightCount, tPlatformId pid, b32 multiObject )
		{
			...
			fPsGlobals( ss, normalMap, shadows, pid, multiObject );
			...
			if( lightCount > 0 || deferred )
			{
				...
				// Choose alignment info
				if( multiObject )
				{
					ss << "   float4 alignInfo = "; Gfx::tTransitionObjectMaterialHelper::fAddFunctionCall( ss, "i.vWorldPos.xyz" );
					ss << "   float bestAlignment = trunc( alignInfo.w );" << std::endl;
					ss << "   float edgeNess = frac( alignInfo.w );" << std::endl;
				}
				else
					ss << "   float bestAlignment = " << Gfx::tTransitionObjectMaterialHelper::cDefaultIdName << ";\n";

				//shift all indices into materials table by our chosen alignment
				ss << "materialIdsExpanded.x += bestAlignment;\n";
				ss << "materialIdsExpanded.y += bestAlignment;\n";
				ss << "materialIdsExpanded.z += bestAlignment;\n";
				...
				// mix the materials' diffuse samples using material masks
				...
				if( multiObject )
					ss << "   diffuseMaterialColor = lerp( diffuseMaterialColor, float4( alignInfo.rgb, diffuseMaterialColor.a ), edgeNess );" << std::endl;
				...
				if( !deferred )
				{
					if( shadows )
					{
						...
						shadowParams.mSs						= &ss;
						shadowParams.mWorldPosName				= "i.vWorldPos";
						//shadowParams.mNDotL					= "nDotL"; // dot( n, gLights[ 0 ].vN??? )
						tHeightFieldMaterialGenBase::fCallComputeShadowTerm( shadowParams );
					}
		...
		void fGenerate( Gfx::tMaterialFile& mtlFile, tHeightFieldMaterialGenBase::tShaderBufferSet& shaderBuffers, tPlatformId pid )
		{
			...
			const tFilePathPtr tmpFolder = tFilePathPtr::fConstructPath( ToolsPaths::fGetEngineTempFolder( ), tFilePathPtr( "heightfield_transition/" ), tFilePathPtr( fPlatformIdString(pid) ) );
			...
			{
				fPS( s, false, false, false, iLightCount, pid, false );
				...
				fPS( s, false, true, false, iLightCount, pid, false );
				...
				fPS( s, true, false, false, iLightCount, pid, false );
				...
				fPS( s, true, true, false, iLightCount, pid, false );
				...
				fPS( s, false, false, false, iLightCount, pid, true );
				FileSystem::fWriteBufferToFile( s.c_str( ), s.length( ), tFilePathPtr::fConstructPath( tmpFolder, tFilePathPtr( "diffuse_mo.psh" ) ) );
				tHeightFieldMaterialGenBase::fAddPixelShader( pid, Gfx::tHeightFieldMaterialBase::cShaderCategoryLightingPS,
					fPixelShaderSlot( cPSDiffuseMultiObject, iLightCount ), mtlFile, shaderBuffers, s );


				fPS( s, false, true, false, iLightCount, pid, true );
				FileSystem::fWriteBufferToFile( s.c_str( ), s.length( ), tFilePathPtr::fConstructPath( tmpFolder, tFilePathPtr( "diffuse_shadow_mo.psh" ) ) );
				tHeightFieldMaterialGenBase::fAddPixelShader( pid, Gfx::tHeightFieldMaterialBase::cShaderCategoryLightingPS,
					fPixelShaderSlot( cPSDiffuseShadowMultiObject, iLightCount ), mtlFile, shaderBuffers, s );


				fPS( s, true, false, false, iLightCount, pid, true );
				FileSystem::fWriteBufferToFile( s.c_str( ), s.length( ), tFilePathPtr::fConstructPath( tmpFolder, tFilePathPtr( "diffuse_nmap_mo.psh" ) ) );
				tHeightFieldMaterialGenBase::fAddPixelShader( pid, Gfx::tHeightFieldMaterialBase::cShaderCategoryLightingPS,
					fPixelShaderSlot( cPSDiffuseNormalMapMultiObject, iLightCount ), mtlFile, shaderBuffers, s );


				fPS( s, true, true, false, iLightCount, pid, true );
				FileSystem::fWriteBufferToFile( s.c_str( ), s.length( ), tFilePathPtr::fConstructPath( tmpFolder, tFilePathPtr( "diffuse_nmap_shadow_mo.psh" ) ) );
				tHeightFieldMaterialGenBase::fAddPixelShader( pid, Gfx::tHeightFieldMaterialBase::cShaderCategoryLightingPS,
					fPixelShaderSlot( cPSDiffuseNormalMapShadowMultiObject, iLightCount ), mtlFile, shaderBuffers, s );


				fPS( s, false, false, true, 1, pid, false );
				...
				fPS( s, true, false, true, 1, pid, false );
				...
				fPS( s, false, false, true, 1, pid, true );
				FileSystem::fWriteBufferToFile( s.c_str( ), s.length( ), tFilePathPtr::fConstructPath( tmpFolder, tFilePathPtr( "diffuseGBuff_mo.psh" ) ) );
				tHeightFieldMaterialGenBase::fAddPixelShader( pid, Gfx::tHeightFieldMaterialBase::cShaderCategoryLightingPS,
					fPixelShaderSlot( cPSDiffuseGBufferMultiObject, iLightCount ), mtlFile, shaderBuffers, s );

				fPS( s, true, false, true, 1, pid, true );
				FileSystem::fWriteBufferToFile( s.c_str( ), s.length( ), tFilePathPtr::fConstructPath( tmpFolder, tFilePathPtr( "diffuseGBuff_nmap_mo.psh" ) ) );
				tHeightFieldMaterialGenBase::fAddPixelShader( pid, Gfx::tHeightFieldMaterialBase::cShaderCategoryLightingPS,
					fPixelShaderSlot( cPSDiffuseNormalMapGBufferMultiObject, iLightCount ), mtlFile, shaderBuffers, s );
			}
		}
	};


	struct tHeightFieldTransitionShadowMapPassGen : public Gfx::tHeightFieldTransitionMaterial::tShadowMapPass
	{
		static void fVsGlobals( std::stringstream& ss )
		{
			ss << "#pragma pack_matrix(row_major)" << std::endl;
	...
	tHeightFieldTransitionMaterialGen::tHeightFieldTransitionMaterialGen( )
	{
	}

	Gfx::tMaterial* tHeightFieldTransitionMaterialGen::fCreateHeightFieldGfxMaterial( tLoadInPlaceFileBase& lipFileCreator, b32 skinned, b32 compressedVerts )
	{
		Gfx::tHeightFieldTransitionMaterial* mtl = new Gfx::tHeightFieldTransitionMaterial( );
		...
		const tResourceId mtlFileRid = tResourceId::fMake<Gfx::tMaterialFile>( Gfx::tHeightFieldTransitionMaterial::fMaterialFilePath( ) );
		...
		{
			if( compressedVerts )
				mtl->mLightingPass.mVS = Gfx::tHeightFieldTransitionMaterial::tLightingPass::cVSDiffuseNormalMapCompressed;
			else
				mtl->mLightingPass.mVS = Gfx::tHeightFieldTransitionMaterial::tLightingPass::cVSDiffuseNormalMap;
			mtl->mLightingPass.mPS = Gfx::tHeightFieldTransitionMaterial::tLightingPass::cPSDiffuseNormalMapShadow;
		}
		else
		{
			if( compressedVerts )
				mtl->mLightingPass.mVS = Gfx::tHeightFieldTransitionMaterial::tLightingPass::cVSDiffuseCompressed;
			else
				mtl->mLightingPass.mVS = Gfx::tHeightFieldTransitionMaterial::tLightingPass::cVSDiffuse;
			mtl->mLightingPass.mPS = Gfx::tHeightFieldTransitionMaterial::tLightingPass::cPSDiffuseShadow;
		}
		mtl->mShadowMapPass.mVS = Gfx::tHeightFieldTransitionMaterial::tShadowMapPass::cVSDepth;
		mtl->mShadowMapPass.mPS = Gfx::tHeightFieldTransitionMaterial::tShadowMapPass::cPSDepth;

		return mtl;
	}

	tFilePathPtr tHeightFieldTransitionMaterialGen::fMaterialFilePath( ) const
	{
		return Gfx::tHeightFieldTransitionMaterial::fMaterialFilePath( );
	}

	void tHeightFieldTransitionMaterialGen::fGenerateMaterialFileWii( tPlatformId pid )
	{
	}

	void tHeightFieldTransitionMaterialGen::fGenerateMaterialFilePcDx9( tPlatformId pid )
	{
		Gfx::tMaterialFile mtlFile;
		mtlFile.mMaterialCid = Rtti::fGetClassId<Gfx::tHeightFieldTransitionMaterial>( );
		...
		tHeightFieldTransitionLightingPassGen lightingPassGen;
		lightingPassGen.fGenerate( mtlFile, shaderBuffers, pid );

		tHeightFieldTransitionShadowMapPassGen shadowMapPassGen;
		shadowMapPassGen.fGenerate( mtlFile, shaderBuffers, pid );

		fSaveMaterialFile( mtlFile, shaderBuffers, pid );
	}

	void tHeightFieldTransitionMaterialGen::fGenerateMaterialFilePcDx10( tPlatformId pid )
	{
	}

	void tHeightFieldTransitionMaterialGen::fGenerateMaterialFileXbox360( tPlatformId pid )
	{
		fGenerateMaterialFilePcDx9( pid );
	}

	void tHeightFieldTransitionMaterialGen::fGenerateMaterialFilePs3Ppu( tPlatformId pid )
	{
	}
