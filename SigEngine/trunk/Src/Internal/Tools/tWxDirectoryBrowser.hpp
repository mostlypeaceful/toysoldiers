...
namespace Sig
{
	...
	protected:
		...
		tFilePathPtrList			mFilteredPaths;
		...
		tHashTable< tHashTablePtrInt, wxTreeItemId > mXmlToIdMap;
	...
	public:
		...
		void fSelectItem( const tFilePathPtr& xmlFilePath );
		...
		tFilePathPtrList fGetAllPathsOfType( const tGrowableArray<const char*>& extList ) const;

		// Returns paths filtered by this browser's fFilterPath function. Not guaranteed to return a full set of a specific file type.
		const tFilePathPtrList& fGetFilteredPaths( ) const { return mFilteredPaths; }
		tFilePathPtrList fGetFilteredPathsOfType( const tGrowableArray<const char*>& extList ) const;

		static tFilePathPtrList fGetPathsOfType( const tFilePathPtrList& files, const tGrowableArray<const char*>& extList );
	private:
