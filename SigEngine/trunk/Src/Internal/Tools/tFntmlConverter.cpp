...
namespace Sig
{
	...
	b32 tFntmlConverter::fLoad( const tFilePathPtr& path )
	{
		...
		{
			log_warning( "Fonts only support a single page (1 font texture map) - in the export settings for the font, adjust the output texture size to ensure it is big enough." );
			return false;
		}
	...
	void tFntmlConverter::fSaveGameBinary( const tFilePathPtr& path, tPlatformId pid )
	{
		...
		if( !ofile.fIsOpen( ) )
		{
			log_warning( "Couldn't open output file [" << path << "] for writing (it's probably read-only for some reason." );
			return;
		}

		mOutputFont.fSetSignature<Gui::tFont>( pid );
