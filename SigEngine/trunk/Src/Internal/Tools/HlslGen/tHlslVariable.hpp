...
namespace Sig { namespace HlslGen
{
	...
	struct tools_export tShaderRequirements
	{
		...
		void fSortGlobals( );
		void fComputeVertexFormat( Gfx::tVertexFormat& vtxFormat, b32 instanced ) const;
	...
	public:
		...
		std::string			fSwizzle( const tHlslVariablePtr& matchThisVar, const std::string& extraText = "" ) const { return fSwizzle( *matchThisVar, extraText ); }
		std::string			fSwizzle( const tHlslVariableConstPtr& matchThisVar, const std::string& extraText = "" ) const { return fSwizzle( *matchThisVar, extraText ); }
