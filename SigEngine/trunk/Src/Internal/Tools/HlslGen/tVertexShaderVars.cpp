...
namespace Sig { namespace HlslGen
{
	...
	tVertexShaderVars::tVertexShaderVars( )
	{
		...
		fAddGlobal( mVariables[ cGlobalShadowMapSplits ][ cPidDefault ], tHlslVariable::fMakeGlobalVector( "gShadowMapSplits", baseReg, 4 ), baseReg,
			new Gfx::tVsGlueShadowMapSplits( baseReg ) );
		fAddGlobal( mVariables[ cGlobalInstancingData ][ cPidDefault ], tHlslVariable::fMakeVectorFloat( "gIndiciesPerInstance_CameraEye2D_ViewDistSq", tHlslVariable::cGlobal, 4, tHlslVariable::tSemantic( "c", baseReg ) ), baseReg,
			new Gfx::tVsGlueInstancingData( baseReg ) );

		// Pallet must be last, it will take up remaining registers.
		...
		mVariables[ cOutWorldSpacePos ][ cPidDefault ].fReset( tHlslVariable::fMakeVectorFloat( "oWorldSpacePos", tHlslVariable::cOutput, 3, tPixelShaderVars::fVariable( tPixelShaderVars::cInWorldSpacePos )->fSemantic( ) ) );
		...
		mVariables[ cOutProjSpaceDepth ][ cPidDefault ].fReset( tHlslVariable::fMakeVectorFloat( "oProjSpaceDepth", tHlslVariable::cOutput, 3, tPixelShaderVars::fVariable( tPixelShaderVars::cInProjSpaceDepth )->fSemantic( ) ) );
