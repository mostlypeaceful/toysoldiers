...
define_unittest( TestFilePackage )
{
	...
	tFileSystemResourceProvider* fsp = NEW tFileSystemResourceProvider( );
	resDepot.fAddResourceProvider( tResourceProviderPtr( fsp ) );
	tFilePackageResourceProvider* fpp = NEW tFilePackageResourceProvider( );
	resDepot.fAddResourceProvider( tResourceProviderPtr( fpp ) );

	tFilePackagePtr thePkg( NEW tFilePackage( ) );

	thePkg->fLoad( this, resDepot, cfg.mOutputPath );
	thePkg->fBlockUntilLoaded( );
	fpp->fAddPackage( *thePkg );
	...
	{
		tResourcePtr resource = resDepot.fQuery( tResourceId::fMake( 0, cfg.mFilesToAdd[i] ) );
