...
namespace Sig 
{
	...
	void tLeaderboard::fSelectBoard( u32 boardId )
	{
		...
		log_warning( "Leaderboard " << boardId << " not accessible because it wasn't added for read" );
	}

	//------------------------------------------------------------------------------
	b32 tLeaderboard::fReading( ) const
	{
		return mState == cStateReading || mState == cStateReadingFriends;
	}
	...
	void tLeaderboard::fAddBoardToRead( u32 boardId )
	{
		...
		{
			log_warning( "Leaderboard already has max of " << mBoardsToRead.fCount( ) << " boards to read set." );
			return;
		}
		...
		log_warning( "Leaderboard " << boardId << " could not found for reading" );
	}
	...
	void tLeaderboard::fRemoveBoardToRead( u32 boardId )
	{
		...
		log_warning( "Leaderboard " << boardId << " was not added for reading" );
	}
	...
	{
		Sqrat::Class< tLeaderboard, Sqrat::NoCopy<tLeaderboard> > classDesc( vm.fSq( ) );
		classDesc
