...
namespace Sig
{
	//static_assert( sizeof( tDynamicQuadtree::tObject ) == 32 );

	devvar_clamp( u32, SceneGraph_Quadtree_ObjectCountBeforeSplit, 0, 0, 1024, 0 );
	devvar_clamp( u32, SceneGraph_Quadtree_MaxDepth, 6, 0, 1024, 0 );
	...
	{
		fClear( );
		mMaxDepth = fMin( maxDepth, (u32)SceneGraph_Quadtree_MaxDepth );
	...
	{
		object->fRemove<tDynamicQuadtree>( );
	}

	void tDynamicQuadtreeRoot::fMove( const tObjectPtr& object )
	{
		tDynamicQuadtree* ignoreOnInsert = object->fMove<tDynamicQuadtree>( mMaxDepth );
		if( ignoreOnInsert )
