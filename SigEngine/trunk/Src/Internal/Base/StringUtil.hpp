...
{
	base_export u32			fStrLen( const char* a ); //like strlen() this does not include null-terminator. fStrLen( "hello" ) == 5
	base_export u32			fStrLen( const wchar_t* a );
	base_export s32			fStricmp( const char* a, const char* b );
	base_export s32			fStrnicmp( const char* a, const char* b, u32 count );
	base_export s32			fWStricmp( const wchar_t* a, const wchar_t* b );
	...
	base_export void		fSplit( tGrowableArray<std::wstring>& subStringsOut, const wchar_t* src, const wchar_t* symbolToSplitOn, b32 keepEmpty = false );
	...
	base_export std::string	fAddCommaEvery3Digits( const char* numberValue, const char* prepend, const char* append, const char* commaValue );
	base_export std::string fTrim( const char* text, const char* trimchars, b32 trimBegin, b32 trimEnd );
	...
	base_export b32         fIsAnyOf( const char* options, char character ); // Checks if character is any of "options"
	base_export b32         fStartsWith( const char* searchIn, const char* prefix );
	base_export b32         fStartsWithI( const char* searchIn, const char* prefix );
	base_export b32         fEndsWith( const char* searchIn, const char* postfix );
	base_export b32         fEndsWithI( const char* searchIn, const char* postfix );
	base_export b32			fAndSearch( const char* searchIn, const tGrowableArray< std::string >& includes );
	base_export b32			fNotSearch( const char* searchIn, const tGrowableArray< std::string >& excludes );
	base_export void		fSplitSearchParams( tGrowableArray< std::string >& includesOut, tGrowableArray< std::string >& excludesOut, const char* src ); // Splits a string into Includes and Excludes
