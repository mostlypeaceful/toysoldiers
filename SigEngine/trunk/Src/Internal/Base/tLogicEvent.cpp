...
namespace Sig { namespace Logic
{
	...
	namespace
	{
		...
		static Logic::tVec3fEventContext* fAsVec3fEventContext( const Logic::tEventContext* obj )
		{
			return obj ? obj->fDynamicCast< Logic::tVec3fEventContext >( ) : NULL;
		}
	}

	void tStringEventContext::fSaveLoadDerived( tGameArchive& archive )
	{
		archive.fSaveLoad( mString );
	}

	void tIntEventContext::fSaveLoadDerived( tGameArchive& archive )
	{
		archive.fSaveLoad( mInt );
	}

	void tBoolEventContext::fSaveLoadDerived( tGameArchive& archive )
	{
		b32 value = mBool;
		archive.fSaveLoad( value );
		mBool = !!value;
	}
	...
	void tEvent::fExportScriptInterface( tScriptVm& vm )
	{
		...
		Sqrat::Class<Logic::tVec3fEventContext, Sqrat::DefaultAllocator<Logic::tVec3fEventContext> > classDescVContext( vm.fSq( ) );
		classDescVContext
			.StaticFunc(_SC("Construct"), &Logic::tVec3fEventContext::fConstructDefault)
			.Prop(_SC("Vec3f"),           &Logic::tVec3fEventContext::fVec3f)
			.StaticFunc(_SC("Convert"),   &fAsVec3fEventContext)
			;
		vm.fRootTable( ).Bind( _SC("Vec3fEventContext"), classDescVContext );
	}
}}//Sig::Logic
