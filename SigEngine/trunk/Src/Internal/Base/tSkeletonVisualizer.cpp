...
namespace Sig
{
	...
	void tSkeletonVisualizer::fSetSkeleton( const Anim::tAnimatedSkeletonPtr& animatedSkel )
	{
		mSkeleton = animatedSkel;

		Anim::tAnimatedSkeleton::tBoneLine boneLines;
	...
	void tSkeletonVisualizer::fSyncToSkeleton( const Math::tMat3f& ogXform )
	{
		...
		u32 ithProxy = 0;
		for( Anim::tAnimatedSkeleton::tBoneProxyList::tConstIterator iproxy = mSkeleton->fBoneProxiesBegin( ), iproxyEnd = mSkeleton->fBoneProxiesEnd( );
	...
	void tSkeletonVisualizer::fGenerateRenderables( const Anim::tAnimatedSkeleton::tBoneLine& root )
	{
		++mCurrentJoint;
	...
	void tSkeletonVisualizer::fUpdateRenderables( const Anim::tAnimatedSkeleton::tBoneLine& root )
	{
