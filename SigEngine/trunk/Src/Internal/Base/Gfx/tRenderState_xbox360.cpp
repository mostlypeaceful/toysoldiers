...
namespace Sig { namespace Gfx
{
	...
	void tRenderState::fEnableDisableColorWrites( const tDevicePtr& device, b32 enable )
	{
		...
		d3ddev->SetRenderState( D3DRS_COLORWRITEENABLE, enable ? D3DCOLORWRITEENABLE_ALL : 0 );
	}
...
#define set_render_state d3ddev->SetRenderState
		...
		const b32 properAlpha = fQuery( cWriteProperAlpha );
		...
		if( !fCompareStateToLastApplied( cWriteProperAlpha, *this, lastApplied, lastStateInvalid ) )
		{
			set_render_state( D3DRS_SEPARATEALPHABLENDENABLE, properAlpha ? TRUE : FALSE );
			set_render_state( D3DRS_DESTBLENDALPHA, properAlpha ? D3DBLEND_INVSRCALPHA : D3DBLEND_ZERO );
		}
		...
		// The game shouldn't need to control this.
		////
		...
		b32 depthModeGreater = fQuery( cDepthModeGreater );
		if( gInvertedViewportDepth ) depthModeGreater = !depthModeGreater;
		...
		if( !fCompareStateToLastApplied( cDepthModeGreater, *this, lastApplied, lastStateInvalid ) )
			set_render_state( D3DRS_ZFUNC, depthModeGreater ? D3DCMP_GREATEREQUAL : D3DCMP_LESSEQUAL  );
		...
		const f32 oneBitOf24BitDepthBuffer = 1.f / 16777215.f;
		const f32 defaultDepthBiasIncrement = 100.f * oneBitOf24BitDepthBuffer;
