	...
	void tIndexBufferVRam::fRender( const tDevicePtr& device, const tRenderBatchData& data ) const
	{
		if( data.mPrimitiveCount > 0 )
			device->fGetDevice( )->DrawIndexedPrimitive( 
				fConvertPrimitiveType( (tIndexFormat::tPrimitiveType)data.mPrimitiveType ),
				data.mBaseVertexIndex,
				0,
				data.mVertexCount,
				data.mBaseIndexIndex,
				data.mPrimitiveCount );
	}

	void tIndexBufferVRam::fRenderInstanced( const tDevicePtr& device, const tRenderBatchData& data ) const
	{
		const u32 numInstances = data.mGeometryBuffer2->fNumInstances( );
		const u32 indexCountPerInstance = fIndexCountPerInstance( );
		sigassert( indexCountPerInstance <= mNumIndices );
		sigassert( mNumIndices % indexCountPerInstance == 0 );
		sigassert( numInstances <= mNumIndices / indexCountPerInstance );
		device->fGetDevice( )->DrawIndexedVertices(
			fConvertPrimitiveType( (tIndexFormat::tPrimitiveType)data.mPrimitiveType ),
			0,
			0,
			indexCountPerInstance * numInstances );
	}
