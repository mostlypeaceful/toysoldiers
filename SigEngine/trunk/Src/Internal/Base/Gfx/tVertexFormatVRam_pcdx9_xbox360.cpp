...
namespace Sig { namespace Gfx
{
	namespace
	{
		void fConvertVertexElement( D3DVERTEXELEMENT9& dxve, const tVertexElement& ve )
		{
			...
			static_assert( array_length( semanticMap ) == tVertexElement::cSemanticCount );
			...
			static_assert( array_length( formatMap ) == tVertexElement::cFormatCount );
		...
		static IDirect3DVertexDeclaration9* fGlobalRegister( tVertexFormatVRam* format, IDirect3DDevice9* d3ddev )
		{
			...
			HRESULT hr = d3ddev->CreateVertexDeclaration(
				dxVtxElems.fBegin( ),
				&dxvd );
			log_assert( dxvd, "[" << hr << "]");// - [" << Win32Util::fErrorCodeToString( hr ) << "]" );
