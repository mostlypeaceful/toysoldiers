...
namespace Sig { namespace Gfx
{
	...
	b32 tRenderState::gInvertedViewportDepth = false;
	b32 tRenderState::gConsiderCutoutXParent = false;
	b32 tRenderState::gWriteProperAlphaWhenWritingCutOut = false;
	...
	void tRenderState::fApply( const tDevicePtr& device, const tRenderContext& context ) const
	{
		...
		if( gWriteProperAlphaWhenWritingCutOut && copy.fQuery( cCutOut ) )
		{
			copy.fEnableDisable( cWriteProperAlpha, true );
			copy.fEnableDisable( cAlphaBlend, true );
			//when writing depth we want to ensure we only write alpha
			// values of 1
			if( copy.fQuery( cDepthWrite ) )
				copy.fSetCutOutThreshold( 254 );
		}
