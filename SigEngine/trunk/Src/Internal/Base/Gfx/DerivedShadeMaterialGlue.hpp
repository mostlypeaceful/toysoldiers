...
namespace Sig { namespace Gfx
{
	...
	class base_export tVsGlueShadowMapSplits : public tShadeMaterialGlue
	{
		declare_reflector( );
		implement_rtti_serializable_base_class( tVsGlueShadowMapSplits, 0xC2E2B264 );
	private:
		u32 mRegister;
	public:
		explicit tVsGlueShadowMapSplits( u32 reg = 0 ) : mRegister( reg ) { }
		tVsGlueShadowMapSplits( tNoOpTag ) { }
		virtual tShadeMaterialGlue* fClone( ) const { return NEW tVsGlueShadowMapSplits( mRegister ); }
		virtual void fApplyShared( const tMaterial& mtl, const tShadeMaterialGlueValues& glueVals, const tDevicePtr& device, const tRenderContext& context ) const;
	};
	class base_export tVsGlueInstancingData : public tShadeMaterialGlue
	{
		declare_reflector( );
		implement_rtti_serializable_base_class( tVsGlueInstancingData, 0x762052F0 );
	private:
		u32 mRegister;
	public:
		explicit tVsGlueInstancingData( u32 reg = 0 ) : mRegister( reg ) { }
		tVsGlueInstancingData( tNoOpTag ) { }
		virtual tShadeMaterialGlue* fClone( ) const { return NEW tVsGlueInstancingData( mRegister ); }
		virtual b32 fIsShared( ) const { return false; }
		virtual void fApplyInstance( const tMaterial& mtl, const tShadeMaterialGlueValues& glueVals, const tDevicePtr& device, const tRenderContext& context, const tDrawCall& drawCall ) const;
	};
	...
	private:
		...
		u32 mEpsRegister;
	public:
		explicit tPsGlueBackFacePlusShadowMapParams( u32 reg = 0, u32 epsReg = 0 ) : mRegister( reg ), mEpsRegister( epsReg ) { }
		tPsGlueBackFacePlusShadowMapParams( tNoOpTag ) { }
		virtual tShadeMaterialGlue* fClone( ) const { return NEW tPsGlueBackFacePlusShadowMapParams( mRegister, mEpsRegister ); }
		virtual void fApplyShared( const tMaterial& mtl, const tShadeMaterialGlueValues& glueVals, const tDevicePtr& device, const tRenderContext& context ) const;
	};
	...
	///
	/// \class tPsGlueMaterialAtlasSampler
	/// \brief Handles binding texture atlas and atlas information
	class base_export tPsGlueMaterialAtlasSampler : public tShadeMaterialGlue
	{
		declare_reflector( );
		implement_rtti_serializable_base_class( tPsGlueMaterialAtlasSampler, 0xDD14F1D0 );
	private:
		u16 mSamplerRegister;
		u16 mVectorRegister;
		u16 mSamplerIndex;
	public:
		explicit tPsGlueMaterialAtlasSampler( u32 sReg = 0, u32 vReg = 0, u32 sIdx = 0 ) 
			: mSamplerRegister( sReg )
			, mVectorRegister( vReg )
			, mSamplerIndex( sIdx ) { }
		tPsGlueMaterialAtlasSampler( tNoOpTag ) { }

		virtual b32 fIsUnique( ) const { return false; }
		virtual tShadeMaterialGlue* fClone( ) const { return NEW_TYPED(tPsGlueMaterialAtlasSampler)( mSamplerRegister, mVectorRegister, mSamplerIndex ); }

		virtual void fApplyShared( 
			const tMaterial& mtl, 
	...
	///
	/// \class tPsGlueTransitionDefaultId
	/// \brief Handles binding transition default
	class base_export tPsGlueTransitionDefaultId : public tShadeMaterialGlue
	{
		declare_reflector( );
		implement_rtti_serializable_base_class( tPsGlueTransitionDefaultId, 0x454037E3 );

	public:

		explicit tPsGlueTransitionDefaultId( u32 reg = 0 ) : mRegisterIndex( reg ) { }
		tPsGlueTransitionDefaultId( tNoOpTag ) { }

		virtual tShadeMaterialGlue * fClone( ) const { return NEW_TYPED( tPsGlueTransitionDefaultId )( mRegisterIndex ); }
	...
	///
	/// \class tPsGlueTransitionEdgeColors
	/// \brief Handles binding transition edge colors
	class base_export tPsGlueTransitionEdgeColors : public tShadeMaterialGlue
	{
		declare_reflector( );
		implement_rtti_serializable_base_class( tPsGlueTransitionEdgeColors, 0xEDF34893 );
	public:

		explicit tPsGlueTransitionEdgeColors( u32 reg = 0 ) : mRegisterIndex( reg ) { }
		tPsGlueTransitionEdgeColors( tNoOpTag ) { }

		virtual tShadeMaterialGlue * fClone( ) const { return NEW_TYPED( tPsGlueTransitionEdgeColors )( mRegisterIndex ); }
	...
	///
	/// \class tPsGlueTransitionObjects
	/// \brief Handles binding transition spheres
	class base_export tPsGlueTransitionObjects : public tShadeMaterialGlue
	{
		declare_reflector( );
		implement_rtti_serializable_base_class( tPsGlueTransitionObjects, 0x79D7857E );

	public:

		explicit tPsGlueTransitionObjects( u32 reg = 0 ) : mRegisterIndex( reg ) { }
		tPsGlueTransitionObjects( tNoOpTag ) { }

		virtual b32 fIsShared( ) const { return false; }
		virtual tShadeMaterialGlue * fClone( ) const { return NEW_TYPED( tPsGlueTransitionObjects )( mRegisterIndex ); }

		virtual void fApplyInstance( 
			const tMaterial& mtl, 
			const tShadeMaterialGlueValues& glueVals, 
			const tDevicePtr& device, 
	...
	//just for debugging:
	class base_export tPsGlueHalfLambert : public tShadeMaterialGlue
	{
		declare_reflector( );
		implement_rtti_serializable_base_class( tPsGlueHalfLambert, 0xE21F59A );
	private:
		u32 mRegister;
	public:
		explicit tPsGlueHalfLambert( u32 reg = 0 ) : mRegister( reg ) { }
		tPsGlueHalfLambert( tNoOpTag ) { }
		virtual tShadeMaterialGlue* fClone( ) const { return NEW tPsGlueHalfLambert( mRegister ); }
		virtual void fApplyShared( const tMaterial& mtl, const tShadeMaterialGlueValues& glueVals, const tDevicePtr& device, const tRenderContext& context ) const;
	};

	class base_export tPsGlueSphericalHarmonics : public tShadeMaterialGlue
	{
		declare_reflector( );
		implement_rtti_serializable_base_class( tPsGlueSphericalHarmonics, 0x2B1AEF00 );
	private:
		u32 mRegister;
	public:
		explicit tPsGlueSphericalHarmonics( u32 reg = 0 ) : mRegister( reg ) { }
		tPsGlueSphericalHarmonics( tNoOpTag ) { }
		virtual tShadeMaterialGlue* fClone( ) const { return NEW tPsGlueSphericalHarmonics( mRegister ); }

		virtual void fApplyShared( const tMaterial& mtl, const tShadeMaterialGlueValues& glueVals, const tDevicePtr& device, const tRenderContext& context ) const;
	};


	// Spherical harmonics is temporarily global.
	//	virtual b32 fIsShared( ) const { return false; }
