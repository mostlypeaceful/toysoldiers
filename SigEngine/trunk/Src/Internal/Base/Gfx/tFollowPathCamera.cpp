...
namespace Sig { namespace Math
{
	...
	{
		tPRSXform<t> temp;
		temp.mPosition = fCatmullRom( p0.mPosition, p1.mPosition, p2.mPosition, p3.mPosition, x );
		temp.mRotation = fCatmullRom( p0.mRotation, p1.mRotation, p2.mRotation, p3.mRotation, x );
		temp.mScale = fLerp( p1.mScale, p2.mScale, x );
		return temp;
	}
}}
...
namespace Sig { namespace Gfx
{
	...
	void tFollowPathCamera::tControlPointList::fSeal( )
	{
		...
		else
		{
			...
			{
				const f32 segLength = ( fIndex( i + 1 ).mXform.mPosition - fIndex( i ).mXform.mPosition ).fLength( );
			...
			{
				if( fIndex( i ).mXform.mRotation.fDot( fIndex( i - 1 ).mXform.mRotation ) < 0.f )
					fIndex( i ).mXform.mRotation = -fIndex( i ).mXform.mRotation;
			}
		}
	}
	...
	{
		tCameraControllerPtr preserveMe( this ); //TREAD LIGHTLY YE WHO ENTERS HERE, THIS CLASS CAN NUKE ITSELF THROUGH DELEGATE FUNCTION CALLS
		fOnTickInternal( dt );
		fSetCamera( );
	}
	...
	void tFollowPathCamera::fOnTickInternal( f32 dt )
	{
		if( !tGameAppBase::fInstance( ).fSceneGraph( )->fIsPaused( ) )
		{
			...
			for( u32 i = 0 ; i < mSkipConditions.fCount( ) ; ++i )
			{
				if( mSkipConditions[i]->fSkip( ) )
				{
					skipPressed = true;
					mSkipped = true;
				}
	...
	void tFollowPathCamera::fAddSkipCondition( const tFollowPathCameraSkipConditionPtr& skipCondition )
	{
		mSkipConditions.fPushBack( skipCondition );
	}
