...
namespace Sig { namespace Gfx
{
	u32 tVertexColor::fForGpu( tPlatformId pid ) const
	{
		...
		case cPlatformPcDx11:	return tVertexColor( mB, mG, mR, mA ).mAsU32;
		case cPlatformMetro:	return tVertexColor( mB, mG, mR, mA ).mAsU32;
		...
		case cPlatformiOS:		return tVertexColor( mA, mR, mG, mB ).mAsU32;
		case cPlatformAndroid:  return tVertexColor( mA, mR, mG, mB ).mAsU32;
	...
	u32 tVertexFormat::fVertexSize( u32 stream ) const
	{
		u32 size = 0;
		for( u32 i = 0; i < mVertexElements.fCount( ); ++i )
		{
			if( mVertexElements[ i ].mStreamIndex == stream )
				size += tVertexElement::fSizeFromFormat( mVertexElements[ i ].mFormat );
		}
		return size;
	}
	...
	void tVertexFormat::fReset( const tVertexElement* elems, u32 numElems )
	{
		...
		tFixedArray<u32,8> streamIndexToSize;
		streamIndexToSize.fFill( 0 );
		...
		for( u32 i = 0; i < mVertexElements.fCount( ); ++i )
		{
			...
			u32& streamSize = streamIndexToSize[ mVertexElements[ i ].mStreamIndex ];
			mVertexElements[ i ].mOffsetFromBase = streamSize;
			streamSize += mVertexElements[ i ].mSize;
		}
	}
