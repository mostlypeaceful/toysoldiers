...
namespace Sig { namespace Gfx
{
	...
	{
		//this is more accurate than above but still not perfect.
		IDirect3DBaseTexture9* d3dtex = (IDirect3DBaseTexture9*)mPlatformHandle;

		u32 baseSize = 0;
		u32 mipSize = 0;
		XGGetTextureLayout(d3dtex,NULL,&baseSize,NULL,NULL,0,NULL,&mipSize,NULL,NULL,0);		

		return baseSize + mipSize;
	}
	...
	tTextureFile::tLockedMip tTextureFile::fLockMip(
		...
		b32 isArray )
	{
		sigcheckfail( rawHandle, return tLockedMip( ) );
	...
	{
		const u32 packMipStart = XGGetMipTailBaseLevel( mWidth, mHeight, FALSE );

		if( imip < packMipStart )
		{
		...
		else
		{
			...
			sigassert( XGIsPackedTexture( d3dtexture ) );

			D3DLOCKED_TAIL tail;
			d3dtexture->LockTail( iface, &tail, 0 );

			return (Sig::byte*)tail.pBits;
		}
	}
	...
	void tTextureFile::fSetTextureNameForProfiler( const char* name )
	{
		...
		if( mPlatformHandle )
		{
			HRESULT hr = PIXSetTextureName( (D3DBaseTexture*)mPlatformHandle, name );
