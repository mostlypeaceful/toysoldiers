...
namespace Sig { namespace Gfx
{
		...
		b32 autoDepthStencil )
	{
		...
		switch( vsync )
		{
			...
			case VSYNC_60HZ: d3dpp.PresentationInterval = D3DPRESENT_INTERVAL_ONE; break;
			case VSYNC_30HZ: //fall-through
	...
	void tDevice::fCreateDevice( D3DPRESENT_PARAMETERS& d3dpp )
	{
		...
		// create the device
		Memory::tHeap::fSetVramContext( vram_alloc_stamp( cAllocStampContextSysVram ) );
	...
	{
		if( vsGprs != 0 || psGprs != 0 )
		{
			sigcheckfail( vsGprs + psGprs == 128, return ); // XDK requirement
			sigcheckfail( vsGprs >= 16, return ); // XDK requirement
			sigcheckfail( psGprs >= 16, return ); // XDK requirement
		}
		static_cast<tDevice*>( this )->fGetDevice( )->SetShaderGPRAllocation( 0, vsGprs, psGprs );
	}
