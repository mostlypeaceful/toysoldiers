...
namespace Sig { namespace Gfx
{
	...
	protected:
		...
		u32						mNumIndices;
		u32						mNumPrimitives;
		u32						mNumIndiciesPerInstance; //for instanced rendering
		u32						mAllocFlags;
	...
	public:
		...
		void fAllocate( const tDevicePtr& device, const tIndexFormat& format, u32 numIndices, u32 numPrimitives, u32 allocFlags, u32 numIndiciesPerInstance = 0 );

		///
		/// \brief "Fake" allocate so that we can save to binary load-in-place stream.
		void fPseudoAllocate( const tIndexFormat& format, u32 numIndices, u32 numPrimitives, u32 allocFlags, u32 numIndiciesPerInstance = 0 );
		...
		void fAllocateInPlace( const tDevicePtr& device, const Sig::byte* indices );
		...
		inline u32 fIndexCountPerInstance( ) const { return mNumIndiciesPerInstance; }
		...
		void fRender( const tDevicePtr& device, const tRenderBatchData& data ) const;
		void fRenderInstanced( const tDevicePtr& device, const tRenderBatchData& data ) const;
