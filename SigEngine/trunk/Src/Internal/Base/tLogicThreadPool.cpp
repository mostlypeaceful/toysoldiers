...
namespace Sig { namespace Threads
{
	...
	public:
		explicit tLogicThread( u32 hwThread )
			: tWorkerThread( "LogicThread", hwThread )
...
namespace Sig
{
	devvar_clamp( u32, Debug_Threads_TimingLogLevel, 0, 0, 2, 0 );
	...
	void tLogicThreadPool::fBeginWork( tLogic::tRunListId runListId, f32 dt, const tLogic::tRunList& runList, b32 secondaryThreadsOnly, b32 waitToComplete )
	{
		...
		if( runList.fCount( ) > 0 )
		{
			...
			if_logging( mRunListId = runListId );
			...
			for( s32 i = totalThreadCount - 1, job = 0; i >= 0; --i ) //start work of threads backwards so the thread0 doesn't preempt us from starting the other threads
			{
				if( threadFlags & ( 1 << i ) )
				{
	...
	{
		profile_pix("tLogicThreadPool::fWaitForWorkToComplete");
		if( !mCurrentRunList )
			return;
...
#ifdef sig_logging
		mLastJobTimeMs = 0.f;
		if( mLastJobThreads.fCount( ) )
		{
			f32 minThreadTime = Math::cInfinity;
			f32 maxThreadTime = -1.f;

			for( u32 i = 0; i < mLastJobThreads.fCount( ); ++i )
			{
				const f32 threadTime = mLastJobThreads[ i ]->fLastJobTimeMs( );
				minThreadTime = fMin( minThreadTime, threadTime );
				maxThreadTime = fMax( maxThreadTime, threadTime );

				mLastJobTimeMs += threadTime;

				if( Debug_Threads_TimingLogLevel == 2 )
					log_line( 0, tLogic::fRunListString( mRunListId ) << ": Thread " << i << " spent " << threadTime << "ms" );
			}

			if( Debug_Threads_TimingLogLevel )
			{
				const f32 average = mLastJobTimeMs / mLastJobThreads.fCount( );
				const f32 variance = maxThreadTime - minThreadTime;

				log_line( 0, tLogic::fRunListString( mRunListId ) << ": "<<  std::setprecision( 2 ) << 
					"Accum: " << mLastJobTimeMs <<  "ms, " <<
					"Avg: " << average << "ms, " <<
					"Max: " << maxThreadTime << "ms, " << 
					"Min: " << minThreadTime << "ms, " << 
					"Variance: " << variance << "ms, " <<
					"Imbalance: " << maxThreadTime - average << "ms" );
			}
	...
	void tLogicThreadPool::fDisallowHwThread( u32 threadId )
	{
		...
		{
			log_warning( "Cannot disallow primary threads, use \"secondaryThreadsOnly\" argument instead" );
			return;
		}
