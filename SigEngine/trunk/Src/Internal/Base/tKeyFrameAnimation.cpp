...
namespace Sig
{
		...
		const tBone& bone ) const
	{
		...
		if( bone.mPositionFrameNums.fCount( ) == 1 )
			xformOut.mPosition = fFromPositionKey( bone.mPositionKeys.fFront( ), bone.mPMin, bone.mPMax );
		else
		{
			...
			xformOut.mPosition = Math::fLerp( a, b, bracket.mZeroToOne );
		}

		if( bone.mRotationFrameNums.fCount( ) == 1 )
			xformOut.mRotation = fFromRotationKey( bone.mRotationKeys.fFront( ) );
		else
		{
			...
			xformOut.mRotation = Math::fSlerp( a, b, bracket.mZeroToOne );
			sigassert( !xformOut.mRotation.fIsNan( ) );
		}

		if( bone.mScaleFrameNums.fCount( ) == 1 )
			xformOut.mScale = fFromScaleKey( bone.mScaleKeys.fFront( ), bone.mSMin, bone.mSMax );
		else
		{
			...
			xformOut.mScale = Math::fLerp( a, b, bracket.mZeroToOne );
		}
	}
	...
	u32 tKeyFrameAnimation::fComputeStorage( ) const
	{
		u32 numBytes = sizeof( *this );
		numBytes += mEvents.fTotalSizeOf( );
		numBytes += mBones.fTotalSizeOf( );

		const u32 boneCount = mBones.fCount( );
		for( u32 b = 0; b < boneCount; ++b )
		{
			const tBone& bone = mBones[ b ];

			numBytes += bone.mPositionFrameNums.fTotalSizeOf( );
			numBytes += bone.mPositionKeys.fTotalSizeOf( );
			numBytes += bone.mRotationFrameNums.fTotalSizeOf( );
			numBytes += bone.mRotationKeys.fTotalSizeOf( );
			numBytes += bone.mScaleFrameNums.fTotalSizeOf( );
			numBytes += bone.mScaleKeys.fTotalSizeOf( );
		}
