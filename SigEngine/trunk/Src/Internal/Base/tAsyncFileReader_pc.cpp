...
namespace Sig
{
	namespace
	{
		...
		{
			if( !GetDiskFreeSpace( 0, 0, ( DWORD* )&gSectorSize, 0, 0 ) || gSectorSize == 0 )
			{
				log_warning( "Failed to query the sector size." );
				gSectorSize = 4096; // Guess instead of crashing
			}
	...
	b32 tAsyncFileReader::fCreateFileForPlatform( )
	{
		...
		{
			const u32 errCode = GetLastError( );
			log_warning( "Failed to open [" << mFileName << "]. Err(" << errCode << "): " << Win32Util::fErrorCodeToString( errCode ) );
		}
