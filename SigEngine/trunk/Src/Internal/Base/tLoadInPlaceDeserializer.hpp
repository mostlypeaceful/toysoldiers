...
namespace Sig
{
	...
	b32 fLoadInPlaceLoadCallback( tResource& resource, tGenericBuffer* buffer )
	{
		...
		if( !lipfile->fVerifyResource( resource, &tLipType::fConvertToSource, tLipType::cLipMinLoadVersion, tLipType::cLipMaxLoadVersion, Rtti::fGetClassId<tLipType>( ) ) )
		{
			buffer->fFree( );
			return false;
		}
		lipfile->fInitializeLoadInPlaceTables( resource );
		if( !lipfile->fLoadSubResources( resource ) )
		{
			lipfile->fUnloadSubResources( );
			lipfile->fCleanUpLoadInPlaceTables( );
			buffer->fFree( );
			return false;
		}
		if_logging( const f32 fileSize = buffer->fGetBufferSize( ) / ( 1024.f * 1024.f ); )
		lipfile->fOnFileLoaded( resource );
		lipfile->fResizeAfterLoad( buffer );

#if sig_resource_logging
		log_line( Log::cFlagResource, "complete (success) - took " << timer.fGetElapsedMs( ) << " ms. Size: " << std::fixed << std::setprecision( 2 ) << fileSize << "MB." );
#endif//sig_resource_logging
	...
	b32 fLoadInPlaceUnloadCallback( tResource& resource, tGenericBuffer* buffer )
	{
		...
		tLoadInPlaceFileBase* lipfile = ( ( tLoadInPlaceFileBase* )buffer->fGetBuffer( ) );
		lipfile->fOnFileUnloading( resource );
		lipfile->fUnloadSubResources( );
		lipfile->fCleanUpLoadInPlaceTables( );
...
#	define implement_lip_load_unload_callbacks_direct( tLipType ) \
		namespace \
		{ \
			...
			{ \
				...
				props.mFlags			   |= tResource::tTypeSpecificProperties::cIsDirectResource; \
				props.mOffsetToHeaderSize	= offsetof( tLipType, mHeaderSize ); \
