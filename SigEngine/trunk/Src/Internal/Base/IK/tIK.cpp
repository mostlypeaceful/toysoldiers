...
namespace Sig { namespace IK {
	...
	void tIKChain::fClampLocal( tIKJoint& joint )
	{
		...
		{
			tEulerAnglesf angles = fCMLDecompose( joint.mAxesLimitsOrder, joint.mLocalDelta );

			for( u32 a = 0; a < 3; ++ a)
				angles[ a ] = fClamp( angles[ a ], joint.mAxesLimitsLow[ a ], joint.mAxesLimitsHigh[ a ] );

			joint.mLocalDelta = fCMLCompose( joint.mAxesLimitsOrder, angles );
		}
	}
	...
	void tIKChain::fSetupTestData( )
	{
		...
		for( u32 i = 0; i < bCnt; ++i )
		{
			...
			if( i != bCnt -1 )
			{
				...
				if( i == 0 )
				{
					joint.mAxesLimitsOrder = cEulerOrderXZY;
	...
	void tCharacterLegTargets::fCallbackMT( tIKCallbackArgs& args )
	{
		...
		if( callback.mHit.fHit( ) )
		{
			...
			else if( lockMode != cLockModeFree )
			{
				...
				tVec3f pt = rayFromFoot.fPointAtTime( callback.mHit.mT ) + callback.mHit.mN * footHeight;
		...
		else if ( mIdle && lockMode != cLockModeFree )
		{
			...
			if( callback.mHit.fHit( ) || lockedS )
			{
				...
				if( !lockedS )
				{
					...
					tVec3f pt = rayFromHip.fPointAtTime( callback.mHit.mT ) + callback.mHit.mN * footHeight;
