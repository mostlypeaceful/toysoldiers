	...
	public:
			...
			, mExternalFreeFunc( freeFunc )
		{
			...
			for( u32 i = 0; i < mBehavior.mInitialPageCount; ++i )
				fAddSubHeap( tAllocStamp::fNoContextStamp( ) );
		}
		
		u32 fLargestFreeChunkSize( ) const
		{
			u32 best = 0;
			for( u32 i = 0; i < mSubHeaps.fCount( ); ++i )
			{
				const u32 bestSub = mSubHeaps[i].fLargestFreeChunkSize( );
				if( bestSub > best )
					best = bestSub;
			}
			return best;
		...
		const tAllocStamp* fGetStamp( void* memory ) const
		{
			tMutexType lock( mLock );
			...
			{
				if( mSubHeaps[ i ].fContains( memory ) )
					return mSubHeaps[ i ].fGetStamp( memory );
			}
		...
		virtual void fDump( tMemoryDumpOptions& memoryDumpOptions )
		{
			if( memoryDumpOptions.mPost )
			{
				...
				Log::fPrintf( Log::cFlagMemory,  "tChunkyHeap Name: %s SubHeaps: %d, Largest Chunk: %.2fMB\n", mName, mSubHeapCount, Memory::fToMB<f32>(fLargestFreeChunkSize()) );
			}
			...
			{
				memoryDumpOptions.mDump->mHeaps.fPushBack( tHeapDump( mName, tHeapDump::cTypeChunky, fNumBytesBeingManaged( ), tMemoryDump::fComputePercentage(fNumBytesAllocd( ), fNumBytesBeingManaged( ) ), *memoryDumpOptions.mDump ) );
			...
			for( u32 i = 0; i < mSubHeapCount; ++i )
			{
				...
				if( heapDump )
				{
					heapDump->mPages.fPushBack( tPageDump( subHeap.fNumBytesBeingManaged( ), tMemoryDump::fComputePercentage( subHeap.fNumBytesAllocd( ), subHeap.fNumBytesBeingManaged( )), 0.f, 0 ) );
		...
		virtual void fLogFreeMemory( ) const
		{
			...
			Log::fPrintf( Log::cFlagMemory,  "-----------------------------------------------------\n");
			Log::fPrintf( Log::cFlagMemory,  " Name: %s, SubHeaps: %d, Size: %.2f, Free: %.2f, Largest Chunk: %.2fMB\n", mName, mSubHeapCount, fNumBytesBeingManaged( )/cOneMb, fNumBytesFree( )/cOneMb, Memory::fToMB<f32>(fLargestFreeChunkSize()) );
		}
