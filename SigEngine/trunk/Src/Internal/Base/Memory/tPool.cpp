...
{
	devvar( bool, Debug_Memory_CheckForDoubleFrees, false );
	...
	namespace
	{
		...
		{
			/*
				during debugging we occasionally mark large blocks of memory as "garbage"
				this will making pointers inside the stamp invalid and they need to be newed in place.
			*/
			void fFixGarbagedMemoryStamp( tAllocStampPtr& stamp )
			{
				new ( &stamp ) tAllocStampPtr( ); //new in place since this memory has been "garbaged"
			}
		}
	...
	u32 tPool::fNumPagesAllocd( ) const
	{
		u32 numPages = 0;
		tPage* i = mPageList;
		while( i )
	...
	void tPool::fClearPageMemory( )
	{
		...
		if( mNumObjectsAllocd != 0 )
			log_warning( "tPool::fClearPageMemory() [" << mName << "]: objects still allocated (leaking memory)" );
#endif//target_tools
		...
		while( next )
		{
			...
			{
				tObjectHeader* obj = ( tObjectHeader* )((unsigned char*)kill->mMemory + mObjectStride * ithObj);
				obj->mStamp.fRelease( );
			}
#endif
	...
	void* tPool::fAlloc( const tAllocStamp& stamp )
	{
		...
		if( !o )
		{
			Memory::tMemoryDumpOptions discarded;
			Memory::tPool::fDumpGlobalPoolAllocations( discarded );
		...
		fMarkMemAsAllocated( (((Sig::byte*)o) + mObjectHeaderSize), mObjectSize );

#ifdef sig_memory_dump
		o->mStamp.fReset( stamp );
#endif

		return ( void* )(((Sig::byte*)o) + mObjectHeaderSize);
	}
	...
	void tPool::fFree( void* object )
	{
...
#ifdef sig_assert
		if( Debug_Memory_CheckForDoubleFrees )
		{
			if( fFoundInFreeList( i ) )
			{
				sigassert( !"Double-free! We are in dire straits." );
				//Quit before we add ourselves to the free list or do any mem marking.
				// If we don't quit early then memory corruption may occur if this
				// pointer has already been allocated and we mem mark it, or if the
				// pointer is simply returned twice and over time the two objects
				// step on each other.
				return;
			}
		}
		...
		fMarkMemAsFreed( object, mObjectSize );
	...
	void tPool::fNewPage( )
	{
		...
		if( numPageBytes == 0 )
		{
			log_assert( numPageBytes > 0,  "obviously our pages should have a size, something really wrong has happened. ptr: " << this );
			return;
		}
		...
		fMarkMemAsGarbage( newPage->mMemory, numPageBytes );
		...
		for( u32 ithObj = 0; ithObj < mNumObjectsPerPage - 1; ++ithObj )
		{
			obj->mNext = next;
#ifdef sig_memory_dump
			fFixGarbagedMemoryStamp( obj->mStamp );
#endif
			obj = next;
...
#else
		log_warning( "App not compiled with sig_memory_dump!" );
#endif
	}
	...
	void tPool::fDumpNamedAllocations( tMemoryDumpOptions& memoryDumpOptions )
	{
#ifdef sig_memory_dump
		{
			...
			{
				memoryDumpOptions.mDump->mHeaps.fPushBack( tHeapDump( mName, Memory::tHeapDump::cTypePool, 0, 0, *memoryDumpOptions.mDump ) );
			...
			for( tPage* page = mPageList; page; page = page->mNext )
			{
				...
				for( u32 ithObj = 0; ithObj < mNumObjectsPerPage; ++ithObj )
				{
					if( !fFoundInFreeList( obj ) )
					{
						...
						if( pageDump && obj->mStamp.fGetRawPtr( ) )
							pageDump->mAllocations.fPushBack( tAllocDump( tStampDump( *obj->mStamp, (u32)obj, *memoryDumpOptions.mDump ) ) );
					}
