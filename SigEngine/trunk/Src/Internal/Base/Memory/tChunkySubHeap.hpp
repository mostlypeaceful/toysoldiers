...
{
	struct tChunkyHeapAllocHeader : public tUncopyable
	{
		enum tState
		{
			cStateGarbage,
	...
	public:
		...
		const tAllocStamp* fGetStamp( void* address ) const;

		// debug/info methods
		b32 fValidate( ) const;
		void fDump( ) const;
		void fDumpNamedAllocations( tMemoryDumpOptions& memoryDumpOptions, tPageDump* page ) const;
		void fDumpSubset( const char* nameToLookFor ) const;
	...
	protected:
		...
		tChunkyHeapAllocHeader*	fFindBestFit( u32 bytes ) const;
