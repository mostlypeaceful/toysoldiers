...
namespace Sig
{
	...
	void t3dGridEntityDef::fCollectEntities( const tCollectEntitiesParams& params ) const
	{
	...
	void t3dGridEntity::fPropagateSkeleton( Anim::tAnimatedSkeleton& skeleton )
	{
	...
	void t3dGridEntity::fRayCast( const Math::tRayf& ray, Math::tRayCastHit& hit ) const
	{
		...
		{
			hit.mT = i.fT( );
			hit.mN = ( ray.fPointAtTime( hit.mT ) - mBox.fCenter( ) ).fNormalizeSafe( Math::tVec3f::cYAxis ); // sphere-ized normal, not accurate for a box
		}
	}
