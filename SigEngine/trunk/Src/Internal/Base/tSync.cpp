...
namespace Sig
{
	...
	namespace
	{
		...
		{
			static_assert( sizeof( Math::tVec2u ) == 2 * sizeof( u32 ) );
		...
		{
			static_assert( sizeof( Math::tVec3u ) == 3 * sizeof( u32 ) );
		...
		{
			static_assert( sizeof( Math::tVec4u ) == 4 * sizeof( u32 ) );
		...
		{
			static_assert( sizeof( Math::tVec2f ) == 2 * sizeof( f32 ) );
		...
		{
			static_assert( sizeof( Math::tVec3f ) == 3 * sizeof( f32 ) );
		...
		{
			static_assert( sizeof( Math::tVec4f ) == 4 * sizeof( f32 ) );
		...
		{
			static_assert( sizeof( Math::tMat3f ) == 12 * sizeof( f32 ) );
		...
		{
			static_assert( sizeof( Math::tMat4f ) == 16 * sizeof( f32 ) );
		...
		{
			static_assert( sizeof( Math::tAabbf ) == 6 * sizeof( f32 ) );
		...
		template< class t >
		void fWrite( tGrowableArray< byte > & buffer, const t & value )
		{
			//sigassert( tIsBuiltInType<t>::cIs );
		...
		template<>
		void fWrite( tGrowableArray< byte > & buffer, const tSync::tSyncStrPtr & str )
		{
			fWrite( buffer, str.fCStr( ), str.fLength( ) );
		}

		template< class t >
		const byte * fRead( tPlatformId source, const byte * data, u32 dataLen, t & out )
		{
			//sigassert( tIsBuiltInType<t>::cIs );
		...
		template<>
		const byte * fRead( tPlatformId source, const byte * data, u32 dataLen, tSync::tSyncStrPtr & str )
		{
		...
		template< class t >
		const byte * fRead( tPlatformId source, const byte * data, u32 dataLen, tGrowableArray<t> & out )
		{
			const byte * term = data + dataLen;
	...
	const byte * tSync::tFrameHeader::fReadStacks( tPlatformId source, const byte * data, u32 dataLen )
	{
		...
		{
			log_warning( "Sync: Couldn't read callstacks because count did not match event count" );
			return data;
		}
	...
	//------------------------------------------------------------------------------
	void tSync::tSyncReader::fAdvance( )
	{
		...
		if( mHeaderIdx >= mFrameHeaders.fCount( ) )
		{
			...
			if( mReader )
			{
				...
				{
					log_warning( __FUNCTION__ << " Error: File is corrupt" );
					fClose( );
					return;
				}
		...
		const tFilePathPtr & desyncFolder )
	{
		...
		while( aR.fIsOpen( ) && bR.fIsOpen( ) )
		{
			...
			if( !aR.fEventsAreEqual( bR ) )
			{
				...
				std::string info;
				while( matching.fTryPopFirst( info ) )
					ss << "\n\t" << info;
			...
			{
				matching.fPushLast( aR.fEventInfo( ) );
			}
		}
	...
	b32 tSync::fFilesEqual( 
		...
		const tFilePathPtr & outFilePath,
		u32 prependCount,
		u32 appendCount )
	{
		...
		{
			log_warning( __FUNCTION__ << ": File 1 ( " << file1Path << " ) couldn't be opened" );
			return false;
		}

		if( !r2.fOpen( file2Path ) )
		{
			log_warning( __FUNCTION__ << ": File 2 ( " << file2Path << " ) couldn't be opened" );
			return false;
		}
		...
		while( r1.fIsOpen( ) && r2.fIsOpen( ) )
		{
			...
			matching.fPushLast( r1.fEventInfo( ) );
		}
	...
	void tSync::fRegisterThread( b32 mainThread )
	{
		...
		{
			log_warning( "Thread was already registered with sync" );
			return;
		}
	...
	void tSync::fDeregisterThread( )
	{
		...
		if( tFrameHeader * ptr = ( tFrameHeader * )Threads::LocalStorage::GetValue( mFrameHeaderTLS ) )
		{
			...
			log_warning( "Thread was not registered with sync" );
	...
	void tSync::fStartSync( const tFilePathPtr & logPath, const tFilePathPtr & compareTo )
	{
		...
		if( !logPath.fNull( ) )
		{
			...
			{
				log_warning( "Sync Error: Couldn't open log file: " << logPath );
		...
		if( !compareTo.fNull( ) )
		{
			...
			{
				log_warning( "Sync Error: Couldn't open compare file: " << compareTo );
		...
		const tFilePathPtr & desyncFolder, const tSyncReader & reader, u64 stamp, u32 idx )
	{
		...
		{
			log_warning( "Failed to open desync file path" );
			return;
		}
