...
namespace Sig { namespace FX
{
	...
	void tQuadParticleList::fFreeVram( Gfx::tRenderableEntity& parent )
	{
		mValidGeometry = false;
		mGpuVerts = NULL;
		mGpuIndices = NULL;
		mGeometry.fFreeSlice( );
		parent.fSetRenderBatch( Gfx::tRenderBatchPtr( ) );
	}
	...
	void tQuadParticleList::fEmitParticles( tParticleListUpdateParams& updateParams, u32 emitCount )
	{
		...
		for( u32 iemit = 0; iemit < emitCount; ++iemit )
		{
			...
			p->mAlphaCutout = state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cCutoutGraph, 0.f );
			p->mEnergy0 = state.fSampleEmissionGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cSpawnEnergyGraph, systemDelta );
			p->mScale0 = state.fSampleEmissionGraph< FX::tBinaryV3Graph >( updateParams.mSystemRand, cSpawnSizeGraph, systemDelta ).fXY( ) * xformScale;			
			...
			else
			{
				...
				//log_warning( "Particles could not push back!? tried to push " << emitCount << " with capacity " << mParticles.fCapacity( ) );
				break;
			}
		}
	}
	...
	{
		profile_pix( "tQuadParticleList::fUpdateParticles" );
#if defined( target_game ) && defined( sig_logging )
		if( mParticles.fCount( ) > 10000 )
		{
			log_warning( "over 10k particles from one particle system. game is probably runnin pretty slow right now... " << mPath );
		}
		else if( mGhostParticles.fCount( ) > 10000 )
		{
			log_warning( "over 10k ghost particles from one particle system. game is probably runnin pretty slow right now... " << mPath );
		}
#endif//defined( target_game ) && defined( sig_logging )
	...
	void tQuadParticleList::fUpdateParticleList( tGrowableArray< tParticle* >& particleList, tGrowableArray< u32 >& removalList, tParticleListUpdateParams& updateParams )
	{
		...
		for( u32 i = 0; i < particleUpdateCount; i += 4 )
		{
			...
			p1->mAlphaCutout = state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cCutoutGraph, pld1 );
			p2->mAlphaCutout = state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cCutoutGraph, pld2 );
			p3->mAlphaCutout = state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cCutoutGraph, pld3 );
			p4->mAlphaCutout = state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cCutoutGraph, pld4 );
		...
		for( u32 i = particleUpdateCount; i < particleList.fCount( ); ++i )
		{
			...
			p1->mCurrentColor = state.fSamplePerParticleGraph< FX::tBinaryV4Graph >( updateParams.mSystemRand, cColorGraph, particleLifeDelta );

			p1->mAlphaCutout = state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cCutoutGraph, particleLifeDelta );

			if( cUpdatePos )
			{
				p1->mPosition += p1->mDirection * state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cEnergyGraph, particleLifeDelta ) * dt;
	...
	void tQuadParticleList::fWriteToVB( tParticleListUpdateParams& updateParams, const tParticle* p1, const tParticle* p2, const tParticle* p3, const tParticle* p4, f32 d1, f32 d2, f32 d3, f32 d4 )
	{
		...
		const f32 yaw[ ] = { p1->mYaw0 * state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cSpinGraph, d1 ),
							 p2->mYaw0 * state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cSpinGraph, d2 ),
							 p3->mYaw0 * state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cSpinGraph, d3 ),
							 p4->mYaw0 * state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cSpinGraph, d4 ) };

		const Math::tVec2f size[ ] = {
			p1->mScale0 * state.fSamplePerParticleGraph< FX::tBinaryV3Graph >( updateParams.mSystemRand, cScaleGraph, d1 ).fXY( ),
			p2->mScale0 * state.fSamplePerParticleGraph< FX::tBinaryV3Graph >( updateParams.mSystemRand, cScaleGraph, d2 ).fXY( ),
			p3->mScale0 * state.fSamplePerParticleGraph< FX::tBinaryV3Graph >( updateParams.mSystemRand, cScaleGraph, d3 ).fXY( ),
			p4->mScale0 * state.fSamplePerParticleGraph< FX::tBinaryV3Graph >( updateParams.mSystemRand, cScaleGraph, d4 ).fXY( ),
		};
		...
		Math::tAabbf& updateBox = (*updateParams.mBoxToUpdate);
		for( u32 i = 0, s = 0; i < array_length( pos ); i += 2, s += 1 )
		{
			Math::tVec3f sizeVec( size[ s ].fMax( ) );
			Math::tVec3f xformedPt = boxXform.fXformPoint( pos[ i ] );
			updateBox.mMax = fMax( updateBox.mMax, xformedPt + sizeVec );
			updateBox.mMin = fMin( updateBox.mMin, xformedPt - sizeVec );
		}
		...
		if( mValidGeometry )
		{
			const Gfx::tParticleRenderVertex verts[ ] = { 
				Gfx::tParticleRenderVertex( pos[ 0 ], vtxColor[ 0 ], -size[ 0 ].x,  size[ 0 ].y, yaw[ 0 ], p1->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 0 ], vtxColor[ 0 ],  size[ 0 ].x,  size[ 0 ].y, yaw[ 0 ], p1->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 1 ], vtxColor[ 0 ],  size[ 0 ].x, -size[ 0 ].y, yaw[ 0 ], p1->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 1 ], vtxColor[ 0 ], -size[ 0 ].x, -size[ 0 ].y, yaw[ 0 ], p1->mAlphaCutout ),
																						
				Gfx::tParticleRenderVertex( pos[ 2 ], vtxColor[ 1 ], -size[ 1 ].x,  size[ 1 ].y, yaw[ 1 ], p2->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 2 ], vtxColor[ 1 ],  size[ 1 ].x,  size[ 1 ].y, yaw[ 1 ], p2->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 3 ], vtxColor[ 1 ],  size[ 1 ].x, -size[ 1 ].y, yaw[ 1 ], p2->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 3 ], vtxColor[ 1 ], -size[ 1 ].x, -size[ 1 ].y, yaw[ 1 ], p2->mAlphaCutout ),
																						
				Gfx::tParticleRenderVertex( pos[ 4 ], vtxColor[ 2 ], -size[ 2 ].x,  size[ 2 ].y, yaw[ 2 ], p3->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 4 ], vtxColor[ 2 ],  size[ 2 ].x,  size[ 2 ].y, yaw[ 2 ], p3->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 5 ], vtxColor[ 2 ],  size[ 2 ].x, -size[ 2 ].y, yaw[ 2 ], p3->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 5 ], vtxColor[ 2 ], -size[ 2 ].x, -size[ 2 ].y, yaw[ 2 ], p3->mAlphaCutout ),
																						
				Gfx::tParticleRenderVertex( pos[ 6 ], vtxColor[ 3 ], -size[ 3 ].x,  size[ 3 ].y, yaw[ 3 ], p4->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 6 ], vtxColor[ 3 ],  size[ 3 ].x,  size[ 3 ].y, yaw[ 3 ], p4->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 7 ], vtxColor[ 3 ],  size[ 3 ].x, -size[ 3 ].y, yaw[ 3 ], p4->mAlphaCutout ),
				Gfx::tParticleRenderVertex( pos[ 7 ], vtxColor[ 3 ], -size[ 3 ].x, -size[ 3 ].y, yaw[ 3 ], p4->mAlphaCutout )
			};
			...
			fMemCpyToGpu( &mGpuVerts[ sizeof( verts[0] ) * mBaseVertexIndex ], verts, sizeof( verts ) );
			fMemCpyToGpu( &mGpuIndices[ sizeof( ids[0] ) * mCurrentIndiceIndex ], ids, sizeof( ids ) );
			mBaseVertexIndex += array_length( verts );
			mCurrentIndiceIndex += array_length( ids );
		}
	}
	void tQuadParticleList::fWriteToVB( tParticleListUpdateParams& updateParams, const tParticle* particle, f32 delta )
	{
		...
		const f32 yaw = particle->mYaw0 * state.fSamplePerParticleGraph< FX::tBinaryF32Graph >( updateParams.mSystemRand, cSpinGraph, delta );
		const Math::tVec2f size = particle->mScale0 * state.fSamplePerParticleGraph< FX::tBinaryV3Graph >( updateParams.mSystemRand, cScaleGraph, delta ).fXY( );		
		const Math::tVec3f& pos = particle->mPosition;
		const f32 cutout = particle->mAlphaCutout;
		...
		const Math::tMat3f& boxXform = updateParams.mLocalSpace ? Math::tMat3f::cIdentity : updateParams.mObjToWorldInv;
		Math::tVec3f sizeVec( size.fMax( ) );
		Math::tAabbf& updateBox = (*updateParams.mBoxToUpdate);

		for( u32 i = 0; i < 2; ++i )
		{
			Math::tVec3f xformedPt = boxXform.fXformPoint( positions[ i ] );
			updateBox.mMax = fMax( updateBox.mMax, xformedPt + sizeVec );
			updateBox.mMin = fMin( updateBox.mMin, xformedPt - sizeVec );
		}
		...
		{
			const Gfx::tParticleRenderVertex verts[ ] = { 
				Gfx::tParticleRenderVertex( positions[ 0 ], vtxColor, -size.x,  size.y, yaw, cutout ),
				Gfx::tParticleRenderVertex( positions[ 0 ], vtxColor,  size.x,  size.y, yaw, cutout ),
				Gfx::tParticleRenderVertex( positions[ 1 ], vtxColor,  size.x, -size.y, yaw, cutout ),
				Gfx::tParticleRenderVertex( positions[ 1 ], vtxColor, -size.x, -size.y, yaw, cutout )
			};
	...
	void tQuadParticleList::fValidateGeometry( )
	{
#if defined( sig_assert ) && defined( sig_logging )
		if( mValidGeometry )
		{
			/*
				If these warnings fire, it basically means that all of the particles that were allocated were not initialized, or fMemCpyToGpu'd.
				 Or, worse, that too many were. :/
			*/
			Gfx::tRenderBatchData batchData = mGeometry.fGetRenderBatch( )->fBatchData( );
			if( mBaseVertexIndex != batchData.mVertexCount )
				log_warning( "Particle Sanity check failed: mBaseVertexIndex ( " << mBaseVertexIndex << " ) == batchData.mVertexCount ( " << batchData.mVertexCount << " ) Path: " << mPath );
			if( mCurrentIndiceIndex != batchData.mPrimitiveCount * 3 )
				log_warning( "Particle Sanity check failed: mCurrentIndiceIndex ( " << mCurrentIndiceIndex << " ) == batchData.mPrimitiveCount * 3 ( " << batchData.mPrimitiveCount * 3 << " ) Path: " << mPath );
		}
#endif
	}
