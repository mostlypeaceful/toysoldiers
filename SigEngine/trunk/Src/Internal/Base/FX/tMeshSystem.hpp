...
namespace FX
{
	...
	public:
		...
		{
			if( mToolData )		return mToolData->template fSampleGraph< t >( random, idx, x );
			if( mBinaryData )	return mBinaryData->template fSampleGraph< t >( random, idx, x );
			return t( 0.f );
		}
		...
		{
			if( mToolData )	mToolData->mAttractorIgnoreIds.fPushBack( id );
			// Can't modify binary state
		}
