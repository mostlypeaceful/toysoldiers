...
//    Nov 9, 2011:
//
// GCC (on iOS) can't infer template arguments for e.g.:
//    .Prop( ..., &tGameApp::fDeviceFromScript )
// because this function is actually inherited from tGameAppBase.
// I've modified .Prop to handle this.  Blame mrickert.
...
#include "sqratGlobalMemberMethods.h"
	...
	*/
	...
	public:
		...
		*/
		...
		//    .Prop( ..., &tGameApp::fDeviceFromScript )
		// because this function is actually inherited from tGameAppBase.
		// I've added 3 .Prop overloads to handle this.  Blame mrickert.
		template<class V, class Base>
		Class& Prop(const SQChar* name, V (Base::*getMethod)(), void (Base::*setMethod)(V)) {
			V (C::*castedGetMethod)(V) = getMethod; // intentionally an implicit cast
			void (C::*castedSetMethod)(V) = setMethod; // intentionally an implicit cast
			Prop<V>(name,castedGetMethod,castedSetMethod);
			return *this;
		}
		template<class V, class Base>
		Class& Prop(const SQChar* name, V (Base::*getMethod)() const) {
			V (C::*castedGetMethod)() const = getMethod; // intentionally an implicit cast
			Prop<V>(name,castedGetMethod);
			return *this;
		}
		template<class V, class Base>
		Class& Prop(const SQChar* name, V (Base::*getMethod)()) {
			V (C::*castedGetMethod)(V) = getMethod; // intentionally an implicit cast
			Prop<V>(name,castedGetMethod);
			return *this;
		}
		// End of additional .Prop overloads... at the time of writing anyways.
	...
	public:
		DerivedClass(HSQUIRRELVM v = DefaultVM::Get()) : Class<C, A>(v, false) {
			if(!ClassType<C>::Initialized()) {
				...
				sq_addref(Class<C, A>::vm, &classObj); // must addref before the pop!
				sq_pop(Class<C, A>::vm, 1);
