...
namespace Sig
{
	...
	const tStringPtr tUberBreakableLogic::cOnDestroyFX( "destroyFX" ); 
	

	tUberBreakableLogic::tUberBreakableLogic( )
		: mCurrentColorizedGroup( 0 )
		, mDamageableInterface( 0 )
		, mFirstBreak( false )
	{
	...
	void tUberBreakableLogic::fOnBreak( )
	{
		if( !mFirstBreak )
		{
			...
			// Leave UnbrokenPerma Meshes enabled
	...
	void tUberBreakableLogic::fSpawnDestroyFX( )
	{
		...
		if( !ent )
		{
			log_warning( "No attachment point with name " << cOnDestroyFX );
			ent = fOwnerEntity( );
		}
    ...
    Math::tObbf tUberBreakableLogic::fGetOBB ( )
    {
        Math::tAabbf objSpaceBounds = Math::tAabbf::cZeroSized;
        for ( u32 i = 0; i < mClumps.fCount( ); ++i )
            objSpaceBounds |= mClumps[ i ]->fObjSpaceBounds( );
 
        return Math::tObbf::fConstruct(objSpaceBounds, fOwnerEntity( )->fObjectToWorld( ));
    }
	...
	void tUberBreakableLogic::fGatherGridsAndMeshes( tEntity& root, tGrowableArray<t3dGridEntity*>& grids )
	{
		...
		if( mesh )
		{
			u32 mask = mesh->fStateMask( );
			u32 maskIndex = fHighestSetBitIndex( mask );

			// ensure only one index is set
			if( (mask & (~(1<<maskIndex))) != 0 )
			{
				tSceneRefEntity* sre = fOwnerEntity( )->fDynamicCast<tSceneRefEntity>( );
				log_assert( 0, "Uber breakable mesh state mask set up inproperly. Path: " << (sre ? sre->fResourcePath( ).fCStr( ) : "") );
			}
			...
			case 3: mUnbrokenPermaMeshes.fPushBack( mesh ); break; // Store the ubroken, perma pieces
			}
		}
	...
	{
		if( mUnbrokenMeshes.fCount( ) || mUnbrokenPermaMeshes.fCount( ) )
		{

#ifdef sig_logging
			if( ( mUnbrokenMeshes.fCount( ) + mUnbrokenPermaMeshes.fCount( ) ) > 5 )
			{
				...
				log_warning( "Uber breakable entity ( " << path << " ) has " << ( mUnbrokenMeshes.fCount( ) + mUnbrokenPermaMeshes.fCount( ) ) << " unbroken meshes" );
			}
#endif
			...
			for( u32 i = 0; i < mUnbrokenPermaMeshes.fCount( ); ++i )
				mUnbrokenPermaMeshes[ i ]->fSetDisabled( false );
		...
		else
		{
			...
			if( ( mBreakMeshes.fCount( ) + mPermaMeshes.fCount( ) ) > 5 )
			{
				...
				tSceneRefEntity * ent = fOwnerEntity( ) ? fOwnerEntity( )->fDynamicCast<tSceneRefEntity>( ) : NULL;
				tFilePathPtr path = ent ? ent->fSgResource( )->fGetPath( ) : tFilePathPtr::cNullPtr;
				log_warning( "Uber breakable entity ( " << path << " ) has no unbroken meshes and " << mBreakMeshes.fCount( ) << " break meshes and " << mPermaMeshes.fCount( ) << " perma meshes" );
			}
#endif
