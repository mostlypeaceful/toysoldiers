...
namespace Sig
{
	...
	public:
		...
		static void fEncode( const Sig::byte* toEncode, u32 toEncodeLength, std::string& encodedOutput, b32 flatEncode = false );

		///
		/// \brief Decode a character string (convert back to binary). Assumes all the characters in 'toDecode' were
		/// encoded using fEncode.
		static void fDecode( const char* toDecode, u32 toDecodeLength, tGrowableArray<Sig::byte>& decodedOutput, b32 flatDecode = false );
