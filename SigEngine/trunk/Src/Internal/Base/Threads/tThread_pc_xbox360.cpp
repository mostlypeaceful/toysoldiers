...
namespace Sig { namespace Threads
{
...
#elif platform_metro
	u32 tThread::gHardwareThreadCount = 4;
	void tThread::fComputeHardwareThreadCount( )
	{
		// GetSystemInfo is banned for metro, make a random assumption
		log_line( Log::cFlagThread, "Hardware Thread Count: " << gHardwareThreadCount );
	}
#else
	...
	{
		if( gMainThreadId == 0 )
			gMainThreadId = fCurrentThreadId( );
	...
	//------------------------------------------------------------------------------
#if !defined( platform_metro ) // GetExitCodeThread is a banned API.
	...
	b32 tThread::fStartThreadForPlatform( tThreadMain threadMain, void* threadParam, u32 hwThread, const char* threadName )
	{
		...
		const u32 creationFlags = 
#ifdef platform_xbox360
			explicitHwThread ? CREATE_SUSPENDED : 0;
#else
			0;
#endif
