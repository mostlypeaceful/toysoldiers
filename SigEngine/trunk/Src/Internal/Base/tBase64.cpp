...
namespace Sig
{
	void tBase64::fEncode( const Sig::byte* toEncode, u32 toEncodeLength, std::string& encodedOutput, b32 flatEncode )
	{
		std::stringstream ss;
		...
		for( u32 i = 0; i < toEncodeLength; i += 3 )
		{
			...
			// If this encoding isn't flat add the wrappage
			if( !flatEncode && ( i % ( 76 / 4 * 3 ) == 0 ) )
				ss << "\r\n";
		}
	...
	void tBase64::fDecode( const char* toDecode, u32 toDecodeLength, tGrowableArray<Sig::byte>& decodedOutput, b32 flatDecode )
	{
		decodedOutput.fSetCount( 0 );

		// If this data was not flat encoded then we need to rip out the non-base64 characters
		std::string toDecodeCopy;
		if( !flatDecode )
		{
			std::stringstream ss;
