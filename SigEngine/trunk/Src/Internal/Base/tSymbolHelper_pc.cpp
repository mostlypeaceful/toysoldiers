...
namespace Sig
{
		...
		: mProcessHandle( gProcessHandle++ )
	{
		...
		std::stringstream ss;
		ss << symbolSearchPath << ";."; // Add the current directory
		std::string path = ss.str( );

		BOOL result = SymInitialize( ( HANDLE )mProcessHandle, path.c_str( ), FALSE );
		if( !result )
			log_warning( "Could not initialize Sym for search path " << path );
	}
		...
		const tSymbolsSignature & signature )
	{
		...
		sigassert( ( void *)result == baseAddress );
		...
		void * address, std::string & symbol, std::string & file )
	{
		...
		for( u32 m = 0; m < moduleCount; ++m )
		{
			...
			if( module.mBaseAddress == ( void * )info->ModBase )
			{
				ssSym << module.mName;
				break;
			}
