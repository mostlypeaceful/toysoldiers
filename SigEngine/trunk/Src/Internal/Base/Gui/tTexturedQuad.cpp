...
namespace Sig { namespace Gui
{
	...
	{
		// TODO REFACTOR
		//fCreateQuadGeometry( mOrigin, mDimensions, mWidthScale, mHeightScale, mChopTexCoords, mInvert );
	}	
	...
	{
		if( !texturePath.fExists( ) )
		{
			tScriptVm::fDumpCallstack( );
			log_warning( "Non-existant path in tTextureQuad.SetTexture." );
			return;
		}
	...
	{
		log_assert( textureResource && textureResource->fLoaded( ), textureResource->fGetPath( ) << " wasn't loaded in tTexturedQuad.SetTexture." );

		mColorMap = textureResource;
		sigassert( mMaterial );
		mMaterial->mColorMap.fSetDynamic( mColorMap.fGetRawPtr( ) );
		mMaterial->mColorMap.fSetSamplingModes( Gfx::tTextureFile::cFilterModeNone, Gfx::tTextureFile::cAddressModeClamp );

		fSetRectFromTexture( );
	}
	void tTexturedQuad::fSetTexture( const Gfx::tTextureReference & texture, const Math::tVec2f & dims )
	{
		sigassert( mMaterial );
		mMaterial->mColorMap = texture;
		mMaterial->mColorMap.fSetSamplingModes( Gfx::tTextureFile::cFilterModeNone, Gfx::tTextureFile::cAddressModeClamp );
	...
	{
		const Gfx::tTextureFile* textureFile = mMaterial->mColorMap.fGetTextureFile( );
		if( textureFile )
	...
	void tTexturedQuad::fSetPivot( const Math::tVec2f& point )
	{
		...
		fSetRect( Math::tRect( -point, texDims ) );
	}
	void tTexturedQuad::fSetRectFromTexture( b32 center )
	{
		const Math::tVec2f texDims = fTextureDimensions( );
		if( center )
			fSetRect( Math::tRect( -0.5f * texDims, texDims ) );
		else
			fSetRect( Math::tRect( texDims ) );
	}
	...
	void tTexturedQuad::fSetRect( const Math::tRect& rect )
	{
		fSetBounds( rect );
		...
		const f32 xOffset	= topLeft.x;
		const f32 yOffset	= topLeft.y;
		const f32 width		= widthHeight.x;
		const f32 height	= widthHeight.y;
...
#else
		const Math::tVec2f uvOffset	= Math::tVec2f( 0, 0 );
#endif
	...
	void tTexturedQuad::fAutoConfigureSamplingModes( )
	{
		...
		if( aOrBOrC )
			...
			fSetFilterMode( Gfx::tTextureFile::cFilterModeNone );
	}
	void tTexturedQuad::fSetTextureFromScript( Gfx::tTextureReference * texture, const Math::tVec2f & dims )
	{
		fSetTexture( *texture, dims );
	}
...
namespace Sig { namespace Gui
{
	void tTexturedQuad::fExportScriptInterface( tScriptVm& vm )
	{
		...
		classDesc
			.Overload<void (tTexturedQuad::*)(const tFilePathPtr&)>( "SetTexture", &tTexturedQuad::fSetTexture )
			.Overload<void (tTexturedQuad::*)(Gfx::tTextureReference*, const Math::tVec2f&)>( "SetTexture", &tTexturedQuad::fSetTextureFromScript )
