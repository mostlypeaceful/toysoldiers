...
namespace Sig { namespace Gui
{
	...
	void tCanvas::fSetBounds( const Math::tRect& rect )
	{
	...
	void tCanvas::fUpdateWorldBounds( b32 notifyParentOfBoundsChange )
	{
		...
		const Math::tRect& localBounds = fLocalXform( ).mRect;
	...
	b32 tCanvas::fContainsWorldPoint( const Math::tVec2f& pt ) const
	{
		return mWorldXform.mRect.fContains( pt );
	}
}}
...
namespace Sig { namespace Gui
{
	...
	void tCanvasFrame::fOnChildBoundsChanged( )
	{
		...
		{
			mWorldXform.mRect.fInvalidate( );
			for( u32 i = 0; i < mChildren.fCount( ); ++i )
			{
				tCanvas* child = mChildren[ i ].fCanvas( );
				if( !child ) continue;
				mWorldXform.mRect.mT = fMin( mWorldXform.mRect.mT, child->fWorldXform( ).mRect.mT );
				mWorldXform.mRect.mL = fMin( mWorldXform.mRect.mL, child->fWorldXform( ).mRect.mL );
				mWorldXform.mRect.mB = fMax( mWorldXform.mRect.mB, child->fWorldXform( ).mRect.mB );
				mWorldXform.mRect.mR = fMax( mWorldXform.mRect.mR, child->fWorldXform( ).mRect.mR );
			}

			if( mWorldXform.mRect.fIsValid( ) )
			{
				// TODO use inverse world xform to compute local xform (i.e., undo scale/angle/position)
				mLocalXform.mRect = mWorldXform.mRect;
			}
			else
			{
				mLocalXform.mRect = Math::tRect( );
				mWorldXform.mRect = Math::tRect( );
			}
		}
	}
...
namespace Sig { namespace Gui
{
	...
	void tCanvas::fExportScriptInterface( tScriptVm& vm )
	{
		...
		classDesc
			...
			.Prop( "AcceptsCanvasEvents", &tCanvas::fAcceptsCanvasEventsFromScript, &tCanvas::fSetAcceptsCanvasEventsFromScript )
			...
			.Func( "ContainsWorldPoint", &tCanvas::fContainsWorldPoint )
