	...
	void tRenderableCanvas::fOnRenderCanvas( Gfx::tScreen& screen ) const
	{
#if defined( sig_logging )
		{
			...
			if( !fInBounds( z, 0.0f, 1.0f ) )
			{
				...
				if( tq )
				{
					if( tResourcePtr colorMap = tq->fColorMapResource( ) )
						log_warning( "Canvas out of Z range! Texture: " << colorMap->fGetPath( ) );
					else
						log_warning( "Canvas out of Z range! Texture, but doesn't exist" );
				}
				else
					log_warning( "Canvas out of Z range! Type: " << fDebugTypeName( ) );
			}
		}
#endif
