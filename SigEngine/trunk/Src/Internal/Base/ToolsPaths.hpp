...
namespace Sig { namespace ToolsPaths
{
	...
	base_export tFilePathPtr	fGetEngineDataFolder( );
	...
	base_export tFilePathPtr	fMakeEngineDataAbsolute( const char * relativePath );
    ...
    base_export void            fSetCurrentProjectDeployTo( const std::string& name );
	base_export std::string		fGetCurrentProjectName( );
    base_export std::string     fGetCurrentProjectDeployTo( );
	...
	base_export void			fSetCurrentP4Port( const std::string& port );
	...
	/// 
	/// \brief Launch sig apps with the specified command line.
	base_export void			fLaunchSigScript( const std::string& commandLine );
	base_export void			fLaunchSigEd( const std::string& commandLine );
	...
	base_export b32				fCheckoutAdd( const tFilePathPtr& path, b32 msgBoxOnError = true );
	...
	base_export b32				fIsCheckedOut ( const tFilePathPtr& path, std::string& returnedUserList );

	/// 
	/// \brief Controls sequential check out behavior for checking out a batch of files.
	base_export void			fCheckOutAllSubsequent( b32 autoCheckout );
	...
	public:
        ...
        std::string     mDeployToDir;
		std::string		mPerforcePort;
