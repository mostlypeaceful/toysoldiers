...
#pragma warning( default : 4723 )
	...
	const f32 cAntiPodalQuatThreshold = 0.f;

	static_assert( sizeof( tDebugF32 ) == sizeof( f32 ) );

	//------------------------------------------------------------------------------
	u32 fBinomial( u32 n, u32 k )
	{
		sigassert( n >= k && "Binomial must comply with 0 <= n <= k" );
		...
		{
			c *= ( n - i );
			c /= ( i + 1 ); // Divide out k! incrementally to reduce overflow
		}

		sigassert( c <= ~0u && "Overflow in Binomial" );
		return (u32)c;
	}
	...
	namespace
	{
		...
		{\
			classDesc\
				...
				.Func(																	_SC("_add"),			&tVecType::fAdd)\
				.Func(																	_SC("_sub"),			&tVecType::fSub)\
				.Func(																	_SC("_mul"),			&tVecType::fMul)\
				.Func(																	_SC("_div"),			&tVecType::fDiv)\
				.Func(																	_SC("_unm"),			&tVecType::fNeg)\
		...
		{
			// Not used, or recommended to be used.
		...
		static void fMat3fOrientYAxisForScript( tMat3f* mat, tVec3f* newAxis )
		{
			newAxis->fNormalize( );
			mat->fOrientYAxis( *newAxis );
		}
		static void fMat3fScaleGlobalForScript( tMat3f* mat, f32 scale )
		{
			mat->fScaleGlobal( Math::tVec3f( scale ) );
		}
		static void fExportMat3( tScriptVm& vm )
		{
			...
			classDesc
				...
				.GlobalFunc(_SC("SetScale"), &fMat3fScaleGlobalForScript)
				.GlobalFunc(_SC("OrientYAxis"), &fMat3fOrientYAxisForScript)
				;
