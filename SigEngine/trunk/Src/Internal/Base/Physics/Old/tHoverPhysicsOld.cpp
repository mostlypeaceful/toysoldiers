...
namespace Sig { namespace PhysicsOld
{
	...
	{	
		...
		// find ground height
		Physics::tGroundRayCastCallback callback( *logic->fOwnerEntity( ), mProperties.mGroundMask );
		tRayf ray;
	...
	void tHoverPhysics::fAddContactPt( f32 dt, const tContactPoint& cp )
	{
		mContactPoints.fPushBack( cp );
	}

	tVec3f tHoverPhysics::fComputeContactResponse( const tContactPoint& cp, f32 mass )
	{
