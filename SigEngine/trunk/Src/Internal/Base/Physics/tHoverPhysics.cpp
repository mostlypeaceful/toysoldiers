...
namespace Sig { namespace Physics
{
	...
	void tHoverPhysics::fPhysicsMT( tLogic* logic, f32 dt )
	{
		...
		{
			realGroundHeight = ray.fPointAtTime( callback.mHit.mT ).y;
	...
	void tHoverPhysics::fResolveCollisionMT( tLogic* logic, f32 dt )
	{
		...
		//{
		//	tVehicleContact& cp = mContactPoints[ i ];

		//	f32 separation = 0.f;

		//	sigassert( cp.mShape );
		//	cp.mCP.mPoint = cp.mShape->fClosestPoint( worldSphere.fCenter( ) );
		//	cp.mCP.mNormal = worldSphere.fCenter( ) - cp.mCP.mPoint;
		//	cp.mCP.mNormal.fNormalizeSafe( tVec3f::cYAxis, separation );
		//	separation -= worldSphere.fRadius( );

		//	if( Physics_Hover_DrawCollisionDetection ) logic->fSceneGraph( )->fDebugGeometry( ).fRenderOnce( cp.mCP.mPoint, cp.mCP.mPoint + tVec3f( 0,10,0 ), tVec4f(0,1,0,1) );
		//	
		//	if( separation < 0.f )
		//	{
		//		f32 penVel = mV.fDot( cp.mCP.mNormal );

		//		if( penVel < 0.f )
		//			mV -= cp.mCP.mNormal * penVel; // resolve velocity

		//		// add outward velocity to resolve penetration
		//		const f32 sepStrength = 4.f; // How stiff the separation is.
		//		mV += cp.mCP.mNormal * (separation*separation * sepStrength);
		//	}
		//}
	...
	{
		f32 diff = newHeight - mTransform( 1, 3 );
