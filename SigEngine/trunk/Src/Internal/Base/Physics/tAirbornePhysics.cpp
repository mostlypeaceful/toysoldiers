...
namespace Sig { namespace Physics
{
	...
	{	
		...
		tVec3f oldPos = fTransform( ).fGetTranslation( );
		tMat3f xform = tMat3f( yaw * pitch * roll );
		xform.fSetTranslation( oldPos + fTransform( ).fXformVector( tVec3f(0,0,mSpeed * dt) ) );
		fSetTransform( xform );
		...
		if( mSpeed > 0.1f )
		{
			for( u32 i = 0; i < mProperties.mCollisionProbes.fCount( ); ++i )
			{
				...
				tRayf ray;
				tVec3f z = fTransform( ).fZAxis( );
				ray.mExtent = mV * dt;
				ray.mOrigin = fTransform( ).fXformPoint( mProperties.mCollisionProbes[ i ] ) - ray.mExtent;
				
				// give ourselves a little extra check
				f32 extraRay = 2.f;
				ray.mExtent += z * extraRay;

				tGroundRayCastCallback cb( *logic->fOwnerEntity( ), mProperties.mGroundMask );
	...
	{
		tMat3f flatXform;
		flatXform.fSetTranslation( fTransform( ).fGetTranslation( ) );
		flatXform.fOrientYAxis( tVec3f::cYAxis, fTransform( ).fXAxis( ) );
	...
	void tAirbornePhysics::fDrawDebugInfo( tLogic* logicP )
	{
		...
		{
			//center of mass
			logicP->fSceneGraph( )->fDebugGeometry( ).fRenderOnce( fTransform( ), 1.0f );			
		}
	}
