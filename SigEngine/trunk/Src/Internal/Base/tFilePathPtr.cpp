...
namespace Sig
{
	namespace
	{
		...
		tSharedStringCriticalSection& fFilePathPtrCritSec( )
		{
		...
		{
			tSharedStringMutex threadSafe( fFilePathPtrCritSec( ) );
	...
	tFilePathPtr tFilePathPtr::fRelativeTo( const tFilePathPtr& path, tPlatformId pid ) const
	{
		sigassert( ( path.fLength()==0 || !fIsSlash(path.fCStr()[path.fLength()-1]) ) && "path doesn't end with a slash" );
		sigassert( path.fLength() < this->fLength() && "*this starts with path" );
		sigassert( strncmp( fCStr(), path.fCStr(), path.fLength() ) == 0 && "*this starts with path" ); // assert this starts with "path"
		sigassert( fIsSlash( fCStr()[ path.fLength() ] ) && "*this starts with path" ); // to assert we're not doing e.g. "blahetc/derp".fRelativeTo("blah")

		std::string newPath( fCStr() + path.fLength() + 1, fCStr() + this->fLength() );
		return tFilePathPtr( newPath.c_str(), pid );
	}
