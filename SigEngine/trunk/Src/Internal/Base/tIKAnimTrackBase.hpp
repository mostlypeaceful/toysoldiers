...
namespace Sig { namespace Anim
{
	...
	struct tIKAnimTrackDesc : public tAnimTrackDesc
	{
		...
		tIKAnimTrackDesc( )
			...
			, mReuse( true )
		{
			mFlags = tAnimTrack::cFlagPartial | tAnimTrack::cFlagPost;
		}
	...
	public:
			...
			, mBlendInCoolDownTimer( -1.f )
		{
			...
			{
				log_warning( "No bones present in IK key frame anim." );
				return;
			}
		...
		{
			const f32 dt = 1/30.f; //TODO figure out where to get this.
		...
		void fBlendIKResult( tAnimatedSkeleton& animSkel )
		{
			...
			//const f32 cBlendEpsilon = 0.0001f;
			{
				...
				for( u32 i = 0; i < mSolver.fJoints( ).fCount( ); ++i )
				{
					...
					{
						log_warning( "Additive bone ignored in CCDIKAnimTrack." );
						continue;
					}
