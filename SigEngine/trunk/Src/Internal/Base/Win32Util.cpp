...
namespace Sig { namespace Win32Util
{
	...
	{
		tFixedArray<char,512> buf, buf2;
		fZeroOut( buf );
		fZeroOut( buf2 );
		GetModuleFileName( GetModuleHandle(0), buf.fBegin( ), buf.fCount( ) );
		GetFullPathName( buf.fBegin(), buf.fCount(), buf2.fBegin(), NULL );
		return buf2.fBegin( );
	}
	...
	b32 fLaunchProcessAndCaptureOutput( std::string cmdLine, std::string& output )
	{
		output.clear( );
		...
		if( !CreatePipe( &hChildStdOutRead, &hChildStdOutWrite, &secAttribs, 0 ) ) 
			return false;

		// Create a pipe for the child process's STDERR
		HANDLE hChildStdErrorRead = NULL;
		HANDLE hChildStdErrorWrite = NULL;
		if( !CreatePipe( &hChildStdErrorRead, &hChildStdErrorWrite, &secAttribs, 0 ) ) 
			return false;
		...
		// Specify the STDOUT handle for redirection
		STARTUPINFO childStartupInfo;
		ZeroMemory( &childStartupInfo, sizeof( STARTUPINFO ) );
		childStartupInfo.cb = sizeof(STARTUPINFO);
		childStartupInfo.dwFlags |= STARTF_USESTDHANDLES | STARTF_USESHOWWINDOW;
		childStartupInfo.hStdOutput = hChildStdOutWrite;
		childStartupInfo.hStdError = hChildStdErrorWrite;
		childStartupInfo.wShowWindow = SW_HIDE;

		// This will return filled with information about the child
		PROCESS_INFORMATION childProcInfo;
		ZeroMemory( &childProcInfo, sizeof( PROCESS_INFORMATION ) );

		char* cmdLinePtr = const_cast<char*> ( cmdLine.c_str( ) );

		if( !CreateProcess( NULL,
			cmdLinePtr,			// Command line arguments
			...
			&childStartupInfo,	// STARTUPINFO pointer
		...
		if( !CloseHandle( hChildStdErrorWrite ) )
			return false;

		DWORD dwRead;
		char buffer[1024];
		b32 success = ReadFile( hChildStdOutRead, buffer, 1024, &dwRead, NULL );
		if( !success )
			return false;
