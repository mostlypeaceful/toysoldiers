...
namespace Sig
{
	...
	*/
	...
	// Basic xor checksum.
	u8 tEncryption::fCheckSum( const byte* src, u32 srcBytes, u8 sum )
	{
	...
	void tEncryption::fEncrypt( const tGrowableArray<byte>& src, tGrowableArray<byte>& output )
	{
		...
		log_line( 0, "Encrypt: Sum: " << (u32)checkSum << " Loc: " << checkSumLocation << " MaskKey: " << maskKey );
	}
	...
	void tEncryption::fDecrypt( const tGrowableArray<byte>& src, tGrowableArray<byte>& output )
	{
		...
		log_line( 0, "Decrypt: Sum: " << (u32)checksum << " Loc: " << (u32)checkSumLocation << " Valid: " << validation << " MaskKey: " << maskKey );
