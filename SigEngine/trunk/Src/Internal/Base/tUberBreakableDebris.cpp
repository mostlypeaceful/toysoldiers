...
namespace Sig
{
	...
	devvar( u32, Perf_Debris_MaxDebrisUber, 250 );
	...
	namespace
	{
		...
		static void fSpawn( tEntity& ent, tEntity& newParent, const Math::tVec3f& velocity )
		{
			...
			ent.fAcquireLogic( tLogicPtr( dl ) );
	...
	void tUberBreakableDebris::fCollideAndRespond( f32 dt, const Math::tMat3f& prevPos )
	{
		...
		if( mActive && mBounceCount < mMaxBounces )
		{
			...
			if( fEqual( ray.mExtent.fLengthSquared( ), 0.f ) )
				return;
			...
			{
				const Math::tVec3f hitPos = ray.fPointAtTime( rayCastCb.mHit.mT );
				...
				mDeathHeight = ray.fPointAtTime( rayCastCb.mHit.mT ).y - worldObb.fExtents( ).fMax( ) * 2;
			}
		}
		else
			mActive = false;
