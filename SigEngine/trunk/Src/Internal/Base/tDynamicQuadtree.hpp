...
namespace Sig
{
	...
	template<>
	inline b32 fQuadtreeIntersectAabbWithVolume<Math::tFrustumf>( const Math::tAabbf& aabb, const Math::tFrustumf& v )
	{
		return v.fIntersects( aabb );
	}
	
	template<>
	inline b32 fQuadtreeIntersectAabbWithVolume<Math::tAabbf>( const Math::tAabbf& aabb, const Math::tAabbf& v )
	{
		return aabb.fIntersects( v );
	}
	
	template<>
	inline b32 fQuadtreeIntersectAabbWithVolume<Math::tObbf>( const Math::tAabbf& aabb, const Math::tObbf& v )
	{
		return Math::tIntersectionAabbObb<f32>( aabb, v ).fIntersects( );
	}
	
	template<>
	inline b32 fQuadtreeIntersectAabbWithVolume<Math::tSpheref>( const Math::tAabbf& aabb, const Math::tSpheref& v )
	{
	...
	public:
		...
		typedef tSpatialTree::tCellKey tCellKey;
		friend class tSpatialObject;
		typedef tSpatialObject tObject;
	...
	private:
		...
		template<class tVolume>
		void fIntersectDeferred(
			u32 depth,
			const tVolume& v,
			tGrowableArray<void*>& owners ) const;
		...
		const tIntersectionOperator& intersectCb ) const
	{
		...
		{
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i + 0 ]->mWorldSpaceBox, v ) ) intersectCb( v, mContents[ i + 0 ], false );
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i + 1 ]->mWorldSpaceBox, v ) ) intersectCb( v, mContents[ i + 1 ], false );
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i + 2 ]->mWorldSpaceBox, v ) ) intersectCb( v, mContents[ i + 2 ], false );
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i + 3 ]->mWorldSpaceBox, v ) ) intersectCb( v, mContents[ i + 3 ], false );
		}
		for( u32 i = divBy4Count; i < mContents.fCount( ); ++i )
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i ]->mWorldSpaceBox, v ) ) intersectCb( v, mContents[ i ], false );
	...
	template<class tVolume>
	void tDynamicQuadtree::fIntersectDeferred(
		u32 depth,
		const tVolume& v,
		tGrowableArray<void*>& owners ) const
	{
		...
		for( u32 i = 0; i < divBy4Count; i += 4 )
		{
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i + 0 ]->mWorldSpaceBox, v ) ) owners.fPushBack( mContents[ i + 0 ]->fOwner( ) );
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i + 1 ]->mWorldSpaceBox, v ) ) owners.fPushBack( mContents[ i + 1 ]->fOwner( ) );
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i + 2 ]->mWorldSpaceBox, v ) ) owners.fPushBack( mContents[ i + 2 ]->fOwner( ) );
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i + 3 ]->mWorldSpaceBox, v ) ) owners.fPushBack( mContents[ i + 3 ]->fOwner( ) );
		}
		for( u32 i = divBy4Count; i < mContents.fCount( ); ++i )
			if( fQuadtreeIntersectAabbWithVolume( mContents[ i ]->mWorldSpaceBox, v ) ) owners.fPushBack( mContents[ i + 0 ]->fOwner( ) );
		...
		if( recurse )
		{
			...
			{
				if( mChildren[ i ] )
					mChildren[ i ]->fIntersectDeferred( depth + 1, v, owners );
			}
		}
	}
	...
	public:
		...
		void fIntersectDeferred(
			...
			tGrowableArray<void*>& owners ) const
		{
			mRoot.fIntersectDeferred( 0, v, owners );
		}
