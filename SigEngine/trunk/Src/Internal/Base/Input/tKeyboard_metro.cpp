...
namespace Sig { namespace Input
{
	void tKeyboard::fCaptureStateUnbuffered( tStateData& stateData, f32 dt )
	{
		...
		if (!mWindow)
		{
			log_warning( 0, "Null mWindow in tKeyboard::fCaptureStateUnbuffered" );
			return;
		}
		...
		CoreWindow^ window = ((tImmersiveWindow*)mWindow)->fGetCoreWindow();

		// translate keys info to be purely boolean and track keys pressed info
		for( u32 i = 0; i < stateData.mKeysOn.fCount( ); ++i )
		{
			if( (window->GetAsyncKeyState( (Windows::System::VirtualKey)i ) & CoreVirtualKeyStates::Down) != CoreVirtualKeyStates::None )
			{
	...
	void tKeyboard::fStartup( tGenericWindowHandle handle )
	{
		sigassert( mWindow == 0 );
		mWindow = handle;
	}
	...
	// Update to use Windows::System::VirtualKey ?
	// Looks like they currently match native enumerations
	// 
	// If they do need to be updated, see:
	// 
	// http://msdn.microsoft.com/en-us/library/windows/apps/windows.system.virtualkey#Y0
	...
	const tKeyboard::tButton tKeyboard::cButtonMinus		= VK_SUBTRACT;			/* - on main keyboard */
	const tKeyboard::tButton tKeyboard::cButtonEquals		= VK_ADD;
	...
	const tKeyboard::tButton tKeyboard::cButtonLWin			= VK_LWIN;				/* Left Windows key */
	const tKeyboard::tButton tKeyboard::cButtonRWin			= VK_RWIN;				/* Right Windows key */
