...
static enet_uint32 gPeerTimeoutMinimum = ENET_PEER_TIMEOUT_MINIMUM;
static enet_uint32 gPeerTimeoutMaximum = ENET_PEER_TIMEOUT_MAXIMUM;
...
enet_protocol_check_timeouts (ENetHost * host, ENetPeer * peer, ENetEvent * event)
{
    ...
    while (currentCommand != enet_list_end (& peer -> sentReliableCommands))
    {
       ...
       if (peer -> earliestTimeout != 0 &&
             (ENET_TIME_DIFFERENCE (host -> serviceTime, peer -> earliestTimeout) >= gPeerTimeoutMaximum ||
               (outgoingCommand -> roundTripTimeout >= outgoingCommand -> roundTripTimeoutLimit &&
                 ENET_TIME_DIFFERENCE (host -> serviceTime, peer -> earliestTimeout) >= gPeerTimeoutMinimum)))
       {
          enet_protocol_notify_disconnect (host, peer, event);
...
enet_protocol_send_reliable_outgoing_commands (ENetHost * host, ENetPeer * peer)
{
    ...
    while (currentCommand != enet_list_end (& peer -> outgoingReliableCommands))
    {
       ...
       if (outgoingCommand -> roundTripTimeout == 0)
       {
          ...
          // Clamp the round trip timeout to avoid resending packets unnecessarily
          outgoingCommand->roundTripTimeout = ENET_MAX( outgoingCommand->roundTripTimeout, ENET_PEER_PACKET_TIMEOUT_MINIMUM );
       }
...
*/
void
enet_set_peer_timeouts( enet_uint32 peerTimeoutMin, enet_uint32 peerTimeoutMax )
{
	gPeerTimeoutMinimum = peerTimeoutMin;
	gPeerTimeoutMaximum = peerTimeoutMax;
}
