...
*/
static SQBool g_endianswap = 0;
void sq_endianswapenable(SQBool e)
{
	g_endianswap = e;
}
SQBool sq_getendianswapenabled()
{
	return g_endianswap;
}
void sq_swapbytes( char* a, char* b )
{
	char t = *a; *a = *b; *b = t;
}
void sq_endianswap16(SQUserPointer up,SQInteger count)
{
	char* pb = ( char* )up;
	for(int i=0; i<count; ++i)
	{
		sq_swapbytes( &pb[0], &pb[1] );
		pb += sizeof( short );
	}
}
void sq_endianswap32(SQUserPointer up,SQInteger count)
{
	char* pb = ( char* )up;
	for(int i=0; i<count; ++i)
	{
		sq_swapbytes( &pb[0], &pb[3] );
		sq_swapbytes( &pb[1], &pb[2] );
		pb += sizeof( int );
	}
}
...
SQRESULT sq_writeclosure(HSQUIRRELVM v,SQWRITEFUNC w,SQUserPointer up)
{
	/*
		ENDIAN-SWAPPING
		MODIFIED December 17, 2008 by Max Wagner, not part of original Squirrel API.
		Now (optionally) performs relevant endian-swapping prior to serializing.
	*/
